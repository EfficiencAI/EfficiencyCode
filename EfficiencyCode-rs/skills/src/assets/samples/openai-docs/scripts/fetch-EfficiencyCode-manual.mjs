...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/EfficiencyCode/EfficiencyCode-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "EfficiencyCode-manual.md";
const OUTLINE_FILE_NAME = "EfficiencyCode-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "EfficiencyCode-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-EfficiencyCode-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# EfficiencyCode Manual Outline\n\n${outline.text}\n`;
...
const fetchEfficiencyCodeManual = async ({
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchEfficiencyCodeManual(args);
...
export { DEFAULT_MANUAL_URL, fetchEfficiencyCodeManual };
