...
- the user wants EfficiencyCode to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single EfficiencyCode cachebuster suffix:
...
outside EfficiencyCode depends on a specific token:
...
```bash
EfficiencyCode plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `EfficiencyCode plugin marketplace add` for that
path, and `EfficiencyCode plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
EfficiencyCode plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that EfficiencyCode picks up new skills
   and tools.
...
```text
<base-version>+EfficiencyCode.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+EfficiencyCode.local-20260519-184516`
- `0.1.0+EfficiencyCode.old-token` → `0.1.0+EfficiencyCode.local-20260519-184516`
- `1.2.3-beta.1+EfficiencyCode.prev` → `1.2.3-beta.1+EfficiencyCode.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+EfficiencyCode.local-20260519-184516`

Replace the existing EfficiencyCode cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `EfficiencyCode plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `EfficiencyCode plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by EfficiencyCode.
...
- Use `EfficiencyCode plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `EfficiencyCode plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `EfficiencyCode plugin add <plugin-name>@<marketplace-name>`.
