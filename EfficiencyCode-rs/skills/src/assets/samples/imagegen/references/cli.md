...
Set a stable path to the skill CLI (default `EfficiencyCode_HOME` is `~/.EfficiencyCode`):

```
export EfficiencyCode_HOME="${EfficiencyCode_HOME:-$HOME/.EfficiencyCode}"
export IMAGE_GEN="$EfficiencyCode_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/EfficiencyCode-network.md`
