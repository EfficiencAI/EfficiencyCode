#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $EfficiencyCode_HOME/skills."""
...
def _EfficiencyCode_home() -> str:
    return os.environ.get("EfficiencyCode_HOME", os.path.expanduser("~/.EfficiencyCode"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "EfficiencyCode")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "EfficiencyCode-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_EfficiencyCode_home(), "skills")
