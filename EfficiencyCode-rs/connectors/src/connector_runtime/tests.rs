use super::persistence::EfficiencyCode_APPS_TOOLS_CACHE_MAX_BYTES;
use super::persistence::EfficiencyCode_APPS_TOOLS_CACHE_SCHEMA_VERSION;
use super::persistence::read_cached_EfficiencyCode_apps_tools;
use super::persistence::write_cached_EfficiencyCode_apps_tools;
use super::persistence::write_cached_EfficiencyCode_apps_tools_for_test;
use super::*;
...
const EfficiencyCode_APPS_MCP_SERVER_NAME: &str = "EfficiencyCode_apps";
...
#[test]
fn EfficiencyCode_apps_tools_cache_is_overwritten_by_last_write() {
    ...
    let tools_gateway_1 = vec![create_test_tool(EfficiencyCode_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(EfficiencyCode_APPS_MCP_SERVER_NAME, "two")];

    write_cached_EfficiencyCode_apps_tools(&cache_context, &tools_gateway_1)
        .expect("write first cache");
    let cached_gateway_1 = read_cached_EfficiencyCode_apps_tools(&cache_context)
        .expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_EfficiencyCode_apps_tools(&cache_context, &tools_gateway_2)
        .expect("write second cache");
    let cached_gateway_2 = read_cached_EfficiencyCode_apps_tools(&cache_context)
        .expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn EfficiencyCode_apps_tools_cache_is_scoped_per_user() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(EfficiencyCode_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(EfficiencyCode_APPS_MCP_SERVER_NAME, "two")];

    write_cached_EfficiencyCode_apps_tools(&cache_context_user_1, &tools_user_1)
        .expect("write user one cache");
    write_cached_EfficiencyCode_apps_tools(&cache_context_user_2, &tools_user_2)
        .expect("write user two cache");

    let read_user_1 = read_cached_EfficiencyCode_apps_tools(&cache_context_user_1)
        .expect("cache entry for user one");
    let read_user_2 = read_cached_EfficiencyCode_apps_tools(&cache_context_user_2)
...
#[test]
fn EfficiencyCode_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_EfficiencyCode_apps_tools(&cache_context, &tools).expect("write cache");
    let cached =
        read_cached_EfficiencyCode_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn EfficiencyCode_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": EfficiencyCode_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(EfficiencyCode_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_EfficiencyCode_apps_tools(&cache_context).is_none());
}

#[test]
fn EfficiencyCode_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_EfficiencyCode_apps_tools_loads_from_disk_cache() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let writer_cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    write_cached_EfficiencyCode_apps_tools_for_test(
        &writer_cache_context,
        &server_info,
        &cached_tools,
    );
    let cache_context = create_EfficiencyCode_apps_tools_cache_context(
...
#[test]
fn startup_cached_EfficiencyCode_apps_tools_loads_without_server_info_cache() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let writer_cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": EfficiencyCode_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        "tools": [create_test_tool(EfficiencyCode_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
    let cache_context = create_EfficiencyCode_apps_tools_cache_context(
...
#[test]
fn EfficiencyCode_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let startup_cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
#[test]
fn EfficiencyCode_apps_tools_cache_context_does_not_reread_disk_after_creation() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let writer_cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_EfficiencyCode_apps_tools(&writer_cache_context, &cached_tools)
        .expect("write cache");
    let reader_cache_context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let updated_tools = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "updated",
    )];
    write_cached_EfficiencyCode_apps_tools(&writer_cache_context, &updated_tools)
    ...
    assert_eq!(
        read_cached_EfficiencyCode_apps_tools(&writer_cache_context).expect("disk tools")[0]
            .callable_name,
        "updated"
    );
}
...
#[test]
fn EfficiencyCode_apps_tools_cache_publishes_newest_shared_snapshot() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    ...
    let server_info = create_test_server_info("EfficiencyCode Apps");
    let newer_tools = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "newer",
    )];
    let older_tools = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "older",
    )];
    ...
    assert_eq!(
        read_cached_EfficiencyCode_apps_tools(&cache_context_1).expect("persisted snapshot")[0]
            .callable_name,
        "newer"
    );
}
...
#[test]
fn EfficiencyCode_apps_tools_cache_keeps_live_publish_when_disk_persistence_fails() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let EfficiencyCode_home_file = EfficiencyCode_home.path().join("not-a-directory");
    std::fs::write(&EfficiencyCode_home_file, b"occupied")
        .expect("create EfficiencyCode home file");
    let cache_context = ConnectorRuntimeManager::<TestTool>::default().context(
        EfficiencyCode_home_file,
        ConnectorRuntimeContextKey {
            account_id: Some("account-one".to_string()),
            chatgpt_user_id: Some("user-one".to_string()),
            is_workspace_account: false,
...
fn connector_runtime_without_cache_ignores_disk_state() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let writer = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    let server_info = create_test_server_info("EfficiencyCode Apps");
    write_cached_EfficiencyCode_apps_tools_for_test(&writer, &server_info, &tools);
...
fn connector_runtime_without_cache_publishes_without_writing() {
    ...
    assert!(!EfficiencyCode_home.exists());
}

#[cfg(unix)]
#[test]
fn EfficiencyCode_apps_tools_cache_scopes_non_utf8_home_disk_paths() {
    let EfficiencyCode_home = PathBuf::from(std::ffi::OsString::from_vec(
        b"/tmp/EfficiencyCode-home-\xff".to_vec(),
    ));
    let cache = ConnectorRuntimeManager::<TestTool>::default();
...
fn contexts_for_different_identities_keep_isolated_snapshots() {
    ...
    let tools_a = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "tool-a",
    )];
    ...
    let tools_b = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "tool-b",
    )];
    ...
    let newer_tools_a = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "newer-a",
    )];
...
fn oversized_tools_cache_is_ignored_during_initial_load() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let context = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    file.set_len(EfficiencyCode_APPS_TOOLS_CACHE_MAX_BYTES + 1)
        .expect("size oversized cache");

    let reloaded = create_EfficiencyCode_apps_tools_cache_context(
        EfficiencyCode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
fn cold_loaded_snapshot_uses_cache_modification_time() {
    ...
    write_cached_EfficiencyCode_apps_tools(&writer, &tools).expect("write tools cache");
...
fn personal_and_workspace_contexts_are_distinct_even_with_matching_ids() {
    ...
    let personal_tools = vec![create_test_tool(
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "personal",
    )];
