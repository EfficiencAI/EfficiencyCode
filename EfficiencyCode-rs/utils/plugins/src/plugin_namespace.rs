...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".EfficiencyCode-plugin/plugin.json");
    ...
    #[test]
    fn preserves_EfficiencyCode_claude_cursor_legacy_precedence() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let EfficiencyCode_path = plugin_root.join(".EfficiencyCode-plugin/plugin.json");
        let claude_path = plugin_root.join(".claude-plugin/plugin.json");
        let cursor_path = plugin_root.join(".cursor-plugin/plugin.json");
        for path in [&EfficiencyCode_path, &claude_path, &cursor_path] {
        ...
        assert_eq!(
            ...
            Some(EfficiencyCode_path.clone())
        );
        fs::remove_file(EfficiencyCode_path).expect("remove EfficiencyCode manifest");
