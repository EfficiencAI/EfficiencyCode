...
mod tests {
    ...
    fn render_replaces_placeholders_with_and_without_whitespace() {
        let rendered = render(
            ...
            [("name", "EfficiencyCode"), ("place", "EfficiencyCode-rs")],
        )
        .unwrap();

        assert_eq!(
            rendered,
            "Hello, EfficiencyCode. You are in EfficiencyCode-rs. EfficiencyCode is repeated."
        );
    }
    ...
    fn parsed_templates_can_be_reused() {
        ...
        assert_eq!(
            template.render([("greeting", "Hello"), ("name", "EfficiencyCode")]),
            Ok("Hello, EfficiencyCode!".to_string())
        );
        assert_eq!(
    ...
    fn render_supports_literal_delimiter_escapes() {
        ...
        assert_eq!(
            rendered,
            "literal open: {{, literal close: }}, value: EfficiencyCode"
        );
    }
    ...
    fn render_errors_when_extra_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "EfficiencyCode"), ("unused", "extra")]),
    ...
    fn render_errors_when_duplicate_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "EfficiencyCode"), ("name", "other")]),
    ...
    fn render_function_wraps_parse_errors() {
        let err = render("Hello, }} world.", [("name", "EfficiencyCode")]).unwrap_err();
    ...
    fn render_function_wraps_render_errors() {
        let err = render("Hello, {{ name }}.", [("extra", "EfficiencyCode")]).unwrap_err();
