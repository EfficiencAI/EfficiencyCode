...
async fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let child_task = tokio::task::spawn_blocking(move || -> anyhow::Result<ChildOutput> {
        ...
        let mut child = Command::new(EfficiencyCode_utils_cargo_bin::cargo_bin(
            "EfficiencyCode-stdio-to-uds",
        )?)
        .arg(&socket_path)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .context("failed to spawn EfficiencyCode-stdio-to-uds")?;
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                anyhow::bail!(
                    "EfficiencyCode-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                    server_events,
    ...
    assert!(
        ...
        "EfficiencyCode-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",
