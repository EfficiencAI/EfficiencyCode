...
//!
//! The history is stored at `~/.EfficiencyCode/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.EfficiencyCode`.
...
impl HistoryConfig {
    pub fn new(EfficiencyCode_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            EfficiencyCode_home: EfficiencyCode_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.EfficiencyCode_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.EfficiencyCode/history.jsonl` and ensure the parent directory exists.
