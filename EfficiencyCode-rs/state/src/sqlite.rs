...
impl RuntimeDbSpec {
    fn path(self, EfficiencyCode_home: &Path) -> PathBuf {
        EfficiencyCode_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all EfficiencyCode SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable EfficiencyCode SQLite database, creating it if necessary.
    ...
    /// Open an existing EfficiencyCode SQLite database without creating or modifying it.
