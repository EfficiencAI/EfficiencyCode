...
//!
//! # async fn example(state_db: std::sync::Arc<EfficiencyCode_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("EfficiencyCode_rmcp_client", LevelFilter::INFO)
        .with_target("EfficiencyCode_otel.log_only", LevelFilter::OFF)
        .with_target("EfficiencyCode_otel.trace_safe", LevelFilter::OFF)
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout EfficiencyCode environments.
...
mod tests {
    ...
    fn temp_EfficiencyCode_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("EfficiencyCode-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let EfficiencyCode_home = temp_EfficiencyCode_home();
