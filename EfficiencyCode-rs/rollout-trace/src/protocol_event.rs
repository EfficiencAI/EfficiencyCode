//! Mapping from EfficiencyCode protocol events into raw rollout-trace events.
//!
...
//! `EfficiencyCode-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `EfficiencyCode-rollout-trace`.
//!
...
use EfficiencyCode_protocol::protocol::PatchApplyStatus;
use EfficiencyCode_protocol::protocol::SubAgentActivityEvent;
use EfficiencyCode_protocol::protocol::TurnAbortReason;
use serde::Serialize;
use std::time::Duration;

use crate::AgentThreadId;
use crate::EfficiencyCodeTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct EfficiencyCodeTurnTraceEvent {
    pub context_turn_id: EfficiencyCodeTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn EfficiencyCode_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<EfficiencyCodeTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let EfficiencyCode_turn_id = event.turn_id.clone();
            Some(EfficiencyCodeTurnTraceEvent {
                context_turn_id: EfficiencyCode_turn_id.clone(),
                payload: RawTraceEventPayload::EfficiencyCodeTurnStarted {
                    EfficiencyCode_turn_id,
                    thread_id,
                },
            })
        }
        EventMsg::TurnComplete(event) => {
            ...
            Some(EfficiencyCodeTurnTraceEvent {
                context_turn_id: EfficiencyCode_turn_id.clone(),
                payload: RawTraceEventPayload::EfficiencyCodeTurnEnded {
                    EfficiencyCode_turn_id,
        ...
        EventMsg::TurnAborted(event) => {
            let EfficiencyCode_turn_id = event
                .turn_id
                .clone()
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a EfficiencyCode_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a EfficiencyCode_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(
        &'a EfficiencyCode_protocol::protocol::CollabAgentInteractionBeginEvent,
    ),
    CollabAgentInteractionEnd(
        &'a EfficiencyCode_protocol::protocol::CollabAgentInteractionEndEvent,
    ),
    CollabWaitingBegin(&'a EfficiencyCode_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a EfficiencyCode_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a EfficiencyCode_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a EfficiencyCode_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [EfficiencyCode_protocol::parse_command::ParsedCommand],
