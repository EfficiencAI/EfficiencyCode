...
use EfficiencyCode_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical EfficiencyCode tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub EfficiencyCode_turn_id: EfficiencyCodeTurnId,
...
/// Raw invocation payload for the canonical EfficiencyCode tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            EfficiencyCode_turn_id: invocation.EfficiencyCode_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == EfficiencyCode_code_mode::PUBLIC_TOOL_NAME
}
