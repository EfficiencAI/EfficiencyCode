...
use crate::model::EfficiencyCodeTurnId;
...
const INFERENCE_CALL_ID_HEADER: &str = "x-EfficiencyCode-inference-call-id";
...
///
/// A EfficiencyCode turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one EfficiencyCode turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        EfficiencyCode_turn_id: EfficiencyCodeTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                EfficiencyCode_turn_id: attempt.context.EfficiencyCode_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps EfficiencyCode-core focused on
    ...
    /// Records a provider stream that EfficiencyCode intentionally stopped consuming.
    ///
...
/// EfficiencyCode received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        EfficiencyCode_turn_id: Some(context.EfficiencyCode_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.EfficiencyCode_turn_id, "turn-1");
