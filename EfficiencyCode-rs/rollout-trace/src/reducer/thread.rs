...
use crate::model::EfficiencyCodeTurn;
...
impl TraceReducer {
    ...
    /// Starts a EfficiencyCode turn inside an existing thread.
    pub(super) fn start_EfficiencyCode_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self
            ...
            .contains_key(&EfficiencyCode_turn_id)
        {
            bail!("duplicate EfficiencyCode turn start for {EfficiencyCode_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.EfficiencyCode_turns.insert(
    ...
    /// Marks a EfficiencyCode turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_EfficiencyCode_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        {
            bail!(
                "EfficiencyCode turn end for {EfficiencyCode_turn_id} used thread {event_thread_id}, \
        ...
        let Some(turn) = self
            .rollout
            .EfficiencyCode_turns
            .get_mut(&EfficiencyCode_turn_id)
        else {
            bail!("EfficiencyCode turn end referenced unknown turn {EfficiencyCode_turn_id}");
        };
