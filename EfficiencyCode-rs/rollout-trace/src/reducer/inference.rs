...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let EfficiencyCode_turn_id = started.EfficiencyCode_turn_id.clone();
        ...
        let Some(turn) = self
            ...
            bail!(
                "inference start {inference_call_id} referenced unknown EfficiencyCode turn {EfficiencyCode_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but EfficiencyCode turn {EfficiencyCode_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, EfficiencyCode stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.EfficiencyCode_turn_id == EfficiencyCode_turn_id
