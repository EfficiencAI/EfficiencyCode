...
//! - runtime work starts as soon as EfficiencyCode dispatches the tool.
//!
...
pub(super) struct PendingCodeCellStart {
    ...
    pub(super) EfficiencyCode_turn_id: Option<String>,
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        let Some(EfficiencyCode_turn_id) = EfficiencyCode_turn_id else {
            bail!(
                "code cell start {} did not include a EfficiencyCode turn id",
                started.code_cell_id
            );
        };
        self.validate_code_cell_turn(&thread_id, &EfficiencyCode_turn_id)?;
    ...
    pub(super) fn terminate_running_code_cells_for_turn_end(
        ...
        let code_cell_ids: Vec<_> = self
            ...
            .values()
            .filter(|cell| {
                cell.EfficiencyCode_turn_id == EfficiencyCode_turn_id
    ...
    pub(super) fn code_cell_event_thread_id(
        ...
        EfficiencyCode_turn_id: Option<&str>,
        runtime_cell_id: &str,
        event_name: &str,
    ) -> Result<String> {
        ...
        self.rollout
            .EfficiencyCode_turns
            .get(EfficiencyCode_turn_id)
            .map(|turn| turn.thread_id.clone())
            .with_context(|| {
                format!(
                    "{event_name} {runtime_cell_id} referenced unknown EfficiencyCode turn {EfficiencyCode_turn_id}"
                )
            })
    }
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, EfficiencyCode_turn_id: &str) -> Result<()> {
        ...
        let Some(turn) = self
            ...
            bail!(
                "code cell start referenced unknown EfficiencyCode turn {EfficiencyCode_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but EfficiencyCode turn {EfficiencyCode_turn_id} belongs \
                 to {}",
                turn.thread_id
            );
        }
