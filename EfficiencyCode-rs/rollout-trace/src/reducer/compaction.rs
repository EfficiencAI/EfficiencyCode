...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self
            ...
            .get(&started.EfficiencyCode_turn_id)
        else {
            bail!(
                "compaction request {} referenced unknown EfficiencyCode turn {}",
                started.compaction_request_id,
                started.EfficiencyCode_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but EfficiencyCode turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.EfficiencyCode_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                EfficiencyCode_turn_id: started.EfficiencyCode_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self
            ...
            bail!(
                "compaction install {compaction_id} referenced unknown EfficiencyCode turn {EfficiencyCode_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but EfficiencyCode turn {EfficiencyCode_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) EfficiencyCode_turn_id: String,
