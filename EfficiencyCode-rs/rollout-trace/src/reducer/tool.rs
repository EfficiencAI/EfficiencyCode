...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        EfficiencyCode_turn_id: Option<String>,
        ...
        let thread_id = self.tool_thread_id(thread_id, EfficiencyCode_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, EfficiencyCode_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_EfficiencyCode_turn_id: EfficiencyCode_turn_id,
    ...
    fn tool_thread_id(
        ...
        let Some(EfficiencyCode_turn_id) = EfficiencyCode_turn_id else {
            bail!("tool call start did not include thread or EfficiencyCode turn context");
        };
        self.rollout
            ...
            .with_context(|| {
                format!("tool call start referenced unknown EfficiencyCode turn {EfficiencyCode_turn_id}")
            })
    }
    ...
    fn validate_tool_turn(
        ...
        if let Some(EfficiencyCode_turn_id) = EfficiencyCode_turn_id {
            let Some(turn) = self
                .rollout
                .EfficiencyCode_turns
                .get(EfficiencyCode_turn_id)
                ...
                bail!(
                    "tool call start referenced unknown EfficiencyCode turn {EfficiencyCode_turn_id}"
                );
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but EfficiencyCode turn {EfficiencyCode_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
