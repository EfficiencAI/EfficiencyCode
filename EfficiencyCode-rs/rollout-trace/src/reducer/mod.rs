    ...
    fn apply_event(&mut self, event: RawTraceEvent) -> Result<()> {
        ...
        match event.payload {
            ...
            RawTraceEventPayload::EfficiencyCodeTurnStarted {
                ...
                self.start_EfficiencyCode_turn(
                    event.seq,
                    event.wall_time_unix_ms,
                    EfficiencyCode_turn_id,
                    thread_id,
            ...
            RawTraceEventPayload::CodeCellStarted {
                ...
                let thread_id = self.code_cell_event_thread_id(
                    event.thread_id,
                    event.EfficiencyCode_turn_id.as_deref(),
                    &runtime_cell_id,
                    "code cell start",
                )?;
                ...
                self.start_or_queue_code_cell(PendingCodeCellStart {
                    ...
                    thread_id,
                    EfficiencyCode_turn_id: event.EfficiencyCode_turn_id,
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(EfficiencyCode_turn_id) = event.EfficiencyCode_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a EfficiencyCode turn id"
                    );
                };
