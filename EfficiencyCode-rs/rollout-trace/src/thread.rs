...
use crate::protocol_event::EfficiencyCode_turn_trace_event;
...
pub const EfficiencyCode_ROLLOUT_TRACE_ROOT_ENV: &str = "EfficiencyCode_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `EfficiencyCode_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the EfficiencyCode
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(EfficiencyCode_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed EfficiencyCode turn lifecycle events from protocol lifecycle events.
    pub fn record_EfficiencyCode_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            EfficiencyCode_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what EfficiencyCode did while executing that boundary.
    pub fn record_tool_call_event(
        &self,
        EfficiencyCode_turn_id: impl Into<EfficiencyCodeTurnId>,
        event: &EventMsg,
    ) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        ...
        context.append_with_context_best_effort(EfficiencyCode_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_EfficiencyCode_turn_id: impl Into<EfficiencyCodeTurnId>,
    ...
    pub fn record_EfficiencyCode_turn_started(
        &self,
        EfficiencyCode_turn_id: impl Into<EfficiencyCodeTurnId>,
    ) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
        ...
        let EfficiencyCode_turn_id = EfficiencyCode_turn_id.into();
        context.append_with_context_best_effort(
            EfficiencyCode_turn_id.clone(),
            RawTraceEventPayload::EfficiencyCodeTurnStarted {
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(EfficiencyCode_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one EfficiencyCode turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        EfficiencyCode_turn_id: EfficiencyCodeTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            EfficiencyCode_turn_id: Some(EfficiencyCode_turn_id),
        };
