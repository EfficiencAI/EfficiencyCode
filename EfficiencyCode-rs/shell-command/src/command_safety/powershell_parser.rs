...
impl PowershellParserProcess {
    fn spawn(executable: &str) -> std::io::Result<Self> {
        ...
        command
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::null());
        EfficiencyCode_protocol::shell_environment::scrub_non_inheritable_env_vars(&mut command);
...
mod tests {
    ...
    fn parser_process_rejects_stop_parsing_forms() {
        ...
        let parsed = parser
            .parse("git log --% HEAD --output=EfficiencyCode_poc.txt")
            .unwrap();
    ...
    fn parser_process_rejects_using_statements() {
        ...
        let parsed = parser
            .parse("using module ./EfficiencyCode_poc.psm1\nGet-Content Cargo.toml")
            .unwrap();
        assert_eq!(parsed, PowershellParseOutcome::Unsupported);
    }
