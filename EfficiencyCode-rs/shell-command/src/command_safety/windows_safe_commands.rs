...
mod tests {
    ...
    fn rejects_git_subcommand_options_with_side_effects() {
        let results: Vec<(&str, bool)> = [
            "git diff --output EfficiencyCode_poc.txt",
            "git diff --ext-diff HEAD",
            "git log --textconv -1",
            "git show --output=EfficiencyCode_poc.txt HEAD",
        ...
        assert_eq!(
            vec![
                ("git diff --output EfficiencyCode_poc.txt", false),
                ("git diff --ext-diff HEAD", false),
                ("git log --textconv -1", false),
                ("git show --output=EfficiencyCode_poc.txt HEAD", false),
    ...
    fn rejects_stop_parsing_git_forms() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "git log --% HEAD --output=EfficiencyCode_poc.txt",
        ])));
    }
    ...
    fn rejects_powershell_named_blocks() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "begin { Set-Content EfficiencyCode_poc.txt pwned } end { Get-Content Cargo.toml }",
        ])));
    }
    ...
    fn rejects_powershell_using_statements() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "using module ./EfficiencyCode_poc.psm1\nGet-Content Cargo.toml",
        ])));
    }
    ...
    fn rejects_powershell_trap_blocks() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "trap { Set-Content EfficiencyCode_poc.txt pwned; continue } Get-Content missing -ErrorAction Stop",
        ])));
    }
