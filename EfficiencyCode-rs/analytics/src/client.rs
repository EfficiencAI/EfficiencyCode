...
impl AnalyticsEventsDestination {
    ...
    fn from_base_url_and_capture_file(base_url: String, capture_file: Option<PathBuf>) -> Self {
        ...
        Self::Http {
            url: format!("{base_url}/EfficiencyCode/analytics-events/events"),
        }
    }
}
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::EfficiencyCodeCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: EfficiencyCodeGoalEvent) {
    ...
    pub fn track_turn_EfficiencyCode_error(&self, fact: TurnEfficiencyCodeErrorFact) {
        self.record_fact(AnalyticsFact::Custom(
            CustomAnalyticsFact::TurnEfficiencyCodeError(Box::new(fact)),
        ));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_EfficiencyCode_backend() {
        return;
    }
    if events.is_empty() {
        return;
