...
#[cfg(debug_assertions)]
use crate::events::EfficiencyCodePluginMetadata;
#[cfg(debug_assertions)]
use crate::events::EfficiencyCodePluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::EfficiencyCodePluginUsedMetadata;
#[cfg(debug_assertions)]
...
use EfficiencyCode_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
fn sample_artifact_operation_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::ArtifactOperation(EfficiencyCode_artifact_operation_event_request(
        TrackEventsContext {
            model_slug: "gpt-5.1-EfficiencyCode".to_string(),
            thread_id: thread_id.to_string(),
            turn_id: "turn-1".to_string(),
            product_client_id: "EfficiencyCode_desktop".to_string(),
        },
        ArtifactOperation {
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(EfficiencyCodeMcpToolCallEventRequest {
        ...
        event_params: EfficiencyCodeMcpToolCallEventParams {
            base: EfficiencyCodeToolItemEventBase {
                ...
                runtime: EfficiencyCodeRuntimeMetadata {
                    EfficiencyCode_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(EfficiencyCodePluginUsedEventRequest {
        event_type: "EfficiencyCode_plugin_used",
        event_params: EfficiencyCodePluginUsedMetadata {
            plugin: EfficiencyCodePluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                remote_plugin_id: None,
                ...
                product_client_id: Some("EfficiencyCode_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "EfficiencyCode-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://chatgpt.com/backend-api/EfficiencyCode/analytics-events/events"
                .to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = EfficiencyCode_login::EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "EfficiencyCode_accepted_line_fingerprints"
    );
...
#[tokio::test]
#[cfg(debug_assertions)]
async fn api_key_auth_sends_only_plugin_events_to_EfficiencyCode_backend() {
    ...
    let auth_manager = EfficiencyCode_login::AuthManager::from_auth_for_testing(
        EfficiencyCode_login::EfficiencyCodeAuth::from_api_key("sk-test"),
    );
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = EfficiencyCode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn flush_is_noop_when_analytics_is_disabled() {
    let client = AnalyticsEventsClient::new(
        EfficiencyCode_login::AuthManager::from_auth_for_testing(
