...
use EfficiencyCode_skills::LoadedSkills;
use EfficiencyCode_skills::SkillError;
use EfficiencyCode_skills::SkillLoadFuture;
...
use EfficiencyCode_skills::parse_skill_frontmatter_metadata;
...
pub(crate) fn test_plugins_manager(EfficiencyCode_home: PathBuf) -> PluginsManager {
    PluginsManager::new(EfficiencyCode_home, test_skill_root_loader())
}
...
pub(crate) fn write_curated_plugin_sha_with(EfficiencyCode_home: &Path, sha: &str) {
    write_file(
...
pub(crate) async fn load_plugins_config(
    EfficiencyCode_home: &Path,
    cwd: &Path,
) -> PluginsConfigInput {
    let EfficiencyCode_home = AbsolutePathBuf::try_from(EfficiencyCode_home)
        .expect("EfficiencyCode home should be absolute");
