...
#[test]
fn legacy_EfficiencyCode_overlay_keeps_portable_components_fixed() {
    let tmp = tempdir().expect("tempdir");
    let plugin_root = tmp.path().join("demo-plugin");
    write_agent_plugin_manifest(
        &plugin_root,
  ...
  "description": "Portable description""#,
    ...
    fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{
  "name": "different-name",
  "version": "9.9.9",
  "description": "EfficiencyCode description",
  "skills": [],
  "mcpServers": null,
  "interface": {"displayName": "EfficiencyCode Demo"}
}"#,
    )
    ...
    assert_eq!(
        ...
        Some("EfficiencyCode Demo".to_string())
    );
}
...
fn inline_openai_extension_precedes_legacy_overlay() {
  ...
  "extensions": {
    "com.openai": {
      "interface": {"displayName": "Inline EfficiencyCode"}
    }
  }"#,
    );
    fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{"interface":{"displayName":"Legacy EfficiencyCode"}}"#,
    )
    .expect("write overlay");
    ...
    assert_eq!(
        manifest
            .interface
            .and_then(|interface| interface.display_name),
        Some("Inline EfficiencyCode".to_string())
    );
}
