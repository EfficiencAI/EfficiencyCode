...
const REMOTE_PLUGIN_INSTALL_METADATA_FILE: &str = ".EfficiencyCode-remote-plugin-install.json";
...
impl PluginStore {
    pub fn new(EfficiencyCode_home: PathBuf) -> Self {
        Self::try_new(EfficiencyCode_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(EfficiencyCode_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(
            EfficiencyCode_home.join(PLUGINS_CACHE_DIR),
        )
        .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(EfficiencyCode_home.join(PLUGINS_DATA_DIR))
        ...
        let EfficiencyCode_home = AbsolutePathBuf::from_absolute_path_checked(EfficiencyCode_home)
            .map_err(|err| PluginStoreError::io("failed to resolve EfficiencyCode home", err))?;
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".EfficiencyCode-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".EfficiencyCode-plugin/plugin.json");
