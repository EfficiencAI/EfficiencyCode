...
use EfficiencyCode_http_client::RouteAwareRequestError;
...
const EfficiencyCode_PRODUCT_SKU: &str = "EfficiencyCode";
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(
        EfficiencyCode_home,
        config,
        auth,
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(EfficiencyCode_home, config, auth, *scope);
    }
}
...
pub(crate) fn has_fresh_cached_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_directory_plugins(EfficiencyCode_home, config, auth, scope)
        .is_some_and(|cached| {
            matches!(
                cached.freshness,
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(EfficiencyCode_home.clone())
    ...
    let legacy_remote_plugin_cache_root = EfficiencyCode_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode == RemotePluginCatalogCacheMode::PreferCache
        && let Some(EfficiencyCode_home) = EfficiencyCode_home
        && let Some(cached) =
            catalog_cache::load_cached_directory_plugins(EfficiencyCode_home, config, auth, scope)
    {
...
fn ensure_chatgpt_auth(
    auth: Option<&EfficiencyCodeAuth>,
) -> Result<&EfficiencyCodeAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    request
        ...
        .headers(EfficiencyCode_model_provider::auth_provider_from_auth(auth).to_auth_headers())
        .header(OAI_PRODUCT_SKU_HEADER, EfficiencyCode_PRODUCT_SKU)
}
