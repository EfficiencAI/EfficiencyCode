...
use EfficiencyCode_config::RemoveMarketplaceConfigOutcome;
use EfficiencyCode_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || {
        remove_marketplace_sync(EfficiencyCode_home.as_path(), request)
    })
    .await
    .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let destination = marketplace_install_root(EfficiencyCode_home).join(&marketplace_name);
    let config_outcome = remove_user_marketplace_config(EfficiencyCode_home, &marketplace_name)
        .map_err(|err| {
