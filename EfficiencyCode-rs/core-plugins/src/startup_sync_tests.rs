...
fn git_command_sanitizes_ambient_repository_environment() {
    ...
    assert_eq!(
        ...
        [
            OsStr::new("-c"),
            OsStr::new(EfficiencyCode_git_utils::SAFE_BARE_REPOSITORY_CONFIG),
        ]
    );
...
async fn ordinary_clone_rejects_tracked_embedded_bare_repository() {
    ...
    run_setup_git(
        ...
        &[
            "-c",
            "user.name=EfficiencyCode Tests",
            "-c",
            "user.email=EfficiencyCode-tests@example.com",
            "commit",
            "--quiet",
            "-m",
    ...
    let apply_error =
        EfficiencyCode_git_utils::apply_git_patch(&EfficiencyCode_git_utils::ApplyGitRequest {
            cwd: nested.clone(),
            diff: String::new(),
            revert: false,
            preflight: true,
    ...
    assert!(
        EfficiencyCode_git_utils::collect_git_info(&nested)
            .await
            .is_none()
    );
    assert!(
        EfficiencyCode_git_utils::git_diff_to_remote(&nested)
            .await
            .is_none()
    );
    assert!(
...
fn write_curated_plugin_sha(EfficiencyCode_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(EfficiencyCode_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(EfficiencyCode_home.join(".tmp")) else {
        return false;
    };
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.EfficiencyCode-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_EfficiencyCode_home_tmp_dir() {
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/.agents/plugins" "$2/plugins/gmail/.EfficiencyCode-plugin"
  cat > "$2/.agents/plugins/marketplace.json" <<'EOF'
{{"name":"openai-curated","plugins":[{{"name":"gmail","source":{{"source":"local","path":"./plugins/gmail"}}}}]}}
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$2/plugins/gmail/.EfficiencyCode-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.EfficiencyCode-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    run_git(
        ...
        &[
            "-c",
            "user.name=EfficiencyCode Test",
            "-c",
            "user.email=EfficiencyCode@example.com",
            "commit",
            "-m",
            "init",
        ],
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.EfficiencyCode-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.EfficiencyCode-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.EfficiencyCode-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path =
        curated_root.join("plugins/linear/.EfficiencyCode-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.EfficiencyCode-plugin/plugin.json"),
            options,
        )
...
fn curated_repo_backup_archive_zip_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            "plugins/plugins/gmail/.EfficiencyCode-plugin/plugin.json",
            options,
        )
        .expect("start plugin manifest entry");
    writer
