...
use EfficiencyCode_config::MarketplaceAllowedSourceKind;
use EfficiencyCode_config::MarketplaceAllowedSourceToml;
use EfficiencyCode_config::RequirementSource;
use EfficiencyCode_config::types::MarketplaceConfig;
use EfficiencyCode_config::types::MarketplaceSourceType;
...
use EfficiencyCode_utils_path::paths_match_after_normalization;
use regex::Regex;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) =
            managed_marketplace_name(EfficiencyCode_home, marketplace_path, &root)
        {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(EfficiencyCode_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn project_effective_user_config(
    ...
    let allowed_marketplace_names = allowed_configured_marketplace_names_with_policy(
        &user_config,
        &policy,
        EfficiencyCode_home,
    );
...
pub fn allowed_configured_marketplace_names(
    ...
    let policy = MarketplacePolicy::from_requirements(config_layer_stack.requirements());
    allowed_configured_marketplace_names_with_policy(&user_config, &policy, EfficiencyCode_home)
}
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(EfficiencyCode_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(user_config) = project_effective_user_config(config_layer_stack, EfficiencyCode_home)
    else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(EfficiencyCode_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if paths_match_after_normalization(
        ...
        curated_plugins_api_marketplace_path(EfficiencyCode_home),
    ) {
    ...
    managed_local_marketplace_name(EfficiencyCode_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(EfficiencyCode_home, path)
}

fn managed_local_marketplace_name(EfficiencyCode_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = EfficiencyCode_home
...
pub(crate) fn primary_runtime_marketplace_root() -> Option<PathBuf> {
    Some(
        primary_runtime_cache_dir()?
            .join("EfficiencyCode-runtimes/EfficiencyCode-primary-runtime/plugins")
