...
fn config_layer_stack_with_user_config(
    ...
    let requirements = EfficiencyCode_config::ConfigRequirements::try_from(with_sources)
...
fn strict_install_validates_configured_name_source_and_root() {
    ...
    let config_file = AbsolutePathBuf::try_from(EfficiencyCode_home.path().join("config.toml"))
...
"#
    ...
    assert!(
        policy
            .validate_install(&stack, EfficiencyCode_home.path(), &other_path, "company")
...
"#,
    ...
    let marketplace_path = AbsolutePathBuf::try_from(
        curated_plugins_repo_path(EfficiencyCode_home.path())
...
"#,
    ...
    let expected_name = validate_marketplace_source_for_add(
        EfficiencyCode_home.path(),
        stack.requirements(),
        &source,
    )
...
"#,
    ...
    let projected = project_effective_user_config(&stack, EfficiencyCode_home.path())
    ...
    assert_eq!(
        configured_plugins_from_stack(&stack, EfficiencyCode_home.path())
            .into_keys()
            .collect::<Vec<_>>(),
...
"#,
    ...
    let outcome =
        upgrade_configured_git_marketplaces(EfficiencyCode_home.path(), &stack, Some("debug"));
