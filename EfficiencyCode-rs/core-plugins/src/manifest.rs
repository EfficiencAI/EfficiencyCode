...
use EfficiencyCode_utils_path_uri::PathConvention;
use EfficiencyCode_utils_path_uri::PathUri;
use EfficiencyCode_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
...
pub type PluginManifest = EfficiencyCode_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks =
    EfficiencyCode_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface =
    EfficiencyCode_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    EfficiencyCode_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths =
    EfficiencyCode_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = EfficiencyCode_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn load_plugin_manifest_with_format(plugin_root: &Path) -> Option<LoadedPluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let overlay_path = plugin_root.join(".EfficiencyCode-plugin/plugin.json");
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = EfficiencyCode_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(EfficiencyCode_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: EfficiencyCode_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<EfficiencyCode_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| EfficiencyCode_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            ...
            (!hooks.is_empty()).then_some(
                EfficiencyCode_plugin::manifest::PluginManifestHooks::Paths(hooks),
            )
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(EfficiencyCode_plugin::manifest::PluginManifestHooks::Inline(vec![*hooks]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(EfficiencyCode_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<EfficiencyCode_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(EfficiencyCode_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => {
                Some(EfficiencyCode_plugin::manifest::PluginManifestMcpServers::Object(servers))
            }
            Err(err) => {
...
mod tests {
    ...
    use EfficiencyCode_plugin::PluginProvider;
    use EfficiencyCode_plugin::ResolvedPlugin;
    use EfficiencyCode_plugin::manifest::PluginManifest as GenericPluginManifest;
    use EfficiencyCode_plugin::manifest::PluginManifestHooks;
    use EfficiencyCode_plugin::manifest::PluginManifestInterface;
    use EfficiencyCode_plugin::manifest::PluginManifestMcpServers;
    use EfficiencyCode_plugin::manifest::PluginManifestPaths;
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".EfficiencyCode-plugin/plugin.json"))
                .expect("manifest");
