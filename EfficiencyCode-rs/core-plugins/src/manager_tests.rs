...
use EfficiencyCode_config::McpServerOAuthConfig;
...
use EfficiencyCode_config::SkillConfigRules;
...
use EfficiencyCode_plugin::AppDeclaration;
...
fn config_layer_stack_with_requirements(
    ...
    let config_file = AbsolutePathBuf::try_from(EfficiencyCode_home.join(CONFIG_TOML_FILE))
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(EfficiencyCode_home, user_config, requirements),
        String::new(),
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let EfficiencyCode_home = TempDir::new().expect("create EfficiencyCode home");
    write_plugin(
        &EfficiencyCode_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = test_plugins_manager(EfficiencyCode_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let EfficiencyCode_home = TempDir::new().expect("create EfficiencyCode home");
    let marketplace_root = EfficiencyCode_home.path().join("marketplace");
...
"#,
    ...
    let err = test_plugins_manager(EfficiencyCode_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let EfficiencyCode_home = TempDir::new().expect("create EfficiencyCode home");
    let repo_root = EfficiencyCode_home.path().join("repo");
...
fn write_auth_projection_plugin(EfficiencyCode_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(EfficiencyCode_home, name, name);
    }
}

fn write_auth_projection_app(EfficiencyCode_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = EfficiencyCode_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(EfficiencyCode_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&EfficiencyCode_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(EfficiencyCode_home, EfficiencyCode_home).await
}
...
async fn plugin_auth_projection_hides_apps_without_chatgpt_auth() {
    ...
    let config = auth_projection_config(EfficiencyCode_home.path()).await;
...
async fn plugin_auth_projection_keeps_non_conflicting_mcp_with_chatgpt_apps_route() {
    ...
    write_auth_projection_app(EfficiencyCode_home.path(), "sample", "sample_app");
    write_auth_projection_plugin(
        EfficiencyCode_home.path(),
        "docs",
        /*include_app*/ false,
...
"#,
    ...
    let config = load_config(EfficiencyCode_home.path(), EfficiencyCode_home.path()).await;
    let manager = test_plugins_manager_with_options(
        EfficiencyCode_home.path().to_path_buf(),
        Some(Product::EfficiencyCode),
        Some(AuthMode::Chatgpt),
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    ...
    run_git(
        repo,
        &["config", "user.email", "EfficiencyCode-test@example.com"],
    );
    run_git(repo, &["config", "user.name", "EfficiencyCode Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    write_file(&EfficiencyCode_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(EfficiencyCode_home, EfficiencyCode_home).await;
...
async fn load_config(EfficiencyCode_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(EfficiencyCode_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        installed_at: None,
        enabled: true,
        install_policy: EfficiencyCode_app_server_protocol::PluginInstallPolicy::Available,
        install_policy_source: None,
        must_show_installation_interstitial: None,
        auth_policy: EfficiencyCode_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: EfficiencyCode_app_server_protocol::PluginAvailability::Available,
...
fn write_cached_plugin(EfficiencyCode_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &EfficiencyCode_home
            .join("plugins/cache")
...
async fn installed_plugin_telemetry_metadata_collects_capabilities() {
    let EfficiencyCode_home = TempDir::new().unwrap();
    write_cached_plugin(EfficiencyCode_home.path(), "test", "sample");
    let manager = test_plugins_manager(EfficiencyCode_home.path().to_path_buf());
...
async fn installed_plugin_telemetry_metadata_resolves_persisted_remote_identity() {
    ...
    PluginStore::new(EfficiencyCode_home.path().to_path_buf())
...
"#,
    ...
    write_cached_plugin(EfficiencyCode_home.path(), "openai-curated", "linear");
    write_cached_plugin(EfficiencyCode_home.path(), "openai-curated", "calendar");
    write_cached_plugin(EfficiencyCode_home.path(), "openai-api-curated", "linear");
    write_cached_plugin(
        EfficiencyCode_home.path(),
        "openai-curated-remote",
        "linear",
...
"#,
    ...
    let mut config = load_config(EfficiencyCode_home.path(), EfficiencyCode_home.path()).await;
    let manager = test_plugins_manager_with_options(
        EfficiencyCode_home.path().to_path_buf(),
        Some(Product::EfficiencyCode),
        Some(AuthMode::Chatgpt),
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy =
        EfficiencyCode_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = EfficiencyCode_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(EfficiencyCode_app_server_protocol::PluginInterface {
    ...
    assert_eq!(
        plugin.auth_policy,
        EfficiencyCode_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn install_plugin_materializes_default_command_skills() {
    let EfficiencyCode_home = TempDir::new().unwrap();
    let source_root = EfficiencyCode_home.path().join("source/sample");

    write_file(
        &source_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root
            .join(".EfficiencyCode-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
        "---\nname: undeclared-command\ndescription: undeclared command\n---\n",
    );
    let result = PluginStore::new(EfficiencyCode_home.path().to_path_buf())
        .install(
            source_root.abs(),
            PluginId::parse("sample@test").expect("plugin id should parse"),
        )
    ...
    let migrated_skill = result
        .installed_path
        .join(".EfficiencyCode-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".EfficiencyCode-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".EfficiencyCode-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
        ...
        vec![
            ...
            AbsolutePathBuf::from_absolute_path_checked(
                fs::canonicalize(result.installed_path.join(
                    ".EfficiencyCode-plugin/migrated-command-skills/source-command-summarize/SKILL.md"
                ))
                .unwrap()
            )
            .unwrap()
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".EfficiencyCode-plugin/migrated-command-skills")
            .exists()
    );
}
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let EfficiencyCode_home = TempDir::new().unwrap();
    let plugin_a_root = EfficiencyCode_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = EfficiencyCode_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(EfficiencyCode_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        skill_discovery_mode: SkillDiscoveryMode::Recursive,
        disabled_skill_paths: HashSet::new(),
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(EfficiencyCode_home.path(), EfficiencyCode_home.path()).await;
    let outcome = test_plugins_manager(EfficiencyCode_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
  ...
  "mcpServers": {
    ...
    let user_file = EfficiencyCode_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let EfficiencyCode_home = TempDir::new().expect("create EfficiencyCode home");
    let marketplace_root = EfficiencyCode_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config =
        config_layer_stack_with_requirements(EfficiencyCode_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = test_plugins_manager(EfficiencyCode_home.path().to_path_buf());
    ...
    assert!(!EfficiencyCode_home.path().join(CONFIG_TOML_FILE).exists());
...
"#
    ...
    let config = config_layer_stack_with_requirements(
        EfficiencyCode_home.path(),
        &user_config,
        &requirements,
    );
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(
        !plugin_root
            .join(".EfficiencyCode-plugin/plugin.json")
            .exists()
    );
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".EfficiencyCode-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path.join(
                ".EfficiencyCode-plugin/migrated-command-skills/source-command-review/SKILL.md"
            )
        )
        .unwrap(),
...
#[tokio::test]
async fn read_plugin_for_config_filters_mcp_servers_for_EfficiencyCode_backend_auth() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_root = tmp.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).unwrap();
    fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.EfficiencyCode-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".EfficiencyCode-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".EfficiencyCode/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.EfficiencyCode-plugin/plugin.json"),
...
"#,
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing()),
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_EfficiencyCode() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "EfficiencyCode"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["EfficiencyCode-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(
        featured_plugin_ids,
        vec!["EfficiencyCode-plugin".to_string()]
    );
}
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer Access Token"))
        .and(header("chatgpt-account-id", "account_id"))
        .and(header("OAI-Product-Sku", "EfficiencyCode"))
        .respond_with(
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_EfficiencyCode_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_EfficiencyCode_home(tmp.path())
            .into_iter()
...
async fn load_plugins_ignores_project_config_files() {
    let EfficiencyCode_home = TempDir::new().unwrap();
    let project_root = EfficiencyCode_home.path().join("project");
    let plugin_root = EfficiencyCode_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".EfficiencyCode/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_EfficiencyCode_folder: AbsolutePathBuf::try_from(
...
"#,
    ...
    for marketplace_name in [
        ...
        write_cached_plugin(EfficiencyCode_home.path(), marketplace_name, "linear");
        write_file(
            &EfficiencyCode_home
                .path()
                .join("plugins/cache")
