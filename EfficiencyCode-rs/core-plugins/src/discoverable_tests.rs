...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_EfficiencyCode_auth() {
    let EfficiencyCode_home = tempdir().expect("tempdir should succeed");
    write_file(
...
"#,
    ...
    let curated_root = curated_plugins_repo_path(EfficiencyCode_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack", "openai-developers"]);

    let plugins = load_plugins_config(EfficiencyCode_home.path(), EfficiencyCode_home.path()).await;
    let plugins_manager = test_plugins_manager(EfficiencyCode_home.path().to_path_buf());
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = EfficiencyCodeAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(EfficiencyCode_home.path(), curated_root.as_path(), "teams").await;
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_EfficiencyCode_auth() {
    let EfficiencyCode_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(EfficiencyCode_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = EfficiencyCode_home
        .path()
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(EfficiencyCode_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.EfficiencyCode-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.EfficiencyCode-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(EfficiencyCode_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(EfficiencyCode_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!(
                "plugins/{plugin_name}/.EfficiencyCode-plugin/plugin.json"
            )),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        logs.matches("gmail/.EfficiencyCode-plugin/plugin.json")
            .count(),
        4
    );
    assert_eq!(
        logs.matches("openai-developers/.EfficiencyCode-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = EfficiencyCode_home
        .path()
...
"#,
    ...
    let auth = EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins =
        load_plugins_config(EfficiencyCode_home.path(), EfficiencyCode_home.path()).await;
...
async fn install_marketplace_plugin(
    ...
    write_curated_plugin_sha_with(EfficiencyCode_home, TEST_CURATED_PLUGIN_SHA);
    let config = load_plugins_config(EfficiencyCode_home, marketplace_root).await;
    test_plugins_manager(EfficiencyCode_home.to_path_buf())
        .install_plugin(
