...
use EfficiencyCode_shell_command::bash::extract_bash_command;
use EfficiencyCode_shell_command::bash::parse_shell_lc_plain_commands;
use EfficiencyCode_shell_command::parse_command::is_pathish;
...
impl TrustedPluginRoots {
    pub fn from_plugin_load_outcome(
        ...
        let Ok(store) = PluginStore::try_new(EfficiencyCode_home.to_path_buf()) else {
            return Self::default();
        };
    ...
    fn expected_plugin_root(
        ...
        match plugin_id.marketplace_name.as_str() {
            ...
            OPENAI_CURATED_MARKETPLACE_NAME | OPENAI_API_CURATED_MARKETPLACE_NAME => {
                let curated_sha = read_curated_plugins_sha(EfficiencyCode_home)?;
                let expected_root =
