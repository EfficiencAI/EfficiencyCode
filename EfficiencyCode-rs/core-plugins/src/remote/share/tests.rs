...
fn test_auth() -> EfficiencyCodeAuth {
...
async fn save_remote_plugin_share_creates_workspace_plugin() {
    ...
    assert_eq!(
        local_paths::load_plugin_share_local_paths(EfficiencyCode_home.path()).unwrap(),
    ...
    assert_eq!(
        archive_files
            .get(".EfficiencyCode-plugin/plugin.json")
            .map(Vec::as_slice),
...
fn archive_plugin_for_upload_places_manifest_at_archive_root() {
    ...
    assert_eq!(
        ...
        vec![
            ".EfficiencyCode-plugin/plugin.json".to_string(),
...
async fn list_remote_plugin_shares_fetches_created_workspace_plugins() {
    ...
    let local_plugin_path =
        AbsolutePathBuf::try_from(EfficiencyCode_home.path().join("local-plugin")).unwrap();
    ...
    let result = list_remote_plugin_shares(&config, Some(&auth), EfficiencyCode_home.path())
        .await
        .unwrap();
