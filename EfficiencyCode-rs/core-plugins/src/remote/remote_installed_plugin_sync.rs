...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&EfficiencyCode_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(EfficiencyCode_home, &config, Some(&auth))
                .await;
        match result {
            Ok(outcome) => {
                info!(
...
fn remote_plugin_cache_root(EfficiencyCode_home: &Path) -> PathBuf {
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(EfficiencyCode_home.path()),
        };
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let EfficiencyCode_home = tempfile::tempdir().expect("create EfficiencyCode home");
        let cached_manifest = EfficiencyCode_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        let metadata_path = PluginStore::new(EfficiencyCode_home.path().to_path_buf())
    ...
    async fn sync_all_scopes_paginates_and_reconciles_each_marketplace() {
        ...
        for (marketplace_name, plugin_name, _, _) in cached_plugins {
            for cached_plugin_name in [plugin_name, "stale"] {
                let manifest = EfficiencyCode_home
                    .path()
                    .join(PLUGINS_CACHE_DIR)
                    .join(marketplace_name)
        ...
        for (marketplace_name, plugin_name, _, _) in cached_plugins {
            ...
            assert!(
                plugin_root
                    .join("1.2.3/.EfficiencyCode-plugin/plugin.json")
                    .is_file()
            );
            assert_eq!(
                serde_json::from_str::<serde_json::Value>(
                    &std::fs::read_to_string(
                        plugin_root.join(".EfficiencyCode-remote-plugin-install.json")
                    )
                    .expect("read remote plugin install metadata")
                )
                .expect("parse remote plugin install metadata"),
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let EfficiencyCode_home = tempfile::tempdir().expect("create EfficiencyCode home");
        let created_by_me_cached_manifest = EfficiencyCode_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = EfficiencyCode_home
            .path()
            .join(PLUGINS_CACHE_DIR)
