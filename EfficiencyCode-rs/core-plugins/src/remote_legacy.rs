...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    url.query_pairs_mut().append_pair(
        "platform",
        product.unwrap_or(Product::EfficiencyCode).to_app_platform(),
    );
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_EfficiencyCode_backend()) {
...
fn ensure_EfficiencyCode_backend_auth(
    auth: Option<&EfficiencyCodeAuth>,
) -> Result<&EfficiencyCodeAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_EfficiencyCode_backend_auth(auth)?;
