...
use crate::loader::configured_curated_plugin_ids_from_EfficiencyCode_home;
...
use EfficiencyCode_config::clear_user_plugin;
use EfficiencyCode_config::set_user_plugin_enabled;
use EfficiencyCode_config::skill_config_rules_from_stack;
use EfficiencyCode_config::types::PluginConfig;
use EfficiencyCode_config::types::ToolSuggestDisabledTool;
use EfficiencyCode_config::types::ToolSuggestDiscoverableType;
use EfficiencyCode_hooks::plugin_hook_declarations;
...
use EfficiencyCode_model_provider::AMAZON_BEDROCK_PROVIDER_ID;
...
use EfficiencyCode_plugin::PluginIdError;
use EfficiencyCode_plugin::PluginTelemetryMetadata;
use EfficiencyCode_plugin::app_connector_ids_from_declarations;
use EfficiencyCode_plugin::prompt_safe_plugin_description;
...
use EfficiencyCode_skills::SkillRootLoader;
use EfficiencyCode_skills::SkillRootSnapshots;
use EfficiencyCode_tools::DiscoverablePluginInfo;
...
use EfficiencyCode_utils_plugins::PluginIdentity;
use EfficiencyCode_utils_plugins::PluginSkillRoot;
...
pub struct RecommendedPluginCandidatesInput<'a> {
    ...
    pub auth: Option<&'a EfficiencyCodeAuth>,
    ...
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            ...
            && self
                .auth
                .as_ref()
                .and_then(EfficiencyCodeAuth::get_chatgpt_user_id)
                == other
                    .auth
                    .as_ref()
                    .and_then(EfficiencyCodeAuth::get_chatgpt_user_id)
            && self
                .auth
                .as_ref()
                .map(EfficiencyCodeAuth::is_workspace_account)
                == other
                    .auth
                    .as_ref()
                    .map(EfficiencyCodeAuth::is_workspace_account)
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(EfficiencyCodeAuth::get_account_id),
        chatgpt_user_id: auth.and_then(EfficiencyCodeAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(EfficiencyCodeAuth::is_workspace_account),
    }
}
...
fn target_curated_marketplace(
    ...
    match auth_mode {
        Some(auth_mode) => {
            if auth_mode.uses_EfficiencyCode_backend() {
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given EfficiencyCode_HOME:
        ...
        // contents of that EfficiencyCode_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single EfficiencyCode_HOME is only used by one product.
        Self {
            EfficiencyCode_home: EfficiencyCode_home.clone(),
            store: PluginStore::new(EfficiencyCode_home),
    ...
    pub fn plugin_skill_snapshots_for_config(
        ...
        let key = PluginLoadCacheKey::from_config(
            config,
            self.EfficiencyCode_home.as_path(),
    ...
    pub fn maybe_start_remote_installed_plugin_bundle_sync(
        ...
        crate::remote::maybe_start_remote_installed_plugin_bundle_sync(
            self.EfficiencyCode_home.clone(),
            remote_plugin_service_config(config),
            auth,
    ...
    async fn install_resolved_plugin(
        ...
        let EfficiencyCode_home = self.EfficiencyCode_home.clone();
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.EfficiencyCode_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(EfficiencyCode_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let loaded_manifest =
            if EfficiencyCode_utils_plugins::find_plugin_manifest_path(source_path.as_path())
                .is_some()
            {
        ...
        if config.plugins_enabled {
            ...
            tokio::spawn(async move {
                ...
                let mut scopes = crate::remote::cached_remote_plugin_catalog_scopes(
                    manager.EfficiencyCode_home.as_path(),
    ...
    fn start_curated_repo_sync(
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(EfficiencyCode_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_EfficiencyCode_home(
                                EfficiencyCode_home.as_path(),
                            );
            ...
            match target_curated_marketplace(self.auth_mode(), &config.model_provider_id) {
                TargetCuratedMarketplace::OpenAi | TargetCuratedMarketplace::OpenAiWithRemote => {
                    let curated_repo_root =
                        curated_plugins_repo_path(self.EfficiencyCode_home.as_path());
                    curated_repo_root.is_dir().then_some(curated_repo_root)
                }
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.EfficiencyCode_home.as_path());
