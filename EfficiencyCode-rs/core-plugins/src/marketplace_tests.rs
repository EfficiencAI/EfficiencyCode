    ...
    assert_eq!(
        ...
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new(
                ...
                "EfficiencyCode-curated".to_string()
            )
            .unwrap(),
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "npm",
        "package": "@acme/EfficiencyCode-plugin",
        "version": "^1.2.0",
    ...
    assert_eq!(
        ...
        ResolvedMarketplacePlugin {
            ...
            source: MarketplacePluginSource::Npm {
                package: "@acme/EfficiencyCode-plugin".to_string(),
...
fn find_marketplace_plugin_skips_unsafe_npm_sources() {
  ...
  "plugins": [
    ...
    },
    {
      ...
      "source": {
        "source": "npm",
        "package": "_EfficiencyCode-plugin",
        "registry": "https://npm.example.com"
      }
    }
  ]
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `remote-plugin` was not found in marketplace `EfficiencyCode-curated`"
        );
    }
}
...
fn find_marketplace_plugin_reports_missing_plugin() {
    ...
    fs::write(
        repo_root.join(".agents/plugins/marketplace.json"),
        r#"{"name":"EfficiencyCode-curated","plugins":[]}"#,
    )
    .unwrap();
    ...
    assert_eq!(
        err.to_string(),
        "plugin `missing` was not found in marketplace `EfficiencyCode-curated`"
    );
}
...
fn list_marketplaces_supports_repo_root_local_plugin_sources() {
    for path in [".", "./"] {
        ...
        fs::create_dir_all(repo_root.join(".git")).unwrap();
        fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
        fs::create_dir_all(repo_root.join(".EfficiencyCode-plugin")).unwrap();
        fs::write(
            repo_root.join(".agents/plugins/marketplace.json"),
            format!(
                r#"{{
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        .unwrap();
        fs::write(
            repo_root.join(".EfficiencyCode-plugin/plugin.json"),
            r#"{
  "name":"repo-root-plugin",
  "interface": {
...
fn list_marketplaces_returns_home_and_repo_marketplaces() {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        vec![
            Marketplace {
                name: "EfficiencyCode-curated".to_string(),
                path:
  ...
  "plugins": [
    {
      ...
      "policy": {
        ...
        "products": ["EfficiencyCode", "CHATGPT", "ATLAS"]
      },
      "category": "Design"
    }
  ]
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `local-plugin` was not found in marketplace `EfficiencyCode-curated`"
        );
    }
}
...
fn find_installable_marketplace_plugin_rejects_disallowed_product() {
  ...
  "plugins": [
    ...
    assert_eq!(
        err.to_string(),
        "plugin `chatgpt-plugin` is not available for install in marketplace `EfficiencyCode-curated`"
    );
}
...
fn find_marketplace_plugin_allows_missing_products_field() {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        "default-plugin@EfficiencyCode-curated"
    );
}
...
fn find_installable_marketplace_plugin_rejects_explicit_empty_products() {
  ...
  "plugins": [
    ...
    assert_eq!(
        err.to_string(),
        "plugin `disabled-plugin` is not available for install in marketplace `EfficiencyCode-curated`"
    );
}
