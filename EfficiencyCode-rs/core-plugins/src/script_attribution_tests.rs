...
use EfficiencyCode_plugin::PluginLoadOutcome;
...
fn synced_plugin_root(
    ...
    let synced_root = curated_plugins_repo_path(EfficiencyCode_home);
    match marketplace {
    ...
    let root = PluginStore::new(EfficiencyCode_home.to_path_buf()).plugin_root(
        &plugin_id,
...
fn cached_remote_plugin_root(EfficiencyCode_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    let root = PluginStore::new(EfficiencyCode_home.to_path_buf()).plugin_root(&plugin_id, "1.2.3");
    fs::create_dir_all(root.as_path()).expect("create cached remote plugin root");
    root
}
fn installed_remote_plugin_root(EfficiencyCode_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let root = cached_remote_plugin_root(EfficiencyCode_home, plugin_name);
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    PluginStore::new(EfficiencyCode_home.to_path_buf())
...
fn resolves_primary_runtime_scripts_from_the_installed_plugin_cache() {
    ...
    fs::create_dir_all(source_root.join(".EfficiencyCode-plugin"))
...
fn roots_for(EfficiencyCode_home: &Path, plugins: Vec<LoadedPlugin>) -> TrustedPluginRoots {
...
async fn resolves_relocated_script_through_executor_filesystem() {
    ...
    let environment =
        EfficiencyCode_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
fn recognizes_windows_executor_plugin_cache_root() {
    ...
    let script = PathUri::parse(
        "file:///C:/Users/user/.EfficiencyCode/plugins/cache/openai-primary-runtime/presentations/0.1.29/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
    .expect("Windows script URI");

    assert!(executor_plugin_root_matches(&script, &attribution));
}
fn assert_untrusted(EfficiencyCode_home: &Path, config_name: &str, root: &Path) {
    assert!(
        roots_for(
            EfficiencyCode_home,
