...
pub enum Feature {
    ...
    Chronicle,
    /// Compress request bodies (zstd) when sending streaming requests to EfficiencyCode-backend.
    EnableRequestCompression,
    /// Start the managed network proxy for sandboxed sessions.
    NetworkProxy,
    /// Respect host system proxy settings for EfficiencyCode-owned network clients.
    RespectSystemProxy,
    /// Enable collab tools.
    Collab,
    ...
    /// Requirements-only gate: this should be set from requirements, not user config.
    BrowserUseExternal,
    /// Allow EfficiencyCode Computer Use.
    ///
    /// Requirements-only gate: this should be set from requirements, not user config.
    ComputerUse,
    ...
    /// Prompt EfficiencyCode Apps connector auth failures through MCP URL elicitations.
    AuthElicitation,
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.openai.com/EfficiencyCode/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        id: Feature::EfficiencyCodeGitCommit,
        key: "EfficiencyCode_git_commit",
        stage: Stage::Removed,
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        id: Feature::EfficiencyCodeHooks,
        key: "hooks",
        stage: Stage::Stable,
        default_enabled: true,
    },
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: Network proxy can now be enabled from /experimental. Restart EfficiencyCode after enabling it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while EfficiencyCode is running a thread.",
