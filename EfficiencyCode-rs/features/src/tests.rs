...
#[test]
fn EfficiencyCode_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::EfficiencyCodeHooks));
    assert_eq!(
        feature_for_key("EfficiencyCode_hooks"),
        Some(Feature::EfficiencyCodeHooks)
    );
}
