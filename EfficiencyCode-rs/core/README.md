...
This crate implements the business logic for EfficiencyCode. It is designed to be used by the various EfficiencyCode UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //EfficiencyCode-rs/core:core-all-wine-exec-test`.
...
Note that `EfficiencyCode-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.EfficiencyCode` read-only.
...
Expects the binary containing `EfficiencyCode-core` to run the equivalent of `EfficiencyCode sandbox` when `arg0` is `EfficiencyCode-linux-sandbox`. See the `EfficiencyCode-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `EfficiencyCode-resources/bwrap`
binary shipped with EfficiencyCode and EfficiencyCode surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
EfficiencyCode also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so EfficiencyCode rejects sandboxed shell commands that would enter the
...
Expects the binary containing `EfficiencyCode-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--EfficiencyCode-run-as-apply-patch`. See the
`EfficiencyCode-arg0` crate for details.
