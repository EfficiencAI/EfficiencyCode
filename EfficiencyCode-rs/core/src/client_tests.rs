...
use super::X_EfficiencyCode_INSTALLATION_ID_HEADER;
use super::X_EfficiencyCode_PARENT_THREAD_ID_HEADER;
use super::X_EfficiencyCode_TURN_METADATA_HEADER;
use super::X_EfficiencyCode_WINDOW_ID_HEADER;
use super::X_OPENAI_SUBAGENT_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use crate::responses_metadata::EfficiencyCodeResponsesMetadata;
use crate::test_support::TestEfficiencyCodeResponsesRequestKind;
use crate::test_support::responses_metadata as test_responses_metadata;
use EfficiencyCode_api::AgentIdentityTelemetry;
use EfficiencyCode_api::ApiError;
use EfficiencyCode_api::ResponseEvent;
...
use EfficiencyCode_model_provider::BearerAuthProvider;
use EfficiencyCode_model_provider::SharedModelProvider;
use EfficiencyCode_model_provider::create_model_provider;
use EfficiencyCode_model_provider_info::CHATGPT_EfficiencyCode_BASE_URL;
...
use EfficiencyCode_rollout_trace::ExecutionStatus;
...
use EfficiencyCode_rollout_trace::RolloutTrace;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&EfficiencyCode_home, server.uri()).await;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: EfficiencyCode_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn write_chatgpt_auth_json(EfficiencyCode_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(EfficiencyCode_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::EfficiencyCodeTurnStarted {
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        client_metadata
            .get(X_EfficiencyCode_TURN_METADATA_HEADER)
            .expect("turn metadata"),
    )
    .expect("valid turn metadata");
    for (client_key, metadata_key, expected) in [
        (
            X_EfficiencyCode_INSTALLATION_ID_HEADER,
            "installation_id",
        ...
        (
            X_EfficiencyCode_WINDOW_ID_HEADER,
            "window_id",
            expected_window_id.as_str(),
        ),
        (
            X_EfficiencyCode_PARENT_THREAD_ID_HEADER,
            "parent_thread_id",
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart EfficiencyCode, url: {url}"
        )
    );
}
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(
                CHATGPT_EfficiencyCode_BASE_URL.to_string(),
            )),
        )
    } else {
        (
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_EfficiencyCode_responses() {
...
#[tokio::test]
async fn non_chatgpt_EfficiencyCode_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
