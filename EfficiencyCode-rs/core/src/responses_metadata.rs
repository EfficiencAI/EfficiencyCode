...
use EfficiencyCode_analytics::CompactionStrategy;
...
use EfficiencyCode_utils_string::to_ascii_json_string;
use http::HeaderMap as ApiHeaderMap;
use http::HeaderValue;
use serde::Serialize;
use serde_json::Value;

use crate::client::X_EfficiencyCode_INSTALLATION_ID_HEADER;
use crate::client::X_EfficiencyCode_PARENT_THREAD_ID_HEADER;
use crate::client::X_EfficiencyCode_TURN_METADATA_HEADER;
use crate::client::X_EfficiencyCode_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    ...
    X_EfficiencyCode_TURN_METADATA_HEADER,
...
pub(crate) enum EfficiencyCodeResponsesRequestKind {
    Turn,
    Prewarm,
...
impl EfficiencyCodeResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            EfficiencyCodeResponsesRequestKind::Turn => ("turn", None),
            EfficiencyCodeResponsesRequestKind::Prewarm => ("prewarm", None),
            EfficiencyCodeResponsesRequestKind::Compaction(metadata) => {
                ("compaction", Some(metadata))
            }
            EfficiencyCodeResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, EfficiencyCodeResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of EfficiencyCode metadata sent to ResponsesAPI.
///
/// The full EfficiencyCode turn metadata blob is transported canonically as
/// `client_metadata["x-EfficiencyCode-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct EfficiencyCodeResponsesMetadata {
    ...
    pub(crate) request_kind: Option<EfficiencyCodeResponsesRequestKind>,
...
impl EfficiencyCodeResponsesMetadata {
    pub(crate) fn new(
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_EfficiencyCode_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(
                X_EfficiencyCode_TURN_METADATA_HEADER.to_string(),
                turn_metadata_json,
            );
        }
        client_metadata
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        // Direct x-EfficiencyCode-turn-metadata is compatibility output. Keep the unbounded tool inventory
        // in client_metadata only so HTTP and WebSocket compatibility headers remain bounded.
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) =
                to_ascii_json_string(&EfficiencyCodeTurnMetadataPayload {
                    tool_namespaces_info: None,
    ...
    fn turn_metadata_payload(&self) -> EfficiencyCodeTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(EfficiencyCodeResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(EfficiencyCodeResponsesRequestKind::has_turn_identity);
        EfficiencyCodeTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the EfficiencyCode turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved EfficiencyCode-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
...
#[derive(Serialize)]
struct EfficiencyCodeTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
