...
use EfficiencyCode_analytics::GuardianReviewDecision;
use EfficiencyCode_analytics::GuardianReviewFailureReason;
use EfficiencyCode_analytics::GuardianReviewTerminalStatus;
use EfficiencyCode_analytics::GuardianReviewTrackContext;
use EfficiencyCode_analytics::GuardianReviewedAction;
...
use EfficiencyCode_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: EfficiencyCodeErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &EfficiencyCode_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        EfficiencyCode_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("EfficiencyCode-approval-review".to_string())
        .spawn(move || {
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == EfficiencyCode_protocol::openai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == EfficiencyCode_protocol::openai_models::ReasoningEffort::Low
                }),
