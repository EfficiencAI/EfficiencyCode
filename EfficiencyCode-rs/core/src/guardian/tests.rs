...
use EfficiencyCode_analytics::GuardianApprovalRequestSource;
...
use EfficiencyCode_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
...
use EfficiencyCode_models_manager::manager::StaticModelsManager;
...
// The memories extension depends on EfficiencyCode-core, so this probe verifies the nested Guardian config
...
impl EfficiencyCode_extension_api::ThreadLifecycleContributor<Config>
    for GuardianMemoryContextProbe
{
    fn on_thread_start<'a>(
        &'a self,
        input: EfficiencyCode_extension_api::ThreadStartInput<'a, Config>,
...
impl EfficiencyCode_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        ...
        'a,
        Vec<EfficiencyCode_extension_api::PromptFragment>,
    > {
        Box::pin(async move {
            if thread_store
...
fn guardian_shell_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::Shell {
        ...
        cwd: test_path_buf("/repo/EfficiencyCode-rs/core").abs(),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/EfficiencyCode\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    id: None,
                    call_id: "call-1".to_string(),
                    output: EfficiencyCode_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/EfficiencyCode-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[EfficiencyCode_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            EfficiencyCode_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The EfficiencyCode agent has requested the following action:\n"));
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = EfficiencyCode_utils_output_truncation::truncate_text(
        &approval_reason,
        EfficiencyCode_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = prompt
        ...
        .find_map(|item| match item {
            EfficiencyCode_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: EfficiencyCode_protocol::permissions::FileSystemSpecialPath::Root,
                },
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value:
                        EfficiencyCode_protocol::permissions::FileSystemSpecialPath::project_roots(
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The EfficiencyCode agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "EfficiencyCode_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    EfficiencyCode_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = EfficiencyCode_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    turn.config = Arc::clone(&config);
    turn.provider = create_model_provider(config.model_provider.clone(), turn.auth_manager.clone());
    turn.model_info.auto_review_model_override = Some("EfficiencyCode-auto-review".to_string());
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    seed_guardian_parent_history(&session, &turn).await;
    session
    ...
    assert!(matches!(
        ...
        Some(EfficiencyCode_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "EfficiencyCode_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            EfficiencyCode_protocol::user_input::UserInput::Text { text, .. } => text,
            EfficiencyCode_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
    ...
    assert!(
        prompt_text.contains(&format!(
            ">>> TRANSCRIPT END\nReviewed EfficiencyCode session id: {}\n",
            session.thread_id
        )),
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    turn_mut.model_info.auto_review_model_override = Some("EfficiencyCode-auto-review".to_string());
        ...
        .replace_history(
            vec![
                ResponseItem::Compaction {
                    id: Some(EfficiencyCode_protocol::ResponseItemId::from_server(
    ...
    assert!(matches!(
        ...
        Some(EfficiencyCode_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "EfficiencyCode_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(EfficiencyCode_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )
...
async fn guardian_review_session_config_clears_context_overrides_for_distinct_effective_model() {
    ...
    let (session, mut turn) = guardian_test_session_and_turn(&server).await;
    let mut config = (*turn.config).clone();
    config.model = Some("EfficiencyCode-auto-review".to_string());
