...
async fn thread_id_generator_applies_to_roots_children_and_forks() {
    ...
    config.EfficiencyCode_home = temp_dir.path().join("EfficiencyCode-home").abs();
    config.cwd = config.EfficiencyCode_home.abs();
    std::fs::create_dir_all(&config.EfficiencyCode_home).expect("create EfficiencyCode home");
...
impl EfficiencyCode_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: EfficiencyCode_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> EfficiencyCode_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<EfficiencyCode_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> EfficiencyCode_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<EfficiencyCode_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> EfficiencyCode_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl EfficiencyCode_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: EfficiencyCode_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl EfficiencyCode_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: EfficiencyCode_extension_api::McpServerContributionContext<'a, Config>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<
            'a,
            Vec<EfficiencyCode_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(EfficiencyCodeAuth::from_api_key("dummy"));
    let manager = Arc::new(ThreadManager::new(
        &config,
        Arc::clone(&auth_manager),
        build_models_manager(&config, auth_manager),
        crate::EfficiencyCodeAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(EfficiencyCode_exec_server::EnvironmentManager::default_for_tests()),
...
async fn start_thread_keeps_internal_threads_hidden_from_normal_lookups() {
    ...
    assert!(
        EfficiencyCode_diagnostics::snapshot()
            .gauges
            .iter()
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl EfficiencyCode_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl EfficiencyCode_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        fn contribute<'a>(
            &'a self,
            context: EfficiencyCode_extension_api::McpServerContributionContext<'a, Config>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<
            'a,
            Vec<EfficiencyCode_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
                ...
                let mut server = EfficiencyCode_mcp::EfficiencyCode_apps_mcp_server_config(
                ...
                vec![
                    EfficiencyCode_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = EfficiencyCode_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("EfficiencyCode_work_desktop".to_string()),
            environments: Some(Vec::new()),
    ...
    let selected_servers = |config: &EfficiencyCode_mcp::McpConfig| {
        EfficiencyCode_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let EfficiencyCode_apps_server =
        EfficiencyCode_mcp::configured_mcp_servers(&first_resolved.config)
            .remove(EfficiencyCode_mcp::EfficiencyCode_APPS_MCP_SERVER_NAME)
            .expect("EfficiencyCode Apps server should be configured");
    let EfficiencyCode_apps_headers = match EfficiencyCode_apps_server.transport {
        EfficiencyCode_config::McpServerTransportConfig::StreamableHttp {
            http_headers, ..
        } => http_headers,
        EfficiencyCode_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("EfficiencyCode Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        EfficiencyCode_apps_headers
            .expect("EfficiencyCode Apps headers should be configured")
            .get("originator"),
        Some(&"EfficiencyCode_work_desktop".to_string())
    );
}
...
#[tokio::test]
async fn explicit_installation_id_skips_EfficiencyCode_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.EfficiencyCode_home = temp_dir.path().join("EfficiencyCode-home").abs();
    config.cwd = config.EfficiencyCode_home.abs();
