...
use EfficiencyCode_thread_store::ThreadStoreResult;
...
/// Explains why `EfficiencyCodeThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
impl ThreadConfigSnapshot {
    ...
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        EfficiencyCode_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
pub struct EfficiencyCodeThreadSettingsOverrides {
...
pub struct EfficiencyCodeThread {
...
/// (formerly called a conversation) in EfficiencyCode.
impl EfficiencyCodeThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
        session_configured: SessionConfiguredEvent,
    ...
    pub async fn submit(&self, op: Op) -> EfficiencyCodeResult<String> {
    ...
    pub fn thread_extension_data(&self) -> &EfficiencyCode_extension_api::ExtensionData {
        &self.session.services.thread_extension_data
    }

    pub async fn shutdown_and_wait(&self) -> EfficiencyCodeResult<()> {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(EfficiencyCode_extension_api::ThreadResumeInput {
    ...
    pub async fn submit_user_input_and_wait_for_admission(
        ...
        trace: Option<W3cTraceContext>,
        client_user_message_id: Option<String>,
    ) -> EfficiencyCodeResult<UserMessageAdmission> {
        ...
        tokio::select! {
            biased;
            result = admission => result.unwrap_or(Err(EfficiencyCodeErr::InternalAgentDied)),
            () = self.io.session_loop_termination.clone() => {
                Err(EfficiencyCodeErr::InternalAgentDied)
            },
        }
    }
    ...
    /// callers that only hold a `EfficiencyCodeThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: EfficiencyCodeThreadSettingsOverrides,
    ...
    async fn thread_settings_update(
        ...
        let EfficiencyCodeThreadSettingsOverrides {
            environments,
    ...
    pub async fn submit_with_id(&self, mut sub: Submission) -> EfficiencyCodeResult<()> {
        sub.parent_turn_id = None;
        self.io.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> EfficiencyCodeResult<Event> {
    ...
    pub async fn runtime_mcp_config_and_context(
        ...
        EfficiencyCode_mcp::McpConfig,
        EfficiencyCode_mcp::McpRuntimeContext,
    ) {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> EfficiencyCodeResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            EfficiencyCodeErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> EfficiencyCodeResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(EfficiencyCodeErr::InvalidRequest(
