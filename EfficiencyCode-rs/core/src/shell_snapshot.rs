...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            let timer = config
                .session_telemetry
                .start_timer("EfficiencyCode.shell_snapshot.duration_ms", &[]);
    ...
    async fn try_create(
        ...
        let temp_path = EfficiencyCode_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let EfficiencyCode_home = EfficiencyCode_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&EfficiencyCode_home, cleanup_session_id, state_db).await
            {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            EfficiencyCode_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(EfficiencyCode_home, session_id, state_db.as_deref())
                .await?;
