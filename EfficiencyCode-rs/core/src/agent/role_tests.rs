...
async fn test_config_with_cli_overrides(
    ...
    let config = ConfigBuilder::default()
        .EfficiencyCode_home(home_path.clone())
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    config.EfficiencyCode_linux_sandbox_exe =
        Some(PathBuf::from("/tmp/EfficiencyCode-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/EfficiencyCode-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        ...
        Some(PathBuf::from("/tmp/EfficiencyCode-execve-wrapper"))
    );
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&EfficiencyCode_exec_server::LOCAL_FS)),
        )
        .await;
