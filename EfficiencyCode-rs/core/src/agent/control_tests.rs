use super::*;
use crate::EfficiencyCodeThread;
...
use EfficiencyCode_thread_store::ArchiveThreadParams;
...
async fn test_config_with_cli_overrides(
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .EfficiencyCode_home(home.path().to_path_buf())
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(
                EfficiencyCode_exec_server::EnvironmentManager::default_for_tests(),
            ),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<EfficiencyCodeThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<EfficiencyCodeThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &EfficiencyCodeThread) -> String {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<EfficiencyCodeThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<EfficiencyCodeThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(err) => match err.details() {
            EfficiencyCodeErrorDetails::ThreadNotFound(id) => assert_eq!(*id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(
        err.details(),
        EfficiencyCodeErrorDetails::ThreadNotFound(id) if *id == thread_id
    );
}
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(err) => match err.details() {
            EfficiencyCodeErrorDetails::ThreadNotFound(id) => {
                assert_eq!(*id, spawned_agent.thread_id)
            }
            _ => panic!("expected ThreadNotFound, got {err:?}"),
        },
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        EfficiencyCodeAuth::from_api_key("dummy"),
        harness.config.model_provider.clone(),
        harness.config.EfficiencyCode_home.to_path_buf(),
        std::sync::Arc::new(EfficiencyCode_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = EfficiencyCode_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
...
async fn spawn_agent_respects_legacy_max_threads_alias() {
    ...
    let EfficiencyCodeErrorDetails::AgentLimitReached {
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let EfficiencyCodeErrorDetails::AgentLimitReached { max_threads } = err.details() else {
        panic!("expected AgentLimitReached");
    };
...
async fn spawn_thread_subagents_persist_parent_originator_across_new_and_truncated_fork() {
    ...
    assert_eq!(parent_originator, "EfficiencyCode_work_desktop");
