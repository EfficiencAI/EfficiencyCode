...
async fn residency_slot_reservation_unloads_oldest_idle_v2_agent() {
    ...
    config.EfficiencyCode_home = temp_home.path().to_path_buf().try_into().unwrap();
...
async fn mark_thread_completed(thread: &EfficiencyCodeThread) {
...
async fn mark_thread_interrupted(thread: &EfficiencyCodeThread) {
    let turn = thread.session.new_default_turn().await;
    thread
        .session
        .send_event(
...
async fn clear_active_turn(thread: &EfficiencyCodeThread) {
