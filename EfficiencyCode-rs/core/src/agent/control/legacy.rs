...
impl AgentControl {
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> EfficiencyCodeResult<String> {
        let state = self.upgrade()?;
        ...
        match state.get_thread(agent_id).await {
                ...
                if !thread.config_snapshot().await.ephemeral
                    ...
                    && let Err(err) = agent_graph_store
                        .set_thread_spawn_edge_status(
                            agent_id,
                            EfficiencyCode_agent_graph_store::ThreadSpawnEdgeStatus::Closed,
                        )
                        .await
                {
            ...
            Err(err)
                if known_agent
                    && matches!(err.details(), EfficiencyCodeErrorDetails::ThreadNotFound(_)) =>
            {
            ...
            Err(err) if matches!(err.details(), EfficiencyCodeErrorDetails::ThreadNotFound(_)) => {}
            Err(err) => {
