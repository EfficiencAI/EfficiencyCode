...
use EfficiencyCode_model_provider_info::AMAZON_BEDROCK_GPT_5_6_LUNA_MODEL_ID;
use EfficiencyCode_model_provider_info::AMAZON_BEDROCK_GPT_5_6_SOL_MODEL_ID;
...
use EfficiencyCode_protocol::openai_models::ApplyPatchToolType;
...
use EfficiencyCode_protocol::openai_models::WebSearchToolType;
...
use EfficiencyCode_tools::ResponsesApiTool;
use EfficiencyCode_tools::ToolCall as ExtensionToolCall;
use EfficiencyCode_tools::ToolExecutor;
use EfficiencyCode_tools::ToolExposure;
...
fn zsh_fork_config_for_spec_plan_tests() -> EfficiencyCode_tools::ZshForkConfig {
    let placeholder_exe = EfficiencyCode_utils_absolute_path::AbsolutePathBuf::try_from(
    ...
    EfficiencyCode_tools::ZshForkConfig {
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(EfficiencyCode_tools::ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: false,
                defer_loading: None,
                parameters: EfficiencyCode_tools::JsonSchema::default(),
                output_schema: None,
            })],
        })
    }

    fn handle(&self, _call: ExtensionToolCall) -> EfficiencyCode_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(
                Box::new(EfficiencyCode_tools::JsonToolOutput::new(json!({})))
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: EfficiencyCode_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    EfficiencyCode_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = EfficiencyCode_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => DynamicToolSpec::Namespace(
            EfficiencyCode_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    EfficiencyCode_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(
                        function,
                    ),
                ],
            },
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        EfficiencyCode_code_mode::PUBLIC_TOOL_NAME,
        EfficiencyCode_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(EfficiencyCode_code_mode::PUBLIC_TOOL_NAME)
    else {
    ...
    for (tool_name, guardian) in [
        ...
        for allow_login_shell in [false, true] {
            let plan = probe(|turn| {
                ...
                if guardian {
                    turn.session_source =
                        EfficiencyCode_protocol::protocol::SessionSource::SubAgent(
                            EfficiencyCode_protocol::protocol::SubAgentSource::Other(
...
async fn shell_command_is_not_registered_without_a_single_local_environment() {
    let remote_environment = probe(|turn| {
        ...
        environment.environment = Arc::new(
            EfficiencyCode_exec_server::Environment::create_for_tests(Some(
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if EfficiencyCode_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode = EfficiencyCode_tools::UnifiedExecShellMode::ZshFork(
...
async fn environment_tools_follow_the_step_context() {
    ...
    turn.environments.environments.clear();
    let turn = Arc::new(turn);
    let mcp = Arc::new(EfficiencyCode_mcp::McpBinding::empty(mcp_config_for_test(
        &turn.config,
    )));
...
async fn sleep_tool_stays_direct_and_outside_code_mode() {
    for code_mode_only in [false, true] {
        ...
        let ToolSpec::Freeform(exec) =
            plan.visible_spec(EfficiencyCode_code_mode::PUBLIC_TOOL_NAME)
        else {
            panic!("expected code mode exec tool");
        };
        if code_mode_only {
...
async fn strict_namespace_ownership_requires_tool_namespace_inventory_opt_in() {
    for (enabled, second_exposure) in [
        ...
        if enabled && second_exposure != ToolExposure::Hidden {
            ...
            assert!(matches!(
                error.details(),
                EfficiencyCodeErrorDetails::ToolCollision(name) if name == "shared"
            ));
        } else {
...
async fn strict_tool_collisions_reject_external_and_synthetic_duplicates() {
    let cases = [
        ...
        (
            ...
            ToolPlanInputs {
                ...
                dynamic_tools: vec![dynamic_tool(
                    /*namespace*/ None,
                    EfficiencyCode_tools::TOOL_SEARCH_TOOL_NAME,
                    /*defer_loading*/ false,
                )],
                ..ToolPlanInputs::default()
            },
        ...
        cases.into_iter().chain(namespace_cases)
    {
        ...
        assert!(matches!(
            error.details(),
            EfficiencyCodeErrorDetails::ToolCollision(name) if name == expected_name
        ));
        assert_eq!(
            error.to_string(),
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("EfficiencyCode_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("EfficiencyCode_app"),
