...
use EfficiencyCode_execpolicy::MatchOptions;
...
use EfficiencyCode_protocol::protocol::NetworkPolicyRuleAction;
...
use EfficiencyCode_shell_command::bash::parse_shell_lc_plain_commands;
use EfficiencyCode_shell_command::bash::parse_shell_lc_single_command_prefix;
use EfficiencyCode_shell_escalation::EscalateServer;
use EfficiencyCode_shell_escalation::EscalationDecision;
use EfficiencyCode_shell_escalation::EscalationExecution;
use EfficiencyCode_shell_escalation::EscalationPermissions;
use EfficiencyCode_shell_escalation::EscalationPolicy;
use EfficiencyCode_shell_escalation::EscalationPolicyFuture;
use EfficiencyCode_shell_escalation::EscalationSession;
use EfficiencyCode_shell_escalation::ExecParams;
use EfficiencyCode_shell_escalation::ExecResult;
use EfficiencyCode_shell_escalation::PreparedExec;
use EfficiencyCode_shell_escalation::ResolvedPermissionProfile;
use EfficiencyCode_shell_escalation::ShellCommandExecutor;
use EfficiencyCode_shell_escalation::ShellCommandExecutorFuture;
use EfficiencyCode_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(ToolError::EfficiencyCode)?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        EfficiencyCode_linux_sandbox_exe: ctx
            .step_context
            .turn
            .config
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::EfficiencyCode(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<EfficiencyCode_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["EfficiencyCode_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            EfficiencyCode_linux_sandbox_exe: self.EfficiencyCode_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::EfficiencyCode(EfficiencyCodeErr::Sandbox(
            SandboxErr::Timeout {
