...
use EfficiencyCode_execpolicy::PolicyParser;
...
use EfficiencyCode_sandboxing::policy_transforms::effective_permission_profile;
...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "EfficiencyCode_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(EfficiencyCode_execpolicy::Policy::empty())),
        ...
        stopwatch: EfficiencyCode_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = EfficiencyCode_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = EfficiencyCode_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.EfficiencyCode_home)
        .context("recreate EfficiencyCode home for hook fixtures")?;
    ...
    let expected_hook_command = EfficiencyCode_shell_command::parse_command::shlex_join(&[
    ...
    session
        ...
        .await;
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(EfficiencyCode_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        EfficiencyCode_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        EfficiencyCode_shell_escalation::EscalationDecision::Escalate(
            EfficiencyCode_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(
        action,
        EfficiencyCode_shell_escalation::EscalationDecision::Run
    );
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        EfficiencyCode_shell_escalation::EscalationDecision::Deny {
