...
*/
use crate::exec_env::EfficiencyCode_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::EfficiencyCode_SESSION_ID_ENV_VAR;
use crate::exec_env::EfficiencyCode_THREAD_ID_ENV_VAR;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::shell::ShellType;
use crate::tools::sandboxing::ToolError;
use EfficiencyCode_apply_patch::EfficiencyCode_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
#[cfg(unix)]
...
pub(crate) use EfficiencyCode_network_proxy::is_managed_proxy_env_var;
pub(crate) use EfficiencyCode_network_proxy::strip_managed_proxy_env;
...
use EfficiencyCode_protocol::shell_environment::is_non_inheritable_env_var;
...
/// PATH entries owned by EfficiencyCode runtime setup.
///
...
/// runtime-only vars like `EfficiencyCode_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains EfficiencyCode-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        EfficiencyCode_SESSION_ID_ENV_VAR,
        EfficiencyCode_THREAD_ID_ENV_VAR,
        EfficiencyCode_PERMISSION_PROFILE_ENV_VAR,
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__EfficiencyCode_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__EfficiencyCode_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__EfficiencyCode_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__EfficiencyCode_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_EfficiencyCode_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_EfficiencyCode_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!(
        "{}\\ *",
        EfficiencyCode_PROXY_GIT_SSH_COMMAND_MARKER.trim_end()
    );
    (
        format!(
            "__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__EfficiencyCode_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
