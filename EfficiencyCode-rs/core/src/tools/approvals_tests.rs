...
fn approval_resolution_aborts_turn_when_approval_is_aborted() {
    ...
    assert!(matches!(
        resolution.into_tool_result(),
        Err(ToolError::EfficiencyCode(error))
            if matches!(
                error.details(),
                EfficiencyCode_protocol::error::EfficiencyCodeErrorDetails::TurnAborted
            )
    ));
}
...
fn guardian_cwd_preserves_drive_shaped_local_posix_path() {
    ...
    assert_eq!(
        guardian_cwd(EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID, cwd)
...
fn guardian_cwd_rejects_foreign_remote_path() {
    ...
    assert!(guardian_cwd(EfficiencyCode_exec_server::REMOTE_ENVIRONMENT_ID, cwd).is_err());
}
