...
use EfficiencyCode_analytics::ArtifactOperation;
use EfficiencyCode_analytics::ArtifactOperationLifecycle;
use EfficiencyCode_analytics::build_track_events_context;
use EfficiencyCode_apply_patch::AppliedPatchDelta;
use EfficiencyCode_core_plugins::PluginCommandAttribution;
use EfficiencyCode_core_plugins::recognize_artifact_operation;
use EfficiencyCode_otel::ARTIFACT_OPERATION_EXPECTED_OUTPUT_COUNT_METRIC;
use EfficiencyCode_otel::ARTIFACT_OPERATION_STARTED_METRIC;
...
use EfficiencyCode_protocol::items::CommandExecutionStatus;
        ...
        ctx.session
            ...
            .track_artifact_operation(
                ...
                ArtifactOperation {
                    ...
                    occurred_at_ms: EfficiencyCode_analytics::now_unix_millis(),
...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::EfficiencyCode(err)) => match err.details() {
...
mod tests {
    ...
    async fn assert_failed_apply_patch_tracks_committed_delta(
        ...
        let delta = EfficiencyCode_apply_patch::apply_patch(
    ...
    async fn denied_apply_patch_tracks_committed_delta() {
        ...
        assert_failed_apply_patch_tracks_committed_delta(
            Err(ToolError::EfficiencyCode(EfficiencyCodeErr::Sandbox(
                SandboxErr::Denied {
