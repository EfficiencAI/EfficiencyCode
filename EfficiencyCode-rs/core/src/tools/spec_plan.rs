...
use EfficiencyCode_protocol::DEFAULT_FUNCTION_NAMESPACE;
...
use EfficiencyCode_tools::TOOL_SEARCH_TOOL_NAME;
use EfficiencyCode_tools::ToolCall as ExtensionToolCall;
use EfficiencyCode_tools::ToolEnvironmentMode;
use EfficiencyCode_tools::ToolExecutor;
use EfficiencyCode_tools::ToolExposures;
use EfficiencyCode_tools::ToolName;
use EfficiencyCode_tools::ToolSearchInfo;
use EfficiencyCode_tools::ToolSpec;
use EfficiencyCode_tools::UnifiedExecShellMode;
use EfficiencyCode_tools::can_request_original_image_detail;
use EfficiencyCode_tools::collect_code_mode_exec_prompt_tool_definitions;
use EfficiencyCode_tools::collect_request_plugin_install_entries;
use EfficiencyCode_tools::default_namespace_description;
use EfficiencyCode_tools::request_user_input_available_modes;
use EfficiencyCode_tools::shell_command_backend_for_features;
use EfficiencyCode_tools::shell_type_for_model_and_features;
...
struct CoreToolPlanContext<'a> {
    ...
    mcp: &'a EfficiencyCode_mcp::McpBinding,
...
) -> EfficiencyCodeResult<ToolRouter> {
...
fn apply_mcp_tool_exposure_policy(
    turn_context: &TurnContext,
    mcp: &EfficiencyCode_mcp::McpBinding,
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(EfficiencyCode_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(EfficiencyCode_code_mode::WAIT_TOOL_NAME),
        ] {
    ...
    if turn_context.config.tool_registry.error_on_tool_collisions {
        if let Some(tool_name) = registry.first_collision() {
            ...
            return Err(EfficiencyCodeErrorDetails::ToolCollision(name).into());
        }
        ...
        for tool in registry.entries() {
            ...
            if include_tool_namespaces_info && tool.exposure != ToolExposure::Hidden {
                ...
                match namespace_owners.get(namespace_name) {
                    Some(existing_owner) if existing_owner != &owner => {
                        return Err(EfficiencyCodeErrorDetails::ToolCollision(
            ...
            match namespace_descriptions.entry(namespace.name.clone()) {
                ...
                Entry::Occupied(entry) if entry.get() != &namespace.description => {
                    return Err(
                        EfficiencyCodeErrorDetails::ToolCollision(entry.key().clone()).into(),
                    );
                }
                Entry::Occupied(_) => {}
            }
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && EfficiencyCode_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&EfficiencyCode_code_mode::normalize_code_mode_identifier(
                &EfficiencyCode_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        EfficiencyCode_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_EfficiencyCode_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && EfficiencyCode_code_mode::is_code_mode_nested_tool(
            &EfficiencyCode_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !EfficiencyCode_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(EfficiencyCode_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            {
                EfficiencyCode_code_mode::ImageDetailVisibility::Hidden
            } else {
                EfficiencyCode_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, EfficiencyCode_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| EfficiencyCode_code_mode::ToolNamespaceDescription {
...
impl ToolExecutor<ToolInvocation> for MultiAgentV2NamespaceOverride {
    ...
    fn handle(&self, invocation: ToolInvocation) -> EfficiencyCode_tools::ToolExecutorFuture<'_> {
...
fn compare_code_mode_tools(
    left: &EfficiencyCode_code_mode::ToolDefinition,
    right: &EfficiencyCode_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, EfficiencyCode_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &EfficiencyCode_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<
        String,
        EfficiencyCode_code_mode::ToolNamespaceDescription,
    >,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
