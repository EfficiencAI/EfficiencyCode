...
use EfficiencyCode_protocol::protocol::FileSystemAccessMode;
use EfficiencyCode_protocol::protocol::FileSystemPath;
use EfficiencyCode_protocol::protocol::FileSystemSandboxEntry;
use EfficiencyCode_protocol::protocol::FileSystemSandboxPolicy;
use EfficiencyCode_protocol::protocol::InterAgentCommunication;
use EfficiencyCode_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.EfficiencyCode_home)
        .await
        .expect("EfficiencyCode home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    EfficiencyCode_protocol::models::function_call_output_content_items_to_text(
                        &items,
                    )
                    .unwrap_or_default()
                }
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: EfficiencyCode_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.EfficiencyCode_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
