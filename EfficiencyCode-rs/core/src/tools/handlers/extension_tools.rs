...
use EfficiencyCode_tools::ConversationHistory;
use EfficiencyCode_tools::ExtensionTurnItem;
use EfficiencyCode_tools::ResponsesApiNamespaceTool;
use EfficiencyCode_tools::ToolCall as ExtensionToolCall;
use EfficiencyCode_tools::ToolEnvironment;
use EfficiencyCode_tools::ToolName;
use EfficiencyCode_tools::ToolSearchInfo;
use EfficiencyCode_tools::ToolSpec;
use EfficiencyCode_tools::TurnItemEmissionFuture;
use EfficiencyCode_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(
    Arc<dyn EfficiencyCode_tools::ToolExecutor<ExtensionToolCall>>,
);

impl ExtensionToolAdapter {
    pub(crate) fn new(
        executor: Arc<dyn EfficiencyCode_tools::ToolExecutor<ExtensionToolCall>>,
    ) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let EfficiencyCode_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use EfficiencyCode_extension_items::web_search::WebSearchItem;
    use EfficiencyCode_protocol::items::TurnItem;
    ...
    impl EfficiencyCode_extension_api::ToolExecutor<EfficiencyCode_tools::ToolCall>
        for StubExtensionExecutor
    {
        fn tool_name(&self) -> EfficiencyCode_tools::ToolName {
            EfficiencyCode_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> EfficiencyCode_tools::ToolSpec {
            EfficiencyCode_tools::ToolSpec::Function(EfficiencyCode_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: EfficiencyCode_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(
            &self,
            _call: EfficiencyCode_tools::ToolCall,
        ) -> EfficiencyCode_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(Box::new(EfficiencyCode_tools::JsonToolOutput::new(
                    json!({ "ok": true }),
                ))
                    as Box<dyn EfficiencyCode_tools::ToolOutput>)
            })
        }
    }

    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<EfficiencyCode_tools::ToolCall>>>,
    }
        ...
        for CapturingExtensionExecutor
    {
        ...
        fn handle(
            &self,
            call: EfficiencyCode_tools::ToolCall,
        ) -> EfficiencyCode_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            ...
            Box<dyn EfficiencyCode_tools::ToolOutput>,
            EfficiencyCode_tools::FunctionCallError,
        > {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: EfficiencyCode_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = EfficiencyCode_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        EfficiencyCode_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        EfficiencyCode_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
