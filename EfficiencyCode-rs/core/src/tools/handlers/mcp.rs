...
use EfficiencyCode_tools::ToolSearchSourceInfo;
use EfficiencyCode_tools::ToolSpec;
use EfficiencyCode_tools::agent_plugin_mcp_tool_to_responses_api_tool;
use EfficiencyCode_tools::mcp_tool_to_responses_api_tool;
...
pub struct McpHandler {
    tool_info: ToolInfo,
    spec: Arc<ToolSpec>,
    code_mode_tool_definitions: OnceLock<Vec<EfficiencyCode_code_mode::ToolDefinition>>,
}
...
impl CoreToolRuntime for McpHandler {
    ...
    fn cached_code_mode_definitions(&self) -> Option<&[EfficiencyCode_code_mode::ToolDefinition]> {
        Some(
            self.code_mode_tool_definitions
                .get_or_init(|| {
                    let mut definitions = EfficiencyCode_tools::collect_code_mode_tool_definitions(
...
mod tests {
    ...
    async fn mcp_pre_tool_use_payload_uses_prefixed_tool_name_and_raw_args() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: EfficiencyCode_tools::ToolName::namespaced("memory", "create_entities"),
    ...
    async fn mcp_pre_tool_use_payload_keeps_builtin_like_tool_names_namespaced() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: EfficiencyCode_tools::ToolName::namespaced("mcp__foo", "exec_command"),
                source: ToolCallSource::Direct,
                payload,
            }),
            Some(PreToolUsePayload {
        ...
        let invocation = handler
            .with_updated_hook_input(
                ToolInvocation {
                    ...
                    tool_name: EfficiencyCode_tools::ToolName::namespaced(
                        "mcp__foo",
                        "exec_command",
                    ),
                    source: ToolCallSource::Direct,
    ...
    async fn mcp_post_tool_use_payload_uses_prefixed_tool_name_args_and_result() {
        ...
        let output = McpToolOutput {
            result: EfficiencyCode_protocol::mcp::CallToolResult {
                content: vec![json!({
                    "type": "text",
                    "text": "notes"
                })],
            ...
            truncation_policy: EfficiencyCode_utils_output_truncation::TruncationPolicy::Bytes(
                1024,
            ),
        };
        let (session, turn) = make_session_and_context().await;
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: EfficiencyCode_tools::ToolName::namespaced("filesystem", "read_file"),
            source: ToolCallSource::Direct,
            payload,
        };
        assert_eq!(
