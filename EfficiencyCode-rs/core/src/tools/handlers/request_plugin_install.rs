...
use EfficiencyCode_analytics::PluginInstallRequestSource;
use EfficiencyCode_analytics::PluginInstallRequested;
use EfficiencyCode_analytics::PluginInstallRequestedPlugin;
...
use EfficiencyCode_tools::DiscoverableToolAction;
use EfficiencyCode_tools::DiscoverableToolType;
use EfficiencyCode_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use EfficiencyCode_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use EfficiencyCode_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use EfficiencyCode_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use EfficiencyCode_tools::RequestPluginInstallArgs;
use EfficiencyCode_tools::RequestPluginInstallResult;
use EfficiencyCode_tools::ToolName;
use EfficiencyCode_tools::ToolSpec;
use EfficiencyCode_tools::all_requested_connectors_picked_up;
use EfficiencyCode_tools::build_request_plugin_install_elicitation_request;
use EfficiencyCode_tools::filter_request_plugin_install_discoverable_tools_for_client;
use EfficiencyCode_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("EfficiencyCode-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in EfficiencyCode-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.EfficiencyCode_home, tool).await
    {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    EfficiencyCode_home: &EfficiencyCode_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&EfficiencyCode_login::EfficiencyCodeAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session
        .hard_refresh_latest_EfficiencyCode_apps_tools()
        .await
    {
        Ok(mcp_tools) => {
        ...
        Err(err) => {
            warn!(
                "failed to refresh EfficiencyCode apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
