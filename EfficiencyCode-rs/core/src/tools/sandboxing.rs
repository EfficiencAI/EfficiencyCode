    ...
    Fut: Future<Output = ReviewDecision>,
{
    ...
    services.session_telemetry.counter(
        "EfficiencyCode.approval.requested",
        /*inc*/ 1,
        &[
            ("tool", tool_name),
...
pub(crate) enum ExecApprovalRequirement {
    ...
    Skip {
        ...
        /// Only applies if the command fails to run in sandbox and EfficiencyCode prompts the user to run outside the sandbox.
...
pub(crate) enum ToolError {
    Rejected(String),
    EfficiencyCode(EfficiencyCodeErr),
}
...
pub(crate) struct SandboxAttempt<'a> {
    ...
    pub permissions: &'a EfficiencyCode_protocol::models::PermissionProfile,
    /// Canonical permissions before this host materializes workspace roots.
    pub exec_server_permissions: &'a EfficiencyCode_protocol::models::PermissionProfile,
    pub enforce_managed_network: bool,
    pub(crate) manager: &'a SandboxManager,
    pub(crate) sandbox_cwd: &'a PathUri,
    pub(crate) workspace_roots: &'a [PathUri],
    pub EfficiencyCode_linux_sandbox_exe: Option<&'a std::path::PathBuf>,
...
impl<'a> SandboxAttempt<'a> {
    ...
    ) -> Result<crate::sandboxing::ExecRequest, EfficiencyCodeErr> {
