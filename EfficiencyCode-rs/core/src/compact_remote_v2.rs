...
use EfficiencyCode_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    EfficiencyCode_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err) if matches!(err.details(), EfficiencyCodeErrorDetails::TurnAborted) => Err(err),
        Err(err) => {
            sess.track_turn_EfficiencyCode_error(turn_context, &err);
...
) -> EfficiencyCodeResult<RemoteCompactionV2Output> {
...
async fn collect_compaction_output(
    ...
    if compaction_count != 1 {
        return Err(EfficiencyCodeErr::Fatal(format!(
...
fn build_v2_compacted_history(
    ...
    prompt_input_metadata: Vec<Option<EfficiencyCodeHarnessMetadata>>,
    compaction_output: ResponseItem,
...
mod tests {
    ...
    fn response_stream(events: Vec<EfficiencyCodeResult<ResponseEvent>>) -> ResponseStream {
    ...
    fn build_v2_compacted_history_preserves_retained_metadata_sidecar() {
        ...
        let (history, _) = build_v2_compacted_history(
            ...
            vec![None, Some(EfficiencyCodeHarnessMetadata::default())],
            output.clone(),
        );
