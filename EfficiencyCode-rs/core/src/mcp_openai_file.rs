//! Bridges Apps SDK-style `openai/fileParams` metadata into EfficiencyCode's MCP flow.
//!
//! Strategy:
...
//!
//! The model-facing local-path schema is owned by `EfficiencyCode-mcp` alongside MCP tool inventory, so this
...
use EfficiencyCode_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use EfficiencyCode_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err(
            "ChatGPT auth is required to upload files for EfficiencyCode Apps tools".to_string(),
        );
    };
    ...
    let upload_auth = EfficiencyCode_model_provider::auth_provider_from_auth(auth);
...
mod tests {
    ...
    async fn openai_file_argument_rewrite_requires_declared_file_params() {
        ...
        let arguments = Some(serde_json::json!({
            "file": "/tmp/EfficiencyCode-smoke-file.txt"
        }));
