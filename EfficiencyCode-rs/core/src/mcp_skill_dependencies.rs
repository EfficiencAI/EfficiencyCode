...
use EfficiencyCode_config::load_global_mcp_servers;
use EfficiencyCode_login::default_client::is_first_party_originator;
...
use EfficiencyCode_skills::SkillToolDependency;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(EfficiencyCode_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&EfficiencyCode_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&EfficiencyCode_home)
