...
pub(crate) struct PendingUserMessageAdmissions {
    pending: Mutex<HashMap<String, oneshot::Sender<EfficiencyCodeResult<UserMessageAdmission>>>>,
}
...
impl PendingUserMessageAdmissions {
    pub(crate) fn register(
        ...
        oneshot::Receiver<EfficiencyCodeResult<UserMessageAdmission>>,
    ) {
    ...
    pub(crate) fn complete(
        &self,
        submission_id: &str,
        admission: EfficiencyCodeResult<UserMessageAdmission>,
    ) {
        let sender = self
            .pending
            .lock()
