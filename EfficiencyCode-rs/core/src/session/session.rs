...
use EfficiencyCode_skills::SkillError;
...
pub(crate) struct Session {
    ...
    pub(super) mcp_elicitation_reviewer_handle:
        OnceLock<EfficiencyCode_mcp::ElicitationReviewerHandle>,
    pub(super) mcp_elicitation_lifecycle_handle: OnceLock<EfficiencyCode_mcp::ElicitationLifecycle>,
...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all EfficiencyCode state for this session.
    pub(super) EfficiencyCode_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn EfficiencyCode_home(&self) -> &AbsolutePathBuf {
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: EfficiencyCode_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        config: Arc<Config>,
        user_instructions: Option<EfficiencyCode_extension_api::UserInstructions>,
        ...
        code_mode_session_provider: Arc<dyn EfficiencyCode_code_mode::CodeModeSessionProvider>,
        extensions: Arc<EfficiencyCode_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        windows_sandbox_proxy_settings_mode: EfficiencyCode_sandboxing::WindowsSandboxProxySettingsMode,
        ...
        thread_extension_init.insert(EfficiencyCode_extension_api::ThreadOriginator(
            session_configuration.originator.clone(),
        ));
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = EfficiencyCode_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let config_path = config.EfficiencyCode_home.join(CONFIG_TOML_FILE);
            ...
            let account_id = telemetry_auth.and_then(EfficiencyCodeAuth::get_account_id);
            let account_email = telemetry_auth.and_then(EfficiencyCodeAuth::get_account_email);
            let originator = session_configuration.originator.clone();
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                session_configuration.provider.info(),
                auth_manager.EfficiencyCode_api_key_env_enabled(),
            );
            ...
            let mcp_server_names =
                EfficiencyCode_mcp::effective_mcp_servers(
            ...
            let session_extension_data =
                EfficiencyCode_extension_api::ExtensionData::new(session_id.to_string());
            ...
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(EfficiencyCode_extension_api::ThreadStartInput {
                ...
                || mcp_auth_changes.has_changed().unwrap_or(false)
            {
                sess.services
                    .plugins_manager
                    .set_auth_mode(latest_auth.as_ref().map(EfficiencyCodeAuth::api_auth_mode));
                sess.services
                    .mcp_manager
                    .runtime_config_for_step(
                        config.as_ref(),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => EfficiencyCode_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    EfficiencyCode_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => EfficiencyCode_hooks::SessionStartSource::Clear,
            };
