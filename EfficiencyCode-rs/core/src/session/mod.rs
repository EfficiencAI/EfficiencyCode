...
use EfficiencyCode_analytics::ImagePreparationFact;
use EfficiencyCode_analytics::ImagePreparationMetadata;
use EfficiencyCode_analytics::SubAgentThreadStartedInput;
use EfficiencyCode_analytics::TurnEfficiencyCodeErrorFact;
use EfficiencyCode_async_utils::OrCancelExt;
use EfficiencyCode_connectors::connector_runtime_context_key;
use EfficiencyCode_exec_server::Environment;
use EfficiencyCode_exec_server::EnvironmentManager;
use EfficiencyCode_execpolicy::prefix_rule_migration;
use EfficiencyCode_extension_api::ExtensionDataInit;
use EfficiencyCode_extension_api::LoadedUserInstructions;
use EfficiencyCode_extension_api::PromptFragment;
use EfficiencyCode_extension_api::PromptSlot;
use EfficiencyCode_extension_api::TurnContextContributionInput;
use EfficiencyCode_features::FEATURES;
use EfficiencyCode_features::Feature;
use EfficiencyCode_features::unstable_features_warning_event;
use EfficiencyCode_history::RolloutItem;
use EfficiencyCode_hooks::Hooks;
use EfficiencyCode_hooks::HooksConfig;
use EfficiencyCode_login::AuthManager;
use EfficiencyCode_login::EfficiencyCodeAuth;
use EfficiencyCode_login::auth_env_telemetry::collect_auth_env_telemetry;
use EfficiencyCode_mcp::McpResourceClient;
use EfficiencyCode_mcp::McpRuntime;
use EfficiencyCode_mcp::McpRuntimeContext;
use EfficiencyCode_mcp::McpRuntimeInput;
use EfficiencyCode_models_manager::manager::RefreshStrategy;
use EfficiencyCode_models_manager::manager::SharedModelsManager;
use EfficiencyCode_network_proxy::NetworkProxy;
use EfficiencyCode_network_proxy::NetworkProxyAuditMetadata;
use EfficiencyCode_network_proxy::normalize_host;
use EfficiencyCode_otel::current_span_trace_id;
use EfficiencyCode_otel::current_span_w3c_trace_context;
use EfficiencyCode_otel::set_parent_from_w3c_trace_context;
use EfficiencyCode_protocol::SessionId;
use EfficiencyCode_protocol::ThreadId;
use EfficiencyCode_protocol::approvals::ElicitationRequest;
use EfficiencyCode_protocol::approvals::ElicitationRequestEvent;
use EfficiencyCode_protocol::approvals::ExecPolicyAmendment;
use EfficiencyCode_protocol::approvals::NetworkPolicyAmendment;
use EfficiencyCode_protocol::approvals::NetworkPolicyRuleAction;
...
use EfficiencyCode_protocol::dynamic_tools::DynamicToolResponse;
use EfficiencyCode_protocol::dynamic_tools::DynamicToolSpec;
use EfficiencyCode_protocol::items::EnteredReviewModeItem;
use EfficiencyCode_protocol::items::TurnItem;
use EfficiencyCode_protocol::items::UserMessageItem;
use EfficiencyCode_protocol::models::ActivePermissionProfile;
use EfficiencyCode_protocol::models::AdditionalPermissionProfile;
...
use EfficiencyCode_protocol::protocol::AdditionalContextEntry;
use EfficiencyCode_protocol::protocol::FileChange;
use EfficiencyCode_protocol::protocol::HasLegacyEvent;
use EfficiencyCode_protocol::protocol::HistoryPosition;
...
use EfficiencyCode_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use EfficiencyCode_protocol::protocol::MultiAgentVersion;
use EfficiencyCode_protocol::protocol::RawResponseItemEvent;
use EfficiencyCode_protocol::protocol::SessionSource;
use EfficiencyCode_protocol::protocol::SubAgentSource;
use EfficiencyCode_protocol::protocol::ThreadHistoryMode;
use EfficiencyCode_protocol::protocol::ThreadSource;
use EfficiencyCode_protocol::protocol::TurnAbortReason;
use EfficiencyCode_protocol::protocol::TurnContextItem;
use EfficiencyCode_protocol::protocol::TurnContextNetworkItem;
...
use EfficiencyCode_protocol::protocol::WorldStateItem;
...
use EfficiencyCode_protocol::request_permissions::RequestPermissionsArgs;
use EfficiencyCode_protocol::request_permissions::RequestPermissionsEvent;
use EfficiencyCode_protocol::request_permissions::RequestPermissionsResponse;
use EfficiencyCode_protocol::request_user_input::RequestUserInputArgs;
use EfficiencyCode_protocol::request_user_input::RequestUserInputResponse;
use EfficiencyCode_rmcp_client::ElicitationResponse;
use EfficiencyCode_rollout::state_db;
use EfficiencyCode_rollout_trace::AgentResultTracePayload;
use EfficiencyCode_rollout_trace::ThreadStartedTraceMetadata;
use EfficiencyCode_rollout_trace::ThreadTraceContext;
use EfficiencyCode_sandboxing::policy_transforms::intersect_permission_profiles;
use EfficiencyCode_shell_command::parse_command::parse_command;
use EfficiencyCode_terminal_detection::user_agent;
use EfficiencyCode_thread_store::CreateThreadParams;
use EfficiencyCode_thread_store::LiveThread;
use EfficiencyCode_thread_store::LiveThreadInitGuard;
use EfficiencyCode_thread_store::LocalThreadStore;
use EfficiencyCode_thread_store::PersistContext;
use EfficiencyCode_thread_store::ReadThreadParams;
use EfficiencyCode_thread_store::ResumeThreadParams;
use EfficiencyCode_thread_store::ThreadPersistenceMetadata;
use EfficiencyCode_thread_store::ThreadStore;
use EfficiencyCode_utils_audio::prepare_response_items as prepare_audio_response_items;
...
use crate::EfficiencyCode_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use EfficiencyCode_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                EfficiencyCode_error_info: Some(EfficiencyCodeErrorInfo::BadRequest),
            },
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                ...
                ErrorEvent {
                    ...
                    EfficiencyCode_error_info: Some(
                        EfficiencyCodeErrorInfo::ActiveTurnNotSteerable {
                            turn_kind: *turn_kind,
                        },
                    ),
                }
...
use EfficiencyCode_core_plugins::PluginCommandAttribution;
use EfficiencyCode_core_plugins::PluginsManager;
use EfficiencyCode_core_plugins::RecommendedPluginCandidatesInput;
use EfficiencyCode_git_utils::get_git_repo_root;
...
use EfficiencyCode_otel::THREAD_STARTED_METRIC;
...
use EfficiencyCode_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use EfficiencyCode_protocol::config_types::WindowsSandboxLevel;
use EfficiencyCode_protocol::mcp::ClientMcpExtensions;
use EfficiencyCode_protocol::models::LocalImagePreparation;
use EfficiencyCode_protocol::models::ResponseInputItem;
use EfficiencyCode_protocol::models::ResponseItem;
use EfficiencyCode_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use EfficiencyCode_protocol::protocol::ApplyPatchApprovalRequestEvent;
use EfficiencyCode_protocol::protocol::AskForApproval;
use EfficiencyCode_protocol::protocol::DeprecationNoticeEvent;
use EfficiencyCode_protocol::protocol::EfficiencyCodeErrorInfo;
use EfficiencyCode_protocol::protocol::ErrorEvent;
...
use EfficiencyCode_protocol::protocol::ModelRerouteEvent;
use EfficiencyCode_protocol::protocol::ModelRerouteReason;
use EfficiencyCode_protocol::protocol::ModelVerification;
use EfficiencyCode_protocol::protocol::ModelVerificationEvent;
use EfficiencyCode_protocol::protocol::NetworkApprovalContext;
...
use EfficiencyCode_protocol::protocol::RequestUserInputEvent;
...
use EfficiencyCode_protocol::protocol::SessionNetworkProxyRuntime;
use EfficiencyCode_protocol::protocol::StreamErrorEvent;
use EfficiencyCode_protocol::protocol::Submission;
use EfficiencyCode_protocol::protocol::ThreadMemoryMode;
...
use EfficiencyCode_protocol::protocol::TurnModerationMetadataEvent;
use EfficiencyCode_protocol::protocol::WarningEvent;
use EfficiencyCode_protocol::user_input::UserInput;
use EfficiencyCode_skills_extension::HostSkillsService;
use EfficiencyCode_tools::UnifiedExecShellMode;
use EfficiencyCode_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use EfficiencyCode_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider:
        Arc<dyn EfficiencyCode_code_mode::CodeModeSessionProvider>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        EfficiencyCode_sandboxing::WindowsSandboxProxySettingsMode,
}
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/EfficiencyCode/concepts/cyber-safety";

impl Session {
    /// Spawn and initialize a new session.
    pub(crate) async fn spawn(
        args: SessionSpawnArgs,
    ) -> EfficiencyCodeResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if EfficiencyCode_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            {
                let EfficiencyCode_home = config.EfficiencyCode_home.clone();
                let policy_path = default_policy_path(EfficiencyCode_home.as_path());
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| {
                        EfficiencyCodeErr::Fatal(format!("failed to load rules: {err}"))
                    })?,
            )
        };
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            EfficiencyCode_models_manager::manager::RefreshStrategy::Offline
        } else {
            EfficiencyCode_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
            ...
            && !trusted_guardian_reviewer
        {
            ...
            if matches!(
                ...
                config
                    ...
                    .map_err(|err| EfficiencyCodeErr::InvalidRequest(err.to_string()))?;
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.EfficiencyCode_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> EfficiencyCodeResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> EfficiencyCodeResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| EfficiencyCodeErr::InternalAgentDied)?;
        Ok(())
    }

    pub(crate) async fn shutdown_and_wait(&self) -> EfficiencyCodeResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(err) if matches!(err.details(), EfficiencyCodeErrorDetails::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> EfficiencyCodeResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> EfficiencyCodeResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-EfficiencyCode-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &EfficiencyCode_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn EfficiencyCode_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<
            Arc<dyn EfficiencyCode_network_proxy::BlockedRequestObserver>,
        >,
    ...
    #[cfg(test)]
    pub(crate) async fn EfficiencyCode_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.EfficiencyCode_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect EfficiencyCode performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(
        &self,
    ) -> Option<EfficiencyCode_extension_api::UserInstructions> {
    ...
    /// Record a terminal EfficiencyCodeErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_EfficiencyCode_error(
        &self,
        turn_context: &TurnContext,
        error: &EfficiencyCodeErr,
    ) {
        self.services
            .analytics_events_client
            .track_turn_EfficiencyCode_error(TurnEfficiencyCodeErrorFact::from_EfficiencyCode_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                ...
                .as_ref()
                .is_some_and(EfficiencyCodeErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_EfficiencyCode_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &EfficiencyCode_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| EfficiencyCode_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => EfficiencyCode_rollout::existing_rollout_path(&path)
                .await
                .is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&EfficiencyCode_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the EfficiencyCode host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                GuardianReviewOptions {
                    ...
                    approval_request_source:
                        EfficiencyCode_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    ) -> EfficiencyCodeResult<Arc<WorldState>> {
    ...
    pub(crate) async fn capture_step_context(
        ...
        turn_context: Arc<TurnContext>,
        cancellation_token: &CancellationToken,
    ) -> EfficiencyCodeResult<Arc<StepContext>> {
    ...
    pub(crate) async fn capture_step_context_with_required_mcp_servers(
        ...
        let executor_capability_discovery = self
            ...
            .await;
        let extension_data =
            EfficiencyCode_extension_api::ExtensionData::new(turn_context.sub_id.clone());
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-EfficiencyCode, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        }
        {
            ...
            state.queue_pending_session_start_source(
                EfficiencyCode_hooks::SessionStartSource::Compact,
            );
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(EfficiencyCode_protocol::AgentPath::root),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        EfficiencyCode_error: EfficiencyCodeErr,
    ) {
        let additional_details = EfficiencyCode_error.to_string();
        let EfficiencyCode_error_info = EfficiencyCodeErrorInfo::ResponseStreamDisconnected {
            http_status_code: EfficiencyCode_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<EfficiencyCode_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_config(
    ...
    HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::EfficiencyCodeHooks),
