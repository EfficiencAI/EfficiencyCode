...
use EfficiencyCode_analytics::CompactionPhase;
use EfficiencyCode_analytics::CompactionReason;
use EfficiencyCode_analytics::InvocationType;
use EfficiencyCode_analytics::TurnResolvedConfigFact;
...
use EfficiencyCode_extension_api::TurnInputContext;
use EfficiencyCode_extension_api::TurnInputEnvironment;
use EfficiencyCode_features::Feature;
use EfficiencyCode_file_system::FindUpErrorPolicy;
use EfficiencyCode_file_system::find_nearest_ancestor_with_markers;
use EfficiencyCode_login::EfficiencyCodeAuth;
use EfficiencyCode_model_provider::RemoteCompactionSupport;
...
use EfficiencyCode_protocol::items::PlanItem;
...
use EfficiencyCode_protocol::protocol::AgentMessageContentDeltaEvent;
use EfficiencyCode_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use EfficiencyCode_protocol::protocol::PlanDeltaEvent;
use EfficiencyCode_protocol::protocol::RawResponseCompletedEvent;
use EfficiencyCode_protocol::protocol::ReasoningContentDeltaEvent;
use EfficiencyCode_protocol::protocol::ReasoningRawContentDeltaEvent;
use EfficiencyCode_protocol::protocol::SafetyBufferingEvent;
...
use EfficiencyCode_skills::ToolMentionKind;
use EfficiencyCode_skills::app_id_from_path;
use EfficiencyCode_skills::build_skill_name_counts;
use EfficiencyCode_skills::collect_explicit_skill_mentions;
use EfficiencyCode_skills::tool_kind_for_path;
use EfficiencyCode_skills_extension::HostSkillPrompts;
use EfficiencyCode_skills_extension::InjectedHostSkillPrompts;
use EfficiencyCode_thread_store::PersistContext;
use EfficiencyCode_tools::DiscoverableTool;
use EfficiencyCode_tools::ToolName;
use EfficiencyCode_tools::filter_request_plugin_install_discoverable_tools_for_client;
use EfficiencyCode_utils_path_uri::PathUri;
use EfficiencyCode_utils_stream_parser::AssistantTextChunk;
use EfficiencyCode_utils_stream_parser::AssistantTextStreamParser;
use EfficiencyCode_utils_stream_parser::ProposedPlanSegment;
use EfficiencyCode_utils_stream_parser::extract_proposed_plan_text;
use EfficiencyCode_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str =
    "EfficiencyCode_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
...
) -> EfficiencyCodeResult<Option<String>> {
    ...
    .await
    {
        if matches!(err.details(), EfficiencyCodeErrorDetails::TurnAborted) {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
        if matches!(err.details(), EfficiencyCodeErrorDetails::ToolCollision(_)) {
            return Err(err);
        }
        let error = err.to_EfficiencyCode_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), EfficiencyCodeErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
    ...
    // Although from the perspective of EfficiencyCode.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: EfficiencyCodeResult<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                EfficiencyCodeResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            ...
            Err(EfficiencyCode_error)
                if matches!(
                    EfficiencyCode_error.details(),
                    EfficiencyCodeErrorDetails::InvalidImageRequest()
                ) =>
            {
                sess.track_turn_EfficiencyCode_error(turn_context.as_ref(), &EfficiencyCode_error);
                let error = EfficiencyCodeErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    EfficiencyCode_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_EfficiencyCode_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_EfficiencyCode_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &EfficiencyCode_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = EfficiencyCode_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
...
///
/// Returns `None` when the active authentication does not use the EfficiencyCode backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> EfficiencyCodeResult<Option<Arc<StepContext>>> {
    let uses_EfficiencyCode_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(EfficiencyCode_login::AuthManager::current_auth_uses_EfficiencyCode_backend);
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = EfficiencyCode_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<EfficiencyCode_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &EfficiencyCodeResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> EfficiencyCodeResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        {
            EfficiencyCode_protocol::models::bound_executed_tool_calls_for_prompt(
                &mut prompt_input,
            );
        }
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                EfficiencyCodeErrorDetails::UsageLimitReached(e) => {
...
pub(crate) async fn built_tools(
    ...
    mcp: &Arc<EfficiencyCode_mcp::McpBinding>,
    step_store: &ExtensionData,
    prepared_recommendations: PreparedToolRecommendations,
) -> EfficiencyCodeResult<Arc<ToolRouter>> {
...
pub(super) fn agent_message_text(
    item: &EfficiencyCode_protocol::items::AgentMessageItem,
) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            EfficiencyCode_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: EfficiencyCode_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(EfficiencyCode_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &EfficiencyCode_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, EfficiencyCodeResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> EfficiencyCodeResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> EfficiencyCodeResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, EfficiencyCodeResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: EfficiencyCodeResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            EfficiencyCode.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            EfficiencyCode.usage.reasoning_output_tokens = field::Empty,
            EfficiencyCode.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(EfficiencyCode_async_utils::CancelErr::Cancelled) => {
                break Err(EfficiencyCodeErr::TurnAborted);
            }
        };
        ...
        let event = match event {
            ...
            None => {
                break Err(EfficiencyCodeErr::Stream(
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => id
                            .as_ref()
                            .map(EfficiencyCode_protocol::ResponseItemId::as_str),
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![EfficiencyCode_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        EfficiencyCode_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
    ...
    if cancellation_token.is_cancelled() {
        return Err(EfficiencyCodeErr::TurnAborted);
    }
