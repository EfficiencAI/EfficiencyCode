...
use EfficiencyCode_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::EfficiencyCode_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                EfficiencyCode_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(EfficiencyCode_protocol::models::PermissionProfile::Disabled)
    ...
    config.EfficiencyCode_linux_sandbox_exe = EfficiencyCode_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        EfficiencyCode_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: EfficiencyCode_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let EfficiencyCode_home = tempdir().expect("create EfficiencyCode home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_EfficiencyCode_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(EfficiencyCode_thread_store::LocalThreadStore::new(
        EfficiencyCode_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        code_mode_session_provider: Arc::new(
            EfficiencyCode_code_mode::DisabledCodeModeSessionProvider,
        ),
        extensions: EfficiencyCode_extension_api::empty_extension_registry(),
