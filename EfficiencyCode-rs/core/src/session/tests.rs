...
use super::*;
use crate::EfficiencyCode_thread::TryStartTurnIfIdleRejectionReason;
...
use EfficiencyCode_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use EfficiencyCode_config::LoaderOverrides;
use EfficiencyCode_config::NetworkConstraints;
use EfficiencyCode_config::NetworkDomainPermissionToml;
use EfficiencyCode_config::NetworkDomainPermissionsToml;
use EfficiencyCode_config::RequirementSource;
use EfficiencyCode_config::Sourced;
use EfficiencyCode_config::loader::project_trust_key;
use EfficiencyCode_config::types::McpServerConfig;
use EfficiencyCode_config::types::McpServerTransportConfig;
use EfficiencyCode_config::types::ToolSuggestDisabledTool;
use core_test_support::test_EfficiencyCode::local_selections;

use EfficiencyCode_features::Feature;
use EfficiencyCode_http_client::ClientRouteClass;
use EfficiencyCode_http_client::HttpClientFactory;
use EfficiencyCode_http_client::OutboundProxyPolicy;
use EfficiencyCode_http_client::RouteAwareClientPool;
use EfficiencyCode_login::EfficiencyCodeAuth;
use EfficiencyCode_login::auth::AgentIdentityAuthPolicy;
...
use EfficiencyCode_models_manager::model_info;
...
use EfficiencyCode_protocol::AgentPath;
use EfficiencyCode_protocol::ResponseItemId;
use EfficiencyCode_protocol::SessionId;
use EfficiencyCode_protocol::ThreadId;
use EfficiencyCode_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use EfficiencyCode_protocol::config_types::ServiceTier;
use EfficiencyCode_protocol::config_types::TrustLevel;
use EfficiencyCode_protocol::exec_output::ExecToolCallOutput;
use EfficiencyCode_protocol::models::AgentMessageInputContent;
use EfficiencyCode_protocol::models::FileSystemPermissions;
use EfficiencyCode_protocol::models::FunctionCallOutputBody;
use EfficiencyCode_protocol::models::FunctionCallOutputContentItem;
use EfficiencyCode_protocol::models::FunctionCallOutputPayload;
use EfficiencyCode_protocol::models::ImageDetail;
use EfficiencyCode_protocol::models::PermissionProfile;
use EfficiencyCode_protocol::models::SandboxEnforcement;
use EfficiencyCode_protocol::openai_models::ModelServiceTier;
...
use EfficiencyCode_protocol::protocol::NonSteerableTurnKind;
use EfficiencyCode_protocol::protocol::SandboxPolicy;
use EfficiencyCode_protocol::protocol::TurnEnvironmentSelections;
use EfficiencyCode_protocol::request_permissions::PermissionGrantScope;
use EfficiencyCode_protocol::request_permissions::RequestPermissionProfile;
use EfficiencyCode_tools::ToolSpec;
...
use EfficiencyCode_config::config_toml::ConfigToml;
use EfficiencyCode_config::config_toml::ProjectConfig;
use EfficiencyCode_config::permissions_toml::FilesystemPermissionToml;
use EfficiencyCode_config::permissions_toml::FilesystemPermissionsToml;
use EfficiencyCode_config::permissions_toml::NetworkToml;
use EfficiencyCode_config::permissions_toml::PermissionProfileToml;
use EfficiencyCode_config::permissions_toml::PermissionsToml;
use EfficiencyCode_execpolicy::Decision;
use EfficiencyCode_execpolicy::NetworkRuleProtocol;
use EfficiencyCode_execpolicy::Policy;
use EfficiencyCode_history::CompactedItem;
use EfficiencyCode_history::EfficiencyCodeHarnessMetadata;
use EfficiencyCode_history::InitialHistory;
use EfficiencyCode_history::ResponseItemEnvelope;
use EfficiencyCode_history::ResumedHistory;
use EfficiencyCode_history::RolloutItem;
use EfficiencyCode_network_proxy::NetworkProxyConfig;
use EfficiencyCode_otel::MetricsClient;
use EfficiencyCode_otel::MetricsConfig;
use EfficiencyCode_otel::TelemetryAuthMode;
...
use EfficiencyCode_protocol::items::HookPromptFragment;
use EfficiencyCode_protocol::items::build_hook_prompt_message;
use EfficiencyCode_protocol::models::BaseInstructions;
use EfficiencyCode_protocol::models::ContentItem;
use EfficiencyCode_protocol::models::InternalChatMessageMetadataPassthrough;
...
use EfficiencyCode_protocol::protocol::CreditsSnapshot;
use EfficiencyCode_protocol::protocol::EfficiencyCodeErrorInfo;
use EfficiencyCode_protocol::protocol::GranularApprovalConfig;
use EfficiencyCode_protocol::protocol::InterAgentCommunication;
use EfficiencyCode_protocol::protocol::MultiAgentVersion;
use EfficiencyCode_protocol::protocol::NetworkApprovalProtocol;
use EfficiencyCode_protocol::protocol::RateLimitSnapshot;
use EfficiencyCode_protocol::protocol::RateLimitWindow;
use EfficiencyCode_protocol::protocol::RealtimeAudioFrame;
use EfficiencyCode_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use EfficiencyCode_protocol::protocol::RealtimeVoice;
use EfficiencyCode_protocol::protocol::RealtimeVoicesList;
use EfficiencyCode_protocol::protocol::SessionMeta;
use EfficiencyCode_protocol::protocol::SessionMetaLine;
use EfficiencyCode_protocol::protocol::Submission;
use EfficiencyCode_protocol::protocol::ThreadRolledBackEvent;
use EfficiencyCode_protocol::protocol::ThreadSettingsOverrides;
use EfficiencyCode_protocol::protocol::TokenCountEvent;
use EfficiencyCode_protocol::protocol::TokenUsage;
use EfficiencyCode_protocol::protocol::TokenUsageInfo;
use EfficiencyCode_protocol::protocol::TurnAbortedEvent;
use EfficiencyCode_protocol::protocol::TurnCompleteEvent;
use EfficiencyCode_protocol::protocol::TurnStartedEvent;
use EfficiencyCode_protocol::protocol::UserMessageEvent;
use EfficiencyCode_protocol::protocol::W3cTraceContext;
use EfficiencyCode_rmcp_client::ElicitationAction;
...
use core_test_support::test_EfficiencyCode::local;
...
use EfficiencyCode_protocol::mcp::CallToolResult as McpCallToolResult;
...
) -> Arc<EfficiencyCode_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &EfficiencyCode_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(EfficiencyCode_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    let attributes = single_histogram_attributes(&snapshot, "EfficiencyCode.test.extension");
    assert_eq!(
        attributes,
        BTreeMap::from([
            (
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl EfficiencyCode_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: EfficiencyCode_extension_api::WorldStateContributionInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<
            'a,
            Vec<EfficiencyCode_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("EfficiencyCode.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let mut builder =
        EfficiencyCode_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "EfficiencyCode.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("EfficiencyCode-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        EfficiencyCode_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_EfficiencyCode: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_EfficiencyCode)?;
    std::fs::write(
        dot_EfficiencyCode.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        EfficiencyCode_home.join(EfficiencyCode_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
    config: &crate::config::Config,
) -> std::io::Result<Vec<EfficiencyCode_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&EfficiencyCode_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: EfficiencyCode_hooks::StartHookTarget::SessionStart {
                source: EfficiencyCode_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn EfficiencyCode_network_proxy::NetworkPolicyDecider> =
        Arc::new({
            let decider_calls = Arc::clone(&decider_calls);
            move |_request| {
                decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                async { EfficiencyCode_network_proxy::NetworkDecision::ask("not_allowed") }
            }
        });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> EfficiencyCode_sandboxing::SandboxablePreference {
            EfficiencyCode_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(EfficiencyCode_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: EfficiencyCode_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let EfficiencyCode_home = session.EfficiencyCode_home().await;
    std::fs::create_dir_all(&EfficiencyCode_home).expect("create EfficiencyCode home");
    let config_toml_path = EfficiencyCode_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = EfficiencyCode_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let EfficiencyCode_home = session.EfficiencyCode_home().await;
    std::fs::create_dir_all(&EfficiencyCode_home).expect("create EfficiencyCode home");
    let base_config_path = EfficiencyCode_home.join(CONFIG_TOML_FILE);
    let profile_config_path = EfficiencyCode_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(EfficiencyCode_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            .features
            .enable(Feature::EfficiencyCodeHooks)
            .expect("enable EfficiencyCode hooks");
    })
    .await?;
    let EfficiencyCode_home = session.EfficiencyCode_home().await;
    std::fs::create_dir_all(&EfficiencyCode_home)?;
    let config_toml_path = EfficiencyCode_home.join(CONFIG_TOML_FILE);
    let user_config: EfficiencyCode_config::TomlValue =
    ...
    let request = EfficiencyCode_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = EfficiencyCode_hooks::list_hooks(EfficiencyCode_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..EfficiencyCode_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        EfficiencyCode_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: EfficiencyCode_config::TomlValue =
        serde_json::from_value(serde_json::json!({
            "hooks": {
                "SessionStart": [{
                    "hooks": [{
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: EfficiencyCode_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: EfficiencyCode_config::MatcherGroup {
                matcher: None,
                hooks: vec![EfficiencyCode_config::HookHandlerConfig::Command {
        ...
        let identity = EfficiencyCode_config::TomlValue::try_from(identity)?;
        EfficiencyCode_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&EfficiencyCode_home).expect("create EfficiencyCode home");
    std::fs::write(
        EfficiencyCode_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = EfficiencyCode_config::Constrained::normalized(
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let replacement_history = vec![
        ResponseItemEnvelope {
            ...
            metadata: Some(EfficiencyCodeHarnessMetadata::default()),
        },
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = EfficiencyCode_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: EfficiencyCode_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        EfficiencyCodeAuth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    )
    .await;
...
async fn prepares_resumed_history_before_installing_it() {
    ...
    assert_eq!(
        ...
        Some(EfficiencyCodeHarnessMetadata::default())
    );
}
...
async fn record_initial_history_seeds_token_info_from_rollout() {
    ...
    let info1 = TokenUsageInfo {
        total_token_usage: TokenUsage {
            ...
            total_tokens: 30,
            EfficiencyCode_rollout_budget_units: None,
        },
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl EfficiencyCode_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a EfficiencyCode_extension_api::ExtensionData,
            thread_store: &'a EfficiencyCode_extension_api::ExtensionData,
            turn_store: &'a EfficiencyCode_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl EfficiencyCode_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: EfficiencyCode_extension_api::TurnStartInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl EfficiencyCode_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: EfficiencyCode_extension_api::TurnErrorInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: EfficiencyCodeErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, EfficiencyCodeErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl EfficiencyCode_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &EfficiencyCode_extension_api::ExtensionData,
            thread_store: &EfficiencyCode_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: EfficiencyCode_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_EfficiencyCode().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_config(|config| {
        config.permissions.approval_policy =
            EfficiencyCode_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        EfficiencyCode_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "EfficiencyCode_core__EfficiencyCode_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: EfficiencyCode_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            EfficiencyCode_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            EfficiencyCode_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            EfficiencyCode_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.EfficiencyCode_error_info,
        Some(EfficiencyCodeErrorInfo::ThreadRollbackFailed)
    );
    assert_eq!(
...
async fn set_rate_limits_retains_previous_credits() {
    let EfficiencyCode_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(EfficiencyCode_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        EfficiencyCode_home: config.EfficiencyCode_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(EfficiencyCode_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("EfficiencyCode_other".to_string()),
        limit_name: Some("EfficiencyCode_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(EfficiencyCode_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("EfficiencyCode".to_string()),
            limit_name: None,
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.EfficiencyCode_error_info
                    == Some(EfficiencyCodeErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
...
async fn build_test_config(EfficiencyCode_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/EfficiencyCode/analytics-events/events"))
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("EfficiencyCode-tui".to_string()),
                ...
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events.iter().find(|event| {
                            event["event_type"] == "EfficiencyCode_thread_initialized"
                        })
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile =
        EfficiencyCode_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        EfficiencyCode_utils_absolute_path::canonicalize_preserving_symlinks(
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let EfficiencyCode_home = tempfile::tempdir().expect("create EfficiencyCode home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        EfficiencyCode_home
            .path()
            .join(EfficiencyCode_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let EfficiencyCode_home = parent_config.EfficiencyCode_home.clone();
    let skill_dir = EfficiencyCode_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&EfficiencyCode_exec_server::LOCAL_FS));
    ...
    let role_path = EfficiencyCode_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let EfficiencyCode_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(EfficiencyCode_home.path()).await;
    config
        .features
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(EfficiencyCodeAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.EfficiencyCode_home.to_path_buf(),
        auth_manager.clone(),
    ...
    let skills_service = Arc::new(HostSkillsService::new(
        config.EfficiencyCode_home.clone(),
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(EfficiencyCode_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(EfficiencyCode_extension_api::ExtensionRegistryBuilder::new().build()),
        EfficiencyCode_extension_api::ExtensionDataInit::default(),
        ClientMcpExtensions::default(),
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(EfficiencyCode_thread_store::LocalThreadStore::new(
            EfficiencyCode_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        EfficiencyCode_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        EfficiencyCode_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(EfficiencyCode_mcp::McpRuntime::empty(
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: EfficiencyCode_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(EfficiencyCode_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: EfficiencyCode_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: EfficiencyCode_extension_api::ExtensionData::new(
            thread_id.to_string(),
        ),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: EfficiencyCode_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(EfficiencyCode_thread_store::LocalThreadStore::new(
            EfficiencyCode_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .EfficiencyCode_home(config.EfficiencyCode_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(EfficiencyCode_thread_store::LocalThreadStore::new(
            EfficiencyCode_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                EfficiencyCode_state::StateRuntime::init(
                    config.sqlite.clone(),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            EfficiencyCode_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(EfficiencyCode_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    turn_context.as_ref(),
                    call_id,
                    EfficiencyCode_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: EfficiencyCode_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(EfficiencyCode_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            responsesapi_client_metadata: None,
            additional_context: Default::default(),
            thread_settings: EfficiencyCode_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(local_selections(config.cwd.clone())),
                approval_policy: Some(config.permissions.approval_policy.value()),
                approvals_reviewer: Some(
                    EfficiencyCode_config::types::ApprovalsReviewer::AutoReview,
                ),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(
                    EfficiencyCode_protocol::config_types::CollaborationMode {
                        mode: EfficiencyCode_protocol::config_types::ModeKind::Default,
                        settings: EfficiencyCode_protocol::config_types::Settings {
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = EfficiencyCode_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        EfficiencyCode_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        EfficiencyCode_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(EfficiencyCode_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn EfficiencyCode_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<EfficiencyCode_protocol::protocol::HookCompletedEvent>(
                json!({
                    "turn_id": "turn-1",
                    "run": {
    ...
    assert_eq!(
        EfficiencyCode_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl EfficiencyCode_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        fn on_thread_stop<'a>(
            &'a self,
            input: EfficiencyCode_extension_api::ThreadStopInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl EfficiencyCode_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: EfficiencyCode_extension_api::TurnAbortInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(EfficiencyCode_home).await;
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            EfficiencyCode_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = EfficiencyCode_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !EfficiencyCode_mcp::configured_mcp_servers(old_step.mcp.config())
            .contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        EfficiencyCode_mcp::configured_mcp_servers(rematerialized_old.config())
            .contains_key("refreshed"),
    ...
    assert!(
        EfficiencyCode_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_latest_runtime_authority() {
    ...
    let request = EfficiencyCode_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: EfficiencyCode_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        (
                            "EfficiencyCode_approval_kind".to_string(),
                            json!("mcp_tool_call"),
                        ),
                        (
                            "EfficiencyCode_request_type".to_string(),
...
async fn mcp_refresh_updates_plugin_auth_mode_before_checking_pending_state() {
    let EfficiencyCode_home = tempfile::tempdir().expect("create auth test directory");
    let (mut session, _turn_context) = make_session_and_context().await;
    session.services.auth_manager = AuthManager::from_auth_for_testing_with_home(
        EfficiencyCodeAuth::from_api_key("old-api-key"),
        EfficiencyCode_home.path().to_path_buf(),
    );
    let session = Arc::new(session);
    ...
    EfficiencyCode_login::login_with_api_key(
        EfficiencyCode_home.path(),
        "new-api-key",
        EfficiencyCode_login::AuthCredentialsStoreMode::File,
        EfficiencyCode_login::AuthKeyringBackendKind::default(),
    )
...
impl EfficiencyCode_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: EfficiencyCode_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<
            EfficiencyCode_exec_server::NoiseRendezvousConnectBundle,
            EfficiencyCode_exec_server::ExecServerError,
        >,
    > {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let selected_root = |environment_id: &str, path: &str| {
        EfficiencyCode_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: EfficiencyCode_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let EfficiencyCode_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        environment_manager
            .report_environment_provisioning_status(
                ...
                Ok(EfficiencyCode_exec_server::EnvironmentReadyInfo {
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(
        EfficiencyCode_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer")
    );
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(EfficiencyCode_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl EfficiencyCode_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a EfficiencyCode_extension_api::ExtensionData,
        thread_store: &'a EfficiencyCode_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<EfficiencyCode_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    EfficiencyCode_extension_api::PromptFragment::developer_policy(
...
-> Arc<EfficiencyCode_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = EfficiencyCode_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl EfficiencyCode_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: EfficiencyCode_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<EfficiencyCode_extension_api::PromptFragment>>
                + Send
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<EfficiencyCode_thread_store::InMemoryThreadStore> {
    let store = Arc::new(EfficiencyCode_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn EfficiencyCode_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &EfficiencyCode_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            _input: EfficiencyCode_extension_api::ThreadIdleInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = EfficiencyCode_protocol::user_input::TextElement::new(
        EfficiencyCode_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            input: EfficiencyCode_extension_api::ThreadIdleInput<'a>,
        ) -> EfficiencyCode_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(
            EfficiencyCode_extension_api::ThreadIdleCause::Completed,
        )
        .await;
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(EfficiencyCode_extension_api::ExtensionData::new(
            tc.sub_id.clone(),
        )),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use EfficiencyCode_protocol::protocol::AskForApproval;
    use EfficiencyCode_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: EfficiencyCode_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: EfficiencyCode_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let EfficiencyCode_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_EfficiencyCode = project_root.join(".EfficiencyCode");
    let nested_dot_EfficiencyCode = nested.join(".EfficiencyCode");

    std::fs::create_dir_all(&EfficiencyCode_home)?;
    std::fs::create_dir_all(&nested_dot_EfficiencyCode)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_EfficiencyCode)?;
    write_project_hooks(&nested_dot_EfficiencyCode)?;
    write_project_trust_config(&EfficiencyCode_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .EfficiencyCode_home(EfficiencyCode_home)
    ...
    let expected_source_path =
        EfficiencyCode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            nested_dot_EfficiencyCode.join("hooks.json"),
        )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_EfficiencyCode = project_root.join(".EfficiencyCode");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_EfficiencyCode)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let EfficiencyCode_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&EfficiencyCode_home)?;
        write_project_trust_config(&EfficiencyCode_home, &trust_entries).await?;
