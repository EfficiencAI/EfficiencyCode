...
use EfficiencyCode_exec_server::ExecutorCapabilityDiscoveryCache;
...
use EfficiencyCode_exec_server::ResolvedSelectedCapabilityRoot;
use EfficiencyCode_mcp::EfficiencyCode_APPS_MCP_SERVER_NAME;
use EfficiencyCode_mcp::ElicitationReviewRequest;
use EfficiencyCode_mcp::ElicitationReviewer;
use EfficiencyCode_mcp::ElicitationReviewerHandle;
...
use EfficiencyCode_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use EfficiencyCode_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use EfficiencyCode_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::PERSIST_KEY as MCP_ELICITATION_PERSIST_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use EfficiencyCode_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::STRICT_AUTO_REVIEW_KEY as MCP_ELICITATION_STRICT_AUTO_REVIEW_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use EfficiencyCode_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use EfficiencyCode_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        EfficiencyCode_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn refresh_mcp_if_dirty(self: &Arc<Self>) {
        ...
        loop {
            ...
            if self
                .services
                .plugins_manager
                .set_auth_mode(auth.as_ref().map(EfficiencyCodeAuth::api_auth_mode))
                || !self
                    .services
                    .mcp_runtime
    ...
    pub(crate) async fn hard_refresh_latest_EfficiencyCode_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<EfficiencyCode_mcp::ToolInfo>> {
        ...
        anyhow::ensure!(
            input
                .mcp_servers
                .contains_key(EfficiencyCode_APPS_MCP_SERVER_NAME),
            "unknown MCP server '{EfficiencyCode_APPS_MCP_SERVER_NAME}'"
        );
    ...
    pub(crate) async fn mcp_runtime_for_step(
    ...
    ) -> Arc<EfficiencyCode_mcp::McpBinding> {
        let ready_selected_capability_roots =
            Self::ready_selected_capability_roots(selected_capability_roots);
        if self
            .services
        ...
        Arc::new(EfficiencyCode_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> EfficiencyCode_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                EfficiencyCode_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: EfficiencyCode_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                EfficiencyCode_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                EfficiencyCode_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    if matches!(
        ...
        let trusted_guardian_request = if request.server_name == EfficiencyCode_APPS_MCP_SERVER_NAME
        {
    ...
    match approval_policy {
        AskForApproval::Never => {
            if EfficiencyCode_mcp::mcp_permission_prompt_is_auto_approved(
                approval_policy,
                &mcp_config.permission_profile,
                EfficiencyCode_mcp::McpPermissionPromptAutoApproveContext::default(),
            ) && matches!(
                &request.elicitation,
                Elicitation::Mcp(
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let EfficiencyCode_protocol::approvals::ElicitationRequest::Form {
