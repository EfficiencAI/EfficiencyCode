...
use EfficiencyCode_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use EfficiencyCode_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: AbortOnDropHandle<EfficiencyCodeResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    pub(crate) fn new(
        task: JoinHandle<EfficiencyCodeResult<ModelClientSession>>,
        started_at: Instant,
        timeout: Duration,
    ) -> Self {
        Self {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<
            EfficiencyCodeResult<ModelClientSession>,
...
async fn schedule_startup_prewarm_inner(
    ...
    let responses_metadata = startup_turn_context
        ...
        .to_responses_metadata(
            ...
            window_id,
            EfficiencyCodeResponsesRequestKind::Prewarm,
        );
