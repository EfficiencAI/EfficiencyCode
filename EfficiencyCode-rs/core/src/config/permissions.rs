...
use EfficiencyCode_config::permissions_toml::NetworkUnixSocketPermissionToml;
use EfficiencyCode_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use EfficiencyCode_features::NetworkProxyDomainPermissionToml;
use EfficiencyCode_features::NetworkProxyModeToml;
use EfficiencyCode_features::NetworkProxyUnixSocketPermissionToml;
use EfficiencyCode_network_proxy::NetworkMode;
use EfficiencyCode_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use EfficiencyCode_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use EfficiencyCode_protocol::permissions::project_roots_glob_pattern;
...
/// for EfficiencyCode to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_EfficiencyCode_runtime(
    EfficiencyCode_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root =
        AbsolutePathBuf::from_absolute_path(EfficiencyCode_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older EfficiencyCode versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of EfficiencyCode. Filesystem access will remain restricted. Upgrade EfficiencyCode if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of EfficiencyCode and will be ignored. Upgrade EfficiencyCode if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of EfficiencyCode and will be ignored. Upgrade EfficiencyCode if this path is required."
            ),
        },
    );
}
