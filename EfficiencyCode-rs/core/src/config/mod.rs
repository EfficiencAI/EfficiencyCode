...
use EfficiencyCode_config::ConfigRequirements;
use EfficiencyCode_config::ConfigRequirementsToml;
use EfficiencyCode_config::ConstrainedWithSource;
use EfficiencyCode_config::FeatureRequirementsToml;
use EfficiencyCode_config::ManagedAuthPolicy;
use EfficiencyCode_config::McpServerRequirement;
use EfficiencyCode_config::PluginRequirementsToml;
use EfficiencyCode_config::ProfileV2Name;
use EfficiencyCode_config::ResidencyRequirement;
use EfficiencyCode_config::SandboxModeRequirement;
use EfficiencyCode_config::Sourced;
use EfficiencyCode_config::ThreadConfigLoader;
use EfficiencyCode_config::config_toml::ConfigToml;
use EfficiencyCode_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use EfficiencyCode_config::config_toml::ProjectConfig;
use EfficiencyCode_config::config_toml::RealtimeAudioConfig;
use EfficiencyCode_config::config_toml::RealtimeConfig;
use EfficiencyCode_config::config_toml::ThreadStoreToml;
use EfficiencyCode_config::config_toml::validate_model_providers;
use EfficiencyCode_config::loader::load_config_layers_state;
use EfficiencyCode_config::loader::project_trust_key;
use EfficiencyCode_config::permissions_toml::PermissionsToml;
use EfficiencyCode_config::sandbox_mode_requirement_for_permission_profile;
use EfficiencyCode_config::types::ApprovalsReviewer;
use EfficiencyCode_config::types::AuthCredentialsStoreMode;
use EfficiencyCode_config::types::AuthKeyringBackendKind;
use EfficiencyCode_config::types::History;
use EfficiencyCode_config::types::McpServerConfig;
use EfficiencyCode_config::types::McpServerDisabledReason;
...
use EfficiencyCode_config::types::OAuthCredentialsStoreMode;
use EfficiencyCode_config::types::ResumeCwdMode;
use EfficiencyCode_config::types::SessionPickerViewMode;
use EfficiencyCode_config::types::ToolSuggestConfig;
use EfficiencyCode_config::types::ToolSuggestDisabledTool;
use EfficiencyCode_config::types::ToolSuggestDiscoverable;
...
use EfficiencyCode_config::types::UriBasedFileOpener;
use EfficiencyCode_config::types::WindowsSandboxModeToml;
use EfficiencyCode_core_plugins::PluginLoadOutcome;
use EfficiencyCode_core_plugins::PluginsConfigInput;
use EfficiencyCode_exec_server::ExecutorFileSystem;
use EfficiencyCode_exec_server::LOCAL_FS;
use EfficiencyCode_features::CodeModeConfigToml;
use EfficiencyCode_features::CurrentTimeReminderConfigToml;
use EfficiencyCode_features::CurrentTimeReminderDeliveryMode;
use EfficiencyCode_features::CurrentTimeSource;
use EfficiencyCode_features::Feature;
use EfficiencyCode_features::FeatureConfigSource;
use EfficiencyCode_features::FeatureOverrides;
use EfficiencyCode_features::FeatureToml;
use EfficiencyCode_features::Features;
use EfficiencyCode_features::FeaturesToml;
use EfficiencyCode_features::MultiAgentV2ConfigToml;
use EfficiencyCode_features::NetworkProxyConfigToml;
use EfficiencyCode_features::TokenBudgetConfigToml;
...
use EfficiencyCode_install_context::InstallContext;
use EfficiencyCode_login::AuthManagerConfig;
use EfficiencyCode_login::AuthRouteConfig;
use EfficiencyCode_mcp::McpConfig;
use EfficiencyCode_mcp::McpPluginAttribution;
use EfficiencyCode_mcp::McpProtocolMode;
use EfficiencyCode_mcp::McpServerRegistration;
use EfficiencyCode_mcp::ResolvedMcpCatalog;
use EfficiencyCode_memories_read::memory_root;
use EfficiencyCode_model_provider::ProviderCapabilities;
use EfficiencyCode_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use EfficiencyCode_model_provider_info::ModelProviderInfo;
use EfficiencyCode_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use EfficiencyCode_model_provider_info::built_in_model_providers;
use EfficiencyCode_model_provider_info::merge_configured_model_providers;
use EfficiencyCode_models_manager::ModelsManagerConfig;
use EfficiencyCode_protocol::config_types::AltScreenMode;
use EfficiencyCode_protocol::config_types::AutoCompactTokenLimitScope;
...
use EfficiencyCode_protocol::config_types::ShellEnvironmentPolicy;
use EfficiencyCode_protocol::config_types::TrustLevel;
use EfficiencyCode_protocol::config_types::Verbosity;
use EfficiencyCode_protocol::config_types::WebSearchConfig;
use EfficiencyCode_protocol::config_types::WebSearchMode;
use EfficiencyCode_protocol::config_types::WindowsSandboxLevel;
use EfficiencyCode_protocol::models::ActivePermissionProfile;
use EfficiencyCode_protocol::models::BaseInstructionsProvenance;
use EfficiencyCode_protocol::models::PermissionProfile;
use EfficiencyCode_protocol::models::SandboxEnforcement;
use EfficiencyCode_protocol::openai_models::ModelsResponse;
...
pub use EfficiencyCode_thread_store::ExtraConfig;
use EfficiencyCode_utils_absolute_path::AbsolutePathBuf;
use EfficiencyCode_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_EfficiencyCode_runtime;
...
#[cfg(test)]
mod schema;
pub use EfficiencyCode_config::ConfigLoadOptions;
pub use EfficiencyCode_config::Constrained;
pub use EfficiencyCode_config::ConstraintError;
pub use EfficiencyCode_config::ConstraintResult;
pub use EfficiencyCode_config::LoaderOverrides;
pub use EfficiencyCode_network_proxy::NetworkProxyAuditMetadata;
use EfficiencyCode_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use EfficiencyCode_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(EfficiencyCode_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(EfficiencyCode_home.path())
...
impl Permissions {
    ...
    /// selection. Internal EfficiencyCode-only writable roots are intentionally excluded.
...
// when EfficiencyCode is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<EfficiencyCode_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, EfficiencyCode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - EfficiencyCode
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.EfficiencyCode/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "EfficiencyCode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send EfficiencyCode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the EfficiencyCode home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that EfficiencyCode can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by EfficiencyCode unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/EfficiencyCode/blob/main/EfficiencyCode-rs/rmcp-client/src/oauth.rs#L2
    /// file: EfficiencyCode_HOME/.credentials.json
    ///       This file will be readable to EfficiencyCode and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, EfficiencyCode will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all EfficiencyCode state (defaults to `~/.EfficiencyCode` but can be
    /// overridden by the `EfficiencyCode_HOME` environment variable).
    pub EfficiencyCode_home: AbsolutePathBuf,

    /// Resolved configuration shared by all EfficiencyCode SQLite databases.
    pub sqlite: EfficiencyCode_state::SqliteConfig,

    /// Directory where EfficiencyCode writes log files (defaults to `$EfficiencyCode_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.EfficiencyCode/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current EfficiencyCode executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub EfficiencyCode_self_exe: Option<PathBuf>,

    /// Path to the `EfficiencyCode-linux-sandbox` executable. This must be set if
    /// [`EfficiencyCode_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `EfficiencyCode-linux-sandbox`.
    pub EfficiencyCode_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `EfficiencyCode-execve-wrapper` executable used for shell
    ...
    /// Whether EfficiencyCode-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for EfficiencyCode updates on startup and surfaces update prompts.
    /// Set to `false` only if your EfficiencyCode updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across EfficiencyCode product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across EfficiencyCode product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: EfficiencyCode_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn EfficiencyCode_home(&self) -> PathBuf {
        self.EfficiencyCode_home.to_path_buf()
    }
...
pub struct ConfigBuilder {
    EfficiencyCode_home: Option<PathBuf>,
...
impl ConfigBuilder {
    pub fn EfficiencyCode_home(mut self, EfficiencyCode_home: PathBuf) -> Self {
        self.EfficiencyCode_home = Some(EfficiencyCode_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let EfficiencyCode_home = match EfficiencyCode_home {
            Some(EfficiencyCode_home) => AbsolutePathBuf::from_absolute_path(EfficiencyCode_home)?,
            None => find_EfficiencyCode_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&EfficiencyCode_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) =
                    EfficiencyCode_config::first_layer_config_error::<ConfigToml>(
                        &config_layer_stack,
                        EfficiencyCode_config::CONFIG_TOML_FILE,
                    )
                    .await
                {
                    return Err(EfficiencyCode_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &EfficiencyCode_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &EfficiencyCode_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            EfficiencyCode_home: self.EfficiencyCode_home.to_path_buf(),
            ...
            EfficiencyCode_linux_sandbox_exe: self.EfficiencyCode_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                EfficiencyCode_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.EfficiencyCode_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_EfficiencyCode_home(
            EfficiencyCode_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific EfficiencyCode home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_EfficiencyCode_home(
        ...
        let cli_layer = EfficiencyCode_config::build_cli_overrides_layer(&cli_overrides);
        EfficiencyCode_config::merge_toml_values(&mut merged, &cli_layer);
        let EfficiencyCode_home = AbsolutePathBuf::from_absolute_path_checked(EfficiencyCode_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &EfficiencyCode_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `EfficiencyCode exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `EfficiencyCode_self_exe`, `EfficiencyCode_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(EfficiencyCode_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, EfficiencyCode_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .EfficiencyCode/ folders.
...
/// Patch `EfficiencyCode_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(EfficiencyCode_home: &Path, provider: &str) -> std::io::Result<()> {
    EfficiencyCode_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &EfficiencyCode_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            EfficiencyCode_protocol::permissions::FileSystemSandboxEntry {
                path: EfficiencyCode_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: EfficiencyCode_protocol::permissions::FileSystemAccessMode::Deny,
            ...
            EfficiencyCode_protocol::permissions::FileSystemSandboxEntry {
                path: EfficiencyCode_protocol::permissions::FileSystemPath::Path { path },
                access: EfficiencyCode_protocol::permissions::FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&EfficiencyCode_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    async fn load_from_base_config_with_overrides(
        ...
        EfficiencyCode_home: AbsolutePathBuf,
    ) -> std::io::Result<Self> {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let ConfigOverrides {
            ...
            EfficiencyCode_linux_sandbox_exe,
            main_execve_wrapper_exe,
            default_zsh_path,
            base_instructions,
        ...
        let memories_root = memory_root(&EfficiencyCode_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(EfficiencyCode_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| EfficiencyCode_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| EfficiencyCode_home.clone());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_EfficiencyCode_runtime(
            &EfficiencyCode_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            EfficiencyCode_home,
            sqlite: EfficiencyCode_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        EfficiencyCode_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
/// Returns the path to the EfficiencyCode configuration directory, which can be
/// specified by the `EfficiencyCode_HOME` environment variable. If not set, defaults to
/// `~/.EfficiencyCode`.
///
/// - If `EfficiencyCode_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `EfficiencyCode_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_EfficiencyCode_home() -> std::io::Result<AbsolutePathBuf> {
    EfficiencyCode_utils_home_dir::find_EfficiencyCode_home()
}

/// Returns the path to the folder where EfficiencyCode logs are stored. Does not verify
