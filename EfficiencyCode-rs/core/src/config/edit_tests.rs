...
fn blocking_set_model_top_level() {
    ...
    let EfficiencyCode_home = tmp.path();
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = EfficiencyCode_home.join("config-link.toml");
    let config_path = EfficiencyCode_home.join(CONFIG_TOML_FILE);
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let EfficiencyCode_home = tmp.path();
    let link_a = EfficiencyCode_home.join("a.toml");
    let link_b = EfficiencyCode_home.join("b.toml");
    let config_path = EfficiencyCode_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(EfficiencyCode_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
#[test]
fn blocking_set_hide_gpt_5_1_EfficiencyCode_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let EfficiencyCode_home = tmp.path();
    std::fs::write(
        EfficiencyCode_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        EfficiencyCode_home,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-EfficiencyCode-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-EfficiencyCode-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
client_id = \"eci-prd-pub-EfficiencyCode-123\"
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !EfficiencyCode_home.join(CONFIG_TOML_FILE).exists(),
