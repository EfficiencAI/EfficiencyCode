...
use EfficiencyCode_config::ConfigRequirementsWithSources;
use EfficiencyCode_config::FilesystemDenyReadPattern;
use EfficiencyCode_config::LoaderOverrides;
use EfficiencyCode_config::RequirementSource;
use EfficiencyCode_config::RequirementsLayerEntry;
use EfficiencyCode_config::SessionThreadConfig;
use EfficiencyCode_config::StaticThreadConfigLoader;
use EfficiencyCode_config::ThreadConfigSource;
use EfficiencyCode_config::compose_requirements;
use EfficiencyCode_config::config_error_from_ignored_toml_fields;
use EfficiencyCode_config::config_error_from_toml;
use EfficiencyCode_config::config_error_from_typed_toml;
...
use EfficiencyCode_config::loader::load_requirements_toml;
...
use EfficiencyCode_protocol::config_types::EnvironmentVariablePattern;
...
async fn write_project_hook_config(
    dot_EfficiencyCode_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_EfficiencyCode_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_EfficiencyCode_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when EfficiencyCode_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .EfficiencyCode_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = EfficiencyCode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        EfficiencyCode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when EfficiencyCode_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_EfficiencyCode_home()
-> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let EfficiencyCode_home = tmp.path().join("EfficiencyCode-home");
...
"#
    ...
    let expected_log_dir =
        AbsolutePathBuf::resolve_path_against_base("~/.EfficiencyCode/logs", &EfficiencyCode_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(EfficiencyCode_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn system_requirements_control_in_app_updates() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let EfficiencyCode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&EfficiencyCode_home).await?;
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.EfficiencyCode".to_string(),
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_EfficiencyCode_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let EfficiencyCode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&EfficiencyCode_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            .expect("bundle requirements path should resolve against EfficiencyCode_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let EfficiencyCode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&EfficiencyCode_home)
        .await
        .expect("create EfficiencyCode home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            EfficiencyCode_config::SandboxModeRequirement::ReadOnly,
            EfficiencyCode_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    EfficiencyCode_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".EfficiencyCode")).await?;
    tokio::fs::create_dir_all(project_root.join(".EfficiencyCode")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".EfficiencyCode").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".EfficiencyCode").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project {
                dot_EfficiencyCode_folder,
            } => Some(dot_EfficiencyCode_folder),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(
        ...
        nested.join(".EfficiencyCode").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".EfficiencyCode").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".EfficiencyCode")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".EfficiencyCode")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".EfficiencyCode"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".EfficiencyCode"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".EfficiencyCode"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".EfficiencyCode"),
        Some("worktree-child"),
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".EfficiencyCode")).await?;
    tokio::fs::create_dir_all(repo_root.join(".EfficiencyCode")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(
        worktree_root.join(".EfficiencyCode").join(CONFIG_TOML_FILE),
        "foo =",
    )
    .await?;
    tokio::fs::write(
        repo_root.join(".EfficiencyCode").join(CONFIG_TOML_FILE),
        [0xff],
    )
    .await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".EfficiencyCode"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(".EfficiencyCode"),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_EfficiencyCode_and_override_in_order()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(
        project_root.join(".EfficiencyCode").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".EfficiencyCode").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let EfficiencyCode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&EfficiencyCode_home).await?;
    tokio::fs::write(EfficiencyCode_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_EfficiencyCode_exists_without_config_toml()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_EfficiencyCode_folder: AbsolutePathBuf::from_absolute_path(
                project_root.join(".EfficiencyCode"),
            )?,
        },
        TomlValue::Table(toml::map::Map::new()),
...
#[tokio::test]
async fn EfficiencyCode_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let EfficiencyCode_home = home_dir.join(".EfficiencyCode");
    tokio::fs::create_dir_all(&EfficiencyCode_home).await?;
    tokio::fs::write(
        EfficiencyCode_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn EfficiencyCode_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_EfficiencyCode = project_root.join(".EfficiencyCode");
    let nested_dot_EfficiencyCode = nested.join(".EfficiencyCode");

    tokio::fs::create_dir_all(&nested_dot_EfficiencyCode).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_EfficiencyCode.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_EfficiencyCode).await?;
    make_config_for_test(
        &project_dot_EfficiencyCode,
        &project_root,
        TrustLevel::Trusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let user_config_path = project_dot_EfficiencyCode.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let EfficiencyCode_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&EfficiencyCode_home_untrusted).await?;
    make_config_for_test(
        &EfficiencyCode_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = EfficiencyCode_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let EfficiencyCode_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&EfficiencyCode_home_unknown).await?;
    tokio::fs::write(
        EfficiencyCode_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_EfficiencyCode).await?;
    ...
    // `.EfficiencyCode` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_EfficiencyCode.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
responses_api_metadata = { EfficiencyCode_security_surface = "attacker" }
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_EfficiencyCode.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.EfficiencyCode` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        let EfficiencyCode_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&EfficiencyCode_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let EfficiencyCode_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&EfficiencyCode_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &EfficiencyCode_home,
...
mod requirements_exec_policy_tests {
    ...
    use EfficiencyCode_config::ConfigRequirementsWithSources;
    use EfficiencyCode_config::RequirementSource;
    use EfficiencyCode_config::RequirementsExecPolicyDecisionToml;
    use EfficiencyCode_config::RequirementsExecPolicyParseError;
    use EfficiencyCode_config::RequirementsExecPolicyPatternTokenToml;
    use EfficiencyCode_config::RequirementsExecPolicyPrefixRuleToml;
    use EfficiencyCode_config::RequirementsExecPolicyToml;
    use EfficiencyCode_execpolicy::Decision;
    use EfficiencyCode_execpolicy::Evaluation;
    use EfficiencyCode_execpolicy::RuleMatch;
    use EfficiencyCode_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_EfficiencyCode_folder_with_requirements(
        dot_EfficiencyCode_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_EfficiencyCode_folder =
            AbsolutePathBuf::from_absolute_path(dot_EfficiencyCode_folder)
                .expect("absolute dot_EfficiencyCode_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack = config_stack_for_dot_EfficiencyCode_folder_with_requirements(
            temp_dir.path(),
            requirements,
        );
