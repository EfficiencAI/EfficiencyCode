...
use EfficiencyCode_config::ConfigLayerEntry;
use EfficiencyCode_config::ConfigLayerSource;
use EfficiencyCode_config::ConfigLayerStack;
use EfficiencyCode_config::McpServerCommandMatcher;
use EfficiencyCode_config::McpServerIdentity;
use EfficiencyCode_config::McpServerRequirement;
use EfficiencyCode_config::McpServerValueMatcher;
use EfficiencyCode_config::ProfileV2Name;
use EfficiencyCode_config::RequirementSource;
use EfficiencyCode_config::Sourced;
use EfficiencyCode_config::config_toml::AgentRoleToml;
use EfficiencyCode_config::config_toml::AgentsToml;
use EfficiencyCode_config::config_toml::AutoReviewToml;
use EfficiencyCode_config::config_toml::ConfigToml;
use EfficiencyCode_config::config_toml::ExperimentalRequestUserInput;
use EfficiencyCode_config::config_toml::ProjectConfig;
use EfficiencyCode_config::config_toml::RealtimeConfig;
use EfficiencyCode_config::config_toml::RealtimeToml;
use EfficiencyCode_config::config_toml::RealtimeTransport;
use EfficiencyCode_config::config_toml::RealtimeWsMode;
use EfficiencyCode_config::config_toml::RealtimeWsVersion;
use EfficiencyCode_config::config_toml::ToolsToml;
...
use EfficiencyCode_config::permissions_toml::NetworkDomainPermissionToml;
use EfficiencyCode_config::permissions_toml::NetworkDomainPermissionsToml;
use EfficiencyCode_config::permissions_toml::NetworkMitmActionToml;
use EfficiencyCode_config::permissions_toml::NetworkMitmHookToml;
use EfficiencyCode_config::permissions_toml::NetworkMitmToml;
...
use EfficiencyCode_config::permissions_toml::WorkspaceRootsToml;
use EfficiencyCode_config::types::AppToolApproval;
use EfficiencyCode_config::types::ApprovalsReviewer;
use EfficiencyCode_config::types::BundledSkillsConfig;
use EfficiencyCode_config::types::FeedbackConfigToml;
use EfficiencyCode_config::types::HistoryPersistence;
use EfficiencyCode_config::types::McpServerEnvVar;
use EfficiencyCode_config::types::McpServerOAuthConfig;
use EfficiencyCode_config::types::McpServerToolConfig;
use EfficiencyCode_config::types::McpServerTransportConfig;
use EfficiencyCode_config::types::MemoriesConfig;
use EfficiencyCode_config::types::MemoriesToml;
use EfficiencyCode_config::types::ModelAvailabilityNuxConfig;
use EfficiencyCode_config::types::Notice;
use EfficiencyCode_config::types::NotificationCondition;
use EfficiencyCode_config::types::NotificationMethod;
use EfficiencyCode_config::types::Notifications;
use EfficiencyCode_config::types::OtelConfigToml;
use EfficiencyCode_config::types::OtelExporterKind;
use EfficiencyCode_config::types::ResumeCwdMode;
use EfficiencyCode_config::types::SandboxWorkspaceWrite;
use EfficiencyCode_config::types::SessionPickerViewMode;
use EfficiencyCode_config::types::SkillsConfig;
use EfficiencyCode_config::types::ToolSuggestDisabledTool;
use EfficiencyCode_config::types::ToolSuggestDiscoverableType;
use EfficiencyCode_config::types::Tui;
use EfficiencyCode_config::types::TuiKeymap;
use EfficiencyCode_config::types::TuiNotificationSettings;
use EfficiencyCode_config::types::TuiPetAnchor;
use EfficiencyCode_config::types::WindowsSandboxModeToml;
use EfficiencyCode_config::types::WindowsToml;
use EfficiencyCode_exec_server::LOCAL_FS;
use EfficiencyCode_features::Feature;
use EfficiencyCode_features::FeaturesToml;
use EfficiencyCode_model_provider::ProviderCapabilities;
use EfficiencyCode_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use EfficiencyCode_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use EfficiencyCode_model_provider_info::WireApi;
use EfficiencyCode_models_manager::bundled_models_response;
use EfficiencyCode_network_proxy::NetworkMode;
use EfficiencyCode_protocol::config_types::ModelProviderAuthInfo;
...
use EfficiencyCode_protocol::models::ActivePermissionProfile;
use EfficiencyCode_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use EfficiencyCode_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use EfficiencyCode_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use EfficiencyCode_protocol::models::ManagedFileSystemPermissions;
...
use EfficiencyCode_protocol::protocol::NetworkAccess;
use EfficiencyCode_protocol::protocol::RealtimeVoice;
use EfficiencyCode_protocol::protocol::SandboxPolicy;
use EfficiencyCode_utils_path_uri::LegacyAppPathString;
...
use EfficiencyCode_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: EfficiencyCode_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
async fn load_config_normalizes_relative_cwd_override() -> std::io::Result<()> {
    ...
    let EfficiencyCode_home = tempdir()?;
...
excluded_tool_namespaces = ["mcp__EfficiencyCode_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            "mcp__EfficiencyCode_apps".to_string(),
...
profile = "EfficiencyCode-bedrock"
"#,
    )
    .unwrap_err();
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        EfficiencyCode_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        EfficiencyCode_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        EfficiencyCode_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = EfficiencyCode_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let EfficiencyCode_home = temp_dir.path().join("EfficiencyCode-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".EfficiencyCode"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".EfficiencyCode".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".EfficiencyCode"), cwd.as_path()),
            "expected .EfficiencyCode carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".EfficiencyCode"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".EfficiencyCode"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of EfficiencyCode and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of EfficiencyCode and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of EfficiencyCode."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    EfficiencyCode_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            EfficiencyCode_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, EfficiencyCode_home.path());
    let project_dot_EfficiencyCode = AbsolutePathBuf::resolve_path_against_base(
        "project/.EfficiencyCode",
    ...
    let requirements_toml = EfficiencyCode_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = EfficiencyCode_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_EfficiencyCode_folder: project_dot_EfficiencyCode.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(EfficiencyCode_home.path().to_path_buf()),
            ..Default::default()
        },
        EfficiencyCode_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_EfficiencyCode_folder: project_dot_EfficiencyCode,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let plugin_root = EfficiencyCode_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin"))?;
    std::fs::write(
        plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            EfficiencyCode_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let default_zsh_path = EfficiencyCode_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_EfficiencyCode_home_for_workspace_write() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), EfficiencyCode_home.path());
...
-> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let memories_root = EfficiencyCode_home.path().join("memories");
    let writable_root = EfficiencyCode_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".EfficiencyCode");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let managed_path = EfficiencyCode_home.path().join("managed_config.toml");
    let config_path = EfficiencyCode_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = EfficiencyCode_home.path().abs();
    let config_layer_stack = load_config_layers_state(
        ...
        &Vec::new(),
        overrides,
        &EfficiencyCode_config::NoopThreadConfigLoader,
    )
    .await?;
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(EfficiencyCode_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(EfficiencyCode_home.path());
    ...
    let loaded = load_global_mcp_servers(EfficiencyCode_home.path()).await?;
...
selected-avatar-id = "EfficiencyCode"
...
"#,
    ...
    assert_eq!(
        ...
        Some(&serde_json::json!("EfficiencyCode"))
    );
    assert_eq!(
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    assert_eq!(
        ...
        EfficiencyCode_mcp::McpProtocolMode::Legacy
    );
    ...
    assert_eq!(
        mcp_config.protocol_mode,
        EfficiencyCode_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let err = load_global_mcp_servers(EfficiencyCode_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/EfficiencyCode-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/EfficiencyCode-mcp""#),
    ...
    let servers = BTreeMap::from([
        (
            ...
            McpServerConfig {
                ...
                environment_id: EfficiencyCode_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID
                    .to_string(),
                enabled: true,
                required: false,
                supports_parallel_tool_calls: false,
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-EfficiencyCode-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-EfficiencyCode-123""#));
    ...
    assert_eq!(
        docs.oauth_client_id(),
        Some("eci-prd-pub-EfficiencyCode-123")
    );
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(EfficiencyCode_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized =
        tokio::fs::read_to_string(EfficiencyCode_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let base_config = EfficiencyCode_home.path().join(CONFIG_TOML_FILE);
    let selected_config = EfficiencyCode_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(EfficiencyCode_home.path(), &profile_name),
        EfficiencyCode_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn EfficiencyCode_home(&self) -> AbsolutePathBuf {
        self.EfficiencyCode_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let workspace = EfficiencyCode_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, EfficiencyCode_home.abs())
            .await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        EfficiencyCode_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let missing_path = EfficiencyCode_home
        .path()
        .join("agents")
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let role_config_path = EfficiencyCode_home
        .path()
        .join("agents")
        .join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = EfficiencyCode_config::ConfigLayerStack::new(
        vec![EfficiencyCode_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: EfficiencyCode_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        EfficiencyCode_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".EfficiencyCode").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let researcher_path = EfficiencyCode_home
        .path()
        .join("agents")
        .join("researcher.toml");
    let reviewer_path = EfficiencyCode_home
        .path()
        .join("agents")
        .join("reviewer.toml");
    tokio::fs::create_dir_all(
...
"#
    ...
    let home_agents_dir = EfficiencyCode_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let catalog_path = EfficiencyCode_home.path().join("catalog.json");
...
"#;
    ...
    let EfficiencyCode_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        EfficiencyCode_home: EfficiencyCode_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: EfficiencyCode_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let EfficiencyCode_home = fixture.EfficiencyCode_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = EfficiencyCode_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![
            EfficiencyCode_config::WebSearchModeRequirement::Cached,
        ]),
    ...
    let requirement_source = EfficiencyCode_config::RequirementSource::Unknown;
    ...
    let requirements = EfficiencyCode_config::ConfigRequirements {
        web_search_mode: EfficiencyCode_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        EfficiencyCode_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/EfficiencyCode4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/EfficiencyCode3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/EfficiencyCode2");
...
[projects."/Users/mbolin/code/EfficiencyCode4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/EfficiencyCode3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let EfficiencyCode_home = temp_dir.path();
    let config_path = EfficiencyCode_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(EfficiencyCode_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(EfficiencyCode_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(EfficiencyCode_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(EfficiencyCode_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; EfficiencyCode would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let cwd = EfficiencyCode_home.path().join("workspace");
    let outside_root = EfficiencyCode_home.path().join("outside-write");
...
-> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let selected_config = EfficiencyCode_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let catalog_path = EfficiencyCode_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = EfficiencyCode_home.path().join("required-state");
    let required_log_dir = EfficiencyCode_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&EfficiencyCode_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let config =
        load_with_enterprise_requirement(&EfficiencyCode_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(EfficiencyCode_home.path().join("required-state"))?;
    let environment = EfficiencyCode_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$EfficiencyCode_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );
