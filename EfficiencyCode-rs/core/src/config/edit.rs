...
pub fn apply_blocking(EfficiencyCode_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(EfficiencyCode_home: &Path) -> Self {
        Self::for_config_path(&EfficiencyCode_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(EfficiencyCode_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| {
                config
                    .EfficiencyCode_home
