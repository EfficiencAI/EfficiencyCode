...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: EfficiencyCodeResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, EfficiencyCodeResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        EfficiencyCodeResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-EfficiencyCode-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-EfficiencyCode-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert_eq!(
        json["EfficiencyCode_security_surface"].as_str(),
        Some("sdk")
    );
    ...
    assert!(json.get("x-EfficiencyCode-installation-id").is_none());
    assert!(json.get("x-EfficiencyCode-parent-thread-id").is_none());
    ...
    assert_eq!(
        model_request_json["EfficiencyCode_security_surface"].as_str(),
        Some("sdk")
    );
    assert_eq!(
    ...
    let compatibility_metadata: Value = serde_json::from_str(
        compatibility_headers
            .get("x-EfficiencyCode-turn-metadata")
    ...
    assert!(meta.get("EfficiencyCode_security_surface").is_none());
...
fn turn_metadata_state_overlays_compaction_only_on_compaction_requests() {
    ...
    assert_eq!(
        compact_json["EfficiencyCode_security_surface"].as_str(),
        Some("sdk")
    );
    assert_eq!(
    ...
    assert_eq!(
        regular_json["EfficiencyCode_security_surface"].as_str(),
        Some("sdk")
    );
