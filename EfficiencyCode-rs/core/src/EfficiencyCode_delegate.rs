...
/// Start an interactive sub-EfficiencyCode thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_EfficiencyCode_thread_interactive(
    config: Config,
...
) -> Result<(Arc<Session>, SessionIo), EfficiencyCodeErr> {
    ...
    let (session, io) = Box::pin(Session::spawn(SessionSpawnArgs {
        ...
        parent_rollout_thread_trace: EfficiencyCode_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
        environment_selections: parent_environments.to_selections(),
        thread_extension_init: EfficiencyCode_extension_api::ExtensionDataInit::default(),
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_EfficiencyCode_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                EfficiencyCode_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                EfficiencyCode_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                EfficiencyCode_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            EfficiencyCode_protocol::request_user_input::RequestUserInputAnswer {
...
) -> EfficiencyCode_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = EfficiencyCode_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(
                    approval_id,
                    EfficiencyCode_protocol::protocol::ReviewDecision::Abort,
                )
                .await;
            EfficiencyCode_protocol::protocol::ReviewDecision::Abort
        }
...
#[cfg(test)]
#[path = "EfficiencyCode_delegate_tests.rs"]
mod tests;
