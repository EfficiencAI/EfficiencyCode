...
//!
//! `ModelClient` is intended to live for the lifetime of a EfficiencyCode session and holds the stable
...
//! per-turn state such as the `x-EfficiencyCode-turn-state` token used for sticky routing.
//!
...
use EfficiencyCode_api::AgentIdentityTelemetry;
use EfficiencyCode_api::ApiError;
use EfficiencyCode_api::AuthProvider;
use EfficiencyCode_api::CompactClient as ApiCompactClient;
use EfficiencyCode_api::CompactionInput as ApiCompactionInput;
use EfficiencyCode_api::Compression;
use EfficiencyCode_api::MemoriesClient as ApiMemoriesClient;
use EfficiencyCode_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use EfficiencyCode_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use EfficiencyCode_api::Provider as ApiProvider;
use EfficiencyCode_api::RawMemory as ApiRawMemory;
use EfficiencyCode_api::RealtimeCallClient as ApiRealtimeCallClient;
use EfficiencyCode_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use EfficiencyCode_api::Reasoning;
use EfficiencyCode_api::ReasoningContext;
use EfficiencyCode_api::RequestTelemetry;
use EfficiencyCode_api::ReqwestTransport;
use EfficiencyCode_api::ResponseCreateWsRequest;
use EfficiencyCode_api::ResponsesApiRequest;
use EfficiencyCode_api::ResponsesClient as ApiResponsesClient;
use EfficiencyCode_api::ResponsesOptions as ApiResponsesOptions;
use EfficiencyCode_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use EfficiencyCode_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use EfficiencyCode_api::ResponsesWsRequest;
use EfficiencyCode_api::SharedAuthProvider;
use EfficiencyCode_api::SseTelemetry;
use EfficiencyCode_api::StreamOptions;
use EfficiencyCode_api::TransportError;
use EfficiencyCode_api::WebsocketTelemetry;
use EfficiencyCode_api::auth_header_telemetry;
use EfficiencyCode_api::build_session_headers;
use EfficiencyCode_api::create_text_param_for_request;
use EfficiencyCode_api::response_create_client_metadata;
...
use EfficiencyCode_login::RefreshTokenError;
use EfficiencyCode_login::UnauthorizedRecovery;
use EfficiencyCode_login::default_client::add_originator_header;
use EfficiencyCode_login::default_client::create_client_for_route;
use EfficiencyCode_otel::SessionTelemetry;
use EfficiencyCode_otel::current_span_w3c_trace_context;
use EfficiencyCode_protocol::auth::AuthMode;

use EfficiencyCode_protocol::ThreadId;
use EfficiencyCode_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use EfficiencyCode_protocol::config_types::Verbosity as VerbosityConfig;
...
use EfficiencyCode_protocol::protocol::InternalSessionSource;
use EfficiencyCode_protocol::protocol::SessionSource;
use EfficiencyCode_protocol::protocol::W3cTraceContext;
use EfficiencyCode_rollout_trace::CompactionTraceContext;
use EfficiencyCode_rollout_trace::InferenceTraceAttempt;
use EfficiencyCode_rollout_trace::InferenceTraceContext;
use EfficiencyCode_tools::create_tools_json_for_responses_api;
use EfficiencyCode_tools::create_tools_json_for_responses_lite;
use EfficiencyCode_tools::create_tools_raw_json_for_responses_api;
...
use EfficiencyCode_feedback::FeedbackRequestTags;
use EfficiencyCode_feedback::emit_feedback_request_tags_with_auth_env;
use EfficiencyCode_login::auth::AgentIdentityAuthPolicy;
use EfficiencyCode_login::auth_env_telemetry::AuthEnvTelemetry;
use EfficiencyCode_login::auth_env_telemetry::collect_auth_env_telemetry;
use EfficiencyCode_model_provider::AgentIdentitySessionFallback;
use EfficiencyCode_model_provider::ProviderAuthScope;
use EfficiencyCode_model_provider::SharedModelProvider;
use EfficiencyCode_model_provider::create_model_provider;
#[cfg(test)]
use EfficiencyCode_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use EfficiencyCode_protocol::error::Result;
use EfficiencyCode_response_debug_context::extract_response_debug_context;
use EfficiencyCode_response_debug_context::extract_response_debug_context_from_api_error;
use EfficiencyCode_response_debug_context::telemetry_api_error_message;
use EfficiencyCode_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_EfficiencyCode_INSTALLATION_ID_HEADER: &str = "x-EfficiencyCode-installation-id";
pub const X_EfficiencyCode_ROUTING_HINT_HEADER: &str = "x-EfficiencyCode-routing-hint";
pub const X_EfficiencyCode_TURN_STATE_HEADER: &str = "x-EfficiencyCode-turn-state";
pub const X_EfficiencyCode_TURN_METADATA_HEADER: &str = "x-EfficiencyCode-turn-metadata";
pub const X_EfficiencyCode_PARENT_THREAD_ID_HEADER: &str = "x-EfficiencyCode-parent-thread-id";
pub const X_EfficiencyCode_WINDOW_ID_HEADER: &str = "x-EfficiencyCode-window-id";
...
const X_EfficiencyCode_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-EfficiencyCode-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_EfficiencyCode_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_EfficiencyCode_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-EfficiencyCode-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a EfficiencyCode session
...
/// - The `x-EfficiencyCode-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each EfficiencyCode turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-EfficiencyCode-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-EfficiencyCode-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a EfficiencyCode session. Per-turn values
    ...
    pub fn new(
        ...
        let EfficiencyCode_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.EfficiencyCode_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), EfficiencyCode_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &EfficiencyCodeResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "EfficiencyCode.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &EfficiencyCodeResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup
                    .auth
                    .as_ref()
                    .map(EfficiencyCodeAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_EfficiencyCode_INSTALLATION_ID_HEADER, header_value);
        }
        ...
        if let Some(header_value) = self.build_routing_hint_header(
            ...
            extra_headers.insert(X_EfficiencyCode_ROUTING_HINT_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery:
                EfficiencyCode_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    fn build_routing_hint_header(
        ...
        if !auth.is_some_and(EfficiencyCodeAuth::uses_EfficiencyCode_backend)
            || !provider.is_openai()
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: EfficiencyCode_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &EfficiencyCodeResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                EfficiencyCode_login::default_client::default_headers(),
                /*turn_state*/ None,
    ...
    async fn build_websocket_headers(
        ...
        if let Some(routing_hint) = &responses_metadata.routing_hint {
            headers.insert(X_EfficiencyCode_ROUTING_HINT_HEADER, routing_hint.clone());
        }
        if let Some(header_value) = self.generate_attestation_header_for().await {
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&EfficiencyCodeAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(EfficiencyCodeAuth::uses_EfficiencyCode_backend)
    ...
    async fn stream_responses_api(
        ...
        loop {
            ...
            if let Some(header_value) = self.client.build_routing_hint_header(
                ...
                options
                    .extra_headers
                    .insert(X_EfficiencyCode_ROUTING_HINT_HEADER, header_value);
            }
            self.client
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(
                    X_EfficiencyCode_TURN_STATE_HEADER.to_string(),
                    turn_state.clone(),
                );
            }
            match self
    ...
    /// Permanently disables WebSockets for this EfficiencyCode session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .insert(
            X_EfficiencyCode_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement EfficiencyCode-specific conventions:
///
/// - `x-EfficiencyCode-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-EfficiencyCode-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-EfficiencyCode-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_EfficiencyCode_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_EfficiencyCode_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: EfficiencyCode_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let EfficiencyCode_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = EfficiencyCode_api::ResponseStream {
        rx_event,
...
/// the mapped `EfficiencyCodeErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: EfficiencyCode_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a EfficiencyCodeResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(EfficiencyCodeErr::RefreshTokenFailed(failed))
            }
            Err(RefreshTokenError::Transient(other)) => {
                ...
                Err(EfficiencyCodeErr::Io(other))
            }
        };
    }
