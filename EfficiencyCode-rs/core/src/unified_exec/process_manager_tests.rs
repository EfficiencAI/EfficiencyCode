...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("EfficiencyCode_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    assert_eq!(
        ...
        HashMap::from([
            ("PATH".to_string(), "/sandbox-path".to_string()),
            ("EfficiencyCode_THREAD_ID".to_string(), "thread-1".to_string()),
            (
                EfficiencyCode_PERMISSION_PROFILE_ENV_VAR.to_string(),
                "current-profile".to_string(),
            ),
            (
                EfficiencyCode_apply_patch::EfficiencyCode_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
                "1".to_string(),
            ),
            (
                "CODEX_SANDBOX_NETWORK_DISABLED".to_string(),
...
fn exec_env_policy_excludes_non_inheritable_and_runtime_variables() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "EfficiencyCode_permission_profile".to_string(),
            ...
            (
                "EfficiencyCode_apply_patch_preserve_line_endings".to_string(),
                "1".to_string(),
            ),
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: EfficiencyCode_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = EfficiencyCode_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        env: HashMap::from([
            ...
            (
                "EfficiencyCode_NETWORK_PROXY_ACTIVE".to_string(),
                "1".to_string(),
            ),
            (
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: EfficiencyCode_exec_server::ExecEnvPolicy {
                inherit: EfficiencyCode_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: EfficiencyCode_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = EfficiencyCode_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_sandbox = Some(
        EfficiencyCode_exec_server::FileSystemSandboxContext::from_permission_profile(
            permission_profile,
        ),
    );
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: EfficiencyCode_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let EfficiencyCode_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg
    else {
        panic!("expected ItemCompleted event");
    };
    let EfficiencyCode_protocol::items::TurnItem::CommandExecution(item) = completed_event.item
    else {
    ...
    assert_eq!(
        item.status,
        EfficiencyCode_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            EfficiencyCode_exec_server::WriteStatus::Accepted,
            /*terminate_error*/ None,
            EfficiencyCode_sandboxing::SandboxType::None,
        )
        .await,
    );
    exited_process
