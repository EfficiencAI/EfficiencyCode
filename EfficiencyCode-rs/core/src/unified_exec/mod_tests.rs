...
use EfficiencyCode_exec_server::ExecProcess;
use EfficiencyCode_exec_server::ExecProcessEventReceiver;
use EfficiencyCode_exec_server::ExecProcessFuture;
use EfficiencyCode_exec_server::ProcessId;
use EfficiencyCode_exec_server::ProcessSignal;
use EfficiencyCode_exec_server::ReadResponse;
use EfficiencyCode_exec_server::StartedExecProcess;
use EfficiencyCode_exec_server::WriteResponse;
use EfficiencyCode_exec_server::WriteStatus;
...
use core_test_support::test_EfficiencyCode::test_env as remote_test_env;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, EfficiencyCode_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, EfficiencyCode_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), EfficiencyCode_exec_server::ExecServerError> {
...
async fn blocking_terminate_unified_process(
    ...
    Ok(Arc::new(
        UnifiedExecProcess::from_exec_server_started(StartedExecProcess {
            ...
            sandbox_type: Some(EfficiencyCode_sandboxing::SandboxType::None),
        })
        .await?,
    ))
}
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export EfficiencyCode_INTERACTIVE_SHELL_VAR=EfficiencyCode\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $EfficiencyCode_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export EfficiencyCode_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $EfficiencyCode_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = EfficiencyCode_exec_server::Environment::default_for_tests();
