...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command::new("/bin/bash")
        ...
        .env("PWD", "/tmp/stale")
        .env(
            "NEXTEST_BIN_EXE_EfficiencyCode-write-config-schema",
            "/path/to/bin",
        )
        .env("BAD-NAME", "broken")
        .output()?;
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_EfficiencyCode-write-config-schema"));
...
fn zsh_snapshot_restores_tied_path() -> Result<()> {
    ...
    let readonly_restored = Command::new("/bin/zsh")
        ...
        .arg("-c")
        .arg(
            "set -e; . \"$1\"; export PATH='/EfficiencyCode-path':\"$PATH\"; print -r -- \"$PATH\"",
        )
        .arg("zsh")
        .arg(&snapshot_path)
        .env_clear()
    ...
    assert_eq!(
        ...
        "/EfficiencyCode-path:/usr/bin:/bin"
    );
...
async fn write_rollout_stub(EfficiencyCode_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    let dir = tempdir()?;
    let EfficiencyCode_home = dir.path().abs();
    let snapshot_dir = EfficiencyCode_home.join(SNAPSHOT_DIR);
    ...
    write_rollout_stub(&EfficiencyCode_home, live_session).await?;
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&EfficiencyCode_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&EfficiencyCode_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(&EfficiencyCode_home, active_session, /*state_db*/ None).await?;
