...
use EfficiencyCode_memories_read::citations::parse_memory_citation;
use EfficiencyCode_memories_read::citations::thread_ids_from_memory_citation;
use EfficiencyCode_protocol::error::EfficiencyCodeErr;
use EfficiencyCode_protocol::error::Result;
use EfficiencyCode_protocol::memory_citation::MemoryCitation;
...
use EfficiencyCode_utils_stream_parser::strip_proposed_plan_blocks;
use futures::Future;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<EfficiencyCode_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                EfficiencyCode_protocol::models::ContentItem::OutputText { text } => {
                    Some(text.as_str())
                }
                _ => None,
            })
            ...
            TurnItem::AgentMessage(agent_message) => {
                let combined = agent_message
                    ...
                    .map(|entry| match entry {
                        EfficiencyCode_protocol::items::AgentMessageContent::Text { text } => {
                            text.as_str()
                        }
                    })
                    .collect::<String>();
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(EfficiencyCodeErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![EfficiencyCode_protocol::items::AgentMessageContent::Text { text: stripped }];
