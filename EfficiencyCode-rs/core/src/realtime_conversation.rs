...
use EfficiencyCode_api::RealtimeAudioFrame;
use EfficiencyCode_api::RealtimeContextAppendChannel;
use EfficiencyCode_api::RealtimeEvent;
use EfficiencyCode_api::RealtimeEventParser;
use EfficiencyCode_api::RealtimeSessionConfig;
use EfficiencyCode_api::RealtimeSessionMode;
use EfficiencyCode_api::RealtimeWebsocketClient;
use EfficiencyCode_api::RealtimeWebsocketEvents;
use EfficiencyCode_api::RealtimeWebsocketWriter;
use EfficiencyCode_api::build_session_headers;
use EfficiencyCode_api::map_api_error;
...
use EfficiencyCode_login::read_openai_api_key_from_env;
...
use EfficiencyCode_protocol::protocol::EfficiencyCodeResponseHandoffMode;
...
use EfficiencyCode_protocol::protocol::RealtimeConversationClosedEvent;
use EfficiencyCode_protocol::protocol::RealtimeConversationRealtimeEvent;
use EfficiencyCode_protocol::protocol::RealtimeConversationSdpEvent;
use EfficiencyCode_protocol::protocol::RealtimeConversationStartedEvent;
use EfficiencyCode_protocol::protocol::RealtimeHandoffRequested;
use EfficiencyCode_protocol::protocol::RealtimeOutputModality;
use EfficiencyCode_protocol::protocol::RealtimeTranscriptEntry;
use EfficiencyCode_protocol::protocol::RealtimeVoice;
use EfficiencyCode_protocol::protocol::RealtimeVoicesList;
use EfficiencyCode_utils_output_truncation::approx_bytes_for_tokens;
use EfficiencyCode_utils_string::approx_token_count;
use EfficiencyCode_utils_string::take_bytes_at_char_boundary;
use anyhow::Context;
...
const STANDALONE_HANDOFF_ID: &str = "EfficiencyCode";
...
struct RealtimeHandoffState {
    ...
    EfficiencyCode_responses_as_items: bool,
    EfficiencyCode_response_item_prefix: Option<String>,
    EfficiencyCode_response_handoff_mode: EfficiencyCodeResponseHandoffMode,
    EfficiencyCode_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.EfficiencyCode_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.EfficiencyCode_response_handoff_mode
...
struct RealtimeStart {
    ...
    EfficiencyCode_response_item_prefix: Option<String>,
    EfficiencyCode_response_handoff_mode: EfficiencyCodeResponseHandoffMode,
    EfficiencyCode_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    ) -> EfficiencyCodeResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> EfficiencyCodeResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            ...
            EfficiencyCode_responses_as_items,
            EfficiencyCode_response_item_prefix,
            EfficiencyCode_response_handoff_mode,
            EfficiencyCode_response_handoff_channel_prefixes,
        ...
        let handoff = RealtimeHandoffState {
            ...
            EfficiencyCode_response_item_prefix,
            EfficiencyCode_response_handoff_mode,
            EfficiencyCode_response_handoff_channel_prefixes: Arc::new(
                EfficiencyCode_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> EfficiencyCodeResult<()> {
        let sender = {
        ...
        let Some(sender) = sender else {
            return Err(EfficiencyCodeErr::InvalidRequest(
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(EfficiencyCodeErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }
    ...
    pub(crate) async fn text_in(
        ...
        sender.send(params).await.map_err(|_| {
            EfficiencyCodeErr::InvalidRequest("conversation is not running".to_string())
        })?;
        Ok(())
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.EfficiencyCode_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.EfficiencyCode_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.EfficiencyCode_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> EfficiencyCodeResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> EfficiencyCodeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.EfficiencyCode_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> EfficiencyCodeResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> EfficiencyCodeResult<PreparedRealtimeConversationStart> {
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        EfficiencyCode_responses_as_items: params.EfficiencyCode_responses_as_items,
        EfficiencyCode_response_item_prefix: params.EfficiencyCode_response_item_prefix,
        EfficiencyCode_response_handoff_mode: params.EfficiencyCode_response_handoff_mode,
        EfficiencyCode_response_handoff_channel_prefixes: params
            .EfficiencyCode_response_handoff_channel_prefixes,
...
) -> EfficiencyCodeResult<RealtimeSessionConfig> {
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(EfficiencyCodeAuth::api_key) {
    ...
    Err(EfficiencyCodeErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    event_parser: RealtimeEventParser,
    originator: &str,
) -> EfficiencyCodeResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            EfficiencyCodeErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: EfficiencyCodeResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        EfficiencyCodeResponseHandoffMode::Thinking => None,
        EfficiencyCodeResponseHandoffMode::Commentary => {
            Some(RealtimeContextAppendChannel::Commentary)
        }
        EfficiencyCodeResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.EfficiencyCode_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    EfficiencyCode_error_info: EfficiencyCodeErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
