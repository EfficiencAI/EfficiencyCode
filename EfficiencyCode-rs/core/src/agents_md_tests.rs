...
fn foreign_agents_md_uses_environment_native_paths() {
    let (cwd, rendered_cwd) = if cfg!(windows) {
        (
            PathUri::parse("file:///EfficiencyCode%20runtime").expect("POSIX cwd URI"),
            "/EfficiencyCode runtime",
        )
    } else {
        (
            PathUri::parse("file:///C:/EfficiencyCode%20runtime").expect("Windows cwd URI"),
            r"C:\EfficiencyCode runtime",
        )
    };
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.EfficiencyCode_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".EfficiencyCode-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_EfficiencyCode_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(
                root.path().join(".EfficiencyCode").abs(),
                ".ignored-root-marker",
            ),
            project_layer(config.cwd.join(".EfficiencyCode"), ".ignored-nested-marker"),
        ],
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.EfficiencyCode_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.EfficiencyCode_home).unwrap();
...
async fn skills_are_not_appended_to_agents_md() {
    ...
    let cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    create_skill(
        cfg.EfficiencyCode_home.to_path_buf(),
        "pdf-processing",
        "extract from pdfs",
    );
...
fn create_skill(EfficiencyCode_home: PathBuf, name: &str, description: &str) {
    let skill_dir = EfficiencyCode_home.join(format!("skills/{name}"));
