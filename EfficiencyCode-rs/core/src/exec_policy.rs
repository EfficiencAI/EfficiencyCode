...
use EfficiencyCode_execpolicy::AmendError;
use EfficiencyCode_execpolicy::Decision;
use EfficiencyCode_execpolicy::Error as ExecPolicyRuleError;
...
use EfficiencyCode_execpolicy::blocking_append_allow_prefix_rule;
use EfficiencyCode_execpolicy::blocking_append_network_rule;
...
use EfficiencyCode_shell_command::is_dangerous_command::DangerousCommandMatch;
use EfficiencyCode_shell_command::is_dangerous_command::dangerous_command_match;
use EfficiencyCode_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(EfficiencyCode_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: EfficiencyCode_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(EfficiencyCode_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &EfficiencyCode_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            EfficiencyCode_shell_command::is_dangerous_command::dangerous_powershell_words_match(
                command,
            )
        }
    }
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            EfficiencyCode_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
pub(crate) fn default_policy_path(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home
        .join(RULES_DIR_NAME)
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            EfficiencyCode_shell_command::powershell::parse_powershell_command_into_plain_commands(
                command,
            )
            && !commands.is_empty()
        {
...
/// - Note: we only use this amendment when the command fails to run in sandbox and EfficiencyCode prompts the user to run outside the sandbox
