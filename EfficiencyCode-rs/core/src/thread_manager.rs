use crate::EfficiencyCode_thread::EfficiencyCodeThread;
use crate::EfficiencyCodeAppsToolsCache;
...
use EfficiencyCode_agent_graph_store::AgentGraphStore;
use EfficiencyCode_agent_graph_store::LocalAgentGraphStore;
...
use EfficiencyCode_code_mode::CodeModeSessionProvider;
use EfficiencyCode_code_mode::DisabledCodeModeSessionProvider;
use EfficiencyCode_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use EfficiencyCode_login::default_client::EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
use EfficiencyCode_model_provider_info::OPENAI_PROVIDER_ID;
...
use EfficiencyCode_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use EfficiencyCode_thread_store::InMemoryThreadStore;
use EfficiencyCode_thread_store::LoadThreadHistoryParams;
use EfficiencyCode_thread_store::LocalThreadStore;
use EfficiencyCode_thread_store::LocalThreadStoreConfig;
use EfficiencyCode_thread_store::MoveThreadToSectionParams;
use EfficiencyCode_thread_store::PreparedFork;
use EfficiencyCode_thread_store::ReadThreadByRolloutPathParams;
use EfficiencyCode_thread_store::ReadThreadParams;
use EfficiencyCode_thread_store::StoredModelContext;
use EfficiencyCode_thread_store::StoredThread;
use EfficiencyCode_thread_store::ThreadMetadataPatch;
use EfficiencyCode_thread_store::ThreadStore;
use EfficiencyCode_thread_store::ThreadStoreError;
use EfficiencyCode_thread_store::UpdateThreadMetadataParams;
...
struct TempEfficiencyCodeHomeGuard {
    path: PathBuf,
}

impl Drop for TempEfficiencyCodeHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created EfficiencyCode thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<EfficiencyCodeThread>,
...
pub struct ThreadManager {
    ...
    _test_EfficiencyCode_home_guard: Option<TempEfficiencyCodeHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<EfficiencyCode_protocol::dynamic_tools::DynamicToolSpec>,
...
fn originator_from_service_name(service_name: Option<&str>) -> Option<String> {
    ...
    for originator in [
        ...
        "EfficiencyCode_work_web",
        "EfficiencyCode_work_mobile",
        "EfficiencyCode_work_cca",
        "chatgpt_cca",
    ] {
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<EfficiencyCodeThread>>>>,
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            if has_state_db && background_migration_enabled {
                ...
                let EfficiencyCode_home = config.EfficiencyCode_home.to_path_buf();
                tokio::spawn(async move {
                    ...
                    if compression_enabled {
                        EfficiencyCode_rollout::spawn_rollout_compression_worker(
                            EfficiencyCode_home,
                        );
                    }
                });
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        EfficiencyCode_apps_tools_cache: EfficiencyCodeAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
        ...
        Self {
            ...
            _test_EfficiencyCode_home_guard: None,
        }
    }
    ...
    /// Construct with a dummy AuthManager containing the provided EfficiencyCodeAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let EfficiencyCode_home = std::env::temp_dir().join(format!(
            "EfficiencyCode-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&EfficiencyCode_home)
            .unwrap_or_else(|err| panic!("temp EfficiencyCode home dir create failed: {err}"));
        ...
        manager._test_EfficiencyCode_home_guard = Some(TempEfficiencyCodeHomeGuard {
            path: EfficiencyCode_home,
        });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided EfficiencyCodeAuth and EfficiencyCode home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_EfficiencyCode_home =
            match AbsolutePathBuf::from_absolute_path_checked(&EfficiencyCode_home) {
                Ok(EfficiencyCode_home) => EfficiencyCode_home,
                Err(err) => panic!("test EfficiencyCode_home should be absolute: {err}"),
            };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        let restriction_product = SessionSource::Exec.restriction_product();
        let skills_service = Arc::new(HostSkillsService::new_with_restriction_product(
            absolute_EfficiencyCode_home.clone(),
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: EfficiencyCode_state::SqliteConfig::new_for_testing(
                    absolute_EfficiencyCode_home,
                ),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(EfficiencyCode_home, /*config_model_catalog*/ None),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
                plugins_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            ...
            self.state
                ...
                .ok_or_else(|| {
                    EfficiencyCodeErr::InvalidRequest(format!(
    ...
    pub async fn list_models(
        ...
        http_client_factory: EfficiencyCode_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> EfficiencyCodeResult<Arc<EfficiencyCodeThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `EfficiencyCodeThread`/`LiveThread`, so metadata changes stay ordered
    ...
    ) -> EfficiencyCodeResult<StoredThread> {
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    EfficiencyCodeErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        &self,
        thread_id: ThreadId,
    ) -> EfficiencyCodeResult<Vec<ThreadId>> {
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    EfficiencyCodeErr::Fatal(format!(
    ...
    pub async fn start_thread(
        &self,
        options: StartThreadOptions,
    ) -> EfficiencyCodeResult<NewThread> {
    ...
    /// as `Arc<EfficiencyCodeThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<EfficiencyCodeThread>> {
    ...
    async fn initial_history_from_rollout_path(
        &self,
        rollout_path: PathBuf,
    ) -> EfficiencyCodeResult<InitialHistory> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        ...
        match threads.get(&thread_id) {
            ...
            Some(_) | None => Err(EfficiencyCodeErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => EfficiencyCodeErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> EfficiencyCodeResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => EfficiencyCodeErr::Fatal(format!(
    ...
    pub(crate) async fn remove_thread(
        &self,
        thread_id: &ThreadId,
    ) -> Option<Arc<EfficiencyCodeThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> EfficiencyCodeResult<NewThread> {
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(EfficiencyCodeErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(EfficiencyCodeThread::new(
                    session,
                    io,
        ...
        Err(EfficiencyCodeErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> EfficiencyCode_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return EfficiencyCode_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(EfficiencyCode_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> EfficiencyCodeErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => {
            EfficiencyCodeErr::ThreadNotFound(thread_id)
        }
        ThreadStoreError::InvalidRequest { message } => EfficiencyCodeErr::InvalidRequest(message),
        err => EfficiencyCodeErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}
...
fn thread_store_metadata_update_error(
    ...
    match err {
        ...
        ThreadStoreError::InvalidRequest { message } => EfficiencyCodeErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => EfficiencyCodeErr::UnsupportedOperation(
