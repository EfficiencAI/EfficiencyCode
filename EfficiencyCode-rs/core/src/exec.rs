...
use EfficiencyCode_protocol::error::SandboxErr;
...
use EfficiencyCode_protocol::protocol::ExecCommandOutputDeltaEvent;
use EfficiencyCode_protocol::protocol::ExecOutputStream;
use EfficiencyCode_sandboxing::SandboxCommand;
use EfficiencyCode_sandboxing::SandboxManager;
use EfficiencyCode_sandboxing::SandboxTransformRequest;
use EfficiencyCode_sandboxing::SandboxType;
use EfficiencyCode_sandboxing::SandboxablePreference;
use EfficiencyCode_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use EfficiencyCode_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use EfficiencyCode_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use EfficiencyCode_sandboxing::record_filesystem_sandbox_violation;
use EfficiencyCode_sandboxing::resolve_windows_elevated_filesystem_overrides;
use EfficiencyCode_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use EfficiencyCode_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use EfficiencyCode_sandboxing::windows_sandbox_uses_elevated_backend;
...
use EfficiencyCode_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: EfficiencyCode_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: EfficiencyCode_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    EfficiencyCodeErr::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let (program, args) = command.split_first().ok_or_else(|| {
        EfficiencyCodeErr::Io(io::Error::new(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            EfficiencyCode_linux_sandbox_exe: EfficiencyCode_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
        ...
        .map_err(EfficiencyCodeErr::from)?;
    ...
    .map_err(EfficiencyCodeErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| EfficiencyCodeErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| EfficiencyCodeErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        EfficiencyCode_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = EfficiencyCode_otel::global() {
        let _ = metrics.counter(
            "EfficiencyCode.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_EfficiencyCode_home;
    use EfficiencyCode_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use EfficiencyCode_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let network_proxy_restricting_sid = network
        ...
        .map(|network| {
            network
                ...
                .ok_or_else(|| {
                    EfficiencyCodeErr::Io(io::Error::other(
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            EfficiencyCode_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let EfficiencyCode_home = find_EfficiencyCode_home().map_err(|err| {
        EfficiencyCodeErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve EfficiencyCode_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                EfficiencyCode_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    EfficiencyCode_home: EfficiencyCode_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(EfficiencyCodeErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, EfficiencyCodeErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(EfficiencyCodeErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(EfficiencyCodeErr::Sandbox(SandboxErr::Timeout {
            ...
            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                ...
                return Err(EfficiencyCodeErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        EfficiencyCode_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                EfficiencyCode_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
