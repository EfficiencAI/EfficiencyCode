...
use EfficiencyCode_analytics::CompactionTrigger;
use EfficiencyCode_analytics::HookRunFact;
use EfficiencyCode_analytics::build_track_events_context;
use EfficiencyCode_hooks::PermissionRequestDecision;
use EfficiencyCode_hooks::PermissionRequestOutcome;
use EfficiencyCode_hooks::PermissionRequestRequest;
use EfficiencyCode_hooks::PostToolUseOutcome;
use EfficiencyCode_hooks::PostToolUseRequest;
use EfficiencyCode_hooks::PreToolUseOutcome;
use EfficiencyCode_hooks::PreToolUseRequest;
use EfficiencyCode_hooks::SessionStartOutcome;
use EfficiencyCode_hooks::StartHookTarget;
use EfficiencyCode_hooks::StopHookTarget;
use EfficiencyCode_hooks::StopOutcome;
use EfficiencyCode_hooks::SubagentHookContext;
use EfficiencyCode_hooks::UserPromptSubmitOutcome;
use EfficiencyCode_hooks::UserPromptSubmitRequest;
use EfficiencyCode_otel::HOOK_RUN_DURATION_METRIC;
use EfficiencyCode_otel::HOOK_RUN_METRIC;
...
use EfficiencyCode_protocol::protocol::HookCompletedEvent;
use EfficiencyCode_protocol::protocol::HookEventName;
use EfficiencyCode_protocol::protocol::HookExecutionMode;
use EfficiencyCode_protocol::protocol::HookOutputEntryKind;
use EfficiencyCode_protocol::protocol::HookRunStatus;
use EfficiencyCode_protocol::protocol::HookRunSummary;
use EfficiencyCode_protocol::protocol::HookSource;
use EfficiencyCode_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    EfficiencyCode_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = EfficiencyCode_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = EfficiencyCode_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = EfficiencyCode_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = EfficiencyCode_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(EfficiencyCode_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: EfficiencyCode_hooks::HookEvent::AfterAgent {
                event: EfficiencyCode_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            EfficiencyCode_hooks::HookResult::Success => continue,
            EfficiencyCode_hooks::HookResult::FailedContinue(error) => (error, false),
            EfficiencyCode_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(EfficiencyCode_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (EfficiencyCode_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use EfficiencyCode_protocol::protocol::HookEventName;
    use EfficiencyCode_protocol::protocol::HookExecutionMode;
    use EfficiencyCode_protocol::protocol::HookHandlerType;
    use EfficiencyCode_protocol::protocol::HookRunStatus;
    use EfficiencyCode_protocol::protocol::HookScope;
    use EfficiencyCode_protocol::protocol::HookSource;
    ...
    async fn hook_lifecycle_notifications_only_report_synchronous_runs() {
        ...
        assert!(matches!(
            started.msg,
            EfficiencyCode_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            EfficiencyCode_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
    }
