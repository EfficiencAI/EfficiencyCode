...
use EfficiencyCode_protocol::protocol::GuardianAssessmentAction;
...
use EfficiencyCode_protocol::request_user_input::RequestUserInputEvent;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    assert_eq!(
        forwarded.trace,
        Some(EfficiencyCode_protocol::protocol::W3cTraceContext {
            traceparent: Some(
                "00-1234567890abcdef1234567890abcdef-1234567890abcdef-01".to_string(),
            ),
            tracestate: Some("vendor=state".to_string()),
...
#[tokio::test]
async fn run_EfficiencyCode_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    let result = timeout(
        ...
        run_EfficiencyCode_thread_interactive(
