...
use EfficiencyCode_config::types::AppConfig;
use EfficiencyCode_config::types::AppToolConfig;
use EfficiencyCode_config::types::AppToolsConfig;
use EfficiencyCode_config::types::ApprovalsReviewer;
use EfficiencyCode_config::types::AppsConfigToml;
...
fn approval_config(turn_context: &TurnContext) -> EfficiencyCode_mcp::McpConfig {
...
fn write_sample_plugin_mcp(EfficiencyCode_home: &std::path::Path) {
    let plugin_root = EfficiencyCode_home.join("plugins/cache/test/sample/local");
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.EfficiencyCode_home)
        .expect("create EfficiencyCode home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.EfficiencyCode_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = EfficiencyCode_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.EfficiencyCode_home,
        hook_list.hooks,
    );
...
#[test]
fn openai_file_params_are_only_honored_for_EfficiencyCode_apps() {
    let params = HashMap::from([("file".to_string(), Vec::new())]);

    assert_eq!(
        openai_file_input_optional_fields_for_server(EfficiencyCode_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        &turn_context,
        McpToolCallSpanFields {
            server_name: EfficiencyCode_APPS_MCP_SERVER_NAME,
            tool_name: "calendar_search",
            call_id: "call-123",
            server_origin: Some("https://chatgpt.com/api/EfficiencyCode/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("EfficiencyCode.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("EfficiencyCode.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("EfficiencyCode.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("EfficiencyCode.mcp.target.id=")
            && !invalid_logs.contains("EfficiencyCode.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("EfficiencyCode.mcp.target.id=")
            && !missing_logs.contains("EfficiencyCode.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("EfficiencyCode.mcp.target.id=")
            && !no_meta_logs.contains("EfficiencyCode.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("EfficiencyCode.mcp.target.id=\"{truncated}\""))
            && !logs.contains("tail"),
...
#[test]
fn EfficiencyCode_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_EfficiencyCode_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn EfficiencyCode_apps_tool_question_without_elicitation_omits_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        EfficiencyCode_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn EfficiencyCode_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: EfficiencyCode_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_EfficiencyCode_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "callId": "call-custom",
            crate::X_EfficiencyCode_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_EfficiencyCode_apps_identity() {
    let mut metadata = approval_metadata(
...
#[tokio::test]
async fn EfficiencyCode_apps_tool_call_request_meta_includes_turn_metadata_and_EfficiencyCode_apps_meta()
 {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            ...
            crate::X_EfficiencyCode_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_EfficiencyCode_APPS_META_KEY: {
...
#[tokio::test]
async fn EfficiencyCode_apps_tool_call_request_meta_includes_call_id_without_existing_EfficiencyCode_apps_meta()
 {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
...
fn EfficiencyCode_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn EfficiencyCode_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn EfficiencyCode_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let result = EfficiencyCode_apps_auth_failure_result();
    let metadata = EfficiencyCode_apps_auth_failure_metadata();

    let returned = maybe_request_EfficiencyCode_apps_auth_elicitation(
        &session,
        &turn_context,
...
#[tokio::test]
async fn EfficiencyCode_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn EfficiencyCode_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
    let mutable_turn_context = Arc::get_mut(&mut turn_context).expect("single turn context ref");
...
#[tokio::test]
async fn EfficiencyCode_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_EfficiencyCode_apps_auth_elicitation(
                &session,
                &turn_context,
                turn_context.approval_policy(),
                "call_123",
    ...
    assert_eq!(request.server_name, EfficiencyCode_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        EfficiencyCode_protocol::mcp::RequestId::String(
            "EfficiencyCode_apps_auth_call_123".to_string()
        )
    );
    assert!(matches!(
        request.request,
        EfficiencyCode_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            EfficiencyCode_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("EfficiencyCode_apps_auth_call_123".into()),
            ElicitationResponse {
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_EfficiencyCode_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            EfficiencyCode_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_EfficiencyCode_app_tool_approval_writes_tool_override() {
    ...
    persist_EfficiencyCode_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(EfficiencyCode_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .EfficiencyCode_home(EfficiencyCode_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents =
        std::fs::read_to_string(EfficiencyCode_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_EfficiencyCode_dir = project_dir.path().join(".EfficiencyCode");
    std::fs::create_dir_all(&project_EfficiencyCode_dir)
        .expect("create project .EfficiencyCode dir");
    std::fs::write(
        project_EfficiencyCode_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&EfficiencyCode_home)
        .set_project_trust_level(
            project_dir.path(),
            EfficiencyCode_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_EfficiencyCode_dir.join(CONFIG_TOML_FILE))
