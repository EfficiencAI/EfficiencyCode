...
const PROXY_TEST_SUBPROCESS_ENV_VAR: &str = "EfficiencyCode_MCP_HTTP_PROXY_TEST_SUBPROCESS";
...
const SERVER_URL: &str = "http://mcp-proxy.invalid/api/EfficiencyCode/ps/mcp";
...
async fn local_mcp_startup_and_refresh_use_configured_http_client() -> Result<()> {
    ...
    if std::env::var_os(PROXY_TEST_SUBPROCESS_ENV_VAR).is_none() {
        ...
        command
            ...
            .env(
                "NO_PROXY",
                EfficiencyCode_network_proxy::DEFAULT_NO_PROXY_VALUE,
            )
            .env(
                "no_proxy",
                EfficiencyCode_network_proxy::DEFAULT_NO_PROXY_VALUE,
...
async fn skill_mcp_dependency_oauth_uses_configured_http_client() -> Result<()> {
    ...
    if std::env::var_os(PROXY_TEST_SUBPROCESS_ENV_VAR).is_none() {
        ...
        Mock::given(method("GET"))
            .and(path("/api/EfficiencyCode/ps/mcp"))
    ...
    fixture
        ...
        .await?;
    core_test_support::wait_for_event(fixture.EfficiencyCode.as_ref(), |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;

    let servers =
        EfficiencyCode_config::load_global_mcp_servers(&fixture.config.EfficiencyCode_home).await?;
