...
use EfficiencyCode_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "EfficiencyCode-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://EfficiencyCode-network-test.invalid:80";
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.EfficiencyCode.next_event())
            .await
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#EfficiencyCode-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("EfficiencyCode-network-test.invalid is not in the allowed_domains")
    );
    test.EfficiencyCode
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="EfficiencyCode-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): EfficiencyCode-network-test.invalid",
        )
    }));
...
async fn approved_network_host_for_one_environment_still_prompts_in_another() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-network-approval-{}",
...
async fn wait_for_completion_without_network_prompt(test: &TestEfficiencyCode) {
...
async fn wait_for_turn_complete(test: &TestEfficiencyCode) {
    wait_for_event(&test.EfficiencyCode, |event| {
