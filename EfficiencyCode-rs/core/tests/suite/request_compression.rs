...
async fn request_body_is_zstd_compressed_for_EfficiencyCode_backend_when_enabled()
-> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/EfficiencyCode/v1", server.uri());
    ...
    let EfficiencyCode = builder.build(&server).await?.EfficiencyCode;
