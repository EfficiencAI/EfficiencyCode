...
use core_test_support::test_EfficiencyCode::RecordingUserInstructionsProvider;
...
async fn agents_instructions(mut builder: TestEfficiencyCodeBuilder) -> Result<String> {
...
async fn submit_thread_turn(
    thread: &Arc<EfficiencyCode_core::EfficiencyCodeThread>,
    prompt: &str,
) -> Result<()> {
    thread
        .submit(Op::UserInput {
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions = agents_instructions(test_EfficiencyCode().with_workspace_setup(
        |cwd, fs| async move {
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        EfficiencyCodeHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_EfficiencyCode().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.EfficiencyCode.instruction_sources().await,
    ...
    let mut resume_builder = test_EfficiencyCode().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.EfficiencyCode.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.EfficiencyCode.ensure_rollout_materialized().await;
    parent.EfficiencyCode.flush_rollout().await?;
    let rollout_path = parent.EfficiencyCode.rollout_path().expect("rollout path");
    ...
    fork_config.EfficiencyCode_self_exe = parent.config.EfficiencyCode_self_exe.clone();
    let forked = parent
        .thread_manager
        .fork_thread(
