...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    EfficiencyCode
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&EfficiencyCode, |ev| {
...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    let EventMsg::Error(error) = wait_for_event(&test.EfficiencyCode, |event| {
    ...
    assert_eq!(error.EfficiencyCode_error_info, None);
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let fixture = builder
        ...
        .await
        .expect("create new conversation");
    let EfficiencyCode = &fixture.EfficiencyCode;
    ...
    let _begin = wait_for_event_match(EfficiencyCode, |ev| match ev {
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.EfficiencyCode, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.EfficiencyCode.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder =
        core_test_support::test_EfficiencyCode::test_EfficiencyCode().with_config(move |config| {
            config
                .features
    ...
    let begin_event = wait_for_event_match(&test.EfficiencyCode, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.EfficiencyCode, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.EfficiencyCode, |ev| match ev {
    ...
    let _ = wait_for_event(&test.EfficiencyCode, |ev| {
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder =
        core_test_support::test_EfficiencyCode::test_EfficiencyCode().with_config(|config| {
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.EfficiencyCode, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_EfficiencyCode::test_EfficiencyCode();
    let test = builder
        .with_config(|config| {
