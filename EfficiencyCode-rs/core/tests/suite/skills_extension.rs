...
use EfficiencyCode_extension_api::ExtensionEventSink;
use EfficiencyCode_extension_api::ExtensionRegistryBuilder;
use EfficiencyCode_extension_api::ExtensionWarning;
...
use EfficiencyCode_skills_extension::HostSkillProvider;
use EfficiencyCode_skills_extension::OrchestratorSkillProvider;
use EfficiencyCode_skills_extension::SkillProvider;
use EfficiencyCode_skills_extension::SkillProviderSource;
use EfficiencyCode_skills_extension::SkillProviders;
use EfficiencyCode_skills_extension::SkillsExtensionConfig;
use EfficiencyCode_skills_extension::catalog::SkillAuthority;
use EfficiencyCode_skills_extension::catalog::SkillCatalog;
use EfficiencyCode_skills_extension::catalog::SkillCatalogEntry;
use EfficiencyCode_skills_extension::catalog::SkillPackageId;
use EfficiencyCode_skills_extension::catalog::SkillProviderError;
use EfficiencyCode_skills_extension::catalog::SkillReadResult;
use EfficiencyCode_skills_extension::catalog::SkillResourceId;
use EfficiencyCode_skills_extension::catalog::SkillSearchResult;
use EfficiencyCode_skills_extension::catalog::SkillSourceKind;
use EfficiencyCode_skills_extension::install;
use EfficiencyCode_skills_extension::install_with_providers;
use EfficiencyCode_skills_extension::provider::SkillListQuery;
use EfficiencyCode_skills_extension::provider::SkillProviderFuture;
use EfficiencyCode_skills_extension::provider::SkillReadRequest;
use EfficiencyCode_skills_extension::provider::SkillSearchRequest;
...
fn write_host_skills(EfficiencyCode_home: &std::path::Path, skills: &[(&str, &str)]) -> Result<()> {
...
fn catalog_extensions(
    ...
    Arc<EfficiencyCode_extension_api::ExtensionRegistry<Config>>,
...
fn configure_catalog_test(config: &mut Config) {
    ...
    let system_config_path = config.EfficiencyCode_home.join("config.toml");
...
async fn rendered_catalogs_for_turns(
    ...
    if !host_skills.is_empty() {
        write_host_skills(EfficiencyCode_home.path(), host_skills)?;
    }
    ...
    for _ in 0..turn_count {
        test.EfficiencyCode
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.EfficiencyCode, |_| true).await {
    ...
    // while core warnings are delivered through the TestEfficiencyCode event stream.
    ...
    let _EfficiencyCode_home_guard = EfficiencyCode_home;
...
async fn capability_sections_render_in_order_with_host_repo_and_plugin_skills() -> Result<()> {
    ...
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let host_skill_dir = EfficiencyCode_home.path().join("skills/host-search");
    ...
    core_test_support::wait_for_event(&test.EfficiencyCode, |event| {
...
async fn agent_plugin_skill_prompt_stays_bounded_without_skills_extension() -> Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_home(EfficiencyCode_home);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    test.EfficiencyCode
        ...
        .await?;
    let warning = core_test_support::wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
...
async fn explicit_only_orchestrator_skill_is_hidden_but_can_be_invoked() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/EfficiencyCode/ps/mcp/?$"))
        .and(|request: &Request| {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.EfficiencyCode, EfficiencyCode_APPS_MCP_SERVER_NAME).await?;
...
async fn executor_skill_invocation_is_environment_scoped_and_deduplicated() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let skill_path = EfficiencyCode_home.path().join("executor-skill/SKILL.md");
...
async fn production_turn_aliases_combined_skill_catalogs_under_shared_budget() -> Result<()> {
    ...
    .await;
    let EfficiencyCode_home = Arc::new(
        tempfile::Builder::new()
            .prefix("EfficiencyCode-integration-shared-skill-catalog-roots-")
            .tempdir()?,
    );
    write_host_skills(
    ...
    let developer_text = response
        ...
        .message_input_texts("developer")
        .join("\n");
    let host_root = dunce::canonicalize(EfficiencyCode_home.path().join("skills"))?
        .to_string_lossy()
        .replace('\\', "/");
    for (alias, root) in [
        ("e0", EXECUTOR_ROOT),
...
async fn production_turn_preserves_host_alias_root_order_across_turns() -> Result<()> {
    ...
    let long_parent = temp_parent
        .path()
        .join("EfficiencyCode-home-with-long-shared-prefix-for-production-alias-order-test");
    std::fs::create_dir_all(&long_parent)?;
    let EfficiencyCode_home = Arc::new(TempDir::new_in(&long_parent)?);
...
async fn production_turn_uses_provider_host_catalog_and_core_snapshot_injection() -> Result<()> {
    ...
    let skill_path = EfficiencyCode_home
        .path()
        .join("skills")
        .join(skill_name)
        .join("SKILL.md");
...
async fn production_turn_suppresses_only_the_superseded_host_skill_prompt() -> Result<()> {
    ...
    let first_skill_path = EfficiencyCode_home
        .path()
        .join("skills/first-host/SKILL.md");
    let second_skill_path = EfficiencyCode_home
        .path()
        .join("skills/second-host/SKILL.md");
...
async fn production_turn_warns_and_omits_unreadable_host_skill() -> Result<()> {
    ...
    let missing_skill_path = dunce::canonicalize(
        EfficiencyCode_home
            .path()
            .join("skills/missing-host/SKILL.md"),
    )?;
...
async fn production_turn_keeps_full_snapshot_host_skill_prompt() -> Result<()> {
    ...
    .await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let skill_dir = EfficiencyCode_home.path().join("skills").join("long-host");
    std::fs::create_dir_all(&skill_dir)?;
...
async fn production_turn_keeps_orchestrator_world_state_incremental_across_turns() -> Result<()> {
    ...
    let skill_resource = "skill://EfficiencyCode_apps/orchestrator-search/SKILL.md";
    let catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. EfficiencyCode can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );
