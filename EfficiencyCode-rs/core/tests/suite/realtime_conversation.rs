...
use EfficiencyCode_core::test_support::auth_manager_from_auth;
...
use EfficiencyCode_protocol::protocol::RealtimeConversationVersion;
use EfficiencyCode_protocol::protocol::RealtimeEvent;
use EfficiencyCode_protocol::protocol::RealtimeNoopRequested;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from EfficiencyCode.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = EfficiencyCode_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "EfficiencyCode_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in EfficiencyCode_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let db = test.EfficiencyCode.state_db().context("state db enabled")?;
    let thread_id = ThreadId::new();
    let updated_at = Utc::now();
    let rollout_path = test
        .EfficiencyCode_home_path()
    ...
    let mut metadata_builder = EfficiencyCode_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.EfficiencyCode
        ...
        .await?;
    let closed = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_chatgpt_sends_EfficiencyCode_headers_to_backend()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    Mock::given(method("POST"))
        .and(path_regex(".*/backend-api/EfficiencyCode/realtime/calls$"))
        .and(capture.clone())
        .respond_with(
    ...
    let backend_base_url = format!("{}/backend-api/EfficiencyCode", server.uri());
    ...
    let created = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
    ...
    assert_eq!(
        ...
        (
            "/backend-api/EfficiencyCode/realtime/calls",
    ...
    test.EfficiencyCode
        .submit(Op::RealtimeConversationClose)
        .await?;
    let _closed = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=EfficiencyCode-realtime-call-boundary")
    );
    ...
    assert_eq!(
        body,
        format!(
            "--EfficiencyCode-realtime-call-boundary\r\n\
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        ...
        loop {
            match test.EfficiencyCode.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
                    payload: RealtimeEvent::Error(message),
                }) => {
        ...
        test.EfficiencyCode.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn assert_transport_close_tail_flush(
    ...
    let mut builder = test_EfficiencyCode().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(
        err.EfficiencyCode_error_info,
        Some(EfficiencyCodeErrorInfo::BadRequest)
    );
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.EfficiencyCode
        ...
        .await?;
    let _ = wait_for_event_match(&test.EfficiencyCode, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let history = user_turns
        ...
        .collect::<Vec<_>>();
    test.EfficiencyCode.shutdown_and_wait().await?;
    let resumed_thread = test
        .thread_manager
        .resume_thread_with_history(
            test.config.clone(),
            InitialHistory::Forked(history),
            auth_manager_from_auth(EfficiencyCodeAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
        )
        .await?;
    let EfficiencyCode = resumed_thread.thread;
    ...
    EfficiencyCode.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("EfficiencyCode-rs/core"))?;
    ...
    assert!(startup_context.contains("EfficiencyCode-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.EfficiencyCode, |event| match event {
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.EfficiencyCode
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            EfficiencyCode_response_item_prefix: None,
            EfficiencyCode_response_handoff_mode:
                EfficiencyCode_protocol::protocol::EfficiencyCodeResponseHandoffMode::BemTags,
            EfficiencyCode_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.EfficiencyCode.next_event().await?;
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_EfficiencyCode().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
