...
async fn user_message_admission_starts_turn_for_empty_input() {
    ...
    loop {
        let event = timeout(Duration::from_secs(10), test.EfficiencyCode.next_event())
            .await
...
async fn default_turn_start_persists_developer_and_user_input_before_model_request() {
    ...
    loop {
        let event = timeout(Duration::from_secs(5), test.EfficiencyCode.next_event())
            .await
...
async fn user_message_admission_reports_started_and_steered_for_concurrent_submissions() {
    ...
    let first_submission = tokio::spawn({
        let EfficiencyCode = Arc::clone(&EfficiencyCode);
    ...
    loop {
        let event = timeout(Duration::from_secs(10), EfficiencyCode.next_event())
            .await
...
async fn user_message_admission_reports_invalid_settings_and_recovers() {
    ...
    let EfficiencyCode = &test.EfficiencyCode;
    ...
    let error = timeout(
        Duration::from_secs(5),
        EfficiencyCode.submit_user_input_and_wait_for_admission(
            invalid_input,
            /*trace*/ None,
    ...
    let error_event = loop {
        let event = timeout(Duration::from_secs(5), EfficiencyCode.next_event())
            .await
