...
const FIXED_CWD: &str = "/tmp/EfficiencyCode_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> EfficiencyCodeAuth {
        match self {
            AuthCase::ChatGpt => EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => EfficiencyCodeAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().EfficiencyCode_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestEfficiencyCodeHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    TestEfficiencyCodeHarness::with_builder(builder.with_config(move |config| {
        config.cwd = EfficiencyCode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn rollout_path(harness: &TestEfficiencyCodeHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn wait_for_turn_complete(EfficiencyCode: &EfficiencyCode_core::EfficiencyCodeThread) {
    wait_for_event(EfficiencyCode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
}
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<EfficiencyCode_HOME>");
            search_start = start_index + "<EfficiencyCode_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <EfficiencyCode_HOME>/skills/.system/imagegen/SKILL.md and \
         <EfficiencyCode_HOME>/skills/custom/SKILL.md"
    );
}
