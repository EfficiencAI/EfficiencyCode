use EfficiencyCode_config::types::Personality;
...
use EfficiencyCode_protocol::openai_models::default_input_modalities;
...
fn read_only_user_turn(test: &TestEfficiencyCode, items: Vec<UserInput>, model: String) -> Op {
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_rollout_path = test.EfficiencyCode.rollout_path().expect("rollout path");
    let source_history =
        EfficiencyCode_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
async fn rollback_first_turn_model_change_removes_its_instructions(
    ...
    let mut builder = test_EfficiencyCode().with_model(initial_model);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    let test = match followup {
        RollbackFollowup::ColdResume => {
            let mut resume_builder = test_EfficiencyCode().with_model(switched_model);
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_model("gpt-5.2");
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-EfficiencyCode-personality";
...
async fn unsupported_configured_service_tier_warns_at_session_start() -> Result<()> {
    ...
    let warning = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
