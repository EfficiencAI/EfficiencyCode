...
-> Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_config(|config| {
        ...
        config.responses_api_metadata.insert(
            "EfficiencyCode_security_surface".to_string(),
            "sdk".to_string(),
        );
    });
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-EfficiencyCode-window-id"))?;
    let child_window_id = child
        .header("x-EfficiencyCode-window-id")
        .ok_or_else(|| anyhow!("child request missing x-EfficiencyCode-window-id"))?;
    ...
    assert_eq!(
        child.header("x-EfficiencyCode-parent-thread-id").as_deref(),
        Some(parent_thread_id)
    );
    let parent_turn_metadata: serde_json::Value = serde_json::from_str(
        &parent
            .header("x-EfficiencyCode-turn-metadata")
            .ok_or_else(|| anyhow!("parent request missing x-EfficiencyCode-turn-metadata"))?,
    )?;
    assert_eq!(
        parent_turn_metadata["EfficiencyCode_security_surface"],
        json!("sdk")
    );
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-EfficiencyCode-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-EfficiencyCode-turn-metadata"))?,
    )?;
    ...
    assert_eq!(
        child_turn_metadata["EfficiencyCode_security_surface"],
        json!("sdk")
    );
...
async fn submit_turn_with_timeout(test: &TestEfficiencyCode, prompt: &str) -> Result<()> {
