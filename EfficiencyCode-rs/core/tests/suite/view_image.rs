...
fn disabled_user_turn(test: &TestEfficiencyCode, items: Vec<UserInput>, model: String) -> Op {
...
async fn assert_user_turn_local_image_resizes_to(
    ...
    let builder = match image_budget_policy {
        ImageBudgetPolicy::DetailBased | ImageBudgetPolicy::Unified => test_EfficiencyCode(),
        ImageBudgetPolicy::UnifiedResponsesLiteWithoutOriginalSupport => test_EfficiencyCode()
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    match item_started.expect("view image item started event emitted") {
        EfficiencyCode_protocol::items::TurnItem::ImageView(item) => {
