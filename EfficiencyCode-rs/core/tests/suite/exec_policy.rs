...
async fn submit_user_turn(
    test: &core_test_support::test_EfficiencyCode::TestEfficiencyCode,
    prompt: &str,
...
"#;
    ...
    let mut migrated_builder = test_EfficiencyCode().with_config(|config| {
        let policy_path = config.EfficiencyCode_home.join("rules/default.rules");
    ...
    let mut ignored_builder = test_EfficiencyCode().with_config(|config| {
        let policy_path = config.EfficiencyCode_home.join("rules/default.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(policy_path, LEGACY_POLICY).expect("write legacy policy");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.EfficiencyCode, |event| {
