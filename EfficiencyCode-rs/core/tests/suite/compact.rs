...
async fn assert_compaction_uses_turn_lifecycle_id(
    EfficiencyCode: &std::sync::Arc<EfficiencyCode_core::EfficiencyCodeThread>,
) {
    ...
    while turn_completed_id.is_none() {
        let event = EfficiencyCode.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn EfficiencyCode.
    ...
    EfficiencyCode.submit(Op::Compact).await.unwrap();
    let warning_event =
        wait_for_event(&EfficiencyCode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down EfficiencyCode to flush rollout entries before inspecting the file.
    EfficiencyCode.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&EfficiencyCode, |ev| {
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    let completed = wait_for_event_match(&EfficiencyCode, |ev| match ev {
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let EfficiencyCode = builder.build(&server).await.unwrap().EfficiencyCode;
    ...
    let first = wait_for_event_match(&EfficiencyCode, |ev| match ev {
    ...
    let last = wait_for_event_match(&EfficiencyCode, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        EfficiencyCode_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    let mut resume_builder = test_EfficiencyCode().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.EfficiencyCode, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("build test EfficiencyCode");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.EfficiencyCode).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using EfficiencyCode with a ChatGPT account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    let mut initial_builder = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test EfficiencyCode");
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.EfficiencyCode).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&EfficiencyCode, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&EfficiencyCode, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-EfficiencyCode-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-EfficiencyCode-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let EfficiencyCode = builder.build(&server).await.unwrap();

    EfficiencyCode.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    EfficiencyCode
...
-> Result<()> {
    ...
    assert_eq!(
        resumed.EfficiencyCode.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
