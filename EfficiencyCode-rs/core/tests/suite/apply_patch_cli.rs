...
use core_test_support::test_EfficiencyCode::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestEfficiencyCodeHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestEfficiencyCodeBuilder) -> TestEfficiencyCodeBuilder,
) -> Result<TestEfficiencyCodeHarness> {
    let builder = configure(test_EfficiencyCode());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestEfficiencyCodeHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestEfficiencyCodeHarness, prompt: &str) -> Result<()> {
...
async fn submit_without_wait_with_turn_permissions(
    harness: &TestEfficiencyCodeHarness,
    prompt: &str,
...
#[cfg(target_os = "linux")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn apply_patch_cli_uses_EfficiencyCode_self_exe_with_linux_sandbox_helper_alias() -> Result<()>
{
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let EfficiencyCode_linux_sandbox_exe = harness
        .test()
        .config
        .EfficiencyCode_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include EfficiencyCode-linux-sandbox helper");
    assert_eq!(
        EfficiencyCode_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(EfficiencyCode_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&EfficiencyCode_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
async fn apply_patch_turn_diff_tracks_local_and_remote_environment_paths() -> Result<()> {
    ...
    let shared_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-turn-diff-{}",
