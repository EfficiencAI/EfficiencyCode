...
use EfficiencyCode_core::REVIEW_PROMPT;
use EfficiencyCode_core::config::Config;
use EfficiencyCode_core::config::Constrained;
use EfficiencyCode_core::find_thread_path_by_id_str;
...
use EfficiencyCode_protocol::openai_models::ModelTokenBudgetConfig;
...
use EfficiencyCode_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use EfficiencyCode_protocol::protocol::ReviewCodeLocation;
use EfficiencyCode_protocol::protocol::ReviewFinding;
use EfficiencyCode_protocol::protocol::ReviewLineRange;
use EfficiencyCode_protocol::protocol::ReviewOutputEvent;
...
use EfficiencyCode_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under EfficiencyCode sandbox network restrictions.
    skip_if_no_network!();
    ...
    let EfficiencyCode =
        new_conversation_for_server(&server, EfficiencyCode_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&EfficiencyCode, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&EfficiencyCode, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&EfficiencyCode, |ev| {
    ...
    let exited_started = wait_for_event(&EfficiencyCode, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&EfficiencyCode, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&EfficiencyCode, |ev| {
    ...
    let _complete = wait_for_event(&EfficiencyCode, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;

    let path = EfficiencyCode.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match EfficiencyCode.next_event().await.expect("review event").msg {
    ...
    EfficiencyCode.submit(Op::Interrupt).await.unwrap();

    let mut exited_review = false;
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match EfficiencyCode
                .next_event()
                .await
...
async fn review_op_with_plain_text_emits_review_fallback() {
    ...
    let _entered = wait_for_event(&EfficiencyCode, |ev| {
        matches!(ev, EventMsg::EnteredReviewMode(_))
    })
    .await;
    let closed = wait_for_event(&EfficiencyCode, |ev| {
...
async fn review_uses_updated_turn_permissions_and_approval_policy() {
    ...
    let EfficiencyCode = Arc::clone(&test.EfficiencyCode);
...
async fn review_uses_custom_review_model_from_config() {
    ...
    .await;
    let _closed = wait_for_event(&EfficiencyCode, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = EfficiencyCode_home.path().join("resume.jsonl");
    {
        ...
        let user = EfficiencyCode_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = EfficiencyCode_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let EfficiencyCode = resume_conversation_for_server(
        &server,
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let EfficiencyCode =
        new_conversation_for_server(&server, EfficiencyCode_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, EfficiencyCode_home, resume_path)
        .await
