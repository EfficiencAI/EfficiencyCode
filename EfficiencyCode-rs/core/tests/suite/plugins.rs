...
use EfficiencyCode_core_plugins::store::PluginStore;
...
use EfficiencyCode_skills_extension::SkillsExtensionConfig;
use EfficiencyCode_skills_extension::install;
use anyhow::Result;
...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__EfficiencyCode_apps__google_calendar";
...
async fn build_analytics_plugin_test_EfficiencyCode(
    server: &MockServer,
    EfficiencyCode_home: Arc<TempDir>,
) -> Result<TestEfficiencyCode> {
    let chatgpt_base_url = server.uri();
    let mut builder = test_EfficiencyCode()
        .with_home(EfficiencyCode_home)
...
async fn build_apps_enabled_plugin_test_EfficiencyCode(
    server: &MockServer,
    EfficiencyCode_home: Arc<TempDir>,
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context() -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local EfficiencyCode home cache"
    );

    let server = start_mock_server().await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let script_path = write_remote_plugin_script_and_config(EfficiencyCode_home.as_ref());
    ...
    let mut builder = test_EfficiencyCode()
        .with_home(Arc::clone(&EfficiencyCode_home))
        .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model("gpt-5.2");
    let test_EfficiencyCode = builder.build_with_auto_env(&server).await?;
    let EfficiencyCode = Arc::clone(&test_EfficiencyCode.EfficiencyCode);
    let cwd = test_EfficiencyCode.config.cwd.clone();
    let session_model = test_EfficiencyCode.session_configured.model.clone();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, cwd.as_path());
    EfficiencyCode
        .submit(Op::UserInput {
            items: vec![EfficiencyCode_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&EfficiencyCode, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&EfficiencyCode, |event| match event {
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    .await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_agent_plugin_skill_plugin(
        EfficiencyCode_home.as_ref(),
    ))?;
    let mut builder = test_EfficiencyCode()
        .with_home(Arc::clone(&EfficiencyCode_home))
    ...
    test_EfficiencyCode
        ...
        .await?;
    let warning = wait_for_event(&test_EfficiencyCode.EfficiencyCode, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_EfficiencyCode.EfficiencyCode, |ev| {
...
#[test_case("EfficiencyCode", true, Some("native review skill"); "native skill wins over migrated command")]
...
async fn plugin_skill_product_policy_and_migrated_command_precedence_reach_agent_turns(
    ...
    .await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let plugin_root = write_sample_plugin_manifest_and_config(EfficiencyCode_home.as_ref());
    ...
    if include_migrated_command {
        let migrated_skill_dir = plugin_root
            .join(".EfficiencyCode-plugin/migrated-command-skills/source-command-review");
...
async fn legacy_plugin_skill_prompt_remains_complete() -> Result<()> {
    ...
    .await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let skill_path = write_plugin_skill_plugin(EfficiencyCode_home.as_ref());
...
async fn agent_plugin_root_mcp_stdio_tool_round_trip_expands_reserved_paths() -> Result<()> {
    ...
    .await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    write_agent_plugin_skill_plugin(EfficiencyCode_home.as_ref());
    ...
    let mut builder = test_EfficiencyCode().with_home(Arc::clone(&EfficiencyCode_home));
    let test_EfficiencyCode = builder.build_with_remote_and_local_env(&server).await?;
    wait_for_mcp_server(&test_EfficiencyCode.EfficiencyCode, "agent").await?;
    let data_root = dunce::canonicalize(
        std::fs::read_dir(
            EfficiencyCode_home
                .path()
                .join("plugins/data/agent-plugins"),
        )?
        .next()
    ...
    test_EfficiencyCode
        ...
        .await?;
    let end = wait_for_event(&test_EfficiencyCode.EfficiencyCode, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_EfficiencyCode.EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn agent_turns_route_curated_plugin_skills_after_auth_switch() -> Result<()> {
    ...
    const FIXTURES: &[Fixture] = &[
        ...
        Fixture {
            ...
            target_auth: TargetAuth::NoEfficiencyCodeAuth,
            target_model_provider_id: AMAZON_BEDROCK_PROVIDER_ID,
    ...
    async fn skills_for_agent_turn(
        test_EfficiencyCode: &TestEfficiencyCode,
        response: &ResponseMock,
        model_provider_id: &str,
        prompt: &str,
        expected_request_count: usize,
    ) -> Result<String> {
        let mut config = test_EfficiencyCode.config.clone();
        config.model_provider_id = model_provider_id.to_string();
        let thread = test_EfficiencyCode
            .thread_manager
...
"#,
        ...
        let mut builder = test_EfficiencyCode()
            ...
            .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing());
        let test_EfficiencyCode = builder.build_with_auto_env(&server).await?;
        let plugins_manager = test_EfficiencyCode.thread_manager.plugins_manager();
        let skills_service = test_EfficiencyCode.thread_manager.skills_service();
        ...
        match fixture.target_auth {
            ...
            TargetAuth::NoEfficiencyCodeAuth => {
                test_EfficiencyCode
                    .thread_manager
                    .auth_manager()
                    .logout()
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins(
    ...
    write_plugin_skill_plugin(EfficiencyCode_home.as_ref());
    write_plugin_mcp_plugin(EfficiencyCode_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(EfficiencyCode_home.as_ref());
    ...
    let test_EfficiencyCode = build_apps_enabled_plugin_test_EfficiencyCode(
        &server,
        EfficiencyCode_home,
    ...
    .await?;
    let EfficiencyCode = Arc::clone(&test_EfficiencyCode.EfficiencyCode);
    wait_for_mcp_server(&EfficiencyCode, EfficiencyCode_APPS_MCP_SERVER_NAME).await?;

    EfficiencyCode
        .submit(Op::UserInput {
            items: vec![EfficiencyCode_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(EfficiencyCode_home.as_ref());
    write_plugin_mcp_plugin(EfficiencyCode_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(EfficiencyCode_home.as_ref(), "sample_app");
    ...
    .await?;
    let EfficiencyCode = Arc::clone(&test_EfficiencyCode.EfficiencyCode);
    wait_for_mcp_server(&EfficiencyCode, "sample").await?;
...
async fn explicitly_requested_mcp_waits_for_startup(request: ExplicitMcpRequest) -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(write_plugin_skill_plugin(EfficiencyCode_home.as_ref()))?;
    write_plugin_mcp_plugin(EfficiencyCode_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(EfficiencyCode_home.as_ref());
    let initialize_barrier =
        block_plugin_mcp_startup(EfficiencyCode_home.as_ref(), &rmcp_test_server_bin);
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(EfficiencyCode_home.as_ref());
    let test_EfficiencyCode =
        build_analytics_plugin_test_EfficiencyCode(&server, EfficiencyCode_home).await?;
    let EfficiencyCode = Arc::clone(&test_EfficiencyCode.EfficiencyCode);
    ...
    let event = wait_for_analytics_event(&server, "EfficiencyCode_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(EfficiencyCode_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_remote_plugin_skill_plugin(
        EfficiencyCode_home.as_ref(),
    ))?;
    persist_sample_remote_plugin_id(EfficiencyCode_home.as_ref());
    let test_EfficiencyCode =
        build_analytics_plugin_test_EfficiencyCode(&server, EfficiencyCode_home).await?;
    let EfficiencyCode = Arc::clone(&test_EfficiencyCode.EfficiencyCode);
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id(
    ...
    let server = start_mock_server().await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(EfficiencyCode_home.as_ref());
    persist_sample_remote_plugin_id(EfficiencyCode_home.as_ref());
...
async fn wait_for_analytics_event(server: &MockServer, event_type: &str) -> serde_json::Value {
    ...
    loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/EfficiencyCode/analytics-events/events")
            .find_map(|request| {
