...
use EfficiencyCode_core::ModelClient;
use EfficiencyCode_core::NewThread;
use EfficiencyCode_core::Prompt;
use EfficiencyCode_core::ResponseEvent;
use EfficiencyCode_core::StartThreadOptions;
use EfficiencyCode_core::ThreadManager;
use EfficiencyCode_core::X_EfficiencyCode_ROUTING_HINT_HEADER;
use EfficiencyCode_core::resolve_installation_id;
use EfficiencyCode_core::thread_store_from_config;
use EfficiencyCode_extension_api::empty_extension_registry;
use EfficiencyCode_features::Feature;
use EfficiencyCode_history::RolloutItem;
use EfficiencyCode_history::RolloutLine;
use EfficiencyCode_login::AuthKeyringBackendKind;
...
use EfficiencyCode_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use EfficiencyCode_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use EfficiencyCode_protocol::models::LocalShellAction;
use EfficiencyCode_protocol::models::LocalShellExecAction;
use EfficiencyCode_protocol::models::LocalShellStatus;
use EfficiencyCode_protocol::models::MessagePhase;
use EfficiencyCode_protocol::models::ReasoningItemContent;
use EfficiencyCode_protocol::models::ReasoningItemReasoningSummary;
use EfficiencyCode_protocol::models::ResponseItem;
use EfficiencyCode_protocol::models::WebSearchAction;
use EfficiencyCode_protocol::openai_models::InputModality;
...
use core_test_support::TestEfficiencyCodeResponsesRequestKind;
...
use core_test_support::test_EfficiencyCode::TestEfficiencyCode;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> EfficiencyCode_core::EfficiencyCodeResponsesMetadata {
...
fn assert_EfficiencyCode_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-EfficiencyCode-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-EfficiencyCode-turn-metadata"]
        .as_str()
        .expect("missing x-EfficiencyCode-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-EfficiencyCode-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-EfficiencyCode-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_EfficiencyCode().build(&server).await.unwrap();
...
async fn non_openai_responses_requests_include_item_ids_without_passthrough_metadata() {
    ...
    let EfficiencyCode = test_EfficiencyCode()
    ...
    EfficiencyCode
        ...
        .await
        .unwrap();
    wait_for_event(&EfficiencyCode, |event| {
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.EfficiencyCode.submit(Op::Shutdown).await?;
    wait_for_event(&initial.EfficiencyCode, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let EfficiencyCode_home = Arc::new(TempDir::new()?);
    let mut builder = test_EfficiencyCode();
    let first = builder
        .resume(
            &server,
            Arc::clone(&EfficiencyCode_home),
            session_path.clone(),
        )
        .await?;

    first.submit_turn("first resume").await?;
    first.EfficiencyCode.submit(Op::Shutdown).await?;
    wait_for_event(&first.EfficiencyCode, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder
        .resume(&server, EfficiencyCode_home, session_path)
        .await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    test.EfficiencyCode.submit(Op::Compact).await?;
    wait_for_event(&test.EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
/// Writes an `auth.json` into the provided `EfficiencyCode_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        EfficiencyCode_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: EfficiencyCode_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = EfficiencyCode_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = EfficiencyCode_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![EfficiencyCode_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = EfficiencyCode_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure EfficiencyCode to resume from our file
    let EfficiencyCode_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_EfficiencyCode()
        .with_home(EfficiencyCode_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, EfficiencyCode_home, session_path.clone())
        .await
        .expect("resume conversation");
    let EfficiencyCode = test.EfficiencyCode.clone();
    ...
    EfficiencyCode
        ...
        .await
        .unwrap();
    wait_for_event(&EfficiencyCode, |ev| {
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let EfficiencyCode_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_EfficiencyCode().with_model("gpt-5.4");
    let test = builder
        .resume(&server, EfficiencyCode_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder =
        test_EfficiencyCode().with_auth(EfficiencyCodeAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    assert_eq!(request.header(X_EfficiencyCode_ROUTING_HINT_HEADER), None);
    ...
    let request_body = request.body_json();
    let installation_id = std::fs::read_to_string(
        test.EfficiencyCode_home_path()
    ...
    assert_EfficiencyCode_client_metadata(
        &request_body,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    let EfficiencyCode_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&EfficiencyCode_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = EfficiencyCode_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        EfficiencyCode_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            EfficiencyCodeAuth::from_api_key("unused-api-key"),
        )),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &EfficiencyCode_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::from_api_key("Test API Key"))
        .with_config(|config| {
            config.base_instructions = Some("test instructions".to_string());
        });
    let EfficiencyCode = builder
        .build(&server)
        .await
        .expect("create new conversation")
        .EfficiencyCode;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/EfficiencyCode", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_EfficiencyCode()
        .with_auth(create_dummy_EfficiencyCode_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/EfficiencyCode/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
    ...
    assert_eq!(
        request.header(X_EfficiencyCode_ROUTING_HINT_HEADER),
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = EfficiencyCodeAuth::from_auth_storage(
    ...
    .await
    .expect("Failed to load EfficiencyCodeAuth")
    .expect("No EfficiencyCodeAuth found in EfficiencyCode_home");
    let auth_manager = EfficiencyCode_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.EfficiencyCode_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        EfficiencyCode_core::build_models_manager(&config, auth_manager),
        ...
        Arc::new(EfficiencyCode_core::test_support::EmptyUserInstructionsProvider),
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__EfficiencyCode_apps"),
        "did not expect EfficiencyCode_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect EfficiencyCode_apps resource calls: {resource_methods:?}"
    );
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestEfficiencyCode { EfficiencyCode, .. } = test_EfficiencyCode()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let model_info =
        EfficiencyCode_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = EfficiencyCode_core::test_support::auth_manager_from_auth(
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-EfficiencyCode-primary-used-percent", "12.5")
        .insert_header("x-EfficiencyCode-secondary-used-percent", "40.0")
        .insert_header("x-EfficiencyCode-primary-window-minutes", "10")
        .insert_header("x-EfficiencyCode-secondary-window-minutes", "60")
        .insert_header("x-EfficiencyCode-primary-reset-at", "1704069000")
        .insert_header("x-EfficiencyCode-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let EfficiencyCode = builder
    ...
    wait_for_event(&EfficiencyCode, |msg| {
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-EfficiencyCode-primary-used-percent", "100.0")
        .insert_header("x-EfficiencyCode-secondary-used-percent", "87.5")
        .insert_header(
            "x-EfficiencyCode-primary-over-secondary-limit-percent",
            "95.0",
        )
        .insert_header("x-EfficiencyCode-primary-window-minutes", "15")
        .insert_header("x-EfficiencyCode-secondary-window-minutes", "60")
        .insert_header("x-EfficiencyCode-credits-has-credits", "true")
        .insert_header("x-EfficiencyCode-credits-unlimited", "false")
        .insert_header("x-EfficiencyCode-credits-balance", "")
        .insert_header(
            "x-EfficiencyCode-rate-limit-reached-type",
    ...
    let mut builder = test_EfficiencyCode();
    let EfficiencyCode_fixture = builder.build(&server).await?;
    let EfficiencyCode = EfficiencyCode_fixture.EfficiencyCode.clone();
    ...
    let submission_id = EfficiencyCode
        .submit(Op::UserInput {
            items: vec![UserInput::Text {
                text: "hello".into(),
                text_elements: Vec::new(),
    ...
    let token_event = wait_for_event(&EfficiencyCode, |msg| {
    ...
    let error_event =
        wait_for_event(&EfficiencyCode, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&EfficiencyCode, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&EfficiencyCode, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = EfficiencyCodeErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn env_var_overrides_loaded_auth() {
    ...
    assert_eq!(
        request.headers.get(X_EfficiencyCode_ROUTING_HINT_HEADER),
        None
    );
}

fn create_dummy_EfficiencyCode_auth() -> EfficiencyCodeAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under EfficiencyCode sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
