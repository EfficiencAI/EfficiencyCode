...
async fn run_turn(test: &TestEfficiencyCode, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestEfficiencyCode, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_EfficiencyCode_with_test_tool(
    server: &wiremock::MockServer,
) -> anyhow::Result<TestEfficiencyCode> {
    let mut builder = test_EfficiencyCode().with_model("test-gpt-5.1-EfficiencyCode");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_EfficiencyCode_with_test_tool(&server).await?;
