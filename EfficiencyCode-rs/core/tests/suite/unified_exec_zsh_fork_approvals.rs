...
async fn unified_exec_zsh_fork_guardian_reviews_intercepted_execve() -> Result<()> {
    ...
    loop {
        let event = tokio::time::timeout(Duration::from_secs(30), test.EfficiencyCode.next_event())
            .await
...
async fn unified_exec_zsh_fork_guardian_reviews_persistent_terminal_in_current_turn() -> Result<()>
{
    ...
    let first_completion = wait_for_event(&test.EfficiencyCode, |event| {
...
async fn build_unified_exec_zsh_fork_test_or_skip<F>(
    ...
    pre_build_hook: F,
) -> Result<Option<(MockServer, TestEfficiencyCode)>>
where
...
async fn approve_expected_exec(test: &TestEfficiencyCode, expected_command: &str) -> Result<()> {
    let approval = expect_exec_approval(test, expected_command).await;
    approve_exec(test, approval.effective_approval_id()).await
}

async fn approve_exec(test: &TestEfficiencyCode, approval_id: String) -> Result<()> {
    test.EfficiencyCode
