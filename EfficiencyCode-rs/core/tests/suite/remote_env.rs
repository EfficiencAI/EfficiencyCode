...
use EfficiencyCode_core::EnvironmentConfig;
use EfficiencyCode_core::StartThreadOptions;
use EfficiencyCode_core::WaitForEnvironmentToolConfig;
use EfficiencyCode_core::compact::SUMMARIZATION_PROMPT;
use EfficiencyCode_core::config::Config;
use EfficiencyCode_core::config::Constrained;
use EfficiencyCode_exec_server::CopyOptions;
use EfficiencyCode_exec_server::CreateDirectoryOptions;
use EfficiencyCode_exec_server::EnvironmentReadyInfo;
...
use EfficiencyCode_exec_server::NoiseChannelPublicKey;
use EfficiencyCode_exec_server::NoiseRendezvousConnectBundle;
use EfficiencyCode_exec_server::NoiseRendezvousConnectProvider;
use EfficiencyCode_exec_server::REMOTE_ENVIRONMENT_ID;
use EfficiencyCode_exec_server::RemoteEnvironmentConfig;
use EfficiencyCode_exec_server::RemoveOptions;
use EfficiencyCode_extension_api::ContextContributor;
use EfficiencyCode_extension_api::ExtensionDataInit;
use EfficiencyCode_extension_api::ExtensionFuture;
use EfficiencyCode_extension_api::ExtensionRegistryBuilder;
use EfficiencyCode_extension_api::RenderedWorldStateFragment;
use EfficiencyCode_extension_api::ThreadLifecycleContributor;
use EfficiencyCode_extension_api::ThreadStartInput;
use EfficiencyCode_extension_api::WorldStateContributionInput;
use EfficiencyCode_extension_api::WorldStateSectionContribution;
...
use EfficiencyCode_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_EfficiencyCode_with_wait_for_environment() -> TestEfficiencyCodeBuilder {
    let mut extensions = ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(Arc::new(WaitForEnvironmentTestExtension));
    test_EfficiencyCode().with_extensions(Arc::new(extensions.build()))
}

async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestEfficiencyCode> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections =
        EfficiencyCode_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_patch_approval(test: &TestEfficiencyCode) {
    let event = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/EfficiencyCode-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\EfficiencyCode-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_EfficiencyCode_with_wait_for_environment()
            } else {
                test_EfficiencyCode()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    test.EfficiencyCode
        ...
        .await?;
    let request = wait_for_event_match(&test.EfficiencyCode, |event| match event {
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    test.EfficiencyCode.ensure_rollout_materialized().await;
    test.EfficiencyCode.flush_rollout().await?;
    let rollout = fs::read_to_string(test.EfficiencyCode.rollout_path().context("rollout path")?)?;
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    for (index, (thread, prompt)) in [
        (&test.EfficiencyCode, "first"),
        (&second.thread, "second"),
        (&test.EfficiencyCode, "first-updated"),
...
-> Result<()> {
    ...
    let remote_environment = tokio::spawn(EfficiencyCode_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_stays_pending_after_materialization() -> Result<()> {
    ...
    .await;
    let mut builder = test_EfficiencyCode_with_wait_for_environment().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        assert!(config.features.enable(Feature::UnifiedExec).is_ok());
    });
    ...
    test.EfficiencyCode.submit(Op::Interrupt).await?;
    wait_for_event(&test.EfficiencyCode, |event| {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_EfficiencyCode_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
    ...
    assert_eq!(
        test.EfficiencyCode.instruction_sources().await,
        vec![agents_path]
    );
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.EfficiencyCode.ensure_rollout_materialized().await;
    test.EfficiencyCode.flush_rollout().await?;
    let rollout_path = test.EfficiencyCode.rollout_path().context("rollout path")?;
...
async fn exec_command_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-routing-{}",
...
async fn remote_exec_materializes_target_roots_before_sandbox_selection() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-target-roots-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/EfficiencyCode-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-readable-{}",
        std::process::id()
    ));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-dotdot-{}",
        std::process::id()
    ));
...
async fn remote_test_env_remove_removes_symlink_not_target() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-remove-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
...
async fn remote_test_env_copy_preserves_symlink_source() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/EfficiencyCode-remote-copy-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
