...
use EfficiencyCode_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "EfficiencyCode-test-ttl";
const VERSIONED_MODEL: &str = "EfficiencyCode-test-versioned";
const MISSING_VERSION_MODEL: &str = "EfficiencyCode-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "EfficiencyCode-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.EfficiencyCode_home.join(CACHE_FILE);
    ...
    EfficiencyCode
        .submit(Op::UserInput {
            ...
            thread_settings: EfficiencyCode_protocol::protocol::ThreadSettingsOverrides {
                ...
                approval_policy: Some(EfficiencyCode_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&EfficiencyCode, |event| {
...
async fn matching_models_etag_does_not_rewrite_recent_cache() -> Result<()> {
    ...
    let cache_path = test.config.EfficiencyCode_home.join(CACHE_FILE);
