...
use EfficiencyCode_core::TurnInput;
use EfficiencyCode_core::config::CurrentTimeReminderConfig;
use EfficiencyCode_extension_items::ExtensionItem;
use EfficiencyCode_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_EfficiencyCode().build_with_auto_env(&server).await?;

    submit_queue_only_agent_mail(test.EfficiencyCode.as_ref(), "pending mailbox input").await;
    test.EfficiencyCode
    ...
    wait_for_turn_complete(test.EfficiencyCode.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message =
        core_test_support::wait_for_event_match(test.EfficiencyCode.as_ref(), |event| {
...
async fn build_EfficiencyCode(server: &StreamingSseServer) -> Arc<EfficiencyCodeThread> {
    test_EfficiencyCode()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming EfficiencyCode test session")
        .EfficiencyCode
}

async fn submit_user_input(EfficiencyCode: &EfficiencyCodeThread, text: &str) {
    EfficiencyCode
        .submit(Op::UserInput {
...
async fn submit_danger_full_access_user_turn(test: &TestEfficiencyCode, text: &str) {
...
async fn steer_user_input(EfficiencyCode: &EfficiencyCodeThread, text: &str) {
    EfficiencyCode
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn enqueue_queue_only_agent_mail(EfficiencyCode: &EfficiencyCodeThread, text: &str) {
    EfficiencyCode
...
async fn submit_queue_only_agent_mail(EfficiencyCode: &EfficiencyCodeThread, text: &str) {
    enqueue_queue_only_agent_mail(EfficiencyCode, text).await;
    EfficiencyCode
...
async fn wait_for_reasoning_item_started(EfficiencyCode: &EfficiencyCodeThread) {
    wait_for_event(EfficiencyCode, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(EfficiencyCode: &EfficiencyCodeThread, text: &str) {
...
async fn wait_for_turn_complete(EfficiencyCode: &EfficiencyCodeThread) {
    wait_for_event(EfficiencyCode, |event| {
...
async fn wait_for_sleep_item_started(
    EfficiencyCode: &EfficiencyCodeThread,
    call_id: &str,
    duration_ms: u64,
) {
    let event = wait_for_event(EfficiencyCode, |event| {
        matches!(
            event,
...
impl EfficiencyCode_extension_api::ThreadLifecycleContributor<EfficiencyCode_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: EfficiencyCode_extension_api::ThreadStartInput<
            'a,
...
async fn queue_only_agent_mail_wakes_sleeping_root_and_persists_message() {
    ...
    let mut extensions = EfficiencyCode_extension_api::ExtensionRegistryBuilder::<
        EfficiencyCode_core::config::Config,
    >::new();
    ...
    let EfficiencyCode = test_EfficiencyCode()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build EfficiencyCode test session")
        .EfficiencyCode;

    enqueue_queue_only_agent_mail(&EfficiencyCode, CHILD_MESSAGE).await;
    wait_for_turn_complete(&EfficiencyCode).await;

    assert_eq!(server.requests().await.len(), 1);
    let history = EfficiencyCode
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    EfficiencyCode_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            EfficiencyCode_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&EfficiencyCode, INITIAL_PROMPT).await;
    wait_for_event(&EfficiencyCode, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&EfficiencyCode, STEER_PROMPT).await;
    wait_for_turn_complete(&EfficiencyCode).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&EfficiencyCode, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&EfficiencyCode, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&EfficiencyCode, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&EfficiencyCode, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&EfficiencyCode, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&EfficiencyCode, "new mailbox input").await;
    wait_for_sleep_item_completed(&EfficiencyCode, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&EfficiencyCode).await;
    ...
    let rollout_path = EfficiencyCode.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let EfficiencyCode = build_EfficiencyCode(&server).await;

    submit_user_input(&EfficiencyCode, "first prompt").await;

    wait_for_reasoning_item_started(&EfficiencyCode).await;

    submit_queue_only_agent_mail(&EfficiencyCode, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&EfficiencyCode, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&EfficiencyCode, "second prompt").await;
    wait_for_turn_complete(&EfficiencyCode).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&EfficiencyCode, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&EfficiencyCode, "resumed old task").await;
    wait_for_turn_complete(&EfficiencyCode).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&EfficiencyCode, "processed steered prompt").await;
    wait_for_turn_complete(&EfficiencyCode).await;
