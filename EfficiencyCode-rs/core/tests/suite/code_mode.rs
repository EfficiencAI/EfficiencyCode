...
use EfficiencyCode_core::config::CurrentTimeReminderConfig;
use EfficiencyCode_extension_api::ExtensionData;
use EfficiencyCode_extension_api::ExtensionRegistryBuilder;
use EfficiencyCode_extension_api::ToolCallOutcome;
use EfficiencyCode_extension_api::ToolContributor;
use EfficiencyCode_extension_api::ToolFinishInput;
use EfficiencyCode_extension_api::ToolLifecycleContributor;
use EfficiencyCode_extension_api::ToolLifecycleFuture;
use EfficiencyCode_extension_api::ToolStartInput;
...
use EfficiencyCode_protocol::openai_models::ToolMode;
...
use EfficiencyCode_tools::FreeformTool;
use EfficiencyCode_tools::FreeformToolFormat;
use EfficiencyCode_tools::FunctionCallError;
use EfficiencyCode_tools::JsonToolOutput;
use EfficiencyCode_tools::ResponsesApiNamespace;
use EfficiencyCode_tools::ResponsesApiNamespaceTool;
use EfficiencyCode_tools::ToolCall;
use EfficiencyCode_tools::ToolExecutor;
use EfficiencyCode_tools::ToolExecutorFuture;
use EfficiencyCode_tools::ToolName;
use EfficiencyCode_tools::ToolOutput;
use EfficiencyCode_tools::ToolPayload;
use EfficiencyCode_tools::ToolSpec;
use EfficiencyCode_web_search_extension::install as install_web_search_extension;
use anyhow::Result;
use base64::Engine;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestEfficiencyCode, ResponseMock)> {
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_EfficiencyCode()
        .with_model(model)
...
async fn run_code_mode_turn_with_builder(
    ...
    prompt: &str,
    code: &str,
    mut builder: TestEfficiencyCodeBuilder,
) -> Result<(TestEfficiencyCode, ResponseMock)> {
...
async fn run_unavailable_code_mode_turn(
    ...
    loop {
        match wait_for_event(&test.EfficiencyCode, |_| true).await {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_EfficiencyCode()
        .with_model("test-gpt-5.1-EfficiencyCode")
        .with_code_mode_host_program("EfficiencyCode-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("EfficiencyCode-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("EfficiencyCode-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth = EfficiencyCodeAuth::from_api_key("dummy");
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-EfficiencyCode")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-EfficiencyCode").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.EfficiencyCode, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__EfficiencyCode_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__EfficiencyCode_apps__calendar_timezone_option_99",
...
async fn code_mode_wait_timeout_reconnects_on_next_exec() -> Result<()> {
    ...
    test.EfficiencyCode
        ...
        .await?;
    wait_for_event_match(&test.EfficiencyCode, |event| match event {
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let EfficiencyCode_extension_api::ToolCallSource::CodeMode { cell_id, .. } =
                input.source
            else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    EfficiencyCode_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
image({{
  ...
  data,
  mimeType: "image/png",
  _meta: {{ "EfficiencyCode/imageDetail": "original" }}
}});
"#,
    ...
    assert!(!exec_description.contains("EfficiencyCode/imageDetail"));
...
"#,
        ...
        let turn_id = wait_for_event_match(&test.EfficiencyCode, |event| match event {
        ...
        test.EfficiencyCode
            ...
            .await?;
        wait_for_event(&test.EfficiencyCode, |event| match event {
        ...
        if use_responses_lite {
            let metadata: Value = serde_json::from_str(
                first_body["client_metadata"]["x-EfficiencyCode-turn-metadata"]
                    .as_str()
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "EfficiencyCode_app__hidden_dynamic_tool");
const out = await tools.EfficiencyCode_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("EfficiencyCode_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String(
            "EfficiencyCode_app__hidden_dynamic_tool".to_string()
        ))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("EfficiencyCode app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("EfficiencyCode_app__hidden_dynamic_tool(args:")
            })
    );
