...
async fn refresh_keeps_superseded_mcp_server_alive_for_in_flight_calls() -> anyhow::Result<()> {
    ...
    let fixture = test_EfficiencyCode()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&fixture.EfficiencyCode, "refresh_cleanup").await?;
    ...
    let long_call = tokio::spawn({
        let EfficiencyCode = Arc::clone(&fixture.EfficiencyCode);
    ...
    .await;
    fixture.EfficiencyCode.submit(Op::RefreshMcpServers).await?;
