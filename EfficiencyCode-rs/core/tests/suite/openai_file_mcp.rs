...
async fn run_extract_turn(test: &TestEfficiencyCode, server: &MockServer) -> Result<ResponseMock> {
...
async fn EfficiencyCode_apps_file_params_omit_fields_absent_from_tool_schema() -> Result<()> {
    ...
    assert_eq!(
        upload_body.get("EfficiencyCode_connector_id"),
        Some(&json!("calendar"))
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn EfficiencyCode_apps_file_params_pass_uploaded_file_to_post_tool_use_hook() -> Result<()> {
    let server = start_mock_server().await;
    let apps_server = AppsTestServer::mount(&server).await?;
