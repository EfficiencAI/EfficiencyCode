...
enum TestEfficiencyCodeHome {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl TestEfficiencyCodeHome {
    fn path(&self) -> &Path {
        match self {
...
fn EfficiencyCode_home_for_windows_sandbox_test(
    name: &str,
) -> anyhow::Result<TestEfficiencyCodeHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep EfficiencyCode_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let EfficiencyCode_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&EfficiencyCode_home).with_context(|| {
            format!(
                "create stable test EfficiencyCode_HOME {}",
                EfficiencyCode_home.display()
            )
        })?;
        return Ok(TestEfficiencyCodeHome::Persistent(EfficiencyCode_home));
    }

    Ok(TestEfficiencyCodeHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("EfficiencyCode-resources");
    ...
    for helper_name in [
        "EfficiencyCode-windows-sandbox-setup",
        "EfficiencyCode-command-runner",
    ] {
        let helper = EfficiencyCode_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let EfficiencyCode_home = EfficiencyCode_home_for_windows_sandbox_test(
        "windows-restricted-token-deny-read-EfficiencyCode-home",
    )?;
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let EfficiencyCode_home = EfficiencyCode_home_for_windows_sandbox_test(
        "windows-elevated-missing-metadata-EfficiencyCode-home",
    )?;
    let _EfficiencyCode_home_guard = EnvVarGuard::set(
        "EfficiencyCode_HOME",
        EfficiencyCode_home.path().as_os_str(),
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let EfficiencyCode_home = EfficiencyCode_home_for_windows_sandbox_test(
        "windows-elevated-deny-read-EfficiencyCode-home",
    )?;
    let _EfficiencyCode_home_guard = EnvVarGuard::set(
        "EfficiencyCode_HOME",
        EfficiencyCode_home.path().as_os_str(),
    ...
    let public = cwd.join("public.txt");
    let setup_marker = EfficiencyCode_home
        .path()
        .join(".sandbox")
    ...
    assert!(
        sandbox_setup_is_complete(EfficiencyCode_home.path()),
