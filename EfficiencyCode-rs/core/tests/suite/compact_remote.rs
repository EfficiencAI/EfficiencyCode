...
use EfficiencyCode_core::compact::SUMMARY_PREFIX;
...
use EfficiencyCode_login::auth::AgentIdentityAuth;
use EfficiencyCode_login::auth::AgentIdentityAuthRecord;
use EfficiencyCode_login::auth::BedrockApiKeyAuth;
use EfficiencyCode_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
...
use EfficiencyCode_protocol::account::PlanType as AccountPlanType;
use EfficiencyCode_protocol::config_types::ServiceTier;
use EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use EfficiencyCode_protocol::dynamic_tools::DynamicToolFunctionSpec;
use EfficiencyCode_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use EfficiencyCode_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use EfficiencyCode_protocol::protocol::RealtimeConversationRealtimeEvent;
use EfficiencyCode_protocol::protocol::RealtimeEvent;
use EfficiencyCode_protocol::protocol::RealtimeOutputModality;
...
use core_test_support::test_EfficiencyCode::TestEfficiencyCodeBuilder;
use core_test_support::test_EfficiencyCode::TestEfficiencyCodeHarness;
use core_test_support::test_EfficiencyCode::test_EfficiencyCode as base_test_EfficiencyCode;
...
const TURN_STATE_HEADER: &str = "x-EfficiencyCode-turn-state";
...
fn test_EfficiencyCode() -> TestEfficiencyCodeBuilder {
    base_test_EfficiencyCode().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_EfficiencyCode_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestEfficiencyCodeBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(
    EfficiencyCode: &EfficiencyCode_core::EfficiencyCodeThread,
) -> Result<()> {
    EfficiencyCode
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            EfficiencyCode_responses_as_items: false,
            EfficiencyCode_response_item_prefix: None,
            EfficiencyCode_response_handoff_mode:
                EfficiencyCode_protocol::protocol::EfficiencyCodeResponseHandoffMode::Thinking,
            EfficiencyCode_response_handoff_channel_prefixes: None,
            model: None,
    ...
    wait_for_event_match(EfficiencyCode, |msg| match msg {
...
async fn close_realtime_conversation(
    ...
    EfficiencyCode.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(EfficiencyCode, |msg| match msg {
...
async fn wait_for_turn_complete(EfficiencyCode: &EfficiencyCode_core::EfficiencyCodeThread) {
...
fn amazon_bedrock_test_EfficiencyCode() -> TestEfficiencyCodeBuilder {
    let auth = EfficiencyCodeAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn amazon_bedrock_uses_remote_compaction_endpoint() -> Result<()> {
    ...
    let harness =
        TestEfficiencyCodeHarness::with_auto_env_builder(amazon_bedrock_test_EfficiencyCode())
            .await?;
    ...
    harness.test().EfficiencyCode.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().EfficiencyCode).await;
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    initial.EfficiencyCode.shutdown_and_wait().await?;
    annotate_retained_user_in_rollout(&rollout_path, retained_text)?;

    let resumed = builder()
        .resume(&server, home, rollout_path.clone())
        .await?;
    resumed.EfficiencyCode.submit(Op::Compact).await?;
    wait_for_turn_complete(&resumed.EfficiencyCode).await;
    resumed.EfficiencyCode.shutdown_and_wait().await?;
...
async fn amazon_bedrock_automatic_compaction_uses_v1_endpoint_when_v2_is_enabled() -> Result<()> {
    ...
    let harness = TestEfficiencyCodeHarness::with_auto_env_builder(
        amazon_bedrock_test_EfficiencyCode().with_config(|config| {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestEfficiencyCodeHarness::with_builder(
        test_EfficiencyCode()
            .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let EfficiencyCode = harness.test().EfficiencyCode.clone();
    ...
    EfficiencyCode
        ...
        .await?;
    wait_for_turn_complete(&EfficiencyCode).await;

    EfficiencyCode.submit(Op::Compact).await?;
    wait_for_turn_complete(&EfficiencyCode).await;
    ...
    assert_eq!(
        ...
        compact_request
            .header("x-EfficiencyCode-window-id")
            .as_deref()
    );
    assert_eq!(
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestEfficiencyCodeHarness::with_builder(
        test_EfficiencyCode().with_auth(EfficiencyCodeAuth::AgentIdentity(
    ...
    assert_eq!(
        compact_request.header(X_EfficiencyCode_ROUTING_HINT_HEADER),
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let uses_EfficiencyCode_backend = auth.uses_EfficiencyCode_backend();
    let mut builder = test_EfficiencyCode().with_auth(auth);
    ...
    let harness = TestEfficiencyCodeHarness::with_builder(builder).await?;
    let EfficiencyCode = harness.test().EfficiencyCode.clone();
    let image_url =
    ...
    let expected_routing_hint = |body: &Value| {
        if !uses_EfficiencyCode_backend {
            return None;
        }
    ...
    assert_eq!(
        normal_request.header(X_EfficiencyCode_ROUTING_HINT_HEADER),
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        EfficiencyCodeAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-EfficiencyCode-beta-features")
            .as_deref()
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-EfficiencyCode-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "EfficiencyCode_app".to_string(),
        description: "EfficiencyCode app tools.".to_string(),
        tools: vec![
    ...
    let new_thread = test
        ...
        .await?;
    test.EfficiencyCode = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "EfficiencyCode_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "EfficiencyCode_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    test.EfficiencyCode
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.EfficiencyCode, |event| {
    ...
    test.EfficiencyCode
        ...
        .await?;
    wait_for_turn_complete(&test.EfficiencyCode).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&EfficiencyCode, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&EfficiencyCode, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestEfficiencyCodeHarness::with_builder(
        test_EfficiencyCode()
            .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_EfficiencyCode = baseline_harness.test().EfficiencyCode.clone();
    ...
    baseline_EfficiencyCode
        ...
        .await?;
    wait_for_event(&baseline_EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    baseline_EfficiencyCode.submit(Op::Compact).await?;
    wait_for_event(&baseline_EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestEfficiencyCodeHarness::with_builder(
        test_EfficiencyCode()
            .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_EfficiencyCode = override_harness.test().EfficiencyCode.clone();
    ...
    override_EfficiencyCode
        ...
        .await?;
    wait_for_event(&override_EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_EfficiencyCode.submit(Op::Compact).await?;
    wait_for_event(&override_EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = EfficiencyCode.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    EfficiencyCode.submit(Op::Shutdown).await?;
    wait_for_event(&EfficiencyCode, |ev| {
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    let mut start_builder = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    initial.EfficiencyCode.submit(Op::Compact).await?;
    wait_for_event(&initial.EfficiencyCode, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let mut resume_builder = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.EfficiencyCode, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_EfficiencyCode_builder(&realtime_server).with_config(|config| {
            config.model_auto_compact_token_limit = Some(200);
        });
    let test = builder.build(&server).await?;
    ...
    start_realtime_conversation(test.EfficiencyCode.as_ref()).await?;
    ...
    close_realtime_conversation(test.EfficiencyCode.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_EfficiencyCode_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_EfficiencyCode_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.EfficiencyCode.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.EfficiencyCode.as_ref()).await?;
    initial.EfficiencyCode.submit(Op::Shutdown).await?;
    wait_for_event(&initial.EfficiencyCode, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_EfficiencyCode_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.EfficiencyCode.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.EfficiencyCode.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_EfficiencyCode_builder(&realtime_server);
    let test = builder.build(&server).await?;
