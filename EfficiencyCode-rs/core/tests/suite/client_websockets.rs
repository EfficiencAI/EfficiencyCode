...
use EfficiencyCode_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use EfficiencyCode_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use EfficiencyCode_core::EfficiencyCodeResponsesMetadata;
use EfficiencyCode_core::ModelClient;
use EfficiencyCode_core::ModelClientSession;
...
use EfficiencyCode_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use EfficiencyCode_core::test_support::with_parent_turn;
...
use EfficiencyCode_rollout_trace::ConversationPart;
use EfficiencyCode_rollout_trace::InferenceTraceContext;
use EfficiencyCode_rollout_trace::RawTraceEventPayload;
use EfficiencyCode_rollout_trace::TraceWriter;
...
fn websocket_connection_metadata(
    ...
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestEfficiencyCodeResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    let harness = websocket_harness_for_EfficiencyCode_backend(&server).await;
    ...
    assert_eq!(
        handshake.header(X_EfficiencyCode_ROUTING_HINT_HEADER),
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_login::default_client::get_EfficiencyCode_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-EfficiencyCode-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_EfficiencyCode_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_omits_routing_hint_for_provider_with_own_credentials() {
    ...
    assert_eq!(handshake.header(X_EfficiencyCode_ROUTING_HINT_HEADER), None);
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-EfficiencyCode-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::EfficiencyCodeTurnStarted {
            EfficiencyCode_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server
            .single_handshake()
            .header(X_EfficiencyCode_ROUTING_HINT_HEADER),
        None
    );
...
async fn responses_websocket_emits_rate_limit_events() {
    ...
    let rate_limit_event = json!({
        "type": "EfficiencyCode.rate_limits",
        "plan_type": "plus",
        "rate_limits": {
            "allowed": true,
            "limit_reached": false,
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-EfficiencyCode-primary-used-percent": "100.0",
            "x-EfficiencyCode-secondary-used-percent": "87.5",
            "x-EfficiencyCode-primary-over-secondary-limit-percent": "95.0",
            "x-EfficiencyCode-primary-window-minutes": "15",
            "x-EfficiencyCode-secondary-window-minutes": "60",
            "x-EfficiencyCode-credits-has-credits": "true",
            "x-EfficiencyCode-credits-unlimited": "false",
            "x-EfficiencyCode-credits-balance": "",
            "x-EfficiencyCode-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket EfficiencyCode");
    ...
    let token_event = wait_for_event(&test.EfficiencyCode, |msg| {
    ...
    let error_event = wait_for_event(&test.EfficiencyCode, |msg| {
        matches!(msg, EventMsg::Error(_))
    })
    .await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-EfficiencyCode-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-EfficiencyCode-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-EfficiencyCode-turn-metadata"]
            .as_str()
...
async fn websocket_harness_for_EfficiencyCode_backend(
    server: &WebSocketTestServer,
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info =
        EfficiencyCode_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(EfficiencyCode_core::test_support::auth_manager_from_auth);
    let auth_manager = client_auth_manager.clone().unwrap_or_else(|| {
        EfficiencyCode_core::test_support::auth_manager_from_auth(EfficiencyCodeAuth::from_api_key(
            "Test API Key",
        ))
    });
