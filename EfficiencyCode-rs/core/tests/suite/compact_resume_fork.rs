...
//! request payload that EfficiencyCode would send to the model and assert that the
...
fn seed_first_checkpoint_harness_metadata(path: &Path, retained_text: &str) -> Result<()> {
    ...
    for envelope in replacement_history {
        envelope.metadata = Some(EfficiencyCodeHarnessMetadata::default());
    }
...
async fn user_turn(conversation: &Arc<EfficiencyCodeThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<EfficiencyCodeThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<EfficiencyCodeThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<EfficiencyCodeThread>) {
    conversation
        .shutdown_and_wait()
        .await
