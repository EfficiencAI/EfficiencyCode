...
struct ScriptedExternalAuth {
    current: Mutex<EfficiencyCodeAuth>,
...
fn external_chatgpt_auth(access_token: &str) -> std::io::Result<EfficiencyCodeAuth> {
...
async fn header_auth_is_attached_to_responses_requests() -> anyhow::Result<()> {
    ...
    let mut builder =
        test_EfficiencyCode().with_auth(EfficiencyCodeAuth::Headers(AuthHeaders::new(headers)));
...
async fn configured_external_auth_401_retry_uses_refreshed_chatgpt_headers() -> anyhow::Result<()> {
    ...
    let mut builder = test_EfficiencyCode().with_auth(EfficiencyCodeAuth::from_api_key("seed"));
    let test = builder.build_with_auto_env(&server).await?;
