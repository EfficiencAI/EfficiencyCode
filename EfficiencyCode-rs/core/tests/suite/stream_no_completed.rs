...
async fn connection_failure_pauses_retry_budget_until_provider_is_reachable() -> anyhow::Result<()>
{
    ...
    let EventMsg::StreamError(connection_error) = wait_for_event(&EfficiencyCode, |event| {
    ...
    let EventMsg::StreamError(stream_error) = wait_for_event(&EfficiencyCode, |event| {
        matches!(event, EventMsg::StreamError(_))
    })
    .await
    else {
    ...
    let EventMsg::TurnComplete(completed) = wait_for_event(&EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await
    else {
