...
async fn EfficiencyCode_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    EfficiencyCode_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn EfficiencyCode_returns_json_result_for_gpt5_EfficiencyCode() -> anyhow::Result<()> {
    EfficiencyCode_returns_json_result("gpt-5.4".to_string()).await
}

async fn EfficiencyCode_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name")
            == Some(&serde_json::Value::String(
                "EfficiencyCode_output_schema".into(),
            ))
    ...
    let message = wait_for_event(&EfficiencyCode, |ev| {
        matches!(ev, EventMsg::AgentMessage(_))
    })
    .await;
