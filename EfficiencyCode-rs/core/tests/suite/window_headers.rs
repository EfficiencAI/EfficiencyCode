...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.EfficiencyCode);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.EfficiencyCode, "after resume").await?;
    shutdown_thread(&resumed.EfficiencyCode).await?;
...
async fn submit_user_turn(EfficiencyCode: &Arc<EfficiencyCodeThread>, text: &str) -> Result<()> {
    EfficiencyCode
        .submit(Op::UserInput {
...
async fn submit_compact_turn(EfficiencyCode: &Arc<EfficiencyCodeThread>) -> Result<()> {
    EfficiencyCode.submit(Op::Compact).await?;
    let warning_event = wait_for_event(EfficiencyCode, |event| {
...
async fn shutdown_thread(EfficiencyCode: &Arc<EfficiencyCodeThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-EfficiencyCode-window-id header");
