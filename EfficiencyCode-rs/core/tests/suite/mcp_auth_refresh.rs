...
use EfficiencyCode_mcp::EfficiencyCodeAppsToolsCache;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let expected_auth = EfficiencyCodeAuth::from_external_chatgpt_tokens(
        "header.e30.first",
        "test-account",
    ...
    let mut hosted_plugin_runtime_config =
        EfficiencyCode_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let runtime = McpRuntime::new(McpRuntimeInput {
        ...
        EfficiencyCode_apps_tools_cache: EfficiencyCodeAppsToolsCache::default(),
        tool_catalog_cache: McpToolCatalogCache::default(),
        EfficiencyCode_apps_tools_cache_key:
            EfficiencyCode_mcp::EfficiencyCode_apps_tools_cache_key(Some(&expected_auth)),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: Some(expected_auth.clone()),
        EfficiencyCode_apps_auth_manager: Some(Arc::clone(&auth_manager)),
    ...
    auth_manager
        .set_external_auth(Arc::new(StaticExternalAuth(
            EfficiencyCodeAuth::from_external_chatgpt_tokens(
                "header.e30.reloaded",
                "test-account",
                /*chatgpt_plan_type*/ None,
            )?,
    ...
    // so this tool call only sees the new token if the EfficiencyCode Apps provider
    ...
    let tool_call_request = requests
        ...
        .expect("EfficiencyCode Apps should receive a tool call");
    assert_eq!(
        tool_call_request
            .headers
            .get("authorization")
