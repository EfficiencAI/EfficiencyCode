...
use EfficiencyCode_core::config::TokenBudgetConfig;
...
use EfficiencyCode_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
use EfficiencyCode_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_EfficiencyCode()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.EfficiencyCode, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction() -> Result<()> {
    ...
    assert_context_compaction_item_lifecycle(&test.EfficiencyCode).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.EfficiencyCode, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.EfficiencyCode, |event| match event {
        EventMsg::HookCompleted(completed)
