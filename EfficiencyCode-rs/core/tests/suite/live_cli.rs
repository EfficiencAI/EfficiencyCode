...
//! `just test -p EfficiencyCode-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let EfficiencyCode_home = home.path().join(".EfficiencyCode");
    ...
    let mut cmd =
        Command::new(EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn EfficiencyCode-rs");
