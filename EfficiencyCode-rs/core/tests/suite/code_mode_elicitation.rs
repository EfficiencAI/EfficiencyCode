...
await tools.exec_command({
  ...
  justification: "test command approval",
    ...
    .await?;
    let approval = wait_for_event_match(&harness.test.EfficiencyCode, |event| match event {
...
await tools.request_permissions({
  ...
  permissions: { network: { enabled: true } },
    ...
    .await?;
    let request = wait_for_event(&harness.test.EfficiencyCode, |event| {
        matches!(
            event,
