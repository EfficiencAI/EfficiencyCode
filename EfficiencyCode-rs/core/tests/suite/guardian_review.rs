...
use EfficiencyCode_core::SleepFuture;
use EfficiencyCode_core::TimeFuture;
...
#[test_case(EfficiencyCodeAuth::from_api_key("test-api-key"), "gpt-5.6-luna"; "api_key_uses_luna_with_responses_lite")]
#[test_case(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing(), "EfficiencyCode-auto-review"; "chatgpt_uses_EfficiencyCode_auto_review")]
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let bundled_models = EfficiencyCode_models_manager::bundled_models_response()?.models;
    let catalog_auto_review = bundled_models
        .iter()
        .find(|model| model.slug == "EfficiencyCode-auto-review")
    ...
    let mut builder = test_EfficiencyCode()
        ...
        .with_config(move |config| {
            let rules_dir = config.EfficiencyCode_home.join("rules");
...
async fn cyber_model_guardian_denial_interrupts_turn_immediately() -> Result<()> {
    ...
    let aborted = wait_for_event(&test.EfficiencyCode, |event| {
        matches!(event, EventMsg::TurnAborted(_))
    })
    .await;
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the EfficiencyCode agent history whose request action you are assessing."
        ),
