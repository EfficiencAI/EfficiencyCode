...
fn skill_script_command(test: &TestEfficiencyCode, script_name: &str) -> Result<String> {
...
async fn wait_for_turn_complete(test: &TestEfficiencyCode) {
    wait_for_event(test.EfficiencyCode.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/EfficiencyCode-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();
