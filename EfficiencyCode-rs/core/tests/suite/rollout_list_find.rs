...
use EfficiencyCode_core::RolloutRecorder;
use EfficiencyCode_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(
    ...
    let sessions = EfficiencyCode_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(EfficiencyCode_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(EfficiencyCode_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = EfficiencyCode_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_EfficiencyCode_home_directory() {
    let repo = TempDir::new().unwrap();
    let EfficiencyCode_home = repo.path().join(".EfficiencyCode");
    std::fs::create_dir_all(&EfficiencyCode_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".EfficiencyCode/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&EfficiencyCode_home, id);
