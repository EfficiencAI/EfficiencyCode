...
use EfficiencyCode_config::types::McpServerAuth;
...
use EfficiencyCode_exec_server::CreateDirectoryOptions;
use EfficiencyCode_exec_server::Environment;
use EfficiencyCode_exec_server::HttpRedirectPolicy;
use EfficiencyCode_exec_server::HttpRequestParams;
use EfficiencyCode_features::Feature;
use EfficiencyCode_http_client::HttpClientBuilder;
use EfficiencyCode_login::EfficiencyCodeAuth;
use EfficiencyCode_mcp::EfficiencyCode_APPS_MCP_SERVER_NAME;
use EfficiencyCode_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use EfficiencyCode_protocol::openai_models::ConfigShellToolType;
...
use EfficiencyCode_protocol::openai_models::TruncationPolicyConfig;
...
use EfficiencyCode_utils_cargo_bin::cargo_bin;
...
use core_test_support::test_EfficiencyCode::turn_permission_fields;
...
fn auto_approved_user_turn(fixture: &TestEfficiencyCode, text: impl Into<String>) -> Op {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        EfficiencyCode_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/EfficiencyCode-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.EfficiencyCode, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.EfficiencyCode, |ev| {
...
async fn assert_openai_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.EfficiencyCode, server_name).await?;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_EfficiencyCode()
        ...
        .await?;
    wait_for_mcp_server(&fixture.EfficiencyCode, "bounded").await?;

    fixture
        .EfficiencyCode
        .submit(read_only_user_turn(&fixture, "show the bounded MCP tools"))
        .await?;
    wait_for_event(&fixture.EfficiencyCode, |event| {
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.EfficiencyCode, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_EfficiencyCode()
        .with_pre_build_hook(move |EfficiencyCode_home| {
            fs::write(
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.EfficiencyCode, "history").await?;
...
-> anyhow::Result<()> {
    ...
    let startup = loop {
        let event = fixture.EfficiencyCode.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let event = fixture
                ...
                .next_event()
                .await
                .context("event stream ended before EfficiencyCode Apps became ready")?;
            if let EventMsg::McpStartupUpdate(update) = event.msg
                && update.server == EfficiencyCode_APPS_MCP_SERVER_NAME
    ...
    .await
    .context("EfficiencyCode Apps should finish starting before the first turn")??;
    ...
    tokio::time::timeout(
        ...
        fixture.EfficiencyCode.shutdown_and_wait(),
    )
    .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.EfficiencyCode.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.EfficiencyCode, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            EfficiencyCode_linux_sandbox_exe: fixture
                .config
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.EfficiencyCode, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: EfficiencyCode_protocol::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "EfficiencyCode_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that EfficiencyCode should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: EfficiencyCode can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so EfficiencyCode will call the MCP echo tool
    ...
    // Phase 3: configure EfficiencyCode with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert EfficiencyCode begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.EfficiencyCode, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_EfficiencyCode()
        .with_auth(EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.EfficiencyCode, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    let untrusted_mcp_url = format!(
        "{}/api/EfficiencyCode/ps/mcp",
        untrusted_apps.chatgpt_base_url
    );
    ...
    wait_for_mcp_server(&fixture.EfficiencyCode, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/EfficiencyCode/ps/mcp")
        .filter_map(|request| {
...
/// This test writes to a fallback credentials file in EfficiencyCode_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire EfficiencyCode_HOME through the code.
#[test]
#[serial(EfficiencyCode_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so EfficiencyCode will call the OAuth-backed
    ...
    // Phase 3: seed an isolated EfficiencyCode_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _EfficiencyCode_home_guard =
        EnvVarGuard::set("EfficiencyCode_HOME", temp_home.path().as_os_str());
    let unset_authorization_env_var = format!(
        "EfficiencyCode_TEST_UNSET_MCP_OAUTH_AUTHORIZATION_{}",
        std::process::id()
    );
    ...
    // Phase 4: configure EfficiencyCode with the OAuth-backed Streamable HTTP MCP
    ...
    let startup = wait_for_event(&fixture.EfficiencyCode, |event| {
    ...
    let (contended_turn, refreshed_failure_reason, refreshed_failed_servers) =
        tokio::time::timeout(Duration::from_secs(5), async {
            ...
            {
                match fixture.EfficiencyCode.next_event().await?.msg {
    ...
    let discovered_turn = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        fixture
            ...
            .await?;
        loop {
            if let EventMsg::TurnComplete(turn) = fixture.EfficiencyCode.next_event().await?.msg {
    ...
    tokio::time::timeout(
        ...
        wait_for_mcp_server(&fixture.EfficiencyCode, discovered_server_name),
    )
    .await
    .context("the newly discovered OAuth server did not recover after its store was unlocked")??;

    assert!(EfficiencyCode_rmcp_client::delete_oauth_tokens(
        discovered_credential_name.as_ref(),
        http_server.url(),
        OAuthCredentialsStoreMode::File,
        EfficiencyCode_config::types::AuthKeyringBackendKind::default(),
    )?);
    fixture
        .EfficiencyCode
    ...
    // Phase 7: assert EfficiencyCode begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.EfficiencyCode, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the EfficiencyCode fixture.
...
fn write_fallback_oauth_tokens(
    ...
    EfficiencyCode_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        EfficiencyCode_config::types::AuthKeyringBackendKind::default(),
