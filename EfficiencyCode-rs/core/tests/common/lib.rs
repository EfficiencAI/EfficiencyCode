...
use EfficiencyCode_utils_cargo_bin::CargoBinError;
...
pub use EfficiencyCode_core::test_support::TestEfficiencyCodeResponsesRequestKind;
pub use EfficiencyCode_core::test_support::responses_metadata;
use EfficiencyCode_utils_absolute_path::AbsolutePathBuf;
pub use EfficiencyCode_utils_absolute_path::test_support::PathBufExt;
pub use EfficiencyCode_utils_absolute_path::test_support::PathExt;
...
pub mod test_EfficiencyCode;
pub mod test_EfficiencyCode_exec;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    EfficiencyCode_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    EfficiencyCode_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(EfficiencyCode_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = EfficiencyCode_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("EfficiencyCode-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\EfficiencyCode\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.EfficiencyCode`.
pub async fn load_default_config_for_test(EfficiencyCode_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        EfficiencyCode_linux_sandbox_exe: Some(
            find_EfficiencyCode_linux_sandbox_exe()
                .expect("should find binary for EfficiencyCode-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_EfficiencyCode_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-linux-sandbox")
}

pub async fn wait_for_event<F>(
    EfficiencyCode: &EfficiencyCodeThread,
    predicate: F,
) -> EfficiencyCode_protocol::protocol::EventMsg
where
    F: FnMut(&EfficiencyCode_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(EfficiencyCode, predicate, Duration::from_secs(1)).await
}
...
pub async fn submit_thread_settings(
    ...
    let submission_id = EfficiencyCode
        ...
        .await?;
    loop {
        let ev = timeout(Duration::from_secs(10), EfficiencyCode.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(EfficiencyCode: &EfficiencyCodeThread, matcher: F) -> T
where
    F: Fn(&EfficiencyCode_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(EfficiencyCode, |ev| matcher(ev).is_some()).await;
...
pub fn sandbox_env_var() -> &'static str {
    EfficiencyCode_core::spawn::CODEX_SANDBOX_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    EfficiencyCode_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    EfficiencyCode_utils_cargo_bin::cargo_bin("test_stdio_server")
...
#[macro_export]
macro_rules! EfficiencyCode_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_EfficiencyCode_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!(
                        "EfficiencyCode-linux-sandbox binary not available, skipping test: {err}"
                    );
                    return;
                }
            }
