...
use EfficiencyCode_core::TimeProvider;
use EfficiencyCode_core::config::Config;
use EfficiencyCode_core::resolve_installation_id;
use EfficiencyCode_core::shell::Shell;
use EfficiencyCode_core::shell::get_shell_by_model_provided_path;
...
use EfficiencyCode_exec_server::RemoveOptions;
...
use EfficiencyCode_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-EfficiencyCode";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: EfficiencyCode_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: EfficiencyCode_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            EfficiencyCode_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &EfficiencyCode_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment = EfficiencyCode_exec_server::Environment::create_for_tests(Some(
                websocket_url.clone(),
            ))?;
            let cwd = remote_env
...
pub struct TestEfficiencyCodeBuilder {
...
impl TestEfficiencyCodeBuilder {
    ...
    pub fn with_auth(mut self, auth: EfficiencyCodeAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    ) -> anyhow::Result<TestEfficiencyCode> {
    ...
    ///
    /// With no remote test configuration, or with `EfficiencyCode_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `EfficiencyCode_TEST_ENVIRONMENT=docker` or
    /// `EfficiencyCode_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `EfficiencyCode_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `EfficiencyCode_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        &mut self,
        server: &MockServer,
        previous: &TestEfficiencyCode,
    ) -> Result<TestEfficiencyCode> {
        let rollout_path = previous
            .session_configured
            .rollout_path
            .clone()
            .context("rollout path")?;
        previous.EfficiencyCode.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let EfficiencyCode_linux_sandbox_exe = Some(
            crate::find_EfficiencyCode_linux_sandbox_exe()
                .context("should find binary for EfficiencyCode-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let EfficiencyCode_linux_sandbox_exe = None;
        let local_runtime_paths = EfficiencyCode_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            EfficiencyCode_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            EfficiencyCode_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            EfficiencyCode_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<EfficiencyCode_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestEfficiencyCode> {
        let auth = self.auth.clone();
        let state_db = EfficiencyCode_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.EfficiencyCode_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(EfficiencyCodeHomeUserInstructionsProvider::new(
                    config.EfficiencyCode_home.clone(),
                ))
            });
        let auth_manager = EfficiencyCode_core::test_support::auth_manager_from_auth(auth.clone());
        let models_manager = self.models_manager.clone().unwrap_or_else(|| {
            EfficiencyCode_core::build_models_manager(&config, auth_manager.clone())
        });
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            EfficiencyCode_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
            /*attestation_provider*/ None,
            /*external_time_provider*/ self.external_time_provider.clone(),
        );
        let code_mode_host_program = self.code_mode_host_program.take().or_else(|| {
            EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-code-mode-host").ok()
        });
        let thread_manager = if config.features.enabled(Feature::CodeModeHost)
            && let Some(code_mode_host_program) = code_mode_host_program
        {
            EfficiencyCode_core::test_support::with_code_mode_host_program(
                thread_manager,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    EfficiencyCode_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    EfficiencyCode_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
        ...
        Ok(TestEfficiencyCode {
            home,
            cwd,
            config,
            EfficiencyCode: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode") {
            config.EfficiencyCode_self_exe = Some(path);
        } else if let Ok(path) = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-exec") {
            // `EfficiencyCode-exec` also supports `--EfficiencyCode-run-as-apply-patch`, so use it
        ...
        {
            let EfficiencyCode = bin_dir.join("EfficiencyCode");
            let EfficiencyCode_exec = bin_dir.join("EfficiencyCode-exec");
            if EfficiencyCode.is_file() {
                config.EfficiencyCode_self_exe = Some(EfficiencyCode);
            } else if EfficiencyCode_exec.is_file() {
                config.EfficiencyCode_self_exe = Some(EfficiencyCode_exec);
            }
        }
...
pub struct TestEfficiencyCode {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub EfficiencyCode: Arc<EfficiencyCodeThread>,
...
impl TestEfficiencyCode {
    pub fn cwd_path(&self) -> &Path {
        self.cwd.path()
    }

    pub fn EfficiencyCode_home_path(&self) -> &Path {
        self.config.EfficiencyCode_home.as_path()
    }
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.EfficiencyCode, |ev| {
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.EfficiencyCode, |event| match event {
...
pub struct TestEfficiencyCodeHarness {
    server: MockServer,
    test: TestEfficiencyCode,
}

impl TestEfficiencyCodeHarness {
    pub async fn with_builder(mut builder: TestEfficiencyCodeBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestEfficiencyCodeBuilder) -> Result<Self> {
        let server = start_mock_server().await;
    ...
    pub fn test(&self) -> &TestEfficiencyCode {
        &self.test
    }
...
pub fn test_EfficiencyCode() -> TestEfficiencyCodeBuilder {
    TestEfficiencyCodeBuilder {
        ...
        auth: EfficiencyCodeAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
