...
use crate::test_EfficiencyCode::TestEfficiencyCode;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) =
        EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `EfficiencyCode-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_EfficiencyCode().with_config(move |config| {
