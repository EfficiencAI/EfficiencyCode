...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let EfficiencyCode_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-EfficiencyCode-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-EfficiencyCode-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-EfficiencyCode-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-EfficiencyCode-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-EfficiencyCode-sandbox"), None);
}
...
async fn responses_respects_model_info_overrides_from_config() {
    ...
    let thread_id = ThreadId::new();
    let auth_mode = EfficiencyCode_core::test_support::auth_manager_from_auth(
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = initial_request
        ...
        .expect("x-EfficiencyCode-turn-metadata header should be present");
    let initial_parsed: serde_json::Value = serde_json::from_str(&initial_header)
        .expect("x-EfficiencyCode-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-EfficiencyCode-turn-metadata"
    );
    ...
    let body_metadata: serde_json::Value = serde_json::from_str(
        initial_request.body_json()["client_metadata"]["x-EfficiencyCode-turn-metadata"]
            .as_str()
            .expect("request body should include x-EfficiencyCode-turn-metadata"),
    )
    .expect("body x-EfficiencyCode-turn-metadata should be valid JSON");
    assert_eq!(
        body_metadata
            .get("sandbox_mode")
            .and_then(serde_json::Value::as_str),
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/EfficiencyCode.git",
    ]);
