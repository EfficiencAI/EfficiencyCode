# EfficiencyCode-network-proxy

`EfficiencyCode-network-proxy` is EfficiencyCode's local network policy enforcement proxy. It runs:
...
`EfficiencyCode-network-proxy` reads from EfficiencyCode's merged `config.toml` (via `EfficiencyCode-core` config loading).
...
# bundle env vars pointing at immutable public files under $EfficiencyCode_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p EfficiencyCode-network-proxy --
```
...
`EfficiencyCode-network-proxy` can be embedded as a library with a thin API:

```rust
use EfficiencyCode_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `EfficiencyCode-network-proxy` is embedded in managed EfficiencyCode runtime, policy decisions emit structured
OTEL-compatible events with `target=EfficiencyCode_otel.network_proxy`.

Event name:

- `EfficiencyCode.network_proxy.policy_decision`
...
This section documents the protections implemented by `EfficiencyCode-network-proxy`, and the boundaries of
