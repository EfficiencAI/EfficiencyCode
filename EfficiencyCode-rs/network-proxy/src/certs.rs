...
fn managed_ca_dir() -> Result<PathBuf> {
    let EfficiencyCode_home = find_EfficiencyCode_home()
        .context("failed to resolve EfficiencyCode_HOME for managed MITM CA")?;
    Ok(EfficiencyCode_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current EfficiencyCode-generated MITM CA bundle.
