...
use crate::protocol::EfficiencyCodeErrorInfo;
...
use EfficiencyCode_http_client::HttpError;
use EfficiencyCode_utils_string::truncate_middle_chars;
use EfficiencyCode_utils_string::truncate_middle_with_token_budget;
use chrono::DateTime;
use chrono::Datelike;
use chrono::Local;
use chrono::Utc;
...
pub type Result<T> = std::result::Result<T, EfficiencyCodeErr>;
...
pub struct EfficiencyCodeErr {
    details: EfficiencyCodeErrorDetails,
    retry_delay: Option<Duration>,
}

/// The semantic category and diagnostic payload for a [`EfficiencyCodeErr`].
#[derive(Error, Debug, EnumDiscriminants)]
#[strum_discriminants(name(EfficiencyCodeErrKind))]
...
pub enum EfficiencyCodeErrorDetails {
    ...
    Stream(String),
    #[error(
        "EfficiencyCode ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `EfficiencyCodeError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use EfficiencyCode with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("EfficiencyCode-linux-sandbox was required but not provided")]
...
impl From<&EfficiencyCodeErr> for EfficiencyCodeErrKind {
    fn from(error: &EfficiencyCodeErr) -> Self {
        error.details().into()
    }
}

impl fmt::Debug for EfficiencyCodeErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.details {
            EfficiencyCodeErrorDetails::Stream(message) => formatter
                .debug_tuple("Stream")
                .field(message)
...
impl fmt::Display for EfficiencyCodeErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.details, formatter)
    }
}

impl std::error::Error for EfficiencyCodeErr {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        self.details.source()
    }
}

impl From<EfficiencyCodeErrorDetails> for EfficiencyCodeErr {
    fn from(details: EfficiencyCodeErrorDetails) -> Self {
        Self {
            details,
            retry_delay: None,
        }
...
impl From<CancelErr> for EfficiencyCodeErr {
    fn from(error: CancelErr) -> Self {
        EfficiencyCodeErrorDetails::from(error).into()
    }
}

impl From<SandboxErr> for EfficiencyCodeErr {
    fn from(error: SandboxErr) -> Self {
        EfficiencyCodeErrorDetails::from(error).into()
    }
}

impl From<io::Error> for EfficiencyCodeErr {
    fn from(error: io::Error) -> Self {
        EfficiencyCodeErrorDetails::from(error).into()
    }
}

impl From<serde_json::Error> for EfficiencyCodeErr {
    fn from(error: serde_json::Error) -> Self {
        EfficiencyCodeErrorDetails::from(error).into()
    }
}

impl From<JoinError> for EfficiencyCodeErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::RulesetError> for EfficiencyCodeErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::PathFdError> for EfficiencyCodeErr {
    fn from(error: landlock::PathFdError) -> Self {
        EfficiencyCodeErrorDetails::from(error).into()
    }
}

impl From<CancelErr> for EfficiencyCodeErrorDetails {
    fn from(_: CancelErr) -> Self {
        EfficiencyCodeErrorDetails::TurnAborted
    }
}

// TODO(anp): Remove this compatibility macro once callers construct
// `EfficiencyCodeErrorDetails` directly.
macro_rules! EfficiencyCode_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
            #[allow(non_upper_case_globals)]
            pub const $variant: Self = Self {
                details: EfficiencyCodeErrorDetails::$variant,
                retry_delay: None,
            };
        )*
    };
...
// TODO(anp): Remove this compatibility macro once callers construct
// `EfficiencyCodeErrorDetails` directly.
macro_rules! EfficiencyCode_err_tuple_constructors {
    ($($(#[$attr:meta])* $variant:ident($value:ident: $value_type:ty)),* $(,)?) => {
        $(
            ...
            pub fn $variant($value: $value_type) -> Self {
                EfficiencyCodeErrorDetails::$variant($value).into()
            }
        )*
    };
}

impl EfficiencyCodeErr {
    EfficiencyCode_err_unit_constructors!(
        TurnAborted,
        SessionBudgetExceeded,
        ContextWindowExceeded,
        SessionConfiguredNotFirstEvent,
    ...
    EfficiencyCode_err_tuple_constructors!(
    ...
    pub fn InvalidImageRequest() -> Self {
        EfficiencyCodeErrorDetails::InvalidImageRequest().into()
    }

    /// Creates an error with no server-provided retry delay.
    pub fn new(details: EfficiencyCodeErrorDetails) -> Self {
        details.into()
    }

    /// Returns the semantic failure and its diagnostic payload.
    pub fn details(&self) -> &EfficiencyCodeErrorDetails {
        &self.details
    }
    ...
    pub fn is_retryable(&self) -> bool {
        match self.details() {
            EfficiencyCodeErrorDetails::TurnAborted
            | EfficiencyCodeErrorDetails::SessionBudgetExceeded
            | EfficiencyCodeErrorDetails::Interrupted
            | EfficiencyCodeErrorDetails::EnvVar(_)
            | EfficiencyCodeErrorDetails::Fatal(_)
            | EfficiencyCodeErrorDetails::UsageNotIncluded
            | EfficiencyCodeErrorDetails::QuotaExceeded
            | EfficiencyCodeErrorDetails::InvalidImageRequest()
            | EfficiencyCodeErrorDetails::InvalidRequest(_)
            | EfficiencyCodeErrorDetails::ToolCollision(_)
            | EfficiencyCodeErrorDetails::RefreshTokenFailed(_)
            | EfficiencyCodeErrorDetails::UnsupportedOperation(_)
            | EfficiencyCodeErrorDetails::Sandbox(_)
            | EfficiencyCodeErrorDetails::LandlockSandboxExecutableNotProvided
            | EfficiencyCodeErrorDetails::RetryLimit(_)
            | EfficiencyCodeErrorDetails::ContextWindowExceeded
            | EfficiencyCodeErrorDetails::ThreadNotFound(_)
            | EfficiencyCodeErrorDetails::AgentLimitReached { .. }
            | EfficiencyCodeErrorDetails::Spawn
            | EfficiencyCodeErrorDetails::SessionConfiguredNotFirstEvent
            | EfficiencyCodeErrorDetails::UsageLimitReached(_)
            | EfficiencyCodeErrorDetails::ServerOverloaded
            | EfficiencyCodeErrorDetails::CyberPolicy { .. } => false,
            EfficiencyCodeErrorDetails::Stream(..)
            | EfficiencyCodeErrorDetails::Timeout
            | EfficiencyCodeErrorDetails::RequestTimeout
            | EfficiencyCodeErrorDetails::UnexpectedStatus(_)
            | EfficiencyCodeErrorDetails::ResponseStreamFailed(_)
            | EfficiencyCodeErrorDetails::ConnectionFailed(_)
            | EfficiencyCodeErrorDetails::InternalServerError
            | EfficiencyCodeErrorDetails::InternalAgentDied
            | EfficiencyCodeErrorDetails::Io(_)
            | EfficiencyCodeErrorDetails::Json(_)
            | EfficiencyCodeErrorDetails::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            EfficiencyCodeErrorDetails::LandlockRuleset(_)
            | EfficiencyCodeErrorDetails::LandlockPathFd(_) => false,
        }
    }
    ...
    /// Minimal shim so that existing `e.downcast_ref::<EfficiencyCodeErr>()` checks continue to compile
    ...
    pub fn to_EfficiencyCode_protocol_error(&self) -> EfficiencyCodeErrorInfo {
        match &self.details {
            EfficiencyCodeErrorDetails::ContextWindowExceeded => {
                EfficiencyCodeErrorInfo::ContextWindowExceeded
            }
            EfficiencyCodeErrorDetails::SessionBudgetExceeded => {
                EfficiencyCodeErrorInfo::SessionBudgetExceeded
            }
            EfficiencyCodeErrorDetails::UsageLimitReached(_)
            | EfficiencyCodeErrorDetails::QuotaExceeded
            | EfficiencyCodeErrorDetails::UsageNotIncluded => {
                EfficiencyCodeErrorInfo::UsageLimitExceeded
            }
            EfficiencyCodeErrorDetails::ServerOverloaded => {
                EfficiencyCodeErrorInfo::ServerOverloaded
            }
            EfficiencyCodeErrorDetails::CyberPolicy { .. } => EfficiencyCodeErrorInfo::CyberPolicy,
            EfficiencyCodeErrorDetails::RetryLimit(_) => {
                EfficiencyCodeErrorInfo::ResponseTooManyFailedAttempts {
                    http_status_code: self.http_status_code_value(),
                }
            }
            EfficiencyCodeErrorDetails::ConnectionFailed(_) => {
                EfficiencyCodeErrorInfo::HttpConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            EfficiencyCodeErrorDetails::ResponseStreamFailed(_) => {
                EfficiencyCodeErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            EfficiencyCodeErrorDetails::RefreshTokenFailed(_) => {
                EfficiencyCodeErrorInfo::Unauthorized
            }
            EfficiencyCodeErrorDetails::SessionConfiguredNotFirstEvent
            | EfficiencyCodeErrorDetails::InternalServerError
            | EfficiencyCodeErrorDetails::InternalAgentDied => {
                EfficiencyCodeErrorInfo::InternalServerError
            }
            EfficiencyCodeErrorDetails::UnsupportedOperation(_)
            | EfficiencyCodeErrorDetails::ThreadNotFound(_)
            | EfficiencyCodeErrorDetails::AgentLimitReached { .. } => {
                EfficiencyCodeErrorInfo::BadRequest
            }
            EfficiencyCodeErrorDetails::Sandbox(_) => EfficiencyCodeErrorInfo::SandboxError,
            _ => EfficiencyCodeErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            EfficiencyCode_error_info: Some(self.to_EfficiencyCode_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match &self.details {
            EfficiencyCodeErrorDetails::RetryLimit(err) => Some(err.status),
            EfficiencyCodeErrorDetails::UnexpectedStatus(err) => Some(err.status),
            EfficiencyCodeErrorDetails::ConnectionFailed(err) => err.source.status(),
            EfficiencyCodeErrorDetails::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("EfficiencyCode")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/EfficiencyCode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using EfficiencyCode (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/EfficiencyCode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &EfficiencyCodeErr) -> String {
    let message = match e.details() {
        EfficiencyCodeErrorDetails::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        EfficiencyCodeErrorDetails::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
