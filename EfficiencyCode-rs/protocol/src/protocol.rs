//! Defines the protocol for a EfficiencyCode session between a client and an agent.
//!
...
use EfficiencyCode_extension_items::image_generation::ImageGenerationFailure;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for EfficiencyCode:";
...
pub struct ConversationStartParams {
    /// Whether EfficiencyCode response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through EfficiencyCode when the session ends.
    ...
    /// Sends automatic EfficiencyCode responses as realtime conversation items instead of handoff appends.
    pub EfficiencyCode_responses_as_items: bool,
    /// Optional prefix added to automatic EfficiencyCode response items when `EfficiencyCode_responses_as_items` is set.
    pub EfficiencyCode_response_item_prefix: Option<String>,
    /// Selects how automatic EfficiencyCode handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub EfficiencyCode_response_handoff_mode: EfficiencyCodeResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub EfficiencyCode_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append EfficiencyCode's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to EfficiencyCode when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to EfficiencyCode when this realtime session ends.
...
pub enum Op {
    ...
    /// Request EfficiencyCode to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down EfficiencyCode instance.
    Shutdown,
...
/// running the command proposed by EfficiencyCode.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.EfficiencyCode`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum EfficiencyCodeResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// EfficiencyCode errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum EfficiencyCodeErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub EfficiencyCode_error_info: Option<EfficiencyCodeErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.EfficiencyCode_error_info
            .as_ref()
            .is_none_or(EfficiencyCodeErrorInfo::affects_turn_status)
    }
}
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub EfficiencyCode_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::EfficiencyCode),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older EfficiencyCode
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => usize::try_from(
                EfficiencyCode_utils_string::approx_tokens_from_byte_count(*bytes),
            )
            .unwrap_or(usize::MAX),
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                EfficiencyCode_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
impl Product {
    pub fn to_app_platform(self) -> &'static str {
        match self {
            Self::Chatgpt => "chat",
            Self::EfficiencyCode => "EfficiencyCode",
            Self::Atlas => "atlas",
        }
    }
    ...
    pub fn from_session_source_name(value: &str) -> Option<Self> {
        ...
        match normalized.as_str() {
            ...
            "EfficiencyCode" => Some(Self::EfficiencyCode),
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_EfficiencyCode() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("EfficiencyCode".to_string()).restriction_product(),
            Some(Product::EfficiencyCode)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::EfficiencyCode])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::EfficiencyCode]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            EfficiencyCode_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".EfficiencyCode"))
            .expect("create .EfficiencyCode");
        let canonical_cwd =
            EfficiencyCode_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_EfficiencyCode =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".EfficiencyCode"))
                .expect("canonical .EfficiencyCode");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_EfficiencyCode.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_EfficiencyCode =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".EfficiencyCode"))
                .expect("canonical .EfficiencyCode");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_EfficiencyCode.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            EfficiencyCode_error_info: Some(EfficiencyCodeErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
    ...
    fn serialize_event() -> Result<()> {
        ...
        let expected = json!({
            ...
            "msg": {
                ...
                "model": "EfficiencyCode-mini-latest",
