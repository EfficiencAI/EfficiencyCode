...
use EfficiencyCode_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_EfficiencyCode_PATH_NAME: &str = ".EfficiencyCode";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_EfficiencyCode_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer EfficiencyCode degrades to warn-and-ignore
    /// instead of failing to load. EfficiencyCode 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "EfficiencyCode-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_EfficiencyCode*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_EfficiencyCode*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_EfficiencyCode*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_EfficiencyCode = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
            ...
            // Example: if `<root>/.EfficiencyCode -> <root>/decoy`, bwrap must still see
            // `<root>/.EfficiencyCode`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.EfficiencyCode` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_EfficiencyCode: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .EfficiencyCode read-only to the agent by
    ...
    let top_level_EfficiencyCode = writable_root.join(PROTECTED_METADATA_EfficiencyCode_PATH_NAME);
    if protect_missing_dot_EfficiencyCode || top_level_EfficiencyCode.as_path().is_dir() {
        subpaths.push(top_level_EfficiencyCode);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "EfficiencyCode_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_EfficiencyCode() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_EfficiencyCode = expected_root.join(".EfficiencyCode");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_EfficiencyCode)
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".EfficiencyCode".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_EfficiencyCode_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_EfficiencyCode = expected_root.join(".EfficiencyCode");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_EfficiencyCode.clone(),
                },
                access: FileSystemAccessMode::Write,
                missing_path_behavior: None,
            },
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".EfficiencyCode".to_string()),
            "explicit .EfficiencyCode rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_EfficiencyCode),
            "explicit .EfficiencyCode rule should win over the default protected carveout"
        );
        assert!(policy.can_write_path_with_cwd(
            explicit_dot_EfficiencyCode.join("config.toml").as_path(),
            cwd.path()
        ));
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_EfficiencyCode_config = cwd.path().join(".EfficiencyCode").join("config.toml");
        ...
        assert!(
            !file_system_policy.can_write_path_with_cwd(&dot_EfficiencyCode_config, cwd.path())
        );
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_EfficiencyCode_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_EfficiencyCode*/ true,
            )
            .into_iter()
            .map(|path| {
                FileSystemSandboxEntry::skip_missing_path(
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".EfficiencyCode/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let EfficiencyCode_dir = real_root.join(".EfficiencyCode");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&EfficiencyCode_dir).expect("create .EfficiencyCode");
        ...
        let expected_EfficiencyCode = link_root.join(".EfficiencyCode");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_EfficiencyCode)
        );
    }
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_EfficiencyCode = expected_root.join(".EfficiencyCode");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-EfficiencyCode");
        let dot_EfficiencyCode = root.join(".EfficiencyCode");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_EfficiencyCode).expect("create .EfficiencyCode symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_EfficiencyCode = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".EfficiencyCode"),
        )
        .expect("absolute .EfficiencyCode symlink");
        ...
        assert_eq!(
            ...
            vec![expected_dot_EfficiencyCode]
        );
        assert!(
            !writable_roots[0]
                .read_only_subpaths
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let EfficiencyCode_dir = real_tmpdir.join(".EfficiencyCode");
