...
//! EfficiencyCode protocol.
//!
//! We intentionally keep these types TS/JSON-schema friendly (via `ts-rs` and
//! `schemars`) so they can be embedded in EfficiencyCode's own protocol structures.
...
/// MCP extensions supplied by the client that created a EfficiencyCode session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `EfficiencyCode-protocol` so other crates can convert
