...
#[ignore = "requires tmux and a locally built EfficiencyCode binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let EfficiencyCode = EfficiencyCode_binary(&repo_root)?;
    ...
    write_config(EfficiencyCode_home.path(), &repo_root)?;
    write_auth(EfficiencyCode_home.path())?;

    let session_name = format!("EfficiencyCode-resize-reflow-smoke-{}", std::process::id());
    ...
    let EfficiencyCode_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(
        !EfficiencyCode_pane.is_empty(),
        "tmux did not report a pane id"
    );
    ...
    let final_capture = capture_pane(&EfficiencyCode_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("EfficiencyCode-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&EfficiencyCode_pane)?;
    let restored_row =
...
async fn tmux_scrolled_composer_resize_preserves_visible_draft_text() -> Result<()> {
    ...
    let session_name = format!(
        "EfficiencyCode-resize-scrolled-composer-{}",
        std::process::id()
    );
    let _session = TmuxSession {
        name: session_name.clone(),
        ...
        [("narrowed", "28", "9", 2), ("restored", "44", "14", 6)]
    {
        ...
        let capture = capture_pane(&EfficiencyCode_pane)?;
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("EfficiencyCode-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn EfficiencyCode_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode") {
        return Ok(path);
    }

    let fallback = repo_root.join("EfficiencyCode-rs/target/debug/EfficiencyCode");
    anyhow::ensure!(
        fallback.is_file(),
        "EfficiencyCode binary is unavailable; run `cargo build -p EfficiencyCode-cli` first"
    );
    Ok(fallback)
}

fn write_config(EfficiencyCode_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(EfficiencyCode_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(EfficiencyCode_home: &Path) -> Result<()> {
    std::fs::write(
