...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    write_test_config(EfficiencyCode_home.path(), &repo_root)?;

    let mut terminal = PtyEfficiencyCode::start(&repo_root, EfficiencyCode_home)?;
...
impl PtyEfficiencyCode {
    fn start(repo_root: &Path, EfficiencyCode_home: TempDir) -> Result<Self> {
        let mut master_fd = -1;
        let mut slave_fd = -1;
        ...
        let EfficiencyCode = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")
            .or_else(|_| EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-tui"))?;
        let child = Command::new(EfficiencyCode)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(repo_root)
            .arg("-c")
            ...
            .stderr(slave)
            .spawn()
            .context("start EfficiencyCode in focus-test pseudo-terminal")?;
    ...
    fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered && self.screen_contains("OpenAI EfficiencyCode") {
                return Ok(());
            }

            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "EfficiencyCode exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "EfficiencyCode did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
...
impl Drop for PtyEfficiencyCode {
    fn drop(&mut self) {
...
fn write_test_config(EfficiencyCode_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(EfficiencyCode_home.join("config.toml"), config)
        .context("write focus-test EfficiencyCode configuration")?;
    std::fs::write(
