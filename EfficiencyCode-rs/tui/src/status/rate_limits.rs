...
use EfficiencyCode_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use EfficiencyCode_app_server_protocol::RateLimitSnapshot;
use EfficiencyCode_app_server_protocol::RateLimitWindow;
use EfficiencyCode_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use EfficiencyCode_protocol::num_format::format_with_separators;
use chrono::DateTime;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `EfficiencyCode` or `EfficiencyCode_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "EfficiencyCode".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("EfficiencyCode");
        ...
        let combine_non_EfficiencyCode_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_EfficiencyCode_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_EfficiencyCode_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_EfficiencyCode_single_limit_renders_combined_row() {
        let now = Local::now();
        let EfficiencyCode = RateLimitSnapshotDisplay {
            limit_name: "EfficiencyCode".to_string(),
            captured_at: now,
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "EfficiencyCode-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[EfficiencyCode, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "EfficiencyCode-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_EfficiencyCode_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "EfficiencyCode-other".to_string(),
            captured_at: now,
        ...
        assert_eq!(
            labels,
            vec![
                "EfficiencyCode-other limit".to_string(),
