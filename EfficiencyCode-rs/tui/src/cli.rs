use EfficiencyCode_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `EfficiencyCode resume` subcommand.
    // These are not exposed as user flags on the base `EfficiencyCode` command.
    #[clap(skip)]
    ...
    /// top-level `EfficiencyCode resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `EfficiencyCode fork` subcommand.
    // These are not exposed as user flags on the base `EfficiencyCode` command.
    #[clap(skip)]
    ...
    /// top-level `EfficiencyCode fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
