...
//! | `EfficiencyCode_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static EfficiencyCode_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and EfficiencyCode home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `EfficiencyCode_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), EfficiencyCode_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let EfficiencyCode_home_set_ok = EfficiencyCode_HOME.set(EfficiencyCode_home.clone()).is_ok();
    ...
    if !override_set_ok || !EfficiencyCode_home_set_ok {
...
pub(crate) fn validate_theme_name(
    ...
    let name = name?;
    let custom_theme_path_display = EfficiencyCode_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$EfficiencyCode_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = EfficiencyCode_home {
...
fn custom_theme_path(name: &str, EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home
        .join("themes")
        .join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{EfficiencyCode_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, EfficiencyCode_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, EfficiencyCode_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, EfficiencyCode_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {EfficiencyCode_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = EfficiencyCode_home
...
fn build_default_theme() -> Theme {
    ...
    let EfficiencyCode_home = EfficiencyCode_HOME
        .get()
        .and_then(|EfficiencyCode_home| EfficiencyCode_home.as_deref());
    resolve_theme_with_override(name, EfficiencyCode_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = EfficiencyCode_HOME.get()
...
/// `.tmTheme` file under `{EfficiencyCode_HOME}/themes/`.
...
/// found in `{EfficiencyCode_home}/themes/`.
pub(crate) fn list_available_themes(EfficiencyCode_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*EfficiencyCode_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*EfficiencyCode_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*EfficiencyCode_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*EfficiencyCode_home*/ None).is_none());
    }
