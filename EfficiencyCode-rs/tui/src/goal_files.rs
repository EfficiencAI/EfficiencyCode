...
const GOAL_FILE_PREFIX: &str = "Read the EfficiencyCode goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    EfficiencyCode_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, EfficiencyCode_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, EfficiencyCode_home) else {
...
pub(crate) fn objective_file_path(
    ...
    let expected = EfficiencyCode_home?
...
async fn ensure_goal_output_dir(
    ...
    let EfficiencyCode_home = EfficiencyCode_home
        .context("App server did not report $EfficiencyCode_HOME; cannot materialize goal files")?;
