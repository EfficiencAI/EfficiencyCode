...
const DIRECTIVE_PREFIX: &str = "::EfficiencyCode-inline-vis{";
...
impl InlineVisualizationContext {
    pub(crate) fn new(EfficiencyCode_home: &Path, thread_id: ThreadId) -> Option<Self> {
        Self::new_with_writable_roots(EfficiencyCode_home, thread_id, std::iter::empty())
    }
    ...
    fn new_with_writable_roots<'a>(
        ...
        let visualizations_dir = EfficiencyCode_home.join("visualizations");
...
fn link_placeholder() -> String {
    ...
    format!("https://EfficiencyCode.invalid/inline-visualization/{token}")
}
