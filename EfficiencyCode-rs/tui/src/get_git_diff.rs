...
//! `EfficiencyCode-cli`: it returns the diff for tracked changes as well as any
...
use EfficiencyCode_git_utils::FsmonitorOverride;
use EfficiencyCode_git_utils::FsmonitorProbeRunner;
use EfficiencyCode_git_utils::detect_fsmonitor_override;
...
impl FsmonitorProbeRunner for WorkspaceFsmonitorProbeRunner<'_> {
    async fn run_probe(&mut self, args: &[&str]) -> Option<Vec<u8>> {
        let argv = [
            "git",
            "-c",
            EfficiencyCode_git_utils::SAFE_BARE_REPOSITORY_CONFIG,
        ]
        .into_iter()
