...
async fn state_runtime(
    config: &Config,
) -> std::io::Result<Arc<EfficiencyCode_state::StateRuntime>> {
    let runtime = EfficiencyCode_state::StateRuntime::init(
        EfficiencyCode_state::SqliteConfig::new_for_testing(
            config.EfficiencyCode_home.as_path().abs(),
        ),
...
async fn upsert_thread(
    ...
    metadata: EfficiencyCode_state::ThreadMetadata,
) -> std::io::Result<()> {
    runtime
...
fn thread_metadata(
    ...
    title: &str,
) -> EfficiencyCode_state::ThreadMetadata {
    ...
    builder.cwd = config.EfficiencyCode_home.join("project").to_path_buf();
...
fn write_rollout(
    ...
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
            ...
            cwd: config.EfficiencyCode_home.join("project").to_path_buf(),
...
async fn add_scan_sentinel(config: &Config, search_term: &str) -> color_eyre::Result<ThreadId> {
    ...
    EfficiencyCode_rollout::append_thread_name(
...
async fn recovers_legacy_index_name_without_requiring_sidecar_rewrite() -> color_eyre::Result<()> {
    ...
    let index_path = config.EfficiencyCode_home.join("session_index.jsonl");
