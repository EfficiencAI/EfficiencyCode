...
use EfficiencyCode_config::types::MAX_FUNCTION_KEY;
...
Set unique keys in `~/.EfficiencyCode/config.toml` and retry. \
...
Set a different key in `~/.EfficiencyCode/config.toml` and retry. \
See the EfficiencyCode keymap documentation for supported actions and examples."
                ));
            }
        }
