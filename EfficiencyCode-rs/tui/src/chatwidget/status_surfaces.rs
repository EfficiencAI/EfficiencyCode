...
impl ChatWidget {
    ...
    /// Clears the terminal title EfficiencyCode most recently wrote, if any.
    ///
    ...
    fn status_line_workspace_headline_should_fetch(&self, now: Instant) -> bool {
        if self
            ...
            || !self.has_EfficiencyCode_backend_auth
        {
            return false;
        }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::EfficiencyCodeVersion => Some(EfficiencyCode_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("EfficiencyCode".to_string()),
            ...
            StatusSurfacePreviewItem::EfficiencyCodeVersion => {
                StatusLineItem::EfficiencyCodeVersion
            }
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("EfficiencyCode".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::EfficiencyCodeVersion => self
                .status_line_value_for_item(StatusLineItem::EfficiencyCodeVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_EfficiencyCode_window(snapshot, "5h")
...
fn weekly_status_window(
    ...
    find_EfficiencyCode_window(snapshot, "weekly")
        .or_else(|| snapshot.secondary.as_ref().map(|window| (window, true)))
}

fn find_EfficiencyCode_window<'a>(
...
fn find_primary_EfficiencyCode_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {
