...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        EfficiencyCode_error_info: &AppServerEfficiencyCodeErrorInfo,
    ) -> bool {
        matches!(
            EfficiencyCode_error_info,
            AppServerEfficiencyCodeErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "EfficiencyCode is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.EfficiencyCode_rate_limit_reached_type.map(|kind| {
        ...
        self.EfficiencyCode_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        EfficiencyCode_error_info: Option<AppServerEfficiencyCodeErrorInfo>,
    ) {
        if EfficiencyCode_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if EfficiencyCode_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = EfficiencyCode_error_info
            .as_ref()
