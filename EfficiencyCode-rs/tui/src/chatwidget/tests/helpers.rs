...
pub(super) async fn test_config() -> Config {
    ...
    let EfficiencyCode_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config = Config::load_default_with_cli_overrides_for_EfficiencyCode_home(
        EfficiencyCode_home.clone(),
        Vec::new(),
    )
    .await
    .expect("config");
    config.EfficiencyCode_home = EfficiencyCode_home.abs();
    config.sqlite =
        EfficiencyCode_state::SqliteConfig::new_for_testing(EfficiencyCode_home.as_path().abs());
    config.log_dir = EfficiencyCode_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget =
        ChatWidget::new_with_op_target(common, super::EfficiencyCodeOpTarget::Direct(op_tx));
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_EfficiencyCode_backend_auth = true;
...
fn token_usage_breakdown(
    usage: TokenUsage,
) -> EfficiencyCode_app_server_protocol::TokenUsageBreakdown {
    EfficiencyCode_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    EfficiencyCode_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: EfficiencyCode_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    EfficiencyCode_error_info: Option<EfficiencyCodeErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = EfficiencyCode_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            EfficiencyCode_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("EfficiencyCode-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(EfficiencyCode_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    EfficiencyCode_app_server_protocol::PluginHookSummary {
        ...
        hook_run(
            ...
            vec![
                EfficiencyCode_app_server_protocol::HookOutputEntry {
                    kind: EfficiencyCode_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: EfficiencyCode_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        EfficiencyCode_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        EfficiencyCode_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        EfficiencyCode_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        EfficiencyCode_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        EfficiencyCode_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        EfficiencyCode_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        EfficiencyCode_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        EfficiencyCode_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        EfficiencyCode_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        EfficiencyCode_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        EfficiencyCode_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
