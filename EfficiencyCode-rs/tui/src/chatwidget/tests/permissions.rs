...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = EfficiencyCode_config::ConfigRequirementsToml {
        windows: Some(EfficiencyCode_config::WindowsRequirementsToml {
...
fn requirements_stack(
    requirements_toml: EfficiencyCode_config::ConfigRequirementsToml,
) -> ConfigLayerStack {
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    ...
    chat.config.config_layer_stack =
        requirements_stack(EfficiencyCode_config::ConfigRequirementsToml {
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default EfficiencyCode agent sandbox"),
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let op = std::iter::from_fn(|| rx.try_recv().ok())
        .find_map(|event| match event {
            AppEvent::EfficiencyCodeOp(op @ Op::OverrideTurnContext { .. }) => Some(op),
            _ => None,
        })
