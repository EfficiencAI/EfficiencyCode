...
use EfficiencyCode_app_server_protocol::HookErrorInfo;
use EfficiencyCode_app_server_protocol::HooksListEntry;
use EfficiencyCode_app_server_protocol::HooksListResponse;
use EfficiencyCode_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            ...
            (EfficiencyCode_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn plugin_detail_popup_shows_local_share_context_as_read_only_snapshot() {
    ...
    let summary = PluginSummary {
        share_context: Some(PluginShareContext {
            ...
            share_url: Some("https://chatgpt.com/EfficiencyCode/plugins/share/docs".to_string()),
...
async fn experimental_features_popup_snapshot() {
    ...
    let mut config = EfficiencyCode_config::types::TuiKeymap::default();
    config.list.accept = Some(EfficiencyCode_config::types::KeybindingsSpec::One(
        EfficiencyCode_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(EfficiencyCodeErrorInfo::ServerOverloaded),
    );
...
async fn reasoning_up_shortcut_does_not_silently_enter_advanced_effort() {
    for (model, model_path) in [
        ...
        ("EfficiencyCode-auto-test", "EfficiencyCode-auto-test"),
    ] {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
        chat.thread_id = Some(ThreadId::new());
        let mut preset = get_available_model(&chat, "gpt-5.4");
...
async fn auto_model_advertising_advanced_effort_opens_reasoning_picker() {
    ...
    preset.id = "EfficiencyCode-auto-test".to_string();
    preset.model = "EfficiencyCode-auto-test".to_string();
    preset.display_name = "EfficiencyCode-auto-test".to_string();
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &EfficiencyCode_feedback::FeedbackDiagnostics::new(vec![
            EfficiencyCode_feedback::FeedbackDiagnostic {
