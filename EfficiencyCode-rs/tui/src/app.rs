...
use EfficiencyCode_ansi_escape::ansi_escape_line;
...
use EfficiencyCode_app_server_protocol::HooksListEntry;
...
#[cfg(test)]
use EfficiencyCode_app_server_protocol::McpAuthStatus;
...
use EfficiencyCode_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use EfficiencyCode_app_server_protocol::SkillErrorInfo;
...
use EfficiencyCode_models_manager::model_presets::HIDE_GPT_5_1_EfficiencyCode_MAX_MIGRATION_PROMPT_CONFIG;
use EfficiencyCode_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use EfficiencyCode_protocol::permissions::FileSystemSandboxKind;
...
use EfficiencyCode_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        EfficiencyCode_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&EfficiencyCode_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&EfficiencyCode_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[EfficiencyCode_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<
        &EfficiencyCode_app_server_protocol::AdditionalPermissionProfile,
    >,
) -> Vec<EfficiencyCode_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    EfficiencyCode_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.EfficiencyCode_error_info,
        Some(AppServerEfficiencyCodeErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `EfficiencyCode unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_EfficiencyCode_backend_auth: self.chat_widget.has_EfficiencyCode_backend_auth(),
    ...
    pub async fn run(
        ...
        let has_EfficiencyCode_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            EfficiencyCode_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("EfficiencyCode.status_line", /*inc*/ 1, &[]);
        }
        ...
        let (mut chat_widget, initial_started_thread) = match session_selection {
            SessionSelection::StartFresh | SessionSelection::Exit => {
                ...
                let init = crate::chatwidget::ChatWidgetInit {
                    ...
                    has_chatgpt_account,
                    has_EfficiencyCode_backend_auth,
...
See the EfficiencyCode keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.EfficiencyCode_home.clone();
