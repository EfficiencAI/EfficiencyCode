//! Shared implementation for `EfficiencyCode archive`, `EfficiencyCode delete`, and `EfficiencyCode unarchive`.
//!
...
pub async fn run_session_archive_command(
    ...
    let EfficiencyCode_home =
        find_EfficiencyCode_home().wrap_err("failed to find EfficiencyCode home")?;
    let mut app_server =
        start_app_server_for_archive_command(options, EfficiencyCode_home.to_path_buf()).await?;
...
async fn run_session_archive_action_with_app_server(
    ...
    let resolved = resolve_session_target(app_server, EfficiencyCode_home, action, target).await?;
...
async fn resolve_session_target(
    ...
    for &archived in archived_values {
        if let Some(thread) =
            lookup_session_by_exact_name(app_server, EfficiencyCode_home, target, archived).await?
        {
...
async fn start_app_server_for_archive_command(
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(EfficiencyCode_home.as_path()).await
    } else {
        None
    };
