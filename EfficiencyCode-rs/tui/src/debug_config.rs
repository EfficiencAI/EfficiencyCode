...
use EfficiencyCode_config::NetworkUnixSocketPermissionToml;
...
use EfficiencyCode_config::WebSearchModeRequirement;
...
mod tests {
    ...
    use EfficiencyCode_config::ConstraintError;
    use EfficiencyCode_config::FeatureRequirementsToml;
    use EfficiencyCode_config::FilesystemConstraints;
    use EfficiencyCode_config::HookEventsToml;
    use EfficiencyCode_config::HookHandlerConfig;
    use EfficiencyCode_config::LoaderOverrides;
    use EfficiencyCode_config::ManagedHooksRequirementsToml;
    use EfficiencyCode_config::MatcherGroup;
    ...
    use EfficiencyCode_config::NetworkUnixSocketPermissionToml;
    use EfficiencyCode_config::NetworkUnixSocketPermissionsToml;
    use EfficiencyCode_config::RequirementSource;
    use EfficiencyCode_config::ResidencyRequirement;
    use EfficiencyCode_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\EfficiencyCode\\config.toml")
        } else {
            absolute_path("/etc/EfficiencyCode/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.EfficiencyCode")
        } else {
            absolute_path("/repo/.EfficiencyCode")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_EfficiencyCode_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\EfficiencyCode\\requirements.toml")
        } else {
            absolute_path("/etc/EfficiencyCode/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.EfficiencyCode\\state")
        } else {
            absolute_path("/home/alice/.EfficiencyCode/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.EfficiencyCode\\logs")
        } else {
            absolute_path("/home/alice/.EfficiencyCode/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.EfficiencyCode\\models.json")
        } else {
            absolute_path("/home/alice/.EfficiencyCode/models.json")
        };
                ...
                BTreeMap::from([(
                    ...
                    McpServerRequirement::Identity {
                        identity: McpServerIdentity::Command {
                            command: "EfficiencyCode-mcp".to_string(),
                        },
                    },
                )]),
                RequirementSource::LegacyManagedConfigTomlFromMdm,
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.EfficiencyCode\\config.toml")
        } else {
            absolute_path("/home/alice/.EfficiencyCode/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
            ...
            network: Some(Sourced::new(
                NetworkConstraints {
                    unix_sockets: Some(NetworkUnixSocketPermissionsToml {
                        entries: BTreeMap::from([
                            (
                                "/tmp/EfficiencyCode.sock".to_string(),
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/EfficiencyCode.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\EfficiencyCode")
        } else {
            absolute_path("/var/lib/EfficiencyCode")
        };
