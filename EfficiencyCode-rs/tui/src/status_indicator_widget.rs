...
mod tests {
    ...
    fn details_args_can_disable_capitalization_and_limit_lines() {
        ...
        w.update_details(
            Some(
                "cargo test -p EfficiencyCode-core and then cargo test -p EfficiencyCode-tui"
                    .to_string(),
            ),
        ...
        assert_eq!(
            w.details(),
            Some("cargo test -p EfficiencyCode-core and then cargo test -p EfficiencyCode-tui")
        );
