//! The main EfficiencyCode TUI chat surface.
//!
...
use crate::version::EfficiencyCode_CLI_VERSION;
...
use EfficiencyCode_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use EfficiencyCode_app_server_protocol::CreditsSnapshot;
use EfficiencyCode_app_server_protocol::EfficiencyCodeErrorInfo as AppServerEfficiencyCodeErrorInfo;
...
use EfficiencyCode_app_server_protocol::ModelVerification as AppServerModelVerification;
use EfficiencyCode_app_server_protocol::RateLimitReachedType;
...
use EfficiencyCode_app_server_protocol::SkillMetadata;
use EfficiencyCode_app_server_protocol::SkillsListResponse;
use EfficiencyCode_app_server_protocol::ThreadGoal as AppThreadGoal;
use EfficiencyCode_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use EfficiencyCode_config::ConstraintResult;
...
#[cfg(test)]
use EfficiencyCode_git_utils::CommitLogEntry;
use EfficiencyCode_git_utils::current_branch_name;
use EfficiencyCode_git_utils::get_git_repo_root;
use EfficiencyCode_git_utils::local_git_branches;
use EfficiencyCode_git_utils::recent_commits;
use EfficiencyCode_otel::RuntimeMetricsSummary;
...
use EfficiencyCode_protocol::account::PlanType;
use EfficiencyCode_protocol::approvals::GuardianAssessmentAction;
use EfficiencyCode_protocol::approvals::GuardianAssessmentDecisionSource;
use EfficiencyCode_protocol::approvals::GuardianAssessmentEvent;
use EfficiencyCode_protocol::approvals::GuardianAssessmentStatus;
...
use EfficiencyCode_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use EfficiencyCode_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use EfficiencyCode_utils_cli::resume_hint;
...
const MEMORIES_DOC_URL: &str = "https://developers.openai.com/EfficiencyCode/memories";
...
use EfficiencyCode_file_search::FileMatch;
...
use EfficiencyCode_utils_approval_presets::ApprovalPreset;
use EfficiencyCode_utils_approval_presets::builtin_approval_presets;
use chrono::Local;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_EfficiencyCode_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: EfficiencyCode_feedback::EfficiencyCodeFeedback,
...
/// and by sending requests back to EfficiencyCode-core.
///
...
pub(crate) struct ChatWidget {
    ...
    EfficiencyCode_op_target: EfficiencyCodeOpTarget,
    ...
    has_EfficiencyCode_backend_auth: bool,
    ...
    EfficiencyCode_rate_limit_reached_type: Option<RateLimitReachedType>,
    EfficiencyCode_spend_control_reached: Option<bool>,
    ...
    pub(crate) pet_http_client: EfficiencyCode_http_client::RouteAwareClientPool,
...
fn request_permissions_from_params(
    params: EfficiencyCode_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            EfficiencyCode_windows_sandbox::current_log_file_path_for_EfficiencyCode_home(
                &self.config.EfficiencyCode_home,
            )
            .is_file();
    ...
    /// Forward a command directly to EfficiencyCode.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.EfficiencyCode_op_target {
            EfficiencyCodeOpTarget::Direct(EfficiencyCode_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = EfficiencyCode_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            EfficiencyCodeOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::EfficiencyCodeOp(op));
            }
        }
        true
    }
...
const PLACEHOLDER: &str = "Ask EfficiencyCode to do anything";
