...
use EfficiencyCode_protocol::items::ExitedReviewModeItem;
...
use EfficiencyCode_state::SqliteConfig;
...
async fn start_recording_app_server_with_history(
    ...
    let embedded = crate::start_embedded_app_server(
        EfficiencyCode_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        EfficiencyCode_config::LoaderOverrides::default(),
        /*strict_config*/ false,
        EfficiencyCode_config::CloudConfigBundleLoader::default(),
    ...
    .await?;
    let EfficiencyCode_home = config.EfficiencyCode_home.display().to_string();
                        ...
                        .send(Message::Text(
                            serde_json::to_string(&JSONRPCMessage::Response(JSONRPCResponse {
                                ...
                                result: serde_json::json!({
                                    "userAgent": "EfficiencyCode-tui-test",
                                    "EfficiencyCodeHome": EfficiencyCode_home,
                                }),
                            }))?
                            .into(),
                        ))
...
async fn make_history_test_app() -> Result<(App, tempfile::TempDir)> {
    ...
    app.config.sqlite = SqliteConfig::new_for_testing(EfficiencyCode_home.path().abs());
    Ok((app, EfficiencyCode_home))
}

#[tokio::test]
async fn older_pagination_reconciles_review_prompts_across_page_boundaries() -> Result<()> {
    let (mut app, EfficiencyCode_home) = make_history_test_app().await?;
    ...
    let initial_cells = crate::thread_transcript::thread_items_to_transcript_cells(
        ...
        Some(app.config.EfficiencyCode_home.as_path()),
    );
...
async fn transcript_home_loads_every_older_history_page() -> Result<()> {
    ...
    let export_path = EfficiencyCode_home.path().join("complete-export.md");
    app.chat_widget
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    let (app, _EfficiencyCode_home) = make_history_test_app().await?;
...
async fn underfilled_scrollback_fetches_older_pages_without_opening_the_transcript() -> Result<()> {
    ...
    if let Some(Overlay::Transcript(overlay)) = app.overlay.as_mut() {
        ...
        assert!(!partial.contains("OpenAI EfficiencyCode"));
        ...
        assert!(!loading.contains("OpenAI EfficiencyCode"));
...
async fn cold_paginated_subagent_transcript_excludes_inherited_parent_history() -> Result<()> {
    let (app, EfficiencyCode_home) = make_history_test_app().await?;
    let parent_thread_id = create_history_rollout(
        &app.config,
        ThreadHistoryMode::Paginated,
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            runtime.block_on(async {
                ...
                let EfficiencyCode_home = tempdir()?;
                app.config.EfficiencyCode_home = EfficiencyCode_home.path().to_path_buf().abs();
                app.config.sqlite = EfficiencyCode_state::SqliteConfig::new_for_testing(
