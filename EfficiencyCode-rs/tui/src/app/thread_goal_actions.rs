...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `EfficiencyCode` to start a saved session, or `EfficiencyCode resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let EfficiencyCode_home =
            app_server.EfficiencyCode_home_path(&self.config.EfficiencyCode_home);
