...
async fn lookup_legacy_index_target(
    ...
    let candidates = EfficiencyCode_rollout::find_thread_meta_candidates_by_name_str(
        ...
        loop {
            if let Some(thread) = self.pending.next() {
                ...
                let session_meta = if app_server.uses_remote_workspace() {
                    ...
                    let Some(path) = EfficiencyCode_rollout::existing_rollout_path(path).await
                    else {
                        continue;
                    };
                    let expected_root = self.EfficiencyCode_home.join(match self.collection {
                        SessionCollection::Active => EfficiencyCode_rollout::SESSIONS_SUBDIR,
                        SessionCollection::Archived => {
                            EfficiencyCode_rollout::ARCHIVED_SESSIONS_SUBDIR
                        }
                    });
                    ...
                    let Ok(session_meta) =
                        EfficiencyCode_rollout::read_session_meta_line(path.as_path()).await
                    else {
                        continue;
                    };
