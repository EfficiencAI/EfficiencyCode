...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<EfficiencyCode_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<EfficiencyCode_protocol::mcp::CallToolResult, String>,
    ...
    fn render_lines(&self, width: u16, mode: McpToolCallRenderMode) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(EfficiencyCode_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`EfficiencyCode-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `EfficiencyCode mcp add`.
...
pub(crate) fn empty_mcp_output() -> WebHyperlinkHistoryCell {
    ...
    docs_line.push_span(
        "MCP docs".underlined(),
        Some("https://developers.openai.com/EfficiencyCode/mcp"),
    );
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, EfficiencyCode_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let auth_status = match status.auth_status {
            EfficiencyCode_app_server_protocol::McpAuthStatus::Unknown => McpAuthStatus::Unknown,
