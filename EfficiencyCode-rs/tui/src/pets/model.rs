...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under EfficiencyCode_HOME,
...
impl Pet {
    ...
    /// has already been materialized into EfficiencyCode_HOME; if callers skip the
    /// asset-fetch step, they will get a missing-spritesheet error here on
    /// first use.
    pub(super) fn load_with_EfficiencyCode_home(
        value: &str,
        ...
        if let Some(custom_id) = value.strip_prefix(CUSTOM_PET_PREFIX) {
            return load_custom_pet(custom_id, EfficiencyCode_home);
        }

        if let Some(builtin) = catalog::builtin_pet(value) {
            return load_builtin_pet(builtin, EfficiencyCode_home);
        }

        load_custom_pet(value, EfficiencyCode_home)
    }
...
fn load_builtin_pet(pet: catalog::BuiltinPet, EfficiencyCode_home: Option<&Path>) -> Result<Pet> {
    let EfficiencyCode_home =
        EfficiencyCode_home.context("EfficiencyCode_HOME is not available")?;
    let spritesheet_path =
        super::builtin_spritesheet_path(EfficiencyCode_home, pet.spritesheet_file);
...
fn load_custom_pet(value: &str, EfficiencyCode_home: Option<&Path>) -> Result<Pet> {
    let EfficiencyCode_home =
        EfficiencyCode_home.context("EfficiencyCode_HOME is not available")?;
    let pet_dir = EfficiencyCode_home.join("pets").join(value);
    if pet_dir.join("pet.json").is_file() {
        return load_pet_manifest(&pet_dir, "pet.json", value, &custom_pet_cache_id(value));
    }

    let avatar_dir = EfficiencyCode_home.join("avatars").join(value);
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_EfficiencyCode_home(
    ...
    fn load_builtin_pet_uses_app_catalog_storage() {
        let EfficiencyCode_home = tempfile::tempdir().unwrap();
        super::super::asset_pack::write_test_pack(EfficiencyCode_home.path());

        let pet = Pet::load_with_EfficiencyCode_home(
            "dewey",
    ...
    #[test]
    fn custom_pet_selector_loads_EfficiencyCode_home_pet_manifest() {
        let dir = write_minimal_pet();
        let EfficiencyCode_home = tempfile::tempdir().unwrap();
        let pet_dir = EfficiencyCode_home.path().join("pets").join("chefito");
    ...
    fn custom_pet_selector_falls_back_to_legacy_avatar_manifest() {
        let dir = write_minimal_pet();
        let EfficiencyCode_home = tempfile::tempdir().unwrap();
        let avatar_dir = EfficiencyCode_home.path().join("avatars").join("legacy");
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        let EfficiencyCode_home = tempfile::tempdir().unwrap();
        let pet_dir = EfficiencyCode_home.path().join("pets").join("escape");
        fs::create_dir_all(&pet_dir).unwrap();
        fs::write(
        ...
        let err = Pet::load_with_EfficiencyCode_home(
