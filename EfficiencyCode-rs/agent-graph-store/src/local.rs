...
fn to_state_status(
    status: ThreadSpawnEdgeStatus,
) -> EfficiencyCode_state::DirectionalThreadSpawnEdgeStatus {
    match status {
        ThreadSpawnEdgeStatus::Open => EfficiencyCode_state::DirectionalThreadSpawnEdgeStatus::Open,
        ThreadSpawnEdgeStatus::Closed => {
            EfficiencyCode_state::DirectionalThreadSpawnEdgeStatus::Closed
        }
    }
}
...
mod tests {
    ...
    async fn state_runtime() -> TestRuntime {
        let EfficiencyCode_home = TempDir::new().expect("tempdir should be created");
