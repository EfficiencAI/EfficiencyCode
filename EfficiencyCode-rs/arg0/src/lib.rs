...
use EfficiencyCode_apply_patch::EfficiencyCode_CORE_APPLY_PATCH_ARG1;
#[cfg(unix)]
use EfficiencyCode_exec_server::EfficiencyCode_ARG0_EXEC_HELPER_ARG1;
use EfficiencyCode_exec_server::EfficiencyCode_FS_HELPER_ARG1;
use EfficiencyCode_install_context::InstallContext;
use EfficiencyCode_sandboxing::landlock::EfficiencyCode_LINUX_SANDBOX_ARG0;
use EfficiencyCode_utils_home_dir::find_EfficiencyCode_home;
#[cfg(target_os = "windows")]
use EfficiencyCode_windows_sandbox::EfficiencyCode_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "EfficiencyCode-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current EfficiencyCode executable for child re-execs.
    ///
    ...
    /// instead of the real EfficiencyCode CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            EfficiencyCode_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == EfficiencyCode_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        EfficiencyCode_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        EfficiencyCode_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    #[cfg(unix)]
    if argv1 == EfficiencyCode_ARG0_EXEC_HELPER_ARG1 {
        EfficiencyCode_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == EfficiencyCode_FS_HELPER_ARG1 {
        EfficiencyCode_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == EfficiencyCode_WINDOWS_SANDBOX_ARG1 {
        EfficiencyCode_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == EfficiencyCode_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match EfficiencyCode_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let update_file_mode =
                    EfficiencyCode_apply_patch::apply_patch_file_update_mode_from_env();
                match runtime.block_on(EfficiencyCode_apply_patch::apply_patch_with_mode(
                    &patch_arg,
                    update_file_mode,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    EfficiencyCode_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!(
                    "Error: {EfficiencyCode_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument."
                );
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_EfficiencyCode_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that EfficiencyCode will proceed successfully even if
...
/// While we want to deploy the EfficiencyCode CLI as a single executable for simplicity,
...
/// `EfficiencyCode-linux-sandbox` we *directly* execute
/// [`EfficiencyCode_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.EfficiencyCode/.env` before creating any threads.
...
///     `EfficiencyCode-linux-sandbox` helper path (falling back to the current
...
///     [`EfficiencyCode_core::config::Config`].
///
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    let handle = std::thread::Builder::new()
        .name("EfficiencyCode-main".to_string())
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        EfficiencyCode_self_exe: current_exe.clone(),
        EfficiencyCode_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `EfficiencyCode-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().EfficiencyCode_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "EfficiencyCode_";

/// Load env vars from ~/.EfficiencyCode/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `EfficiencyCode_`.
fn load_dotenv() {
    if let Ok(EfficiencyCode_home) = find_EfficiencyCode_home()
        && let Ok(iter) = dotenvy::from_path_iter(EfficiencyCode_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `EfficiencyCode_` keys.
...
///   with the hidden `--EfficiencyCode-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of EfficiencyCode
/// CLI.
...
fn prepare_path_entry_for_EfficiencyCode_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if EfficiencyCode_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (EfficiencyCode_home: {EfficiencyCode_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a EfficiencyCode_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = EfficiencyCode_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(target_os = "linux")]
        EfficiencyCode_LINUX_SANDBOX_ARG0,
        #[cfg(unix)]
        EXECVE_WRAPPER_ARG0,
    ] {
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {EfficiencyCode_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        EfficiencyCode_self_exe: std::env::current_exe().ok(),
        EfficiencyCode_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(EfficiencyCode_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("EfficiencyCode-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(EfficiencyCodePackageLayout {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_EfficiencyCode_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("EfficiencyCode-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                EfficiencyCode_self_exe: Some(PathBuf::from("/usr/bin/EfficiencyCode")),
                EfficiencyCode_linux_sandbox_exe: Some(alias_path.clone()),
        ...
        assert_eq!(
            linux_sandbox_exe_path(
                Some(&path_entry),
                Some(PathBuf::from("/usr/bin/EfficiencyCode"))
            ),
            Some(alias_path),
        );
        Ok(())
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("EfficiencyCode-helper-alias");
