//! Registry of model providers supported by EfficiencyCode.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so EfficiencyCode works out-of-the-box.
//!   2. User-defined entries inside `~/.EfficiencyCode/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use EfficiencyCode_api::Provider as ApiProvider;
use EfficiencyCode_api::RetryConfig as ApiRetryConfig;
...
use EfficiencyCode_protocol::error::EnvVarError;
...
pub const CHATGPT_EfficiencyCode_BASE_URL: &str = "https://chatgpt.com/backend-api/EfficiencyCode";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "EfficiencyCode";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/openai/EfficiencyCode/discussions/7782";
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> EfficiencyCodeResult<HeaderMap> {
    ...
    pub fn to_api_provider(
    ...
    ) -> EfficiencyCodeResult<ApiProvider> {
        let default_base_url = if matches!(
            ...
            CHATGPT_EfficiencyCode_BASE_URL
        } else {
    ...
    pub fn api_key(&self) -> EfficiencyCodeResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        EfficiencyCodeErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with EfficiencyCode CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These EfficiencyCode_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_EfficiencyCode_oss_base_url = format!(
        "http://localhost:{EfficiencyCode_oss_port}/v1",
        EfficiencyCode_oss_port = std::env::var("EfficiencyCode_OSS_PORT")
            .ok()
    ...
    let EfficiencyCode_oss_base_url = std::env::var("EfficiencyCode_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_EfficiencyCode_oss_base_url);
    create_oss_provider_with_base_url(&EfficiencyCode_oss_base_url, wire_api)
}
