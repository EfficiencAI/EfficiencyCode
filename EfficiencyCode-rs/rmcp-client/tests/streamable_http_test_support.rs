...
use EfficiencyCode_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    EfficiencyCode_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        ...
        Implementation::new("EfficiencyCode-test", "0.0.0-test")
            .with_title("EfficiencyCode rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<EfficiencyCode_http_client::HttpClient> {
    Ok(EfficiencyCode_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `EfficiencyCode exec-server --listen`.
