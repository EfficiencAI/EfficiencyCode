...
const MEMO_URI: &str = "memo://EfficiencyCode/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "EfficiencyCode/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of EfficiencyCode TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (EfficiencyCode TUI):
    /// - Build this binary: `cargo build -p EfficiencyCode-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `EfficiencyCode mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in EfficiencyCode TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://EfficiencyCode/{slug}", "EfficiencyCode-memo")
            .with_title("EfficiencyCode Memo")
            .with_description("Template for memo://EfficiencyCode/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise EfficiencyCode TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new(
                        "EfficiencyCode-rmcp-test-server",
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "encrypted_output" => {
                ...
                meta.insert("EfficiencyCode/encryptedContent".to_string(), json!(true));
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                let mut meta = MetaObject::new();
                meta.insert(
                    "EfficiencyCode/imageDetail".to_string(),
