...
pub enum McpOAuthClientRegistration {
    ...
    #[default]
    Auto,
    /// Require a ChatGPT-hosted EfficiencyCode public native Client ID Metadata Document.
    Cimd,
...
pub(crate) async fn start_authorization(
    ...
    let mut request = AuthorizationRequest::new(redirect_uri)
        ...
        .with_client_name("EfficiencyCode");
    if offer_cimd {
        ...
        request = request.with_client_metadata_url(format!(
            "https://chatgpt.com/oauth/EfficiencyCode/{callback_id}/client.json"
        ));
    }
