...
const LOCK_CONTENTION_EVENT_TARGET: &str =
    "EfficiencyCode_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are EfficiencyCode_HOME-backed today: if EfficiencyCode_HOME is unset they use
        // the default home (`~/.EfficiencyCode`), and if an embedder has no local home/filesystem authority
        ...
        let EfficiencyCode_home = find_EfficiencyCode_home()
            .map_err(|source| OAuthStoreLockFailure::EfficiencyCodeHome { store, source })?;
        Self::acquire_in_with_mode(&EfficiencyCode_home, store, acquire_timeout, mode)
    }
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(EfficiencyCode_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve EfficiencyCode_HOME for MCP OAuth {store} aggregate-store lock")]
    EfficiencyCodeHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(EfficiencyCode_home: &Path, store: OAuthStore) -> PathBuf {
    EfficiencyCode_home
        .join(OAUTH_LOCK_DIR)
