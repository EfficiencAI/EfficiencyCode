...
//!
//! If the keyring is not available or fails, we fall back to EfficiencyCode_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "EfficiencyCode MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let EfficiencyCode_home = find_EfficiencyCode_home().map_err(anyhow::Error::from)?;
...
mod tests {
    ...
    fn save_oauth_tokens_with_secrets_backend_writes_encrypted_storage() -> Result<()> {
        let env = TempEfficiencyCodeHome::new();
        let store = MockKeyringStore::default();
        let tokens = sample_tokens();
        let key = super::compute_store_key(&tokens.server_name, &tokens.url)?;
        let serialized = serde_json::to_string(&tokens)?;
