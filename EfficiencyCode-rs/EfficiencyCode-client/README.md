# EfficiencyCode-client

Higher-level request policy layered on `EfficiencyCode-http-client` without any EfficiencyCode/OpenAI API awareness.
...
- Re-exports the low-level HTTP types temporarily so consumers can migrate to `EfficiencyCode-http-client` incrementally.
