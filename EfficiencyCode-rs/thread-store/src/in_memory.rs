...
use EfficiencyCode_rollout::persisted_rollout_items;
use chrono::DateTime;
use chrono::Utc;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state.sections.get(&thread_id).cloned().map(|id| {
            EfficiencyCode_state::ThreadSection {
                name: if id == EfficiencyCode_state::PINNED_THREAD_SECTION_ID {
                    EfficiencyCode_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(
...
) -> Option<EfficiencyCode_protocol::protocol::GitInfo> {
    ...
    Some(EfficiencyCode_protocol::protocol::GitInfo {
