...
use EfficiencyCode_state::RolloutMigrationCursor;
use EfficiencyCode_state::RolloutMigrationSkippedRollout;
...
pub(super) async fn migrate_rollouts_on_startup(store: &LocalThreadStore) -> ThreadStoreResult<()> {
    ...
    if !pending_migration_thread_ids(&store.config.EfficiencyCode_home)
        .await?
        .is_empty()
    {
...
async fn inspect_rollout_path(
    ...
    match EfficiencyCode_rollout::read_session_meta_line(path).await {
...
fn relative_rollout_path(store: &LocalThreadStore, path: &Path) -> String {
    path.strip_prefix(&store.config.EfficiencyCode_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
