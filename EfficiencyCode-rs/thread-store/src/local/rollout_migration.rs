...
use EfficiencyCode_app_server_protocol::project_rollout_line;
...
impl LocalThreadStore {
    ...
    async fn migrate_rollouts_with_progress_inner(
        ...
        let _maintenance_guard = match options.mode {
            ...
            RolloutMigrationMode::Apply => Some(
                EfficiencyCode_rollout::try_acquire_rollout_maintenance_lock(
        ...
        let mut paths = find_rollout_paths(
            &self
                .config
                .EfficiencyCode_home
                .join(EfficiencyCode_rollout::SESSIONS_SUBDIR),
        )
        .await?;
        paths.extend(
            find_rollout_paths(
                &self
                    .config
                    .EfficiencyCode_home
                    .join(EfficiencyCode_rollout::ARCHIVED_SESSIONS_SUBDIR),
            )
            .await?,
        );
        if options.mode == RolloutMigrationMode::Apply {
            let pending_thread_ids =
                pending_migration_thread_ids(&self.config.EfficiencyCode_home).await?;
    ...
    async fn migrate_rollout_path(
        ...
        let metadata = match EfficiencyCode_rollout::read_session_meta_line(&path).await {
        ...
        let journal_path = migration_journal_path(&self.config.EfficiencyCode_home, thread_id);
    ...
    async fn migrate_one_rollout(
            ...
            || current_source_metadata.modified().ok() != source_modified
        {
            return Err(ThreadStoreError::Conflict {
                message: "rollout changed while migration was staging it; close older EfficiencyCode processes and retry".to_string(),
            });
        }
            ...
            if plan.is_none()
                && matches!(
                    ...
                    RolloutItem::EventMsg(
                        EfficiencyCode_protocol::protocol::EventMsg::ThreadRolledBack(_)
                    )
                )
            {
    ...
    async fn cleanup_failed_unpublished_migration(
        ...
        let Ok(metadata) = EfficiencyCode_rollout::read_session_meta_line(rollout_path).await
        else {
            return Ok(());
        };
        if metadata.meta.history_mode == ThreadHistoryMode::Paginated {
    ...
    async fn project_rollout_in_batches(
        ...
        let subagent_history_start_ordinal =
            EfficiencyCode_rollout::read_session_meta_line(rollout_path)
                .await
                .map_err(migration_error)?
                .meta
        ...
        while let Some(entry) = entries.next_entry().await.map_err(migration_error)? {
                ...
                && (name.ends_with(".jsonl") || name.ends_with(".jsonl.zst"))
            {
                ...
                if name.ends_with(".jsonl.zst")
                    && tokio::fs::try_exists(EfficiencyCode_rollout::plain_rollout_path(&path))
                        .await
                        .map_err(migration_error)?
                {
                    continue;
