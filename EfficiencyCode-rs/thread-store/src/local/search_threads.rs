...
use EfficiencyCode_rollout::first_rollout_content_match_snippet;
use EfficiencyCode_rollout::parse_cursor;
use EfficiencyCode_rollout::search_rollout_matches;
...
struct ThreadSearchItem {
    item: EfficiencyCode_rollout::ThreadItem,
    snippet: String,
}
...
pub(super) async fn search_threads(
    ...
    loop {
        ...
        for item in page.items {
            let logical_path = EfficiencyCode_rollout::plain_rollout_path(item.path.as_path());
...
) -> Option<EfficiencyCode_rollout::Cursor> {
