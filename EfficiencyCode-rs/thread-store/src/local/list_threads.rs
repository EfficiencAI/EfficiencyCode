...
use EfficiencyCode_rollout::RolloutConfig;
use EfficiencyCode_rollout::RolloutRecorder;
use EfficiencyCode_rollout::parse_cursor;
use EfficiencyCode_state::ThreadFilterOptions;
use chrono::DateTime;
use chrono::Utc;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => EfficiencyCode_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => EfficiencyCode_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => EfficiencyCode_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => EfficiencyCode_rollout::SortDirection::Asc,
        SortDirection::Desc => EfficiencyCode_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        EfficiencyCode_home: store.config.EfficiencyCode_home.clone(),
        sqlite: store.config.sqlite.clone(),
        cwd: store.config.EfficiencyCode_home.clone(),
...
async fn list_section_threads(
    ...
    let anchor =
        params
            ...
            .as_deref()
            .map(
                |cursor| -> ThreadStoreResult<EfficiencyCode_state::Anchor> {
                    ...
                    let thread_id = EfficiencyCode_protocol::ThreadId::from_string(thread_id)
                        .map_err(|_| ThreadStoreError::InvalidRequest {
                            message: format!("invalid cursor: {cursor}"),
                        })?;
                    Ok(EfficiencyCode_state::Anchor {
                        ts: timestamp,
                        id: Some(thread_id),
                    })
                },
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| EfficiencyCode_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: EfficiencyCode_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => EfficiencyCode_state::SortDirection::Asc,
            SortDirection::Desc => EfficiencyCode_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    EfficiencyCode_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    EfficiencyCode_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
    ...
    let EfficiencyCode_state::ThreadsPage {
...
pub(super) async fn list_rollout_threads(
    state_db: Option<EfficiencyCode_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&EfficiencyCode_rollout::Cursor>,
    sort_key: EfficiencyCode_rollout::ThreadSortKey,
    sort_direction: EfficiencyCode_rollout::SortDirection,
) -> ThreadStoreResult<EfficiencyCode_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    EfficiencyCode_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    EfficiencyCode_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = EfficiencyCode_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let state = EfficiencyCode_state::StateRuntime::init(
            config.sqlite.clone(),
            config.default_model_provider_id.clone(),
        )
        .await
        ...
        for index in 0..3 {
            ...
            EfficiencyCode_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),
