...
use EfficiencyCode_rollout::ThreadItem;
use EfficiencyCode_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(EfficiencyCode_home: &Path, path: &Path) -> bool {
    path.starts_with(EfficiencyCode_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = EfficiencyCode_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) = find_thread_names_by_ids(
