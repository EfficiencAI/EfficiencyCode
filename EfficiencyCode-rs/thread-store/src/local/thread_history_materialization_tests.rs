...
use EfficiencyCode_rollout::RolloutRecorderParams;
...
/// Separate EfficiencyCode and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let EfficiencyCode_home = root.path().join("EfficiencyCode");
    let sqlite_home = root.path().join("sqlite");
    let thread_id = ThreadId::new();
    let sqlite = EfficiencyCode_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
    let rollout_config = RolloutConfig {
        EfficiencyCode_home: EfficiencyCode_home.clone(),
        sqlite: sqlite.clone(),
        cwd: EfficiencyCode_home.clone(),
    ...
    let runtime = EfficiencyCode_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from EfficiencyCode home");
    assert!(
        runtime
    ...
    store
        .resume_thread(ResumeThreadParams {
            ...
            metadata: ThreadPersistenceMetadata {
                cwd: Some(EfficiencyCode_home.clone()),
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !EfficiencyCode_home.join(filename).exists(),
            "SQLite database should not be created under EfficiencyCode home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = EfficiencyCode_state::open_thread_history_db(
...
async fn projection_store(EfficiencyCode_home: &Path) -> LocalThreadStore {
    let config = test_config(EfficiencyCode_home);
