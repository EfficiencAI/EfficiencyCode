...
use EfficiencyCode_api::ResponsesClient;
use EfficiencyCode_api::ResponsesOptions;
use EfficiencyCode_client::HttpTransport;
use EfficiencyCode_client::Request;
use EfficiencyCode_client::RequestBody;
use EfficiencyCode_client::Response;
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: EfficiencyCode_api::RetryConfig {
            max_attempts: 1,
...
struct FlakyTransportState {
    ...
    requests: Vec<(
        RequestBody,
        HeaderMap,
        EfficiencyCode_client::RequestCompression,
    )>,
}
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(
        requests[0].2,
        EfficiencyCode_client::RequestCompression::None
    );
    Ok(())
}
