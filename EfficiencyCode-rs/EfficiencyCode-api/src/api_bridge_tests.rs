...
fn map_api_error_preserves_retry_delay() {
    ...
    assert!(matches!(
        err.details(),
        EfficiencyCodeErrorDetails::Stream(message) if message == "retry later"
    ));
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let EfficiencyCodeErrorDetails::UnexpectedStatus(err) = err.details() else {
        panic!("expected EfficiencyCodeErrorDetails::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let EfficiencyCodeErrorDetails::CyberPolicy { message } = err.details() else {
        panic!("expected EfficiencyCodeErrorDetails::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let EfficiencyCodeErrorDetails::InvalidRequest(message) = err.details() else {
        panic!("expected EfficiencyCodeErrorDetails::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("EfficiencyCode_other"),
    );
    headers.insert(
        "x-EfficiencyCode-other-limit-name",
        http::HeaderValue::from_static("EfficiencyCode_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let EfficiencyCodeErrorDetails::UsageLimitReached(usage_limit) = err.details() else {
        panic!("expected EfficiencyCodeErrorDetails::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_copies_rate_limit_reached_type_to_usage_limit_snapshot() {
    for (active_limit, expected_limit_id) in [
        (None, "EfficiencyCode"),
        (Some("EfficiencyCode_other"), "EfficiencyCode_other"),
    ] {
        let mut headers = HeaderMap::new();
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-EfficiencyCode-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",
