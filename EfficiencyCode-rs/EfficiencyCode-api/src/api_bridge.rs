...
use EfficiencyCode_protocol::auth::PlanType;
use EfficiencyCode_protocol::error::ConnectionFailedError;
use EfficiencyCode_protocol::error::EfficiencyCodeErr;
use EfficiencyCode_protocol::error::EfficiencyCodeErrorDetails;
use EfficiencyCode_protocol::error::RetryLimitReachedError;
use EfficiencyCode_protocol::error::UnexpectedResponseError;
use EfficiencyCode_protocol::error::UsageLimitReachedError;
use base64::Engine;
use chrono::DateTime;
use chrono::Utc;
use http::HeaderMap;
...
pub fn map_api_error(err: ApiError) -> EfficiencyCodeErr {
    match err {
        ApiError::ContextWindowExceeded => EfficiencyCodeErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => EfficiencyCodeErr::QuotaExceeded,
        ApiError::UsageNotIncluded => EfficiencyCodeErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => {
            let error = EfficiencyCodeErr::Stream(message);
            match delay {
                Some(delay) => error.with_retry_delay(delay),
                None => error,
            }
        }
        ApiError::Stream(msg) => EfficiencyCodeErr::Stream(msg),
        ApiError::ServerOverloaded => EfficiencyCodeErr::ServerOverloaded,
        ...
        ApiError::InvalidRequest { message } => EfficiencyCodeErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => {
            EfficiencyCodeErr::new(EfficiencyCodeErrorDetails::CyberPolicy { message })
        }
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return EfficiencyCodeErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                    ...
                    {
                        EfficiencyCodeErr::InvalidImageRequest()
                    } else {
                        EfficiencyCodeErr::InvalidRequest(body_text)
                    }
                } else if status == http::StatusCode::INTERNAL_SERVER_ERROR {
                    EfficiencyCodeErr::InternalServerError
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return EfficiencyCodeErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return EfficiencyCodeErr::UsageNotIncluded;
                        }
                    }
            ...
            TransportError::RetryLimit => EfficiencyCodeErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => EfficiencyCodeErr::RequestTimeout,
            TransportError::Connection(source) => {
                EfficiencyCodeErr::ConnectionFailed(ConnectionFailedError { source })
            }
            TransportError::Network(msg) | TransportError::Build(msg) => {
                EfficiencyCodeErr::Stream(msg)
            }
        },
        ApiError::RateLimit(msg) => EfficiencyCodeErr::Stream(msg),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-EfficiencyCode-active-limit";
