...
const OPENAI_FILE_USE_CASE: &str = "EfficiencyCode";
...
pub async fn upload_openai_file(
    ...
    let create_request = match hosted_upload {
        Some(context) => serde_json::json!({
            ...
            "file_size": file_size_bytes,
            "use_case": OPENAI_FILE_USE_CASE,
            "EfficiencyCode_connector_id": context.connector_id,
            "EfficiencyCode_action_name": context.action_name,
            "EfficiencyCode_model": context.model,
        }),
        None => create_request,
    };
    ...
    let upload_response = client_pool
        ...
        .map_err(|source| {
            ...
            tracing::event!(
                ...
                tracing::Level::WARN,
                event.name = "EfficiencyCode.openai_file_blob_upload_failed",
