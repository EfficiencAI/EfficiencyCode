...
pub fn rollout_path(EfficiencyCode_home: &Path, filename_ts: &str, thread_id: &str) -> PathBuf {
...
/// Create a minimal rollout file under `EfficiencyCode_HOME/sessions/YYYY/MM/DD/`.
///
...
pub fn create_fake_rollout_with_token_usage(
    ...
    let file_path = rollout_path(EfficiencyCode_home, filename_ts, &thread_id);
    let line = json!({
...
fn create_fake_rollout_with_source_and_parent_thread_id(
    ...
    let file_path = rollout_path(EfficiencyCode_home, filename_ts, &uuid_str);
    let dir = file_path
        .parent()
