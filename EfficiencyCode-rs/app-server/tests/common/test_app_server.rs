...
use EfficiencyCode_app_server_protocol::ClientNotification;
...
use EfficiencyCode_app_server_protocol::ConfigReadParams;
use EfficiencyCode_app_server_protocol::ConfigValueWriteParams;
...
use EfficiencyCode_app_server_protocol::FsCopyParams;
use EfficiencyCode_app_server_protocol::FsCreateDirectoryParams;
use EfficiencyCode_app_server_protocol::FsGetMetadataParams;
use EfficiencyCode_app_server_protocol::FsReadDirectoryParams;
use EfficiencyCode_app_server_protocol::FsReadFileParams;
use EfficiencyCode_app_server_protocol::FsRemoveParams;
use EfficiencyCode_app_server_protocol::FsUnwatchParams;
use EfficiencyCode_app_server_protocol::FsWatchParams;
use EfficiencyCode_app_server_protocol::FsWriteFileParams;
...
use EfficiencyCode_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use EfficiencyCode_app_server_protocol::PluginSearchParams;
use EfficiencyCode_app_server_protocol::PluginSkillReadParams;
use EfficiencyCode_app_server_protocol::PluginUninstallParams;
use EfficiencyCode_app_server_protocol::ProcessKillParams;
use EfficiencyCode_app_server_protocol::ProcessSpawnParams;
use EfficiencyCode_app_server_protocol::RemoteControlClientsListParams;
use EfficiencyCode_app_server_protocol::RemoteControlClientsRevokeParams;
use EfficiencyCode_app_server_protocol::RemoteControlPairingStartParams;
use EfficiencyCode_app_server_protocol::RemoteControlPairingStatusParams;
...
use EfficiencyCode_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use EfficiencyCode_app_server_protocol::ThreadSectionMoveParams;
...
use EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_URL_ENV_VAR;
...
use core_test_support::test_EfficiencyCode::TestEnv;
use core_test_support::test_EfficiencyCode::test_env;
...
pub struct TestAppServer {
    ...
    // removing an owned EfficiencyCode_HOME that may still be its cwd on Windows.
    ...
    _owned_EfficiencyCode_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "EfficiencyCode-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "EfficiencyCode_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary EfficiencyCode_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(EfficiencyCode_home);
        cmd.env("EfficiencyCode_HOME", EfficiencyCode_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "EfficiencyCode_APP_SERVER_MANAGED_CONFIG_PATH",
            EfficiencyCode_home.join("managed_config.toml"),
        );
        cmd.env_remove(EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("EfficiencyCode-mcp-server proc should start")?;
            }
            retries += 1;
        ...
        Ok(Self {
            ...
            _owned_EfficiencyCode_home: None,
        })
    }
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: EfficiencyCode_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing EfficiencyCode_HOME instead of a temporary one.
    pub fn with_EfficiencyCode_home(mut self, EfficiencyCode_home: &Path) -> Self {
        self.EfficiencyCode_home = Some(EfficiencyCode_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary EfficiencyCode_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (EfficiencyCode_home, owned_EfficiencyCode_home) = match EfficiencyCode_home {
            Some(EfficiencyCode_home) => (EfficiencyCode_home, None),
            None => {
                let owned_EfficiencyCode_home = TempDir::new()?;
                (
                    owned_EfficiencyCode_home.path().to_path_buf(),
                    Some(owned_EfficiencyCode_home),
                )
            }
        };
        let attribution_settings_server = if EfficiencyCode_home.join("auth.json").is_file() {
            let config_path = EfficiencyCode_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = EfficiencyCode_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            EfficiencyCode_utils_cargo_bin::cargo_bin("exec-server")
                ...
                let mut auto_env_overrides = vec![
                    (
                        EfficiencyCode_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        EfficiencyCode_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        EfficiencyCode_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        EfficiencyCode_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        EfficiencyCode_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-app-server")
                .context("should find binary for EfficiencyCode-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && EfficiencyCode_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-code-mode-host")
        {
        ...
        app_server._owned_EfficiencyCode_home = owned_EfficiencyCode_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `EfficiencyCode-app-server` child process.
        //
