...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(
        ...
        exec_server_program: &Path,
    ) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(EfficiencyCode_home);
        command.env("EfficiencyCode_HOME", EfficiencyCode_home);
        #[cfg(target_os = "linux")]
        command.env(
            EfficiencyCode_LINUX_SANDBOX_EXE_ENV_VAR,
