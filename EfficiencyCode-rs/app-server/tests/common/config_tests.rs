...
fn mock_responses_config_composes_model_provider_features_and_extra_tables() {
    let home = TempDir::new().expect("temporary EfficiencyCode_HOME");
    MockResponsesConfig::new("http://127.0.0.1:1234")
        .with_model("custom-model")
        .with_model_provider("openai-custom")
        .with_provider_name("OpenAI")
        .with_provider_base_url("http://127.0.0.1:1234/api/EfficiencyCode")
    ...
    for expected in [
        ...
        "base_url = \"http://127.0.0.1:1234/api/EfficiencyCode\"",
