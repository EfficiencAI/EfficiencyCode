...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(EfficiencyCode_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "EfficiencyCode_other",
                "metered_feature": "EfficiencyCode_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
    ...
    Mock::given(method("GET"))
        .and(path("/api/EfficiencyCode/usage"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/api/EfficiencyCode/rate-limit-reset-credits"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "credits": [
                {
                    "id": "credit-1",
                    "reset_type": "EfficiencyCode_rate_limits",
                    "status": "available",
    ...
    let expected = GetAccountRateLimitsResponse {
        ...
        rate_limits_by_limit_id: Some(
            [
                ...
                (
                    "EfficiencyCode_other".to_string(),
                    RateLimitSnapshot {
        ...
        rate_limit_reset_credits: Some(RateLimitResetCreditsSummary {
            ...
            credits: Some(vec![
                RateLimitResetCredit {
                    ...
                    reset_type: RateLimitResetType::EfficiencyCodeRateLimits,
...
async fn get_account_rate_limits_preserves_count_when_reset_credit_details_fail() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_chatgpt_base_url(EfficiencyCode_home.path(), &server.uri())?;
...
fn write_chatgpt_base_url(EfficiencyCode_home: &Path, base_url: &str) -> std::io::Result<()> {
