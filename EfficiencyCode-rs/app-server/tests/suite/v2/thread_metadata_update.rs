use EfficiencyCode_app_server_protocol::GitInfo;
...
use EfficiencyCode_app_server_protocol::ThreadSectionListResponse;
...
use EfficiencyCode_protocol::protocol::GitInfo as RolloutGitInfo;
use EfficiencyCode_rollout::state_db::reconcile_rollout;
use EfficiencyCode_state::PINNED_THREAD_SECTION_ID;
use EfficiencyCode_state::PINNED_THREAD_SECTION_NAME;
...
async fn thread_section_move_pins_and_unpins_with_filtered_recency_pagination() -> Result<()> {
    ...
    let state_db = init_state_db(EfficiencyCode_home.path()).await?;
    ...
    for (filename_timestamp, timestamp, preview) in [
        ...
        reconcile_rollout(
            Some(&state_db),
            rollout_path(EfficiencyCode_home.path(), filename_timestamp, &thread_id).as_path(),
            "mock_provider",
            /*builder*/ None,
            &[],
            /*archived_only*/ None,
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(EfficiencyCode_home.path())?;
    let _state_db = init_state_db(EfficiencyCode_home.path()).await?;
...
async fn thread_metadata_update_can_clear_stored_git_fields() -> Result<()> {
    ...
    let thread_id = create_fake_rollout(
        ...
        Some(RolloutGitInfo {
            ...
            branch: Some("feature/sidebar-pr".to_string()),
            repository_url: Some("git@example.com:openai/EfficiencyCode.git".to_string()),
        }),
    )?;
    let _state_db = init_state_db(EfficiencyCode_home.path()).await?;
...
async fn init_state_db(EfficiencyCode_home: &Path) -> Result<Arc<StateRuntime>> {
