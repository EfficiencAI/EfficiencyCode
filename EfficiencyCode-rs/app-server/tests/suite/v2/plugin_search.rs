...
use EfficiencyCode_app_server_protocol::PluginSearchResponse;
use EfficiencyCode_app_server_protocol::PluginSearchScope;
...
async fn plugin_search_stitches_local_results_into_the_first_remote_page() -> Result<()> {
    ...
    write_chatgpt_search_auth(EfficiencyCode_home.path())?;
...
async fn plugin_search_returns_local_matches_for_api_key_auth() -> Result<()> {
    for remote_plugin_enabled in [false, true] {
        let EfficiencyCode_home = TempDir::new()?;
        let repo_root = TempDir::new()?;
        let curated_root = EfficiencyCode_home.path().join(".tmp/plugins");
        let bundled_alpha_root = EfficiencyCode_home
            .path()
...
-> Result<()> {
    ...
    write_installed_plugin(EfficiencyCode_home.path(), "openai-curated", "calendar")?;
...
fn write_chatgpt_search_auth(EfficiencyCode_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        EfficiencyCode_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_installed_plugin(
    ...
    let plugin_manifest = EfficiencyCode_home
        .join("plugins/cache")
