...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    ...
    run_git(
        root,
        &["config", "user.email", "EfficiencyCode@example.com"],
    )?;
    run_git(root, &["config", "user.name", "EfficiencyCode Tests"])?;
...
fn disable_plugin_startup_tasks(EfficiencyCode_home: &Path) -> Result<()> {
...
fn marketplace_install_root(EfficiencyCode_home: &Path) -> std::path::PathBuf {
    EfficiencyCode_home.join(INSTALLED_MARKETPLACES_DIR)
}
...
fn expected_installed_root(
    ...
    AbsolutePathBuf::try_from(
        marketplace_install_root(&EfficiencyCode_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(EfficiencyCode_home.path())?;
    ...
    let debug_root = expected_installed_root(EfficiencyCode_home.path(), "debug")?;
    let tools_root = expected_installed_root(EfficiencyCode_home.path(), "tools")?;
