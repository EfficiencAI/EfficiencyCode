...
use EfficiencyCode_app_server::AppServerRuntimeOptions;
use EfficiencyCode_app_server::AppServerTransport;
use EfficiencyCode_app_server::AppServerWebsocketAuthSettings;
use EfficiencyCode_app_server::PluginStartupTasks;
use EfficiencyCode_app_server::RemoteControlStartupMode;
use EfficiencyCode_app_server::run_main_with_transport_options;
...
use EfficiencyCode_app_server_protocol::RemoteControlClient;
use EfficiencyCode_app_server_protocol::RemoteControlClientsListOrder;
use EfficiencyCode_app_server_protocol::RemoteControlClientsListParams;
use EfficiencyCode_app_server_protocol::RemoteControlClientsListResponse;
use EfficiencyCode_app_server_protocol::RemoteControlClientsRevokeParams;
use EfficiencyCode_app_server_protocol::RemoteControlClientsRevokeResponse;
use EfficiencyCode_app_server_protocol::RemoteControlConnectionStatus;
use EfficiencyCode_app_server_protocol::RemoteControlDisableResponse;
use EfficiencyCode_app_server_protocol::RemoteControlEnableResponse;
use EfficiencyCode_app_server_protocol::RemoteControlPairingStartParams;
use EfficiencyCode_app_server_protocol::RemoteControlPairingStartResponse;
use EfficiencyCode_app_server_protocol::RemoteControlPairingStatusParams;
use EfficiencyCode_app_server_protocol::RemoteControlPairingStatusResponse;
use EfficiencyCode_app_server_protocol::RemoteControlStatusChangedNotification;
use EfficiencyCode_app_server_protocol::RemoteControlStatusReadResponse;
...
use EfficiencyCode_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = EfficiencyCode_home.path().join("managed_config.toml");
    let socket_path = EfficiencyCode_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _EfficiencyCode_home_guard = EnvVarGuard::set(
        "EfficiencyCode_HOME",
        EfficiencyCode_home.path().as_os_str(),
    );
    ...
    let result = timeout(
        ...
        run_main_with_transport_options(
            Arg0DispatchPaths {
                EfficiencyCode_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let listener = configured_remote_control_listener(EfficiencyCode_home.path()).await?;
...
async fn listen_off_exits_without_persisted_remote_control_enable() -> Result<()> {
    for persisted_preference in [None, Some(false)] {
        ...
        if let Some(remote_control_enabled) = persisted_preference {
            ...
            let state_db = StateRuntime::init(
                EfficiencyCode_state::SqliteConfig::new_for_testing(
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(EfficiencyCode_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(EfficiencyCode_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(EfficiencyCode_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(EfficiencyCode_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let mut backend =
        ClientManagementRemoteControlBackend::start(EfficiencyCode_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(EfficiencyCode_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(EfficiencyCode_home).await?;
