use EfficiencyCode_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use EfficiencyCode_app_server::INVALID_PARAMS_ERROR_CODE;
...
use EfficiencyCode_app_server_protocol::ByteRange;
...
use EfficiencyCode_app_server_protocol::FileChangePatchUpdatedNotification;
...
use EfficiencyCode_app_server_protocol::JSONRPCError;
...
use EfficiencyCode_app_server_protocol::TextElement;
...
use EfficiencyCode_core::test_support::all_model_presets;
...
const TEST_ORIGINATOR: &str = "EfficiencyCode_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two EfficiencyCode turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    MockResponsesConfig::new(&server.uri()).write(EfficiencyCode_home.path())?;
    ...
    let image_path = EfficiencyCode_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(EfficiencyCode_home.path())?;
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("EfficiencyCode VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        .enable_feature(Feature::Personality)
        .write(EfficiencyCode_home.path())?;
    write_models_cache(EfficiencyCode_home.path())?;
    let cache_path = EfficiencyCode_home.path().join("models_cache.json");
    ...
    let config_path = EfficiencyCode_home.path().join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("model = \"mock-model\"", &format!("model = \"{model}\"")),
    )?;
    write_test_skill(EfficiencyCode_home.path(), "alpha-skill")?;
    write_test_skill(EfficiencyCode_home.path(), "beta-skill")?;

    let isolated_home = EfficiencyCode_home.path().to_string_lossy();
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .with_env_overrides(&[
...
async fn turn_start_emits_raw_response_completed_with_upstream_usage() -> Result<()> {
    ...
    .await??;
    let notification: EfficiencyCode_app_server_protocol::ServerNotification =
        notification.try_into()?;
    let EfficiencyCode_app_server_protocol::ServerNotification::RawResponseCompleted(notification) =
        notification
    else {
...
async fn turn_start_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    mount_analytics_capture(&server, EfficiencyCode_home.path()).await?;
    ...
    let ThreadStartResponse { thread, .. } = mcp
        .start_thread(ThreadStartParams {
            model: Some("mock-model".to_string()),
            thread_source: Some(ThreadSource::User),
            service_name: Some("EfficiencyCode_work_desktop".to_string()),
            ..Default::default()
        })
        .await?;
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "EfficiencyCode_turn_event")
            .await?;
...
async fn code_mode_exec_emits_correlated_production_analytics() -> Result<()> {
    ...
    let event = wait_for_analytics_event(
        &server,
        DEFAULT_READ_TIMEOUT,
        "EfficiencyCode_dynamic_tool_call_event",
    )
    .await?;
    assert_eq!(
        json!({
...
async fn turn_start_rejects_invalid_permission_selection_before_starting_turn() -> Result<()> {
    ...
    MockResponsesConfig::new("http://localhost/unused")
        ...
        .write(EfficiencyCode_home.path())?;
    std::fs::write(
        EfficiencyCode_home.path().join("managed_config.toml"),
...
async fn turn_start_accepts_managed_network_profile_from_requirements() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(EfficiencyCode_home.path())?;
    std::fs::write(
        EfficiencyCode_home.path().join("requirements.toml"),
        r#"
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: EfficiencyCode_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three EfficiencyCode turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        EfficiencyCode_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let ThreadStartResponse { thread, .. } = mcp
        .start_thread(ThreadStartParams {
            model: Some("exp-EfficiencyCode-personality".to_string()),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let EfficiencyCode_home = tmp.path().to_path_buf();
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(EfficiencyCode_home.as_path())?;

    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.as_path())
        .build_initialized()
        .await?;
    ...
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            ...
            params: TurnStartParams {
                ...
                approval_policy: Some(EfficiencyCode_app_server_protocol::AskForApproval::Never),
                sandbox_policy: Some(
...
async fn turn_start_explicit_local_environment_updates_legacy_cwd_between_turns() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let EfficiencyCode_home = tmp.path().join("EfficiencyCode_home");
    std::fs::create_dir(&EfficiencyCode_home)?;
    ...
    MockResponsesConfig::new(&server.uri())
        .with_approval_policy("untrusted")
        .write(&EfficiencyCode_home)?;

    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(&EfficiencyCode_home)
        .build_initialized()
        .await?;
    ...
    let first_writable_root =
        EfficiencyCode_utils_absolute_path::AbsolutePathBuf::try_from(first_cwd.clone())?;
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            request_id,
            params: TurnStartParams {
                ...
                sandbox_policy: Some(
                    EfficiencyCode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root =
        EfficiencyCode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root =
        EfficiencyCode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    MockResponsesConfig::new(&server.uri()).write(&EfficiencyCode_home)?;
    std::fs::write(
        EfficiencyCode_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![EfficiencyCode_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .disable_feature(Feature::ShellSnapshot)
        .write(&EfficiencyCode_home)?;
    write_models_cache(&EfficiencyCode_home)?;
    let cache_path = EfficiencyCode_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn direct_input_to_multi_agent_v2_subagent_is_rejected() -> Result<()> {
    ...
    let listed: EfficiencyCode_app_server_protocol::ThreadListResponse = mcp
        .request(|request_id| ClientRequest::ThreadList {
            request_id,
            params: EfficiencyCode_app_server_protocol::ThreadListParams {
                cursor: None,
                limit: Some(10),
                sort_key: None,
                sort_direction: None,
                ...
                source_kinds: Some(vec![
                    EfficiencyCode_app_server_protocol::ThreadSourceKind::SubAgentThreadSpawn,
                ]),
                archived: None,
                section_id: None,
                cwd: None,
    ...
    assert!(matches!(
        &listed_child.source,
        EfficiencyCode_app_server_protocol::SessionSource::SubAgent(
            EfficiencyCode_protocol::protocol::SubAgentSource::ThreadSpawn {
                agent_path: Some(_),
                ..
            }
        )
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(EfficiencyCode_home.path())?;
    std::fs::write(
        EfficiencyCode_home.path().join("custom-role.toml"),
...
async fn command_execution_notifications_include_trusted_plugin_id() -> Result<()> {
    ...
    let synced_root = EfficiencyCode_home.path().join(".tmp/plugins");
    for path in [
        plugin_root.join(".EfficiencyCode-plugin"),
        script_path
            .parent()
    ...
    std::fs::write(
        EfficiencyCode_home.path().join(".tmp/plugins.sha"),
        format!("{curated_sha}\n"),
    )?;
    std::fs::write(
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(EfficiencyCode_home.path().join("config.toml"))?;
...
fn write_test_skill(EfficiencyCode_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = EfficiencyCode_home.join("skills").join(name);
