...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let EfficiencyCode_home = configured_EfficiencyCode_home(fixture.base_url())?;
    let mut app_server = start_app_server(EfficiencyCode_home.path()).await?;
...
fn configured_EfficiencyCode_home(base_url: &str) -> Result<TempDir> {
...
impl InstalledAppsFixture {
    async fn start() -> Result<Self> {
        ...
        synthetic_link
            ...
            .0
            .insert(
                "_EfficiencyCode_apps".to_string(),
