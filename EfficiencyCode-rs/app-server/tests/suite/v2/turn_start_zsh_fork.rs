...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&EfficiencyCode_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(EfficiencyCode_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(EfficiencyCode_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(
        command.starts_with(
            &command_packaged_zsh_path(&EfficiencyCode_home)
                .display()
                .to_string()
        )
    );
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(
                EfficiencyCode_app_server_protocol::AskForApproval::UnlessTrusted,
            ),
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(EfficiencyCode_home, zsh_path)?;
...
fn create_test_package_app_server(EfficiencyCode_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = EfficiencyCode_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(EfficiencyCode_home);
    ...
    std::fs::write(package_dir.join("EfficiencyCode-package.json"), "{}")?;

    let app_server = bin_dir.join("EfficiencyCode-app-server");
    copy_with_permissions(
        &EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home
        .join("test-package")
...
fn command_packaged_zsh_path(EfficiencyCode_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(EfficiencyCode_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = EfficiencyCode_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("EfficiencyCode-rs/app-server/tests/suite/zsh");
