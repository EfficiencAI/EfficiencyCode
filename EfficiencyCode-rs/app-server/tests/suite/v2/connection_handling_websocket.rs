...
async fn thread_start_routes_project_exec_policy_warning_to_requester() -> Result<()> {
    ...
    let rules_dir = project.path().join(".EfficiencyCode/rules");
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let EfficiencyCode_home = TempDir::new()?;
    let token_file = EfficiencyCode_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(EfficiencyCode_home.path(), "ws://0.0.0.0:0", &auth_args)
            .await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let EfficiencyCode_home = TempDir::new()?;
    let shared_secret_file = EfficiencyCode_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "EfficiencyCode-enroller".to_string(),
        "--ws-audience".to_string(),
        "EfficiencyCode-app-server".to_string(),
...
pub(super) async fn spawn_websocket_server(
    ...
    spawn_websocket_server_with_args(EfficiencyCode_home, "ws://127.0.0.1:0", &[]).await
}
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    let program = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-app-server")
