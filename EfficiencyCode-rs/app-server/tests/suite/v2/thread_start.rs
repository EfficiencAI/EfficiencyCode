...
use EfficiencyCode_app_server_protocol::TextPosition;
use EfficiencyCode_app_server_protocol::TextRange;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = EfficiencyCode_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml_without_approval_policy(EfficiencyCode_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .build_initialized()
        .await?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml_without_approval_policy(EfficiencyCode_home.path(), &server.uri())?;
    let global_agents_path = EfficiencyCode_home.path().join("AGENTS.md");
...
async fn thread_start_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml_with_required_broken_mcp(EfficiencyCode_home.path(), &server.uri())?;
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(EfficiencyCode_home.path(), &server.uri())?;
...
"#,
    ...
    let config_before = std::fs::read_to_string(EfficiencyCode_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(EfficiencyCode_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
