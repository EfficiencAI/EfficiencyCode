...
use EfficiencyCode_app_server_protocol::ImageGenerationFailure;
...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    let image_request = server
        ...
        .into_iter()
        .find(|request| request.url.path() == "/api/EfficiencyCode/images/generations")
...
async fn standalone_image_generation_failure_emits_terminal_item() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/EfficiencyCode/images/generations"))
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(429)
                .insert_header("x-EfficiencyCode-active-limit", "image_gen")
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    ...
    let (edit_request, _) = run_image_edit_test(|EfficiencyCode_home| {
        let image_path = EfficiencyCode_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let EfficiencyCode_home = TempDir::new()?;
    let (arguments, input) = input(EfficiencyCode_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
    ...
    let image_request = requests
        .iter()
        .find(|request| request.url.path() == "/api/EfficiencyCode/images/edits")
...
fn assert_image_turn_id_header(request: &wiremock::Request, expected_turn_id: &str) -> Result<()> {
    let turn_id = request
        .headers
        .get("x-EfficiencyCode-image-turn-id")
...
async fn mount_image_edit_response(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/api/EfficiencyCode/images/edits"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "created": 1,
...
fn create_config_toml(
    ...
    let mut config = MockResponsesConfig::new(server_uri)
        ...
        .with_provider_base_url(&format!("{server_uri}/api/EfficiencyCode"))
