...
use EfficiencyCode_app_server_protocol::CapabilityRootLocation;
...
use EfficiencyCode_protocol::protocol::ThreadSource as CoreThreadSource;
...
async fn mcp_server_tool_call_forwards_only_server_extensions() -> Result<()> {
    ...
    mcp.initialize_with_capabilities(
        ClientInfo {
            name: "EfficiencyCode_test".to_string(),
            title: None,
...
-> Result<()> {
    mcp_server_tool_call_round_trips_elicitation_for_thread(ElicitationThread::Start {
        params: ThreadStartParams {
            ...
            sandbox: Some(EfficiencyCode_app_server_protocol::SandboxMode::DangerFullAccess),
...
{
    assert_full_access_form_elicitation_is_declined(FullAccessElicitationCase {
        thread_source: Some(EfficiencyCode_app_server_protocol::ThreadSource::Feature(
...
async fn mcp_server_tool_call_declines_full_access_elicitation_for_subagent_thread() -> Result<()> {
    assert_full_access_form_elicitation_is_declined(FullAccessElicitationCase {
        thread_source: Some(EfficiencyCode_app_server_protocol::ThreadSource::Subagent),
        client_advertises_standard_form_input: true,
    })
    .await
}
...
struct FullAccessElicitationCase {
    thread_source: Option<EfficiencyCode_app_server_protocol::ThreadSource>,
...
async fn assert_full_access_form_elicitation_is_declined(
    ...
    assert_eq!(
        approval_policy,
        EfficiencyCode_app_server_protocol::AskForApproval::Never
    );
    assert_eq!(
        sandbox,
...
async fn mcp_server_tool_call_round_trips_elicitation_for_thread(
    ...
    let thread = match elicitation_thread {
        ...
        ElicitationThread::Resume { source, params } => {
            ...
            assert_eq!(
                thread.source,
                EfficiencyCode_app_server_protocol::SessionSource::from(source)
            );
            assert_eq!(
                approval_policy,
                EfficiencyCode_app_server_protocol::AskForApproval::Never
