...
impl CuratedMcpSyncFixture {
    async fn set_up() -> Result<Self> {
        ...
        let EfficiencyCode_home = fixture_root.path().join("EfficiencyCode-home");
        ...
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.email", "EfficiencyCode-tests@openai.com"],
        )?;
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.name", "EfficiencyCode Tests"],
        )?;
...
fn write_plugin(
    ...
    std::fs::create_dir_all(root.join(".EfficiencyCode-plugin"))?;
    std::fs::write(
        root.join(".EfficiencyCode-plugin/plugin.json"),
