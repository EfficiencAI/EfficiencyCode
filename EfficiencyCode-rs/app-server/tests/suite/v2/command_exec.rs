use EfficiencyCode_app_server_protocol::CommandExecOutputDeltaNotification;
use EfficiencyCode_app_server_protocol::CommandExecOutputStream;
use EfficiencyCode_app_server_protocol::CommandExecParams;
use EfficiencyCode_app_server_protocol::CommandExecResizeParams;
use EfficiencyCode_app_server_protocol::CommandExecResponse;
use EfficiencyCode_app_server_protocol::CommandExecTerminalSize;
...
use EfficiencyCode_core::exec_env::EfficiencyCode_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
use EfficiencyCode_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR;
use anyhow::Context;
use anyhow::Result;
...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(EfficiencyCode_home.path(), &server.uri(), "never")?;
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$EfficiencyCode_HOME\" \"$OPENAI_FEDERATION_RULE_ID\" \"$openai_identity_token_file\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${EfficiencyCode_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml(EfficiencyCode_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(EfficiencyCode_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let EfficiencyCode_home = TempDir::new()?;
    let command_dir = EfficiencyCode_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !EfficiencyCode_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(EfficiencyCode_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "EfficiencyCode-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(EfficiencyCode_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(EfficiencyCode_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(EfficiencyCode_home: &Path, inserted_config: &str) -> Result<()> {
