...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    // Verify file moved.
    let archived_directory = EfficiencyCode_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let archived_child_path = EfficiencyCode_home
        .path()
    ...
    wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "EfficiencyCode_thread_archive_event"
    ...
    for request in requests {
        if request.url.path() != "/EfficiencyCode/analytics-events/events" {
            continue;
        }
        ...
        for event in events
            .iter()
            .filter(|event| event["event_type"] == "EfficiencyCode_thread_archive_event")
        {
