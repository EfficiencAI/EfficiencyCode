...
use EfficiencyCode_app_server_protocol::AppBranding;
...
use EfficiencyCode_app_server_protocol::AppReview;
use EfficiencyCode_app_server_protocol::AppScreenshot;
...
#[tokio::test]
async fn list_apps_returns_empty_when_workspace_EfficiencyCode_plugins_disabled() -> Result<()> {
    let connectors = vec![AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: Some("Beta connector".to_string()),
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    write_connectors_and_plugins_config(EfficiencyCode_home.path(), &server_url)?;
    write_plugin_app_fixture(EfficiencyCode_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        EfficiencyCode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
