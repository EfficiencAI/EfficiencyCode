...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_EfficiencyCode_home(fixture.EfficiencyCode_home.path())
    ...
    let mut exec_server =
        spawn_exec_server(fixture.EfficiencyCode_home.path(), &fixture.exec_server_url).await?;
...
fn selected_capability_fixture(
    ...
    let local_skill_dir = EfficiencyCode_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".EfficiencyCode-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__EfficiencyCode_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(EfficiencyCode_home: &std::path::Path, url: &str) -> Result<Child> {
    let mut child = Command::new(EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")?)
