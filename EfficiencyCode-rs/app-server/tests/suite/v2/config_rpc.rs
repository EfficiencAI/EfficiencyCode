use EfficiencyCode_app_server_protocol::AppConfig;
use EfficiencyCode_app_server_protocol::AppToolApproval;
use EfficiencyCode_app_server_protocol::ApprovalsReviewer;
use EfficiencyCode_app_server_protocol::AppsConfig;
use EfficiencyCode_app_server_protocol::AppsDefaultConfig;
use EfficiencyCode_app_server_protocol::AskForApproval;
use EfficiencyCode_app_server_protocol::ConfigBatchWriteParams;
use EfficiencyCode_app_server_protocol::ConfigEdit;
use EfficiencyCode_app_server_protocol::ConfigLayerSource;
...
use EfficiencyCode_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use EfficiencyCode_app_server_protocol::ToolsV2;
...
use EfficiencyCode_protocol::config_types::WebSearchContextSize;
use EfficiencyCode_protocol::config_types::WebSearchLocation;
...
fn write_config(EfficiencyCode_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let EfficiencyCode_home_path = EfficiencyCode_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(EfficiencyCode_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    write_config(&EfficiencyCode_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_EfficiencyCode_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let EfficiencyCode_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", EfficiencyCode_home);
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let EfficiencyCode_home = tmp_dir.path().canonicalize()?;
...
fn assert_layers_user_then_optional_system(
    layers: &[EfficiencyCode_app_server_protocol::ConfigLayer],
