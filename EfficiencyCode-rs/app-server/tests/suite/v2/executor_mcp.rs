...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let executor_config: EfficiencyCode_config::types::McpServerConfig =
    ...
    let oauth_credentials_path = EfficiencyCode_home.path().join(".credentials.json");
    std::fs::write(
        &oauth_credentials_path,
        serde_json::to_vec(&json!({"host": host_oauth_credential.clone()}))?,
    )?;
    let EfficiencyCode_bin = toml::Value::String(
        EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".EfficiencyCode-plugin"))?;
    std::fs::write(
        plugin.path().join(".EfficiencyCode-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(registration_request["client_name"], json!("EfficiencyCode"));
    assert_eq!(
