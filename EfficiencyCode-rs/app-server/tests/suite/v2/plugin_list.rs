...
use EfficiencyCode_app_server_protocol::HookTrustStatus;
...
use EfficiencyCode_app_server_protocol::PluginAuthPolicy;
use EfficiencyCode_app_server_protocol::PluginDisabledReason;
use EfficiencyCode_app_server_protocol::PluginInstallPolicy;
use EfficiencyCode_app_server_protocol::PluginInstallPolicySource;
...
use EfficiencyCode_core::config::set_project_trust_level;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "EfficiencyCode_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
fn write_plugins_enabled_config(EfficiencyCode_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(EfficiencyCode_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = EfficiencyCode_home.path().to_string_lossy().into_owned();
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .with_env_overrides(&[
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&EfficiencyCode_home, "openai-curated", "linear")?;
    std::fs::write(
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_openai_curated_marketplace(EfficiencyCode_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&EfficiencyCode_home, "openai-curated", "linear")?;
    write_installed_plugin(&EfficiencyCode_home, "openai-curated", "calendar")?;
    std::fs::write(
        EfficiencyCode_home.path().join("config.toml"),
        format!(
...
async fn plugin_installed_prefers_api_curated_conflicts_after_switching_to_api_auth() -> Result<()>
{
    let EfficiencyCode_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_openai_api_curated_marketplace(EfficiencyCode_home.path(), &["linear"])?;
    write_installed_plugin(&EfficiencyCode_home, "openai-api-curated", "linear")?;
    let config = format!(
        r#"chatgpt_base_url = "{}/backend-api/"
...
"#,
    ...
    std::fs::write(EfficiencyCode_home.path().join("config.toml"), &config)?;
    write_chatgpt_auth(
        EfficiencyCode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.EfficiencyCode-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.EfficiencyCode-plugin/plugin.json"),
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_EfficiencyCode_plugins_disabled() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_EfficiencyCode_plugins_setting() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.EfficiencyCode-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        EfficiencyCode_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    disabled_reason: None,
                    eligible_plan_types: None,
                    interface: Some(EfficiencyCode_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    std::fs::create_dir_all(EfficiencyCode_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(EfficiencyCode_home.path())?;
    std::fs::write(
        EfficiencyCode_home
            .path()
...
async fn plugin_list_force_refetch_waits_for_same_path_local_plugin_upgrade() -> Result<()> {
    ...
    let source_manifest = marketplace_root
        .path()
        .join("sample-plugin/.EfficiencyCode-plugin/plugin.json");
...
"#,
    ...
    let plugin_cache = EfficiencyCode_home
        .path()
        .join("plugins/cache/sample-marketplace/sample-plugin");
    let installed_manifest = plugin_cache.join("1.1.0/.EfficiencyCode-plugin/plugin.json");
    assert!(
...
[plugins."enabled-plugin@EfficiencyCode-curated"]
enabled = true

[plugins."disabled-plugin@EfficiencyCode-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "EfficiencyCode-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(
        ...
        "enabled-plugin@EfficiencyCode-curated"
    );
    ...
    assert_eq!(
        ...
        "disabled-plugin@EfficiencyCode-curated"
    );
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@EfficiencyCode-curated"
    );
...
[plugins."shared-plugin@EfficiencyCode-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".EfficiencyCode"))?;
    std::fs::write(
        workspace_enabled.path().join(".EfficiencyCode/config.toml"),
        r#"[plugins."shared-plugin@EfficiencyCode-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@EfficiencyCode-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@EfficiencyCode-curated");
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = EfficiencyCode_home
        .path()
        .join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".EfficiencyCode-plugin"))?;
    std::fs::write(
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = EfficiencyCode_home
        .path()
    ...
    wait_for_path_exists(&installed_path.join(".EfficiencyCode-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".EfficiencyCode-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let old_path = EfficiencyCode_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = EfficiencyCode_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = EfficiencyCode_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".EfficiencyCode-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".EfficiencyCode-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        EfficiencyCode_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(
        EfficiencyCode_home
            .path()
...
async fn app_server_startup_skips_disabled_remote_plugin_catalog_scopes() -> Result<()> {
    ...
    write_remote_plugin_catalog_config(EfficiencyCode_home.path(), &base_url)?;
    write_chatgpt_auth(
        EfficiencyCode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_remote_plugins_disabled_config_with_base_url(EfficiencyCode_home.path(), &base_url)?;
    server.reset().await;
    mount_remote_plugin_list(&server, "GLOBAL", &global_body).await;
    mount_remote_plugin_list(&server, "USER", &user_body).await;
    mount_remote_plugin_list(&server, "WORKSPACE", &workspace_body).await;
...
-> Result<()> {
    ...
    write_openai_api_curated_marketplace(EfficiencyCode_home.path(), &["api-plugin"])?;
    login_with_api_key(
        EfficiencyCode_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
#[tokio::test]
async fn plugin_list_includes_api_curated_marketplace_for_bedrock_without_EfficiencyCode_auth()
-> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    std::fs::write(
        EfficiencyCode_home.path().join("config.toml"),
...
"#,
    ...
    write_openai_curated_marketplace(EfficiencyCode_home.path(), &["chatgpt-plugin"])?;
    write_openai_api_curated_marketplace(EfficiencyCode_home.path(), &["api-plugin"])?;
    ...
    assert!(!EfficiencyCode_home.path().join("auth.json").exists());
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "openai-api-curated")
...
"#,
    ...
    wait_for_path_exists(
        &EfficiencyCode_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.EfficiencyCode-plugin/plugin.json",
        ),
    )
    .await?;
    wait_for_remote_installed_snapshot_request(&server).await?;
...
"#,
    ...
    write_remote_plugin_test_auth(EfficiencyCode_home.path())?;
    ...
    wait_for_path_exists(&EfficiencyCode_home.path().join(
        "plugins/cache/workspace-directory/no-hooks/1.2.3/.EfficiencyCode-plugin/plugin.json",
    ))
    .await?;
...
async fn assert_disabled_remote_plugin_metadata(
    ...
    assert_eq!(
        plugin.availability,
        EfficiencyCode_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    write_openai_curated_marketplace(EfficiencyCode_home.path(), &["linear", "gmail"])?;
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(EfficiencyCode_home)?;
...
fn cached_remote_catalog_plugin_ids(EfficiencyCode_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = EfficiencyCode_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
fn remote_plugin_bundle_tar_gz_bytes(
    ...
    let mut entries = vec![
        (
            ".EfficiencyCode-plugin/plugin.json",
            manifest.as_bytes(),
            /*mode*/ 0o644,
        ),
        (
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(EfficiencyCode_home, marketplace_name, plugin_name, "local")
}
...
fn write_remote_plugin_test_auth(EfficiencyCode_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        EfficiencyCode_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_curated_marketplace(
    ...
    let curated_root = EfficiencyCode_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root =
            curated_root.join(format!("plugins/{plugin_name}/.EfficiencyCode-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(EfficiencyCode_home.join(".tmp"))?;
    std::fs::write(
        EfficiencyCode_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(EfficiencyCode_home.join(".tmp"))?;
    std::fs::write(
        EfficiencyCode_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
