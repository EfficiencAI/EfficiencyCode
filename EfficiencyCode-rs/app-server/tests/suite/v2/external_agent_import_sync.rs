...
const NATIVE_USER: &str = "native EfficiencyCode message";
const NATIVE_ASSISTANT: &str = "native EfficiencyCode answer";
...
impl ImportFixture {
    async fn new() -> Result<Self> {
        ...
        let session_path = EfficiencyCode_home
            .path()
            .join(concat!(".", "cla", "ude"))
        ...
        let app_server = start_app_server(EfficiencyCode_home.path()).await?;
        Ok(Self {
    ...
    async fn restart(&mut self) -> Result<()> {
        ...
        self.app_server = start_app_server(self._EfficiencyCode_home.path()).await?;
        Ok(())
    }
...
async fn start_app_server(EfficiencyCode_home: &Path) -> Result<TestAppServer> {
    let home = EfficiencyCode_home.display().to_string();
...
async fn cold_diverged_target_is_deferred_without_checkpoint() -> Result<()> {
    ...
    let mut fixture = ImportFixture::new().await?;
    MockResponsesConfig::new(&model.uri()).write(fixture._EfficiencyCode_home.path())?;
