...
use EfficiencyCode_app_server_protocol::SkillScope;
use EfficiencyCode_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = EfficiencyCode_home.join("plugins/cache/openai-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin"))?;
    std::fs::write(
        plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
fn write_cached_local_curated_plugin_with_skill(
    ...
    let plugin_root =
        EfficiencyCode_home.join("plugins/cache/openai-curated/google-calendar/local");
    std::fs::create_dir_all(plugin_root.join(".EfficiencyCode-plugin"))?;
    std::fs::write(
        plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
...
async fn skills_list_uses_each_cwds_bundled_skills_configuration() -> Result<()> {
    ...
    for (cwd, enabled) in [(disabled_cwd.path(), false), (enabled_cwd.path(), true)] {
        ...
        std::fs::create_dir_all(cwd.join(".EfficiencyCode"))?;
        std::fs::write(
            cwd.join(".EfficiencyCode/config.toml"),
            format!("[skills.bundled]\nenabled = {enabled}\n"),
        )?;
        set_project_trust_level(EfficiencyCode_home.path(), cwd, TrustLevel::Trusted)?;
    }
...
async fn skills_list_runtime_enable_refreshes_shared_system_skill_cache() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    let stale_skill_path = EfficiencyCode_home
        .path()
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(EfficiencyCode_home.path())?;
    std::fs::write(
        EfficiencyCode_home.path().join("config.toml"),
        format!(
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_EfficiencyCode_plugins_disabled()
-> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&EfficiencyCode_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace EfficiencyCode plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&EfficiencyCode_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".EfficiencyCode/skills/repo-skill");
    ...
    let mut mcp = TestAppServer::builder()
        .with_EfficiencyCode_home(EfficiencyCode_home.path())
        .without_auto_env()
        .with_env_overrides(&[(EfficiencyCode_EXEC_SERVER_URL_ENV_VAR, Some("none"))])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(EfficiencyCode_home.path().join(&relative_cwd))?;
...
async fn skills_list_preserves_requested_cwd_order() -> Result<()> {
    ...
    write_skill(&EfficiencyCode_home, "shared-skill")?;
    write_cached_local_curated_plugin_with_skill(EfficiencyCode_home.path())?;
    std::fs::write(
        EfficiencyCode_home.path().join("config.toml"),
        r#"[features]
...
"#,
    ...
    for (request_index, force_reload) in [false, false, true].into_iter().enumerate() {
        if request_index == 1 {
            write_skill(&EfficiencyCode_home, "new-skill")?;
        }
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".EfficiencyCode/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&EfficiencyCode_home, "demo")?;
