...
use EfficiencyCode_rollout::append_thread_name;
...
async fn thread_fork_creates_new_thread_and_emits_started() -> Result<()> {
    ...
    let original_path = EfficiencyCode_home
        .path()
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(EfficiencyCode_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(EfficiencyCode_home.path(), filename_ts, &conversation_id);
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    config.write(EfficiencyCode_home.path())?;
    ...
    assert!(matches!(
        ...
        [
            ...
            &response_item.item,
            EfficiencyCode_protocol::models::ResponseItem::Message { role, .. }
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_EfficiencyCode_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let EfficiencyCode_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(EfficiencyCode_home.path())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(EfficiencyCode_home_path.clone()),
            ..Default::default()
        })
        .await?;
    ...
    let fork_id = app_server
        .send_thread_fork_request(ThreadForkParams {
            ...
            path: Some(EfficiencyCode_home_path),
            ..Default::default()
        })
        .await?;
    let fork_err: JSONRPCError = timeout(
