...
use EfficiencyCode_app_server_protocol::AppInfo;
use EfficiencyCode_app_server_protocol::AppMetadata;
...
use EfficiencyCode_app_server_protocol::HookEventName;
...
use EfficiencyCode_app_server_protocol::ScheduledTaskSchedule;
use EfficiencyCode_app_server_protocol::ScheduledTaskSummary;
use EfficiencyCode_app_server_protocol::ScheduledTaskWeekday;
use EfficiencyCode_app_server_protocol::SkillInterface;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                EfficiencyCode_home.path().join("marketplace.json"),
            )?),
...
"#,
    ...
    write_installed_plugin(&EfficiencyCode_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(EfficiencyCode_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&EfficiencyCode_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "EfficiencyCode-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(EfficiencyCode_home.path().join(".tmp"))?;
    std::fs::write(
        EfficiencyCode_home
            .path()
...
async fn plugin_read_agent_plugin_excludes_nested_skills() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![EfficiencyCode_app_server_protocol::SkillSummary {
...
[plugins."demo-plugin@EfficiencyCode-curated"]
enabled = true

[hooks.state."demo-plugin@EfficiencyCode-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(response.plugin.marketplace_name, "EfficiencyCode-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(
        response.plugin.summary.id,
        "demo-plugin@EfficiencyCode-curated"
    );
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            EfficiencyCode_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@EfficiencyCode-curated:hooks/hooks.json:pre_tool_use:0:0"
                    .to_string(),
                event_name: HookEventName::PreToolUse,
            },
            EfficiencyCode_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@EfficiencyCode-curated:hooks/hooks.json:pre_tool_use:0:1"
                    .to_string(),
                event_name: HookEventName::PreToolUse,
            },
            EfficiencyCode_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@EfficiencyCode-curated:hooks/hooks.json:session_start:0:0"
                    .to_string(),
...
async fn plugin_read_batches_large_app_metadata_requests() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    write_connectors_config(EfficiencyCode_home.path(), &server_url)?;
    write_chatgpt_auth(
        EfficiencyCode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let EfficiencyCode_home = TempDir::new()?;
    write_connectors_config(EfficiencyCode_home.path(), &server.uri())?;
    write_chatgpt_auth(
        EfficiencyCode_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_installed_plugin(
    ...
    let plugin_root = EfficiencyCode_home
        ...
        .join(plugin_name)
        .join("local/.EfficiencyCode-plugin");
    std::fs::create_dir_all(&plugin_root)?;
    std::fs::write(
        plugin_root.join("plugin.json"),
...
fn write_plugins_enabled_config(EfficiencyCode_home: &TempDir) -> Result<()> {
    std::fs::write(
        EfficiencyCode_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn batch_apps(
    ...
    let product_sku_ok = headers
        .get("oai-product-sku")
        .and_then(|value| value.to_str().ok())
        .is_some_and(|value| value == "EfficiencyCode");
