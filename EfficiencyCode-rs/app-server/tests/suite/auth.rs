...
fn create_config_toml(EfficiencyCode_home: &Path) -> std::io::Result<()> {
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(EfficiencyCode_home.path())?;
...
async fn personal_access_token_without_email_supports_auth_status_and_account_read() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("OPENAI_API_KEY", None),
            ("EfficiencyCode_ACCESS_TOKEN", Some("at-test-token")),
            (
                "EfficiencyCode_AUTHAPI_BASE_URL",
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml_forced_login(EfficiencyCode_home.path(), "chatgpt")?;
