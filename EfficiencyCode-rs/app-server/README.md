# EfficiencyCode-app-server

`EfficiencyCode app-server` is the interface EfficiencyCode uses to power rich interfaces such as the [EfficiencyCode VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `EfficiencyCode app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--stdio` or `--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$EfficiencyCode_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
The unix socket transport is intended for local app-server control-plane clients. `EfficiencyCode app-server proxy`
opens exactly one raw stream connection to `$EfficiencyCode_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `EfficiencyCode app-server generate-ts`, or a JSON Schema bundle via `EfficiencyCode app-server generate-json-schema`. Each output is specific to the version of EfficiencyCode you used to run the command, so the generated artifacts are guaranteed to match that version.

```
EfficiencyCode app-server generate-ts --out DIR
EfficiencyCode app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and EfficiencyCode:

- **Thread**: A conversation between a user and the EfficiencyCode agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `EfficiencyCodeHome` for the server's EfficiencyCode home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
when a EfficiencyCode session is created by `thread/start`, `thread/resume`, or
`thread/fork`. EfficiencyCode advertises that profile in the downstream MCP
...
Applications building on top of `EfficiencyCode app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new EfficiencyCode integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-EfficiencyCode
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "EfficiencyCode_vscode",
      "title": "EfficiencyCode VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `memory/reset` — experimental; clear the current `EfficiencyCode_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `thread/realtime/start` — start a thread-scoped realtime session (experimental); pass `outputModality: "text"` or `outputModality: "audio"` to choose model output, optionally pass `model` and `version` to override configured realtime selection for this session only, pass `includeStartupContext: false` to omit EfficiencyCode's generated startup context, and optionally pass `initialItems` to seed V3 with complete role-bearing text messages at session creation. Pass `realtimeStartInstructions` and `realtimeEndInstructions` to control the developer instructions given to the backing EfficiencyCode model when this session starts and ends. Version `"v1"` uses legacy Bidi `conversation.handoff.*`, `"v2"` uses the Realtime Voice API, and `"v3"` preserves V1 EfficiencyCode Voice behavior while using Frameless Bidi `delegation.*`. For V3 automatic EfficiencyCode text, `EfficiencyCodeResponseHandoffMode` accepts `"thinking"` (the default; all output uses channel-less thinking appends), `"commentary"` (all output uses the commentary channel), or `"bemTags"` (the raw BEM envelope selects the API channel: BEM `analysis` and `commentary` use `commentary`, while BEM `final` and unparsable output use `speakable`). The BEM envelope remains in the appended text for the frontend model to interpret. V1 and V2 ignore this setting. For V3, pass `delegationAckFiller: false` to suppress the Realtime API's delegation acknowledgement filler or `true` to restore it; omitting the field preserves the Realtime API's default. V1 and V2 ignore `delegationAckFiller`. V3 handoffs do not prepend the legacy `"Agent Final Message"` label. Pass `clientManagedHandoffs: true` to disable automatic EfficiencyCode response delivery so only the client's explicit append calls produce handoffs. Pass `EfficiencyCodeResponsesAsItems: true` to send automatic EfficiencyCode responses as realtime conversation items instead, and optionally pass `EfficiencyCodeResponseItemPrefix` to prepend experiment instructions to those items. Returns `{}` and streams `thread/realtime/*` notifications. Omit `transport` for the websocket transport, or pass `{ "type": "webrtc", "sdp": "..." }` to create a Bidi WebRTC session from a browser-generated SDP offer; the remote answer SDP is emitted as `thread/realtime/sdp`. Conversation `version: "v2"` requests remain unsupported for WebRTC.
...
- `process/spawn` — experimental; spawn a standalone process without the EfficiencyCode sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
Start a fresh thread when you need a new EfficiencyCode conversation.

```json
{ "method": "thread/start", "id": 10, "params": {
    // Optionally set config settings. If not specified, will use the user's
    // current config settings.
    "model": "gpt-5.1-EfficiencyCode",
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current EfficiencyCode home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text, images, or audio) to a thread and trigger EfficiencyCode generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.EfficiencyCode/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a EfficiencyCode session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use EfficiencyCode's default realtime backend prompt. Send `prompt: null` or
...
the backing EfficiencyCode model when the thread enters realtime mode, and
`realtimeEndInstructions` to provide its developer instructions when the session
ends. These instructions configure EfficiencyCode, not the realtime frontend model. They
...
field preserves EfficiencyCode's existing default instructions.
...
Pass `includeStartupContext: false` to skip EfficiencyCode's startup context for this
...
Pass `clientManagedHandoffs: true` to suppress automatic EfficiencyCode response handoffs
...
Pass `EfficiencyCodeResponsesAsItems: true` to inject automatic EfficiencyCode responses with
`conversation.item.create` instead of the protocol's default speakable output
path. When using that mode, `EfficiencyCodeResponseItemPrefix` can prepend short
experiment instructions to each automatic EfficiencyCode response item. Omit
`EfficiencyCodeResponsesAsItems`, or pass `false`, to preserve the default speakable
behavior. In V3, automatic handoffs default to
`EfficiencyCodeResponseHandoffMode: "thinking"`, which omits the context append `channel`
...
`EfficiencyCodeResponseHandoffChannelPrefixes` to override the accepted prefixes for
...
clients may continue to send the removed `EfficiencyCodeResponseHandoffPrefix` field; the
...
Use `review/start` to run EfficiencyCode’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions EfficiencyCode will run).
...
For an inline review, EfficiencyCode streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). EfficiencyCode will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, EfficiencyCode generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the EfficiencyCode sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a EfficiencyCode session/thread-group id.
...
- `turn/completed` — `{ turn }` where `turn.status` is `completed`, `interrupted`, or `failed`; successful turns include their final agent message when available, and failures carry `{ error: { message, EfficiencyCodeErrorInfo?, additionalDetails? } }`.
...
- `contextCompaction` — `{id}` emitted when EfficiencyCode compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when EfficiencyCode compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
`error` event is emitted whenever the server hits an error mid-turn (for example, upstream model errors or quota limits). Carries the same `{ error: { message, EfficiencyCodeErrorInfo?, additionalDetails? } }` payload as `turn.status: "failed"` and may precede that terminal notification.

`EfficiencyCodeErrorInfo` maps to the `EfficiencyCodeErrorInfo` enum. Common values:
...
When an upstream HTTP status is available (for example, from the Responses API or a provider), it is forwarded in `httpStatusCode` on the relevant `EfficiencyCodeErrorInfo` variant.

## Approvals

Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell EfficiencyCode whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
`EfficiencyCode_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.EfficiencyCode/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a EfficiencyCode skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a EfficiencyCode skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.EfficiencyCode/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.EfficiencyCode/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.EfficiencyCode/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.EfficiencyCode/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.EfficiencyCode/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
EfficiencyCode supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`. Self-serve Business ProLite accounts use the `self_serve_business_prolite` plan type; Enterprise automation accounts use `enterprise_cbp_automation`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): EfficiencyCode owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; EfficiencyCode persists tokens to disk and refreshes them automatically.
- **EfficiencyCode managed Amazon Bedrock auth (`amazonBedrock`, experimental)**: Caller supplies an Amazon Bedrock API key and region via `account/login/start` with `type: "amazonBedrock"`. The client must enable the `experimentalApi` initialization capability for EfficiencyCode-managed Amazon Bedrock login. EfficiencyCode replaces the current primary auth with the Bedrock credential and writes `model_provider = "amazon-bedrock"` to the user config.
- **Personal access token (`personalAccessToken`)**: EfficiencyCode uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `EfficiencyCode login --with-access-token` or `EfficiencyCode_ACCESS_TOKEN`.
...
{ "id": 1, "result": { "account": { "type": "amazonBedrock", "usesEfficiencyCodeManagedCredentials": false }, "requiresOpenaiAuth": false } }
```
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, EfficiencyCode can run without OpenAI credentials.
- Amazon Bedrock reports `usesEfficiencyCodeManagedCredentials: true` when it uses a Bedrock API key managed by EfficiencyCode. It reports `false` for external credential paths, including the AWS credential chain and configured command auth. This identifies whether EfficiencyCode-managed credentials are selected; it does not validate that the credential source can resolve credentials.
...
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   ...
   organization setup to the hosted EfficiencyCode success page instead. When hosted login success is
   enabled, clients may set `appBrand` to `"EfficiencyCode"` or `"chatgpt"` to select the matching hosted
   page artwork; omitted or `null` values default to `"EfficiencyCode"`.
...
1. Start:
   ...
   { "id": 4, "result": { "type": "chatgptDeviceCode", "loginId": "<uuid>", "verificationUrl": "https://auth.openai.com/EfficiencyCode/device", "userCode": "ABCD-1234" } }
   ```
...
When using a EfficiencyCode-managed Bedrock key, logout removes the key and clears `model_provider` if it is still set to `"amazon-bedrock"`. When using AWS-managed credentials, manage them through AWS or switch providers before logging out.
      ...
      "credits": [
        {
          ...
          "resetType": "EfficiencyCodeRateLimits",
          "status": "available",
...
`EfficiencyCode app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
EfficiencyCode app-server generate-ts --out DIR --experimental
EfficiencyCode app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p EfficiencyCode-app-server-protocol
   ```
