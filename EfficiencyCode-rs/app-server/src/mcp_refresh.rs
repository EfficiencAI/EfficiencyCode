...
mod tests {
    ...
    use EfficiencyCode_config::ThreadConfigContext;
    use EfficiencyCode_config::ThreadConfigLoadError;
    use EfficiencyCode_config::ThreadConfigLoadErrorCode;
    ...
    use EfficiencyCode_extension_api::NoopExtensionEventSink;
    ...
    async fn refresh_config_preserves_thread_mcp_overrides() -> anyhow::Result<()> {
        ...
        let thread_config = initial_config_manager
            ...
            .await?;
        let thread = thread_manager
            .start_thread(EfficiencyCode_core::StartThreadOptions::new(thread_config))
            .await?
            .thread;
        std::fs::write(
            temp_dir
...
"#,
        ...
        actual.remove(EfficiencyCode_mcp::EfficiencyCode_APPS_MCP_SERVER_NAME);
...
"#,
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                EfficiencyCode_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        goal_service: Arc::new(EfficiencyCode_goal_extension::GoalService::new()),
                ...
                Arc::new(EfficiencyCodeHomeUserInstructionsProvider::new(
                    good_config.EfficiencyCode_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                EfficiencyCode_core::local_agent_graph_store_from_state_db(Some(&state_db)),
        ...
        thread_manager
            .start_thread(EfficiencyCode_core::StartThreadOptions::new(good_config))
            .await?;
        thread_manager
            .start_thread(EfficiencyCode_core::StartThreadOptions::new(bad_config))
            .await?;
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> EfficiencyCode_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
