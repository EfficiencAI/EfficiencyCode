...
use EfficiencyCode_cloud_config::cloud_config_bundle_loader;
...
/// Shared app-server entry point for loading effective EfficiencyCode configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(EfficiencyCode_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            .EfficiencyCode_home(self.EfficiencyCode_home.clone())
            .cli_overrides(self.current_cli_overrides())
            .loader_overrides(loader_overrides)
            .fallback_cwd(Some(self.EfficiencyCode_home.clone()))
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = EfficiencyCode_core::config::ConfigBuilder::default()
            .EfficiencyCode_home(self.EfficiencyCode_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.EfficiencyCode_self_exe = self.arg0_paths.EfficiencyCode_self_exe.clone();
        config.EfficiencyCode_linux_sandbox_exe =
            self.arg0_paths.EfficiencyCode_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(EfficiencyCode_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(EfficiencyCode_home: PathBuf) -> Self {
        Self::new_for_tests(
            EfficiencyCode_home,
            Vec::new(),
