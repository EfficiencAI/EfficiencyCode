...
use EfficiencyCode_connectors::connector_runtime_cache_path;
use EfficiencyCode_feedback::EfficiencyCode_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME;
use EfficiencyCode_feedback::EfficiencyCode_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME;
#[cfg(target_os = "windows")]
use EfficiencyCode_feedback::WINDOWS_SANDBOX_LOG_ATTACHMENT_FILENAME;
...
pub(crate) struct FeedbackRequestProcessor {
    ...
    config: Arc<Config>,
    feedback: EfficiencyCodeFeedback,
...
impl FeedbackRequestProcessor {
    ...
    async fn upload_feedback_response(
        ...
        if let Some(chatgpt_user_id) = auth
            .as_ref()
            .and_then(EfficiencyCode_login::EfficiencyCodeAuth::get_chatgpt_user_id)
        {
            tracing::info!(target: "feedback_tags", chatgpt_user_id);
        }
        if let Some(account_id) = auth
            .as_ref()
            .and_then(EfficiencyCode_login::EfficiencyCodeAuth::get_account_id)
        {
        ...
        if include_logs {
            ...
            if let Some(sandbox_log_attachment) =
                windows_sandbox_log_attachment(&self.config.EfficiencyCode_home)
...
fn tool_cache_feedback_attachments(
    ...
    let tools_cache_path = connector_runtime_cache_path(EfficiencyCode_home, auth);
    if tools_cache_path.is_file() {
        attachments.push(FeedbackAttachmentPath {
            path: tools_cache_path,
            attachment_filename_override: Some(
                EfficiencyCode_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME.to_string(),
            ),
        });
    }
    ...
    if directory_cache_path.is_file() {
        attachments.push(FeedbackAttachmentPath {
            ...
            attachment_filename_override: Some(
                EfficiencyCode_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME.to_string(),
            ),
        });
    }
...
#[cfg(target_os = "windows")]
fn windows_sandbox_log_attachment(EfficiencyCode_home: &Path) -> Option<FeedbackAttachmentPath> {
...
fn windows_sandbox_log_attachment(_EfficiencyCode_home: &Path) -> Option<FeedbackAttachmentPath> {
    None
}
...
mod tests {
        ...
        let mut lines = vec![RolloutLine {
            ...
            item: RolloutItem::SessionMeta(SessionMetaLine {
                meta: EfficiencyCode_protocol::protocol::SessionMeta {
                    ...
                    base_instructions: Some(EfficiencyCode_protocol::models::BaseInstructions {
        ...
        lines.extend(turns.iter().map(|(turn_id, model, effort)| {
            RolloutLine {
                ...
                item: RolloutItem::TurnContext(TurnContextItem {
                    ...
                    current_date: None,
                    timezone: None,
                    approval_policy: EfficiencyCode_protocol::protocol::AskForApproval::Never,
                    approvals_reviewer: None,
                    sandbox_policy:
                        EfficiencyCode_protocol::protocol::SandboxPolicy::new_read_only_policy(),
    ...
    fn tool_cache_feedback_attachments_include_existing_active_cache_files() {
        let EfficiencyCode_home = tempfile::tempdir().expect("create tempdir");
        let auth = EfficiencyCodeAuth::create_dummy_chatgpt_auth_for_testing();
        let tools_cache_path =
            connector_runtime_cache_path(EfficiencyCode_home.path(), Some(&auth));
        ...
        assert_eq!(
            ...
            vec![
                (
                    tools_cache_path,
                    Some(EfficiencyCode_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME.to_string()),
                ),
                (
                    directory_cache_path,
                    Some(EfficiencyCode_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME.to_string()),
                ),
            ]
        );
    }
    ...
    fn tool_cache_feedback_attachments_include_directory_cache_without_account_id() {
        let EfficiencyCode_home = tempfile::tempdir().expect("create tempdir");
        let auth = EfficiencyCodeAuth::Headers(EfficiencyCode_login::AuthHeaders::new(
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let EfficiencyCode_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = EfficiencyCode_windows_sandbox::sandbox_dir(EfficiencyCode_home.path());
        ...
        let attachment = windows_sandbox_log_attachment(EfficiencyCode_home.path())
            .map(|attachment| (attachment.path, attachment.attachment_filename_override));
