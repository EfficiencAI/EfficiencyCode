...
use EfficiencyCode_app_server_protocol::AutoReviewRequirements;
use EfficiencyCode_app_server_protocol::BrowserUseRequirements;
use EfficiencyCode_app_server_protocol::ClientResponsePayload;
use EfficiencyCode_app_server_protocol::ComputerUseRequirements;
...
use EfficiencyCode_app_server_protocol::ConfigRequirements;
...
use EfficiencyCode_app_server_protocol::ConfigWriteErrorCode;
use EfficiencyCode_app_server_protocol::ConfigWriteResponse;
use EfficiencyCode_app_server_protocol::ConfiguredHookHandler;
use EfficiencyCode_app_server_protocol::ConfiguredHookMatcherGroup;
use EfficiencyCode_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use EfficiencyCode_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use EfficiencyCode_app_server_protocol::FeedbackRequirements;
use EfficiencyCode_app_server_protocol::JSONRPCErrorError;
use EfficiencyCode_app_server_protocol::ManagedHooksRequirements;
use EfficiencyCode_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use EfficiencyCode_app_server_protocol::ModelsRequirements;
use EfficiencyCode_app_server_protocol::NetworkDomainPermission;
use EfficiencyCode_app_server_protocol::NetworkRequirements;
use EfficiencyCode_app_server_protocol::NetworkUnixSocketPermission;
...
use EfficiencyCode_config::HookEventsToml;
use EfficiencyCode_config::HookHandlerConfig as CoreHookHandlerConfig;
use EfficiencyCode_config::ManagedHooksRequirementsToml;
use EfficiencyCode_config::MatcherGroup as CoreMatcherGroup;
use EfficiencyCode_config::ResidencyRequirement as CoreResidencyRequirement;
use EfficiencyCode_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use EfficiencyCode_core::ThreadManager;
use EfficiencyCode_features::canonical_feature_for_key;
use EfficiencyCode_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<EfficiencyCode_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes =
            EfficiencyCode_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(EfficiencyCode_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(EfficiencyCode_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            EfficiencyCode_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            EfficiencyCode_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: EfficiencyCode_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: EfficiencyCode_config::BrowserUseRequirementsToml,
...
) -> EfficiencyCode_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => {
            EfficiencyCode_app_server_protocol::ResidencyRequirement::Us
        }
    }
}

fn map_network_requirements_to_api(
    network: EfficiencyCode_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(EfficiencyCode_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(EfficiencyCode_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(EfficiencyCode_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: EfficiencyCode_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        EfficiencyCode_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        EfficiencyCode_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: EfficiencyCode_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        EfficiencyCode_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        EfficiencyCode_config::NetworkUnixSocketPermissionToml::Deny => {
...
mod tests {
    ...
    use EfficiencyCode_app_server_protocol::FeedbackRequirements;
    use EfficiencyCode_app_server_protocol::WindowsSandboxSetupMode;
    use EfficiencyCode_config::AutoReviewRequirementsToml;
    use EfficiencyCode_config::ComputerUseRequirementsToml;
    use EfficiencyCode_config::ConfigRequirementsToml;
    use EfficiencyCode_config::ModelsRequirementsToml;
    use EfficiencyCode_config::NewThreadModelDefaultsToml;
    use EfficiencyCode_config::WindowsRequirementsToml;
