...
use EfficiencyCode_app_server_protocol::SelectedCapabilityRoot;
use EfficiencyCode_app_server_protocol::ThreadSection;
use EfficiencyCode_app_server_protocol::ThreadSectionAppearance;
use EfficiencyCode_app_server_protocol::ThreadSectionMoveParams;
use EfficiencyCode_app_server_protocol::ThreadSectionMoveResponse;
use EfficiencyCode_extension_api::ExtensionDataInit;
use EfficiencyCode_extension_api::ThreadIdleCause;
...
const EfficiencyCode_TUI_CLIENT_NAME: &str = "EfficiencyCode-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: EfficiencyCode_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                EfficiencyCode_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn latest_persisted_approval_policy(
    history: &[RolloutItem],
) -> Option<EfficiencyCode_protocol::protocol::AskForApproval> {
    history
        .iter()
        .enumerate()
        .rev()
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = EfficiencyCode_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(EfficiencyCode_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<EfficiencyCodeThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            EfficiencyCode_home: self.config.EfficiencyCode_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(EfficiencyCode_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<EfficiencyCode_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<EfficiencyCode_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                EfficiencyCode_core::config::set_project_trust_level(
                    &listener_task_context.EfficiencyCode_home,
        ...
        if let Ok(Some(err)) =
            EfficiencyCode_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(EfficiencyCode_app_server_protocol::ThreadStartSource::Startup)
                {
                    EfficiencyCode_app_server_protocol::ThreadStartSource::Startup => {
                        InitialHistory::New
                    }
                    EfficiencyCode_app_server_protocol::ThreadStartSource::Clear => {
            ...
            .map_err(|err| match err.details() {
                EfficiencyCodeErrorDetails::InvalidRequest(message) => {
                    invalid_request(message.clone())
                }
                EfficiencyCodeErrorDetails::UnsupportedOperation(message) => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<EfficiencyCode_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<EfficiencyCode_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(EfficiencyCode_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(EfficiencyCode_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            EfficiencyCode_linux_sandbox_exe: self
                .arg0_paths
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = EfficiencyCode_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.EfficiencyCode_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.EfficiencyCode_home.display()
                ))
            })?;
    ...
    async fn load_live_thread_view(
        ...
        thread_id: ThreadId,
        include_turns: bool,
        loaded_thread: &EfficiencyCodeThread,
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                EfficiencyCode_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<EfficiencyCode_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                thread_id,
                thread: EfficiencyCode_thread,
                session_configured,
                ..
            }) => {
                if let Err(err) = Self::set_app_server_client_info(
                    EfficiencyCode_thread.as_ref(),
                    app_server_client_name,
                    app_server_client_version,
                )
                .await
                ...
                let instruction_sources = EfficiencyCode_thread.legacy_instruction_sources().await;
                ...
                thread.thread_source = EfficiencyCode_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = EfficiencyCode_thread.config_snapshot().await;
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(EfficiencyCode_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    EfficiencyCode_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    EfficiencyCode_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => EfficiencyCode_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(EfficiencyCode_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            EfficiencyCodeErrorDetails::Io(_) | EfficiencyCodeErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: EfficiencyCode_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        EfficiencyCode_error_info: error.EfficiencyCode_error_info,
...
pub(super) fn core_thread_write_error(
    operation: &str,
    err: EfficiencyCodeErr,
) -> JSONRPCErrorError {
    match err.details() {
        EfficiencyCodeErrorDetails::ThreadNotFound(thread_id) => {
...
pub(crate) fn thread_from_stored_thread(
    ...
    Thread,
    Option<EfficiencyCode_thread_store::StoredThreadHistory>,
) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<EfficiencyCode_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(EfficiencyCode_protocol::protocol::SessionSource::Unknown);
    ...
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => {
                match EfficiencyCode_core::parse_turn_item(&item.item) {
                    Some(EfficiencyCode_protocol::items::TurnItem::UserMessage(user)) => {
                        Some(user.message())
                    }
                    _ => None,
                }
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            EfficiencyCode_protocol::config_types::SandboxMode::WorkspaceWrite
                | EfficiencyCode_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &EfficiencyCode_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        EfficiencyCode_protocol::models::PermissionProfile::Disabled
        | EfficiencyCode_protocol::models::PermissionProfile::External { .. } => true,
        EfficiencyCode_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<EfficiencyCode_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
