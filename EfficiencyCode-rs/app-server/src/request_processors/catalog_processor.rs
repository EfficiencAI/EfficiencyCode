use super::*;
use EfficiencyCode_core::config::permission_profile_catalog;
...
) -> Vec<EfficiencyCode_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            EfficiencyCode_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    EfficiencyCode_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(
                                |tool| EfficiencyCode_app_server_protocol::SkillToolDependency {
                                    r#type: tool.r#type,
                                    value: tool.value,
...
fn hooks_to_info(hooks: &[EfficiencyCode_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[EfficiencyCode_skills::SkillError],
) -> Vec<EfficiencyCode_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| EfficiencyCode_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
    ...
    async fn experimental_feature_list_response(
        ...
        let workspace_EfficiencyCode_plugins_enabled = self
            ...
            .map(|spec| {
                ...
                ApiExperimentalFeature {
                    ...
                    enabled: config.features.enabled(spec.id)
                        && (workspace_EfficiencyCode_plugins_enabled
    ...
    async fn skills_list_response(
        ...
        let (effective_skill_roots, plugin_skill_snapshots) =
            if workspace_EfficiencyCode_plugins_enabled {
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                EfficiencyCode_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![
                                        EfficiencyCode_app_server_protocol::SkillErrorInfo {
                                            path: error_path,
                                            message,
                                        },
                                    ],
                    ...
                    let skills_input = EfficiencyCode_skills_extension::HostSkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(EfficiencyCode_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![EfficiencyCode_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let plugin_hooks = if plugins_enabled {
                ...
                EfficiencyCode_core_plugins::PluginHookLoadOutcome {
                ...
                EfficiencyCode_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = EfficiencyCode_hooks::list_hooks(EfficiencyCode_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.EfficiencyCode_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {
