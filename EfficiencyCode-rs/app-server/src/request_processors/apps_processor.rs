...
impl AppsRequestProcessor {
    ...
    async fn apps_list_task(
        ...
        let should_retry = result
            .as_ref()
            .is_ok_and(|(_, EfficiencyCode_apps_ready)| !EfficiencyCode_apps_ready);
        outgoing
        ...
        if should_retry && !retry_params.force_refetch {
            ...
            {
                warn!(
                    "failed to refresh app list after EfficiencyCode-apps readiness retry: {err:?}"
                );
            }
        }
    }
    ...
    async fn apps_list_response(
        ...
        let mut EfficiencyCode_apps_ready = true;
        ...
        loop {
            ...
            match result {
                AppListLoadResult::Accessible(Ok(status)) => {
                    ...
                    EfficiencyCode_apps_ready = status.EfficiencyCode_apps_ready;
                }
            ...
            if accessible_loaded && all_loaded {
                ...
                return Ok((response, EfficiencyCode_apps_ready));
            }
        }
    }
...
const APPS_INSTALLED_DURATION_METRIC: &str = "EfficiencyCode.apps.installed.duration_ms";
