...
use EfficiencyCode_app_server_protocol::ExperimentalApi;
...
use EfficiencyCode_app_server_protocol::experimental_required_message;
...
use EfficiencyCode_thread_store::LocalQueueStore;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: EfficiencyCodeFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let executor_skill_provider: Arc<dyn EfficiencyCode_skills_extension::SkillProvider> = Arc::new(
            EfficiencyCode_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                EfficiencyCode_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let EfficiencyCode_request = deserialize_client_request(request);
                let result = match EfficiencyCode_request {
                    Ok(EfficiencyCode_request) => {
    ...
    async fn handle_client_request(
        ...
        EfficiencyCode_request: ClientRequest,
        session: Arc<ConnectionSessionState>,
        ...
        if let ClientRequest::Initialize { request_id, params } = EfficiencyCode_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = EfficiencyCode_request.experimental_reason()
        ...
        let serialization_scope = EfficiencyCode_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: EfficiencyCode_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> =
            match EfficiencyCode_request {
