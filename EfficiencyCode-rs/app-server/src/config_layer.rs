use EfficiencyCode_app_server_protocol::ConfigLayer as ApiConfigLayer;
use EfficiencyCode_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use EfficiencyCode_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use EfficiencyCode_config::ConfigLayer;
use EfficiencyCode_config::ConfigLayerMetadata;
use EfficiencyCode_config::ConfigLayerSource;

/// Converts a config-layer source owned by `EfficiencyCode-config` into the app-server wire type owned by
/// `EfficiencyCode-app-server-protocol`.
///
...
/// Converts config-layer metadata owned by `EfficiencyCode-config` into the app-server wire type owned by
/// `EfficiencyCode-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `EfficiencyCode-config` into the app-server wire type owned by
/// `EfficiencyCode-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
