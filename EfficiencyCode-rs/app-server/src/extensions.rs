...
use EfficiencyCode_extension_api::AgentSpawnFuture;
use EfficiencyCode_extension_api::AgentSpawner;
...
use EfficiencyCode_thread_store::QueueStore;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn EfficiencyCode_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = EfficiencyCodeErr> + 'static,
{
    ...
    if let Some(queue) = queue_store {
        let queue_service = Arc::new(EfficiencyCode_queue_extension::QueuedItemService::new(
            queue,
            thread_manager.clone(),
            event_sink,
        ));
        EfficiencyCode_queue_extension::install(&mut builder, queue_service);
    }
    if let Some(state_db) = state_db {
        EfficiencyCode_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config
                    .features
                    .enabled(EfficiencyCode_features::Feature::Goals),
    ...
    EfficiencyCode_guardian::install(&mut builder, guardian_agent_spawner);
    EfficiencyCode_memories_extension::install(&mut builder, EfficiencyCode_otel::global());
    EfficiencyCode_mcp_extension::install(&mut builder);
    EfficiencyCode_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    EfficiencyCode_web_search_extension::install(&mut builder, auth_manager.clone());
    ...
    let skill_providers = EfficiencyCode_skills_extension::SkillProviders::new()
        ...
        .with_orchestrator_provider(Arc::new(
            EfficiencyCode_skills_extension::OrchestratorSkillProvider::new(),
        ))
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = EfficiencyCodeErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, EfficiencyCodeErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                EfficiencyCodeErr::UnsupportedOperation("thread manager dropped".to_string())
            })?;
            thread_manager
...
#[cfg(test)]
mod tests {
    use EfficiencyCode_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use EfficiencyCode_protocol::protocol::ThreadGoalStatus;
    use EfficiencyCode_protocol::protocol::ThreadGoalUpdatedEvent;
