...
use EfficiencyCode_app_server_protocol::Config as ApiConfig;
...
use EfficiencyCode_app_server_protocol::OverriddenMetadata;
...
use EfficiencyCode_config::ConfigLayerMetadata;
...
use EfficiencyCode_config::ShellEnvironmentPolicyFilterRepresentation;
use EfficiencyCode_config::config_toml::ConfigToml;
use EfficiencyCode_config::merge_toml_values;
use EfficiencyCode_config::shell_environment_filter_entry;
use EfficiencyCode_config::validate_shell_environment_policy_filter_config;
use EfficiencyCode_core::config::deserialize_config_toml_with_base;
use EfficiencyCode_core::config::edit::ConfigEdit;
use EfficiencyCode_core::config::edit::ConfigEditsBuilder;
use EfficiencyCode_core::config::validate_feature_requirements_for_config_toml;
use EfficiencyCode_core::path_utils;
use EfficiencyCode_core::path_utils::SymlinkWritePaths;
use EfficiencyCode_core::path_utils::resolve_symlink_write_paths;
use EfficiencyCode_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.EfficiencyCode_home())
                .map_err(|err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .EfficiencyCode/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project {
            ...
            dot_EfficiencyCode_folder.display(),
        ),
