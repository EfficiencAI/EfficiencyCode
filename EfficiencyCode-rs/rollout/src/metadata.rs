...
use EfficiencyCode_state::BackfillState;
use EfficiencyCode_state::BackfillStats;
use EfficiencyCode_state::BackfillStatus;
use EfficiencyCode_state::DB_ERROR_METRIC;
use EfficiencyCode_state::DB_METRIC_BACKFILL;
use EfficiencyCode_state::DB_METRIC_BACKFILL_DURATION_MS;
use EfficiencyCode_state::ExtractionOutcome;
use EfficiencyCode_state::ThreadMetadataBuilder;
use EfficiencyCode_state::apply_rollout_item;
use chrono::DateTime;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = EfficiencyCode_otel::global();
    ...
    let sessions_root = EfficiencyCode_home.join(SESSIONS_SUBDIR);
    let archived_root = EfficiencyCode_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(EfficiencyCode_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(EfficiencyCode_home: &Path, path: &Path) -> String {
    path.strip_prefix(EfficiencyCode_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
