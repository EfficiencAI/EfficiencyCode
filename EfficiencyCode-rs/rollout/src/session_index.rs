...
pub async fn append_thread_name(
    ...
    append_session_index_entry(EfficiencyCode_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(EfficiencyCode_home);
...
fn session_index_path(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home.join(SESSION_INDEX_FILE)
}
