...
pub use EfficiencyCode_state::LogEntry;
...
pub type StateDbHandle = Arc<EfficiencyCode_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        EfficiencyCode_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    EfficiencyCode_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == EfficiencyCode_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, EfficiencyCode_home, default_model_provider_id)
                .await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match EfficiencyCode_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> EfficiencyCode_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == EfficiencyCode_state::BackfillStatus::Complete => {
            Some(runtime)
        }
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<EfficiencyCode_state::Anchor> {
    let cursor = cursor?;
    ...
    Some(EfficiencyCode_state::Anchor {
        ts,
        id: cursor.thread_id(),
    })
}
...
pub async fn list_thread_ids_db(
    context: Option<&EfficiencyCode_state::StateRuntime>,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => EfficiencyCode_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => EfficiencyCode_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => EfficiencyCode_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<EfficiencyCode_state::ThreadRelationFilter>,
    archived: bool,
    section: Option<Option<&str>>,
    search_term: Option<&str>,
) -> Option<EfficiencyCode_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    if let Some(relation_filter) = relation_filter {
        let filters = EfficiencyCode_state::ThreadFilterOptions {
    ...
    Some(EfficiencyCode_state::ThreadsPage {
        items,
