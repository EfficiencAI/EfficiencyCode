//! Persist EfficiencyCode session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use EfficiencyCode_protocol::protocol::GitInfo as ProtocolGitInfo;
use EfficiencyCode_protocol::protocol::HistoryPosition;
use EfficiencyCode_protocol::protocol::MultiAgentVersion;
use EfficiencyCode_protocol::protocol::SessionContextWindow;
...
///
/// ```ignore
/// $ jq -C . ~/.EfficiencyCode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.EfficiencyCode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided EfficiencyCode home directory.
    #[allow(clippy::too_many_arguments)]
    ...
    async fn list_threads_with_db_fallback(
        ...
        let EfficiencyCode_home = config.EfficiencyCode_home();
        ...
        if state_db_ctx.is_none() {
            ...
            EfficiencyCode_state::record_fallback(
                "list_threads",
                "db_unavailable",
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = EfficiencyCode_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(EfficiencyCode_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(EfficiencyCode_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.EfficiencyCode/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.EfficiencyCode_home().to_path_buf();
...
impl From<EfficiencyCode_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: EfficiencyCode_state::ThreadsPage) -> Self {
...
fn thread_item_from_state_metadata(
    item: EfficiencyCode_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &EfficiencyCode_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
