...
//! existing rollout path. They must not do that at the same time for one EfficiencyCode home, so they share
...
/// Try to exclude rollout compression and migration for one EfficiencyCode home.
pub fn try_acquire_rollout_maintenance_lock(
    EfficiencyCode_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = EfficiencyCode_home.join(".tmp");
