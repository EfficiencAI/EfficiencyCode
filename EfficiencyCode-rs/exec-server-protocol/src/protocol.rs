...
pub use EfficiencyCode_file_system::WalkOptions;
pub use EfficiencyCode_file_system::WalkOutcome;
...
use EfficiencyCode_shell_command::shell_detect::DetectedShell;
...
impl EnvironmentInfo {
    ...
    pub fn local() -> Self {
        ...
        Self {
            shell: EfficiencyCode_shell_command::shell_detect::default_user_shell().into(),
...
pub struct ShellInfo {
    /// Stable shell name, for example `zsh`, `bash`, `powershell`, `winuxsh`, `sh`, or `cmd`.
    pub name: String,
...
pub struct ExecParams {
    ...
    #[serde(default)]
    pub pipe_stdin: bool,
    /// Optional process-visible argv0 override. Values such as `EfficiencyCode-linux-sandbox` are command
...
mod tests {
    ...
    use EfficiencyCode_network_proxy::RemoteNetworkProxyLaunchConfig;
    use EfficiencyCode_protocol::config_types::WindowsSandboxProxySettingsMode;
    ...
    fn local_environment_info_resolves_relative_temporary_directory() {
        if std::env::var_os("EfficiencyCode_TEST_RELATIVE_TMPDIR").is_none() {
            let status = std::process::Command::new(std::env::current_exe().expect("test binary"))
                .arg("--exact")
                .arg(
                    "protocol::tests::local_environment_info_resolves_relative_temporary_directory",
                )
                .env("EfficiencyCode_TEST_RELATIVE_TMPDIR", "1")
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::ProjectRoots {
                            subpath: Some(".EfficiencyCode".into()),
                        },
                    },
                    FileSystemAccessMode::Read,
                ),
