...
use EfficiencyCode_login::AuthEnvTelemetry;
...
/// Filename used for the redacted `EfficiencyCode doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "EfficiencyCode-doctor-report.json";
/// Filename used for the raw EfficiencyCode Apps MCP tools cache feedback attachment.
pub const EfficiencyCode_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str =
    "EfficiencyCode-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const EfficiencyCode_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str =
    "EfficiencyCode-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_EfficiencyCode_api_key_present = tracing::field::debug(auth_env.EfficiencyCode_api_key_env_present),
        auth_env_EfficiencyCode_api_key_enabled = tracing::field::debug(auth_env.EfficiencyCode_api_key_env_enabled),
...
#[derive(Clone)]
pub struct EfficiencyCodeFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for EfficiencyCodeFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl EfficiencyCodeFeedback {
    pub fn new() -> Self {
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("EfficiencyCode_http_client::transport", LevelFilter::DEBUG)
                    .with_target("EfficiencyCode_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target(
                        "EfficiencyCode_api::responses_websocket_timing",
                        LevelFilter::OFF,
                    )
                    .with_target(
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `EfficiencyCode-logs.log` and before path-backed rollout
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("EfficiencyCode-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = EfficiencyCodeFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        let fb = EfficiencyCodeFeedback::new();
        ...
        tracing::trace!(target: "EfficiencyCode_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "EfficiencyCode_http_client::transport", "transport-trace");
        tracing::trace!(target: "EfficiencyCode_api::sse", "sse-trace");
        tracing::trace!(target: "EfficiencyCode_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "EfficiencyCode_http_client::transport", "transport-debug");
        tracing::debug!(target: "EfficiencyCode_api::sse::responses", "sse-debug");
        tracing::trace!(target: "EfficiencyCode_feedback_test", "unrelated-trace");
        log::trace!(target: "EfficiencyCode_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("EfficiencyCode-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = EfficiencyCodeFeedback::new()
        ...
        let attachments_without_diagnostics = EfficiencyCodeFeedback::new()
            .snapshot(/*session_id*/ None)
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("EfficiencyCode-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("EfficiencyCode-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = EfficiencyCodeFeedback::new()
            .snapshot(/*session_id*/ None)
