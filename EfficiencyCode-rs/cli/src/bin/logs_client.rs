...
use EfficiencyCode_state::LogQuery;
use EfficiencyCode_state::LogRow;
...
#[derive(Debug, Parser)]
#[command(name = "EfficiencyCode-state-logs")]
#[command(about = "Tail EfficiencyCode logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to EfficiencyCode_HOME. Defaults to $EfficiencyCode_HOME or ~/.EfficiencyCode.
    #[arg(long, env = "EfficiencyCode_HOME")]
    EfficiencyCode_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --EfficiencyCode-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(EfficiencyCode_home) = args.EfficiencyCode_home.as_ref() {
        config_builder = config_builder.EfficiencyCode_home(EfficiencyCode_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["EfficiencyCode-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["EfficiencyCode-state-logs", "--level", "err"]).is_err());
        assert!(
            Args::try_parse_from(["EfficiencyCode-state-logs", "--level", "warn,error"]).is_err()
        );
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["EfficiencyCode-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden EfficiencyCode home.
    #[tokio::test]
    async fn direct_db_skips_EfficiencyCode_home_config() {
        let EfficiencyCode_home = tempfile::tempdir().expect("create EfficiencyCode home");
        std::fs::write(EfficiencyCode_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("EfficiencyCode-state-logs"),
            OsString::from("--EfficiencyCode-home"),
            EfficiencyCode_home.path().as_os_str().to_owned(),
            OsString::from("--db"),
