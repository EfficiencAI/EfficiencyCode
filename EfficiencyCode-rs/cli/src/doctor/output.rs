...
pub(super) fn render_human_report(report: &DoctorReport, options: HumanOutputOptions) -> String {
    ...
    let _ = writeln!(
        out,
        "{} {}",
        bold("EfficiencyCode Doctor", options),
...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run EfficiencyCode doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.EfficiencyCode_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.EfficiencyCode_version);
...
pub(super) fn redact_detail(detail: &str) -> String {
    ...
    let secret_keys = [
        "openai_api_key",
        "EfficiencyCode_api_key",
        "EfficiencyCode_access_token",
        "authorization",
        "bearer_token",
        "token",
        "secret",
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: EfficiencyCode"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
            ...
            .remediation("Run `EfficiencyCode login`."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
EfficiencyCode Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `EfficiencyCode login`.
─────────────────────────────────────────────────────────────
...
Environment
  ...
  ✓ git          git version 2.54.0
      ...
      repo detected            true
  ⚠ terminal     narrow terminal
  ✓ title        default · project EfficiencyCode
      title source             default
      title items              activity, project-name
      project value            EfficiencyCode
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `EfficiencyCode login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project EfficiencyCode
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/EfficiencyCode/target/debug/EfficiencyCode · <redacted>",
        ...
        assert!(
            rendered.contains("\u{1b}[38;5;117m~/code/EfficiencyCode/target/debug/EfficiencyCode")
        );
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, EfficiencyCode_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, EfficiencyCode_API_KEY"
        );
    }
