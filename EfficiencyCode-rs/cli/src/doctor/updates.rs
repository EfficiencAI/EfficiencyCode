//! Diagnoses whether EfficiencyCode update paths target the running installation.
//!
...
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/EfficiencyCode.json";
...
pub(super) fn updates_check(config: &Config) -> DoctorCheck {
    ...
    let version_file = config.EfficiencyCode_home.join(VERSION_FILE_NAME);
...
fn update_action_label(context: &InstallContext) -> &'static str {
    match &context.method {
        InstallMethod::Npm => "npm install -g @openai/EfficiencyCode",
        InstallMethod::Bun => "bun install -g @openai/EfficiencyCode",
        InstallMethod::Pnpm => "pnpm add -g @openai/EfficiencyCode",
        InstallMethod::Brew => "brew upgrade --cask EfficiencyCode",
...
mod tests {
    ...
    fn update_action_labels_install_contexts() {
        ...
        assert_eq!(
            ...
            "pnpm add -g @openai/EfficiencyCode"
        );
        assert_eq!(
            update_action_label(&InstallContext {
