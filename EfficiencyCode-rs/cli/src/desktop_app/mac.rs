...
const EfficiencyCode_BUNDLE_IDENTIFIER: &str = "com.openai.EfficiencyCode";
const EfficiencyCode_DMG_URL_ARM64: &str =
    "https://persistent.oaistatic.com/EfficiencyCode-app-prod/EfficiencyCode.dmg";
const EfficiencyCode_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/EfficiencyCode-app-prod/EfficiencyCode-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_EfficiencyCode_app_path(&EfficiencyCode_app_search_dirs())
    {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_EfficiencyCode_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            EfficiencyCode_DMG_URL_ARM64
        } else {
            EfficiencyCode_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_EfficiencyCode_to_user_applications(&download_url)
        .await
    ...
    open_EfficiencyCode_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_EfficiencyCode_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "EfficiencyCode.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_EfficiencyCode_app_bundle(candidate))
}

fn EfficiencyCode_app_search_dirs() -> Vec<PathBuf> {
...
fn is_EfficiencyCode_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim()
                    == EfficiencyCode_BUNDLE_IDENTIFIER
        })
}

async fn open_EfficiencyCode_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = EfficiencyCode_new_thread_url(workspace);
...
fn EfficiencyCode_new_thread_url(workspace: &Path) -> String {
    let workspace = workspace.as_os_str().to_string_lossy();
    let mut serializer = url::form_urlencoded::Serializer::new(String::new());
    serializer.append_pair("path", workspace.as_ref());
    let query = serializer.finish();
    format!("EfficiencyCode://threads/new?{query}")
}

async fn download_and_install_EfficiencyCode_to_user_applications(
    dmg_url: &str,
) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("EfficiencyCode-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("EfficiencyCode.dmg");
    ...
    let result = async {
        let app_in_volume = find_EfficiencyCode_app_in_mount(&mount_point)
            .context("failed to locate EfficiencyCode.app in mounted dmg")?;
        install_EfficiencyCode_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_EfficiencyCode_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("EfficiencyCode.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install EfficiencyCode.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install EfficiencyCode.app to any applications directory");
}
...
fn find_EfficiencyCode_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("EfficiencyCode.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
#[cfg(test)]
mod tests {
    use super::EfficiencyCode_new_thread_url;
    use super::find_existing_EfficiencyCode_app_path;
    ...
    #[test]
    fn finds_chatgpt_app_with_EfficiencyCode_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.openai.EfficiencyCode");

        assert_eq!(
            find_existing_EfficiencyCode_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let EfficiencyCode_app_path = temp_dir.path().join("EfficiencyCode.app");
        write_app_bundle(&EfficiencyCode_app_path, "com.openai.EfficiencyCode");

        assert_eq!(
            find_existing_EfficiencyCode_app_path(&[temp_dir.path().to_path_buf()]),
            Some(EfficiencyCode_app_path)
        );
    }

    #[test]
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tEfficiencyCode\t/Volumes/EfficiencyCode\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/EfficiencyCode")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tEfficiencyCode Installer\t/Volumes/EfficiencyCode Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/EfficiencyCode Installer")
        );
    }

    #[test]
    fn EfficiencyCode_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&EfficiencyCode_new_thread_url(Path::new(
            "/tmp/EfficiencyCode workspace/#1",
        )))
        ...
        assert_eq!(
            ...
            (
                ...
                vec![(
                    "path".to_string(),
                    "/tmp/EfficiencyCode workspace/#1".to_string()
                )],
            )
        );
    }
