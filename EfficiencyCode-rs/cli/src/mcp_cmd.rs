...
use EfficiencyCode_core::config::load_global_mcp_servers;
use EfficiencyCode_core::plugins_manager_for_config;
use EfficiencyCode_exec_server::EnvironmentManager;
use EfficiencyCode_exec_server::HttpClient;
use EfficiencyCode_exec_server::RouteAwareHttpClient;
use EfficiencyCode_login::AuthManager;
use EfficiencyCode_mcp::McpOAuthLoginSupport;
use EfficiencyCode_mcp::McpRuntimeContext;
use EfficiencyCode_mcp::ResolvedMcpOAuthScopes;
use EfficiencyCode_mcp::compute_auth_statuses;
...
use EfficiencyCode_rmcp_client::delete_oauth_tokens;
use EfficiencyCode_rmcp_client::perform_oauth_login;
use EfficiencyCode_utils_cli::CliConfigOverrides;
use EfficiencyCode_utils_cli::format_env_display;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
/// - `add`    — add a server launcher entry to `~/.EfficiencyCode/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(
    override_usage = "EfficiencyCode mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)"
)]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: EfficiencyCode_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: EfficiencyCode_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&EfficiencyCode_home)
        .await
        .with_context(|| {
            format!(
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `EfficiencyCode mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!(
            "No MCP servers configured yet. Try `EfficiencyCode mcp add my-tool -- my-command`."
        );
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: EfficiencyCode mcp remove {}", get_args.name);
