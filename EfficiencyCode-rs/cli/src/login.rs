...
//! interactive-session layers. Direct `EfficiencyCode login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `EfficiencyCode-login.log` artifact that
...
use EfficiencyCode_login::CLIENT_ID;
use EfficiencyCode_login::ServerOptions;
use EfficiencyCode_login::login_with_access_token;
use EfficiencyCode_login::login_with_api_key;
use EfficiencyCode_login::logout_with_revoke;
use EfficiencyCode_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `EfficiencyCode login` flows.
///
...
/// command produce a durable `EfficiencyCode-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match EfficiencyCode_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("EfficiencyCode-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("EfficiencyCode_cli=info,EfficiencyCode_core=info,EfficiencyCode_login=info")
    });
    ...
    // Direct `EfficiencyCode login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `EfficiencyCode login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | EfficiencyCode login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv EfficiencyCode_ACCESS_TOKEN | EfficiencyCode login --with-access-token`.",
...
/// `EfficiencyCode login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match config
        .auth_config()
        .load_auth(/*enable_EfficiencyCode_api_key_env*/ false)
        .await
    {
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let EfficiencyCode_home = tempdir().expect("create temporary EfficiencyCode home");
        login_with_api_key(
            EfficiencyCode_home.path(),
            "sk-existing",
