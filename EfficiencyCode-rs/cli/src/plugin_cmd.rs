...
use EfficiencyCode_core_plugins::ConfiguredMarketplace;
use EfficiencyCode_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use EfficiencyCode_core_plugins::PluginInstallOutcome;
...
use EfficiencyCode_core_plugins::allowed_configured_marketplace_names;
use EfficiencyCode_core_plugins::installed_marketplaces::marketplace_install_root;
use EfficiencyCode_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use EfficiencyCode_core_plugins::marketplace::MarketplaceListError;
use EfficiencyCode_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use EfficiencyCode_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use EfficiencyCode_core_plugins::marketplace::MarketplacePluginSource;
use EfficiencyCode_core_plugins::marketplace::find_marketplace_manifest_path;
use EfficiencyCode_login::AuthManager;
use EfficiencyCode_plugin::PluginId;
use EfficiencyCode_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(bin_name = "EfficiencyCode plugin")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "EfficiencyCode plugin add",
    after_help = "Examples:\n  EfficiencyCode plugin add sample@debug\n  EfficiencyCode plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "EfficiencyCode plugin list",
    after_help = "Examples:\n  EfficiencyCode plugin list\n  EfficiencyCode plugin list --marketplace debug\n  EfficiencyCode plugin list --json\n  EfficiencyCode plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "EfficiencyCode plugin remove",
    after_help = "Examples:\n  EfficiencyCode plugin remove sample@debug\n  EfficiencyCode plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources =
        configured_marketplace_sources(&plugins_input, EfficiencyCode_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    EfficiencyCode_core_plugins::marketplace::MarketplacePluginSource::Local {
                        path,
                    } => path.as_path().display().to_string(),
                    EfficiencyCode_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    EfficiencyCode_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<EfficiencyCode_core_plugins::ConfiguredMarketplace>,
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    AuthManager::shared_from_config(config, /*enable_EfficiencyCode_api_key_env*/ true)
        .await
        .auth()
        .await
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(EfficiencyCode_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, EfficiencyCode_home, &root)
                {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
...
fn is_implicit_system_marketplace_root(
    ...
    marketplace_name == OPENAI_PRIMARY_RUNTIME_MARKETPLACE_NAME
        && path_ends_with(
            root,
            &[
                "EfficiencyCode-runtimes",
                "EfficiencyCode-primary-runtime",
                "plugins",
                marketplace_name,
            ],
        )
