//! Implements the `EfficiencyCode doctor` diagnostic report.
//!
...
use EfficiencyCode_api::ResponsesWebsocketClient;
use EfficiencyCode_api::is_azure_responses_provider;
...
use EfficiencyCode_install_context::EfficiencyCodePackageLayout;
use EfficiencyCode_install_context::InstallContext;
use EfficiencyCode_install_context::InstallMethod;
use EfficiencyCode_install_context::StandalonePlatform;
use EfficiencyCode_login::AuthDotJson;
use EfficiencyCode_login::AuthManager;
use EfficiencyCode_login::EfficiencyCode_ACCESS_TOKEN_ENV_VAR;
use EfficiencyCode_login::EfficiencyCode_API_KEY_ENV_VAR;
use EfficiencyCode_login::EfficiencyCodeAuth;
use EfficiencyCode_login::OPENAI_API_KEY_ENV_VAR;
use EfficiencyCode_login::default_client::create_client_without_request_logging;
use EfficiencyCode_login::default_client::default_headers;
use EfficiencyCode_login::load_auth_dot_json;
...
use EfficiencyCode_terminal_detection::Multiplexer;
use EfficiencyCode_terminal_detection::TerminalInfo;
use EfficiencyCode_terminal_detection::TerminalName;
use EfficiencyCode_terminal_detection::terminal_info;
...
/// Options for building a local EfficiencyCode diagnostic report.
///
...
///
/// This is the CLI entry point for EfficiencyCode doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation(
                            "Fix the reported config error, then rerun EfficiencyCode doctor.",
                        )
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
    ...
    DoctorReport {
        ...
        overall_status,
        EfficiencyCode_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load EfficiencyCode config")
}
...
/// JSON support report emitted by `EfficiencyCode doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        EfficiencyCode_version: report.EfficiencyCode_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("EfficiencyCode_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("EfficiencyCode_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "EfficiencyCode_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = EfficiencyCode_path_entries();
    ...
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH EfficiencyCode #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary = "npm install -g @openai/EfficiencyCode would update a different install"
                    .to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update EfficiencyCode so the JS shim provides EfficiencyCode_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("EfficiencyCode_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("EfficiencyCode_MANAGED_BY_NPM").is_none()
        && env::var_os("EfficiencyCode_MANAGED_BY_BUN").is_none()
        && env::var_os("EfficiencyCode_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&EfficiencyCodePackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) =
        env::var_os("EfficiencyCode_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("EfficiencyCode");
...
fn EfficiencyCode_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["EfficiencyCode"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "EfficiencyCode"]);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.EfficiencyCode_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        EfficiencyCode_API_KEY_ENV_VAR,
        EfficiencyCode_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check.remediation(
                    "Run EfficiencyCode login again or provide a supported auth env var.",
                );
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no EfficiencyCode credentials were found",
        )
        .details(details)
        .remediation(
            "Run EfficiencyCode login or provide an API key through a supported auth env var.",
        ),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run EfficiencyCode login again."),
    }
}
...
fn stored_auth_mode(auth: &EfficiencyCode_login::AuthDotJson) -> &'static str {
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present = env_var_present(OPENAI_API_KEY_ENV_VAR)
                || env_var_present(EfficiencyCode_API_KEY_ENV_VAR);
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["EfficiencyCode_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "EfficiencyCode-linux-sandbox helper",
        arg0_paths.EfficiencyCode_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.EfficiencyCode_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    ...
    rollout_stats_details(&mut details, &config.EfficiencyCode_home);
...
async fn sqlite_integrity_detail(
    sqlite: &EfficiencyCode_state::SqliteConfig,
    details: &mut Vec<String>,
    ...
    match EfficiencyCode_state::sqlite_integrity_check(sqlite, path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, EfficiencyCode_home: &Path) {
    let active = collect_rollout_stats(&EfficiencyCode_home.join("sessions"));
    let archived = collect_rollout_stats(&EfficiencyCode_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &EfficiencyCodeAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match EfficiencyCode_home {
        Ok(path) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Ok,
            "EfficiencyCode_HOME was resolved without config",
        )
        .detail(format!("EfficiencyCode_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "EfficiencyCode_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(EfficiencyCode_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(EfficiencyCode_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/EfficiencyCode");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("EfficiencyCode")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/EfficiencyCode");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("EfficiencyCode"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].EfficiencyCode_hooks is deprecated. Use [features].hooks instead."
                .to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            EfficiencyCode_self_exe: Some(PathBuf::from("/bin/EfficiencyCode")),
            EfficiencyCode_linux_sandbox_exe: Some(PathBuf::from(
                "/bin/EfficiencyCode-linux-sandbox",
            )),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/EfficiencyCode-execve-wrapper")),
        };
        ...
        assert_eq!(
            overrides.EfficiencyCode_self_exe,
            arg0_paths.EfficiencyCode_self_exe
        );
        assert_eq!(
            overrides.EfficiencyCode_linux_sandbox_exe,
            arg0_paths.EfficiencyCode_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            EfficiencyCode_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "system.environment",
                    "system",
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "EfficiencyCode_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("EfficiencyCode_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running EfficiencyCode."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running EfficiencyCode.".to_string())
        );
    }
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-EfficiencyCode-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-EfficiencyCode-doctor-mcp\" is not resolvable",
            )
        }));
    }
