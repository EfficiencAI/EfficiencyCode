...
use EfficiencyCode_thread_store::RolloutMigrationMode;
use EfficiencyCode_thread_store::RolloutMigrationOptions;
use EfficiencyCode_thread_store::RolloutMigrationProgress;
use EfficiencyCode_thread_store::RolloutMigrationReport;
use EfficiencyCode_thread_store::RolloutMigrationStatus;
...
pub(crate) async fn run(
    ...
    EfficiencyCode_core::otel_init::record_process_start(
        otel.as_ref(),
        "EfficiencyCode_migrate_rollouts",
    );
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            EfficiencyCode_rollout::state_db::try_init(&config)
                .await
...
fn print_human_report(
    ...
    if mode == RolloutMigrationMode::DryRun && counts.eligible > 0 {
        println!("Run `EfficiencyCode migrate-rollouts --apply` to migrate eligible sessions.");
    }
...
fn print_outcome(outcome: &EfficiencyCode_thread_store::RolloutMigrationOutcome) {
