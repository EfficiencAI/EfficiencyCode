...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run EfficiencyCode after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
        conflicts_with = "current_user",
        requires = "EfficiencyCode_home"
    )]
    user: Option<String>,

    /// Use the current Windows user as the EfficiencyCode user.
    #[arg(
        long = "current-user",
    ...
    /// EfficiencyCode_HOME for the EfficiencyCode user. Required with --user.
    #[arg(long = "EfficiencyCode-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`EfficiencyCode sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        .EfficiencyCode_home(identity.EfficiencyCode_home.clone())
        .fallback_cwd(Some(identity.EfficiencyCode_home.clone()))
        .loader_overrides(super::loader_overrides_for_profile_at_EfficiencyCode_home(
            config_profile.as_ref(),
            &identity.EfficiencyCode_home,
        ))
        .cli_overrides(cli_overrides)
        .build()
        .await
        .context("failed to load target user's EfficiencyCode config for sandbox provisioning")?;

    EfficiencyCode_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.EfficiencyCode_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.EfficiencyCode_home.as_path())
    ...
    println!(
        ...
        identity.real_user,
        identity.EfficiencyCode_home.display()
    );
    Ok(())
}
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let EfficiencyCode_home = match cmd.EfficiencyCode_home.clone() {
            Some(EfficiencyCode_home) => EfficiencyCode_home,
            None => find_EfficiencyCode_home()?.to_path_buf(),
        };
    ...
    let EfficiencyCode_home = cmd
        .EfficiencyCode_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--EfficiencyCode-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        let command = SandboxSetupCommand::try_parse_from([
            ...
            "DOMAIN\\alice",
            "--EfficiencyCode-home",
            r"C:\Users\alice\.EfficiencyCode",
        ])
        .expect("parse");
        ...
        assert_eq!(
            command.EfficiencyCode_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.EfficiencyCode"))
        );
    }
    ...
    #[test]
    fn requires_EfficiencyCode_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--EfficiencyCode-home".to_string(),
            r"C:\Users\alice\.EfficiencyCode".to_string(),
        ])
        .expect("parse")
