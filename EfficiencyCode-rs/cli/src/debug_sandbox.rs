...
#[cfg(target_os = "macos")]
use EfficiencyCode_core::spawn::CODEX_SANDBOX_ENV_VAR;
use EfficiencyCode_core::spawn::CODEX_SANDBOX_ENV_VAR;
...
use EfficiencyCode_sandboxing::landlock::allow_network_for_proxy;
use EfficiencyCode_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use EfficiencyCode_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use EfficiencyCode_sandboxing::seatbelt::create_seatbelt_command_args;
use EfficiencyCode_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<EfficiencyCode_mcp::SandboxState>)
        .transpose()
    ...
    let EfficiencyCode_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .EfficiencyCode_linux_sandbox_exe
                .clone()
                .or(EfficiencyCode_linux_sandbox_exe)
        }
        None => EfficiencyCode_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(EfficiencyCode_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let EfficiencyCode_linux_sandbox_exe = config
                .EfficiencyCode_linux_sandbox_exe
                .expect("EfficiencyCode-linux-sandbox executable not found");
            ...
            spawn_debug_sandbox_child(
                EfficiencyCode_linux_sandbox_exe,
                args,
                Some("EfficiencyCode-linux-sandbox"),
                cwd.to_path_buf(),
                network_sandbox_policy,
                env,
                |env_map| {
...
async fn run_command_under_windows_session(
    ...
    use EfficiencyCode_windows_sandbox::WindowsSandboxProxySettingsMode;
    use EfficiencyCode_windows_sandbox::WindowsSandboxSessionRequest;
    use EfficiencyCode_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        EfficiencyCode_home: config.EfficiencyCode_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = EfficiencyCode_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(CODEX_SANDBOX_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_EfficiencyCode_home(
        cli_overrides,
        EfficiencyCode_linux_sandbox_exe,
        options,
...
async fn load_debug_sandbox_config_with_EfficiencyCode_home(
    cli_overrides: Vec<(String, TomlValue)>,
    EfficiencyCode_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `EfficiencyCode sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            EfficiencyCode_linux_sandbox_exe: EfficiencyCode_linux_sandbox_exe.clone(),
            ..Default::default()
        },
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(EfficiencyCode_home) = EfficiencyCode_home {
        builder = builder
            .EfficiencyCode_home(EfficiencyCode_home.clone())
            .fallback_cwd(Some(EfficiencyCode_home));
    }
    builder.build().await
}
...
"#;
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        write_permissions_profile_config(&EfficiencyCode_home, &docs, &private)?;
        let EfficiencyCode_home_path = EfficiencyCode_home.path().to_path_buf();

        let profile_config = build_debug_sandbox_config(
            Vec::new(),
            ConfigOverrides::default(),
            Some(EfficiencyCode_home_path.clone()),
        ...
        let config = load_debug_sandbox_config_with_EfficiencyCode_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = EfficiencyCode_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_EfficiencyCode_home(
            ...
            Some(EfficiencyCode_home.path().to_path_buf()),
            CloudConfigBundleLoader::default(),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = EfficiencyCode_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
