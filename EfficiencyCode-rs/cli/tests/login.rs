...
fn write_file_auth_config(EfficiencyCode_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(EfficiencyCode_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(EfficiencyCode_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(EfficiencyCode_home.path())?;
    ...
    let auth = read_auth_json(EfficiencyCode_home.path())?;
...
async fn debug_prompt_input_follows_authenticated_attribution_setting() -> Result<()> {
    ...
    for enabled in [true, false] {
        let output = EfficiencyCode_command(EfficiencyCode_home.path())?
        ...
        assert_eq!(
            prompt.contains("Co-authored-by: EfficiencyCode <noreply@openai.com>"),
            enabled
        );
