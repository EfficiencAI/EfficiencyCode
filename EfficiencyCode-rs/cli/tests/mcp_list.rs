...
async fn configure_http_oauth_server(EfficiencyCode_home: &Path, url: &str) -> Result<()> {
    let mut servers = load_global_mcp_servers(EfficiencyCode_home).await?;
    servers.insert(
        "oauth".to_string(),
...
"#,
  ...
  "mcpServers": {
    ...
    let mut list_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    list_cmd
        .env(EfficiencyCode_API_KEY_ENV_VAR, "sk-test")
        .args(["mcp", "list", "--json"])
        .assert()
        .success()
        .stdout(contains(r#""name": "api-docs""#));

    let mut get_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    get_cmd
        .env(EfficiencyCode_API_KEY_ENV_VAR, "sk-test")
...
async fn list_discovers_local_oauth_server_through_environment_proxy() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    configure_http_oauth_server(EfficiencyCode_home.path(), "http://mcp-proxy.invalid/mcp").await?;
    std::fs::write(
    ...
    let mut command = EfficiencyCode_command(EfficiencyCode_home.path())?;
    command
...
async fn list_reports_unknown_auth_status_when_oauth_discovery_is_rate_limited() -> Result<()> {
    ...
    wiremock::Mock::given(wiremock::matchers::method("GET"))
        ...
        .mount(&server)
        .await;
    configure_http_oauth_server(EfficiencyCode_home.path(), &format!("{}/mcp", server.uri()))
        .await?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = EfficiencyCode_command(EfficiencyCode_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(EfficiencyCode_home.path()).await?;
    ...
    let mut list_json_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    ...
    assert!(stdout.contains("remove: EfficiencyCode mcp remove docs"));

    let mut get_json_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    get_json_cmd
