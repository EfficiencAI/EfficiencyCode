...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_and_login_discover_oauth_through_configured_http_proxy() -> Result<()> {
    ...
    assert!(
        load_global_mcp_servers(EfficiencyCode_home.path())
            .await?
            .contains_key("oauth")
    );
    ...
    let mut login = EfficiencyCode_command(EfficiencyCode_home.path())?;
    login
        .env("HTTP_PROXY", proxy.uri())
        .env("http_proxy", proxy.uri())
        .env_remove("HTTPS_PROXY")
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!EfficiencyCode_home.path().join(".env").exists());
...
async fn add_streamable_http_with_oauth_options() -> Result<()> {
    ...
    add_cmd
        .args([
            ...
            "https://example.com/mcp",
            "--oauth-client-id",
            "eci-prd-pub-EfficiencyCode-123",
            "--oauth-resource",
