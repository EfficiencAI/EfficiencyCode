...
impl CloudManagedMcpFixture {
    async fn new() -> Result<Option<Self>> {
        ...
        std::fs::write(EfficiencyCode_home.path().join("config.toml"), &user_config)?;
    ...
    fn command(&self, args: &[&str]) -> Result<Command> {
        let mut command =
            Command::new(EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")?);
        command
            .kill_on_drop(true)
            .current_dir(self.EfficiencyCode_home.path())
            .env("EfficiencyCode_HOME", self.EfficiencyCode_home.path())
            .env("NO_PROXY", "127.0.0.1,localhost")
            .env("no_proxy", "127.0.0.1,localhost")
            .env_remove("EfficiencyCode_ACCESS_TOKEN")
    ...
    async fn output(&self, args: &[&str]) -> Result<Output> {
        let output = self.command(args)?.output().await?;
            ...
            "EfficiencyCode {} failed with status {}: stdout={}; stderr={}",
            args.join(" "),
            output.status,
    ...
    fn assert_user_config_unchanged(&self) -> Result<()> {
        assert_eq!(
            std::fs::read_to_string(self.EfficiencyCode_home.path().join("config.toml"))?,
            self.user_config
        );
        Ok(())
    }
...
async fn login_and_logout_persist_only_cloud_managed_mcp_oauth_credentials() -> Result<()> {
    ...
    let credentials_path = fixture.EfficiencyCode_home.path().join(".credentials.json");
...
async fn add_and_remove_do_not_copy_or_delete_cloud_managed_mcp_servers() -> Result<()> {
    ...
    fixture
        ...
        .await?;
    let local_servers = load_global_mcp_servers(fixture.EfficiencyCode_home.path()).await?;
    ...
    assert!(
        load_global_mcp_servers(fixture.EfficiencyCode_home.path())
            .await?
            .is_empty()
    );
    fixture
