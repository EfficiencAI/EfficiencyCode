...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = EfficiencyCode_command(EfficiencyCode_home.path())?;
    cmd.env(
        EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    let mut command =
        tokio::process::Command::new(EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")?);
    command
        .env("EfficiencyCode_HOME", EfficiencyCode_home.path())
        .env("EfficiencyCode_API_KEY", "test-api-key")
        .env(
            EfficiencyCode_exec_server::EfficiencyCode_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined EfficiencyCode_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${EfficiencyCode_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            env_policy: Some(EfficiencyCode_exec_server::ExecEnvPolicy {
                inherit: EfficiencyCode_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let EfficiencyCode_bin = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")?;
    let EfficiencyCode_home = EfficiencyCode_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(EfficiencyCode_bin);
        command
            .env("EfficiencyCode_HOME", EfficiencyCode_home)
            .env("NO_PROXY", "127.0.0.1,localhost")
            .env("no_proxy", "127.0.0.1,localhost")
            .args(["exec-server", "--listen", "stdio"])
            .stdin(Stdio::piped())
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let EfficiencyCode_home = TempDir::new()?;
    let mut child =
        std::process::Command::new(EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode")?)
            .env("EfficiencyCode_HOME", EfficiencyCode_home.path())
