...
fn write_installed_marketplace(EfficiencyCode_home: &Path, marketplace_name: &str) -> Result<()> {
    let root = marketplace_install_root(EfficiencyCode_home).join(marketplace_name);
...
async fn marketplace_remove_deletes_config_and_installed_root() -> Result<()> {
    ...
    write_installed_marketplace(EfficiencyCode_home.path(), "debug")?;
...
async fn marketplace_remove_json_prints_remove_outcome() -> Result<()> {
    ...
    write_installed_marketplace(EfficiencyCode_home.path(), "debug")?;
    let installed_root = marketplace_install_root(EfficiencyCode_home.path()).join("debug");
