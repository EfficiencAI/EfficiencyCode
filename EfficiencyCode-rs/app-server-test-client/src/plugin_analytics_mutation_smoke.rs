...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "EfficiencyCode-plugin-analytics-mutation-{}.jsonl",
            process::id()
        ))
    });
    prepare_capture_file(&capture_path)?;
    let mut client = spawn_client(EfficiencyCode_bin, config_overrides, &capture_path)?;
    ...
    match (sequence_result, restoration) {
        ...
        (sequence_result, RestorationStatus::Unknown(cleanup_err)) => {
            ...
            print_recovery_command(EfficiencyCode_bin, config_overrides, remote_plugin_id);
            Err(cleanup_err)
        }
    }
}
...
pub(super) fn run_cleanup(
    ...
    let mut client = EfficiencyCodeClient::spawn_stdio(EfficiencyCode_bin, &overrides)?;
    client.initialize()?;
    ...
    match restore_uninstalled_state(&mut client, remote_plugin_id) {
        ...
        RestorationStatus::Dirty(err) => {
            print_dirty_recovery(EfficiencyCode_bin, config_overrides, remote_plugin_id, &err);
            Err(err)
        }
...
fn spawn_client(
    ...
    EfficiencyCodeClient::spawn_stdio_with_env(EfficiencyCode_bin, &overrides, &environment)
}
...
fn run_mutation_sequence(
    ...
    let result = (|| {
        ...
        wait_for_remote_plugin_event(
            capture_path,
            &expected.remote_plugin_id,
            "EfficiencyCode_plugin_installed",
        )?;
        ...
        wait_for_remote_plugin_event(
            capture_path,
            &expected.remote_plugin_id,
            "EfficiencyCode_plugin_uninstalled",
        )?;
...
fn print_recovery_command(
    ...
    let test_client = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "EfficiencyCode-app-server-test-client".to_string());
