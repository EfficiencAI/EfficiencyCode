...
use EfficiencyCode_app_server_protocol::JSONRPCMessage;
use EfficiencyCode_app_server_protocol::JSONRPCNotification;
use EfficiencyCode_app_server_protocol::JSONRPCRequest;
use EfficiencyCode_app_server_protocol::JSONRPCResponse;
...
use EfficiencyCode_app_server_protocol::SandboxPolicy;
...
use EfficiencyCode_otel::OtelProvider;
use EfficiencyCode_otel::current_span_w3c_trace_context;
use EfficiencyCode_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "EfficiencyCode-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $EfficiencyCode_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the EfficiencyCode app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "EfficiencyCode", version, about = "Bootstrap EfficiencyCode app-server", long_about = None)]
struct Cli {
    /// Path to the `EfficiencyCode` CLI binary. When set, requests use stdio by
    /// spawning `EfficiencyCode app-server` as a child process.
    #[arg(long, env = "EfficiencyCode_BIN", global = true)]
    EfficiencyCode_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--EfficiencyCode-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "EfficiencyCode_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `EfficiencyCode` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `EfficiencyCode app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/EfficiencyCode-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `EfficiencyCode app-server --listen`.
    ...
    /// Send a user message through the EfficiencyCode app-server.
    SendMessage {
        /// User message to send to EfficiencyCode.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a EfficiencyCode-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the EfficiencyCode app-server.
    GetAccountRateLimits,
    /// List the available models from the EfficiencyCode app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the EfficiencyCode app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "EfficiencyCode_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let EfficiencyCode_bin =
                EfficiencyCode_bin.unwrap_or_else(|| PathBuf::from("EfficiencyCode"));
            serve(&EfficiencyCode_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(EfficiencyCode_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(
                EfficiencyCode_bin,
                url,
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-smoke requires --EfficiencyCode-bin and does not support --url"
                );
            }
            let EfficiencyCode_bin = EfficiencyCode_bin
                .context("plugin-analytics-smoke requires --EfficiencyCode-bin")?;
        ...
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --EfficiencyCode-bin and does not support --url"
                );
            }
            let EfficiencyCode_bin = EfficiencyCode_bin
                .context("plugin-analytics-mutation-smoke requires --EfficiencyCode-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!(
                    "plugin-remote-uninstall requires --EfficiencyCode-bin and does not support --url"
                );
            }
            let EfficiencyCode_bin = EfficiencyCode_bin
                .context("plugin-remote-uninstall requires --EfficiencyCode-bin")?;
...
fn resolve_endpoint(EfficiencyCode_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if EfficiencyCode_bin.is_some() && url.is_some() {
        bail!("--EfficiencyCode-bin and --url are mutually exclusive");
    }
    if let Some(EfficiencyCode_bin) = EfficiencyCode_bin {
        return Ok(Endpoint::SpawnEfficiencyCode(EfficiencyCode_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if EfficiencyCode_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --EfficiencyCode-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(EfficiencyCode_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(EfficiencyCode_bin);
        if let Some(EfficiencyCode_bin_parent) = EfficiencyCode_bin.parent() {
            let mut path = OsString::from(EfficiencyCode_bin_parent.as_os_str());
...
fn serve(
    EfficiencyCode_bin: &Path,
    config_overrides: &[String],
    listen: &str,
    kill: bool,
) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/EfficiencyCode-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,EfficiencyCode_=trace {}",
        shell_quote(&EfficiencyCode_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started EfficiencyCode app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnEfficiencyCode(EfficiencyCode_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: EfficiencyCode_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut EfficiencyCodeClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = EfficiencyCodeClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = EfficiencyCodeClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (EfficiencyCode_bin, url) {
        (Some(_), Some(_)) => bail!("--EfficiencyCode-bin and --url are mutually exclusive"),
        (Some(EfficiencyCode_bin), None) => {
            let server = BackgroundAppServer::spawn(&EfficiencyCode_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve EfficiencyCode-app-server-test-client binary path")?;
...
impl EfficiencyCodeClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnEfficiencyCode(EfficiencyCode_bin) => {
                Self::spawn_stdio(EfficiencyCode_bin, config_overrides)
            }
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(EfficiencyCode_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(EfficiencyCode_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let EfficiencyCode_bin_display = EfficiencyCode_bin.display();
        let mut cmd = Command::new(EfficiencyCode_bin);
        if let Some(EfficiencyCode_bin_parent) = EfficiencyCode_bin.parent() {
            let mut path = OsString::from(EfficiencyCode_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut EfficiencyCode_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| {
                format!("failed to start `{EfficiencyCode_bin_display}` app-server")
            })?;

        let stdin = EfficiencyCode_app_server
            .stdin
            .take()
            .context("EfficiencyCode app-server stdin unavailable")?;
        let stdout = EfficiencyCode_app_server
            .stdout
            .take()
            .context("EfficiencyCode app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: EfficiencyCode_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `EfficiencyCode-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "EfficiencyCode-toy-app-server".to_string(),
                    title: Some("EfficiencyCode Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: EfficiencyCode_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                EfficiencyCode_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: EfficiencyCode_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to EfficiencyCode app-server")?;
                    return Ok(());
                }
                bail!("EfficiencyCode app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from EfficiencyCode app-server")?;
                if bytes == 0 {
                    bail!("EfficiencyCode app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = EfficiencyCode_core::otel_init::build_provider(
            &config,
...
impl Drop for EfficiencyCodeClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[EfficiencyCode app-server exited: {status}]");
            return;
        }
