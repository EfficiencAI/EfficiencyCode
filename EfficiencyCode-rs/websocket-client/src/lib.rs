//! Proxy-aware WebSocket connection setup shared by EfficiencyCode API clients.
...
use EfficiencyCode_http_client::BuildCustomCaTransportError;
use EfficiencyCode_http_client::HttpClientFactory;
use EfficiencyCode_http_client::build_rustls_client_config_with_custom_ca;
use futures::Sink;
use futures::Stream;
...
/// Selects whether WebSocket TLS follows EfficiencyCode custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured EfficiencyCode custom CAs.
    ExplicitEfficiencyCodeTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured EfficiencyCode custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ) -> Result<Self, BuildCustomCaTransportError> {
        Self::new_with_tls_mode(
            http_client_factory,
            WebSocketTlsMode::ExplicitEfficiencyCodeTls,
        )
    }

    /// Creates a connector with explicit EfficiencyCode TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build EfficiencyCode TLS configuration when they establish their
