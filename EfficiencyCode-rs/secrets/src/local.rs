...
const EfficiencyCode_AUTH_SECRETS_FILENAME: &str = "EfficiencyCode_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// EfficiencyCode authentication credentials used by the CLI, TUI, app server, and other clients.
    EfficiencyCodeAuth,
...
impl LocalSecretsBackend {
    pub fn new(EfficiencyCode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.EfficiencyCode_home.join("secrets")
    }

    fn secrets_path(&self) -> PathBuf {
        let filename = match self.namespace {
            LocalSecretsNamespace::ManagedSecrets => LOCAL_SECRETS_FILENAME,
            LocalSecretsNamespace::EfficiencyCodeAuth => EfficiencyCode_AUTH_SECRETS_FILENAME,
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.EfficiencyCode_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(EfficiencyCode_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let EfficiencyCode_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(EfficiencyCode_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        ...
        let EfficiencyCode_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let EfficiencyCode_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        EfficiencyCode_auth_backend.set(&scope, &name, "EfficiencyCode-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            EfficiencyCode_auth_backend.get(&scope, &name)?,
            Some("EfficiencyCode-auth-value".to_string())
        );
        assert!(
            MCP_OAUTH_CACHE
                .lock()
                .unwrap_or_else(PoisonError::into_inner)
                .as_ref()
                .is_none_or(|cached| cached.path != EfficiencyCode_auth_backend.secrets_path())
        );
        assert_eq!(
