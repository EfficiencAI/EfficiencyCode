...
pub fn app_server_control_socket_path(
...
pub fn app_server_startup_lock_path(
    EfficiencyCode_home: &Path,
) -> std::io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(
        EfficiencyCode_home
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let EfficiencyCode_home = find_EfficiencyCode_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve EfficiencyCode_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&EfficiencyCode_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),
