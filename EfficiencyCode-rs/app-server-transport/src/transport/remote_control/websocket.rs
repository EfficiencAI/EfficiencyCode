...
use EfficiencyCode_core::util::backoff;
...
pub(super) const REMOTE_CONTROL_INSTALLATION_ID_HEADER: &str = "x-EfficiencyCode-installation-id";
const REMOTE_CONTROL_SUBSCRIBE_CURSOR_HEADER: &str = "x-EfficiencyCode-subscribe-cursor";
...
fn build_remote_control_websocket_request(
    ...
    set_remote_control_header(headers, "x-EfficiencyCode-server-id", &enrollment.server_id)?;
    set_remote_control_header(
        headers,
        "x-EfficiencyCode-name",
        &base64::engine::general_purpose::STANDARD.encode(&enrollment.server_name),
    )?;
    set_remote_control_header(
        headers,
        "x-EfficiencyCode-protocol-version",
