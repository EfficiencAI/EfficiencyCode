...
use EfficiencyCode_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let EfficiencyCode_home = tempdir().unwrap();
...
#[tokio::test]
#[serial(EfficiencyCode_auth_env)]
...
async fn agent_identity_jwt_uses_explicit_staging_endpoint_overrides() -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/api/EfficiencyCode/agent-identities/jwks"))
        .respond_with(ResponseTemplate::new(200).set_body_json(test_jwks_body()))
        .expect(1)
        .mount(&jwks_server)
        .await;
    ...
    let _authapi_guard = EnvVarGuard::set(
        "EfficiencyCode_AGENT_IDENTITY_AUTHAPI_BASE_URL",
        &authapi_base_url,
    );
    let jwks_base_url = format!("{}/api/EfficiencyCode/", jwks_server.uri());
    let _jwks_guard = EnvVarGuard::set(
        "EfficiencyCode_AGENT_IDENTITY_JWKS_BASE_URL",
        &jwks_base_url,
    );

    let auth = EfficiencyCodeAuth::from_agent_identity_jwt(
        &jwt,
    ...
    let EfficiencyCodeAuth::AgentIdentity(agent_identity) = auth else {
...
fn agent_identity_authapi_override_preserves_chatgpt_environment_validation() {
    ...
    let _jwks_guard = EnvVarGuard::set(
        "EfficiencyCode_AGENT_IDENTITY_JWKS_BASE_URL",
        "https://jwks.example/api/EfficiencyCode",
    );
...
fn agent_identity_custom_jwks_base_requires_explicit_authapi_override() {
    let _authapi_guard = EnvVarGuard::remove("EfficiencyCode_AGENT_IDENTITY_AUTHAPI_BASE_URL");
    let jwks_base_url = "https://jwks.example/api/EfficiencyCode";
    let _jwks_guard =
        EnvVarGuard::set("EfficiencyCode_AGENT_IDENTITY_JWKS_BASE_URL", jwks_base_url);
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let EfficiencyCodeAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(EfficiencyCode_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(EfficiencyCode_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("EfficiencyCode_AUTHAPI_BASE_URL", &server.uri());
    let allowed_workspaces = [WORKSPACE_ID_ALLOWED.to_string()];
    super::login_with_access_token(
        dir.path(),
        "at-login-test",
...
async fn chatgpt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .and(body_partial_json(json!({
            "abom": {
                "agent_harness_id": "EfficiencyCode-cli",
            },
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(EfficiencyCode_home.path().to_path_buf());
    let auth_path = get_auth_file(EfficiencyCode_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = EfficiencyCodeAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = EfficiencyCodeAuth::from_auth_dot_json(
        EfficiencyCode_home.path(),
...
#[derive(Clone)]
struct StaticExternalAuth(EfficiencyCodeAuth);
...
async fn configured_auth_keeps_cached_credentials_after_permanent_reload_failure() {
    let manager = AuthManager::from_auth_for_testing(EfficiencyCodeAuth::from_api_key("seed"));
    let auth = EfficiencyCodeAuth::from_api_key("configured-token");
...
async fn runtime_external_auth_errors_remain_transient() {
    ...
    manager
        .set_external_auth(Arc::new(FailingExternalAuth {
            auth: EfficiencyCodeAuth::from_api_key("runtime-token"),
            resolve_count: AtomicUsize::new(0),
        }))
        .await
...
async fn external_auth_provider_can_install_headers() {
    ...
    let auth = EfficiencyCodeAuth::Headers(AuthHeaders::new(headers));
    ...
    assert!(
        manager
            .auth_cached()
            .is_some_and(|auth| auth.uses_EfficiencyCode_backend())
    );
    assert!(
        !manager
            .auth_cached()
...
async fn configured_external_auth_is_immutable_and_process_local() {
    ...
    let manager = AuthManager::from_auth_for_testing_with_home(
        EfficiencyCodeAuth::from_api_key("seed"),
        EfficiencyCode_home.path().to_path_buf(),
    );
    ...
    assert!(!get_auth_file(EfficiencyCode_home.path()).exists());
...
fn write_auth_file(params: AuthFileParams, EfficiencyCode_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(EfficiencyCode_home);
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(EfficiencyCode_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(EfficiencyCode_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    assert!(
        !get_auth_file(EfficiencyCode_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("EfficiencyCode_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(EfficiencyCode_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
#[tokio::test]
#[serial(EfficiencyCode_auth_env)]
async fn load_auth_keeps_EfficiencyCode_api_key_env_precedence() {
    let EfficiencyCode_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard =
        EnvVarGuard::set(EfficiencyCode_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(EfficiencyCode_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    ...
    assert!(
        !EfficiencyCode_home.path().join("auth.json").exists(),
    ...
    assert!(
        manager
            .set_external_auth(Arc::new(StaticExternalAuth(
                EfficiencyCodeAuth::from_api_key("sk-external",)
            )))
            .await
            .is_err(),
...
async fn api_only_policy_rejects_access_tokens_before_hydration() {
    ...
    let server = MockServer::start().await;
    let _authapi_guard = EnvVarGuard::set("EfficiencyCode_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(EfficiencyCode_ACCESS_TOKEN_ENV_VAR, "at-rejected");
    let mut config = build_config(
        EfficiencyCode_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_rejects_agent_identity_before_hydration() {
    ...
    let access_token_guard = EnvVarGuard::set(EfficiencyCode_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let mut config = build_config(
        EfficiencyCode_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_checks_the_selected_request_account() {
    ...
    let auth_path = EfficiencyCode_home.path().join("auth.json");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(EfficiencyCode_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let EfficiencyCode_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material = EfficiencyCode_agent_identity::generate_agent_key_material()
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/EfficiencyCode-backend/agent-identity",
        "aud": "EfficiencyCode-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = EfficiencyCodeAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
