...
#[test]
fn test_get_EfficiencyCode_user_agent() {
    let user_agent = get_EfficiencyCode_user_agent();
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("EfficiencyCode-tui"), true);
    assert_eq!(is_first_party_originator("EfficiencyCode_vscode"), true);
    assert_eq!(
        is_first_party_originator("EfficiencyCode Something Else"),
        true
    );
    assert_eq!(is_first_party_originator("EfficiencyCode_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("EfficiencyCode_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("EfficiencyCode_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(
        is_first_party_chat_originator("EfficiencyCode_vscode"),
        false
    );
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed EfficiencyCode UA for that originator
    let expected_ua = get_EfficiencyCode_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "EfficiencyCode_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "EfficiencyCode_cli_rs/0.0.0 (bad_suffix)"
    );
}
