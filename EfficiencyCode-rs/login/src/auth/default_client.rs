//! Default EfficiencyCode HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`EfficiencyCode_login::default_client`] from other crates in this
//! workspace.

use EfficiencyCode_http_client::BuildRouteAwareHttpClientError;
...
/// See https://github.com/openai/EfficiencyCode/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by EfficiencyCode. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "EfficiencyCode_cli_rs";
pub const EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str =
    "EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-EfficiencyCode-residency";

pub use EfficiencyCode_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "EfficiencyCode-tui"
        || originator_value == "EfficiencyCode_vscode"
        || originator_value.starts_with("EfficiencyCode ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "EfficiencyCode_atlas"
        || originator_value == "EfficiencyCode_chatgpt_desktop"
}

pub fn get_EfficiencyCode_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized EfficiencyCode user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base EfficiencyCode user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default EfficiencyCode originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// EfficiencyCode's route-aware system/PAC resolution.
...
/// Builds the default EfficiencyCode HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the EfficiencyCode
...
/// Builds the default EfficiencyCode HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without EfficiencyCode default headers.
...
/// Builds the default EfficiencyCode HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_EfficiencyCode_user_agent()) {
