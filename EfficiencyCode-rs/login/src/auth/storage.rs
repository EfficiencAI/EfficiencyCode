...
use EfficiencyCode_agent_identity::AgentIdentityJwtClaims;
use EfficiencyCode_agent_identity::decode_agent_identity_jwt;
use EfficiencyCode_config::types::AuthCredentialsStoreMode;
pub use EfficiencyCode_config::types::AuthKeyringBackendKind;
use EfficiencyCode_keyring_store::DefaultKeyringStore;
use EfficiencyCode_keyring_store::KeyringStore;
use EfficiencyCode_protocol::account::PlanType as AccountPlanType;
use EfficiencyCode_protocol::auth::AuthMode;
use EfficiencyCode_secrets::LocalSecretsNamespace;
use EfficiencyCode_secrets::SecretName;
use EfficiencyCode_secrets::SecretScope;
use EfficiencyCode_secrets::SecretsBackendKind;
use EfficiencyCode_secrets::SecretsManager;
use once_cell::sync::Lazy;

/// Expected structure for $EfficiencyCode_HOME/auth.json.
...
pub(super) fn get_auth_file(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(EfficiencyCode_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(EfficiencyCode_home: PathBuf) -> Self {
        Self {
            EfficiencyCode_home,
        }
    }

    /// Attempt to read and parse the `auth.json` file in the given `EfficiencyCode_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.EfficiencyCode_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.EfficiencyCode_home)
    }
}

static EfficiencyCode_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("EfficiencyCode_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("EfficiencyCode_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "EfficiencyCode Auth";

// turns EfficiencyCode_home path into a stable, short key string
fn compute_store_key(EfficiencyCode_home: &Path) -> std::io::Result<String> {
    let canonical = EfficiencyCode_home
        .canonicalize()
        .unwrap_or_else(|_| EfficiencyCode_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(EfficiencyCode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            EfficiencyCode_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.EfficiencyCode_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.EfficiencyCode_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.EfficiencyCode_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(EfficiencyCode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(EfficiencyCode_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            EfficiencyCode_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::EfficiencyCodeAuth,
        );
        Self {
            EfficiencyCode_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &EfficiencyCode_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(
                &SecretScope::Global,
                &EfficiencyCode_AUTH_SECRET_NAME,
                &serialized,
            )
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &EfficiencyCode_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(EfficiencyCode_home)),
        }
    }
}
...
// A global in-memory store for mapping EfficiencyCode_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(EfficiencyCode_home: PathBuf) -> Self {
        Self {
            EfficiencyCode_home,
        }
    }
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(EfficiencyCode_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(EfficiencyCode_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => {
            Arc::new(EphemeralAuthStorage::new(EfficiencyCode_home))
        }
    }
}
