use super::manager::EfficiencyCodeAuth;
...
impl BearerTokenRefresher {
    ...
    async fn resolve(&self) -> io::Result<EfficiencyCodeAuth> {
        let access_token = {
            ...
            if let Some(cached_token) = cached.as_ref() {
                ...
                if should_use_cached_token {
                    return Ok(EfficiencyCodeAuth::from_api_key(
        ...
        Ok(EfficiencyCodeAuth::from_api_key(access_token.as_str()))
    }

    async fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> io::Result<EfficiencyCodeAuth> {
        let access_token = run_provider_auth_command(&self.state.config).await?;
        let mut cached = self.state.cached_token.lock().await;
        *cached = Some(CachedExternalBearerToken {
            access_token: access_token.clone(),
