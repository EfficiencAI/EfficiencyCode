...
use EfficiencyCode_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = EfficiencyCode_home.clone();

    let opts = ServerOptions {
        EfficiencyCode_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: EfficiencyCode_login::test_support::transport_default_auth_route_config(
        ),
        client_id: EfficiencyCode_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = EfficiencyCode_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/EfficiencyCode/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_EfficiencyCode_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let EfficiencyCode_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = HttpClientBuilder::new().build_direct()?;
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_EfficiencyCode_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to EfficiencyCode"),
        "error body should clearly explain the EfficiencyCode access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_EfficiencyCode_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to EfficiencyCode to retry"),
    ...
    assert!(
        !body.contains("get access to EfficiencyCode"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not EfficiencyCode"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        EfficiencyCode_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_EfficiencyCode_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        EfficiencyCode_home: first_EfficiencyCode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: EfficiencyCode_login::test_support::transport_default_auth_route_config(
        ),
        client_id: EfficiencyCode_login::CLIENT_ID.to_string(),
    ...
    let second_EfficiencyCode_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        EfficiencyCode_home: second_EfficiencyCode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: EfficiencyCode_login::test_support::transport_default_auth_route_config(
        ),
        client_id: EfficiencyCode_login::CLIENT_ID.to_string(),
