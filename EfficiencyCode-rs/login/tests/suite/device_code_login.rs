...
use EfficiencyCode_login::auth::load_auth_dot_json;
...
fn server_opts(
    EfficiencyCode_home: &tempfile::TempDir,
    issuer: String,
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    let opts = server_opts(&EfficiencyCode_home, issuer, AuthCredentialsStoreMode::File);
...
async fn device_code_login_rejects_workspace_mismatch() -> anyhow::Result<()> {
    ...
    let issuer = mock_server.uri();
    let mut opts = server_opts(&EfficiencyCode_home, issuer, AuthCredentialsStoreMode::File);
