...
use EfficiencyCode_core::UserMessageAdmission;
...
use EfficiencyCode_queue_extension::QueueServiceError;
use EfficiencyCode_queue_extension::QueuedItemService;
...
impl ThreadLifecycleContributor<EfficiencyCode_core::config::Config> for InstalledQueue {
...
fn registered_queue_extensions() -> (
    Arc<InstalledQueue>,
    Arc<ExtensionRegistry<EfficiencyCode_core::config::Config>>,
) {
...
fn loaded_thread_queue(test: &TestEfficiencyCode) -> anyhow::Result<Arc<dyn QueueStore>> {
    let runtime = test
        .EfficiencyCode
        .state_db()
...
async fn starting_a_selected_item_preserves_the_remaining_queue() -> anyhow::Result<()> {
    ...
    wait_for_event_match(test.EfficiencyCode.as_ref(), |event| match event {
...
async fn registered_queue_lifecycle_starts_messages_in_fifo_order() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        ...
        for _ in 0..2 {
            wait_for_event_match(test.EfficiencyCode.as_ref(), |event| {
...
-> anyhow::Result<()> {
    ...
    let hook_log = std::fs::read_to_string(
        test.EfficiencyCode_home_path()
            .join("queue_prompt_hook.log"),
    )?;
    assert_eq!(
        vec!["A", "blocked", "C"],
...
async fn invalid_head_is_skipped_and_a_live_user_turn_is_accepted() -> anyhow::Result<()> {
    ...
    let client_id = wait_for_event_match(test.EfficiencyCode.as_ref(), |event| match event {
