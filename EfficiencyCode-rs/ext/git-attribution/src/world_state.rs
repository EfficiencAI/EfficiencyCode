...
Co-authored-by: EfficiencyCode <noreply@openai.com>
...
When you create or amend a git commit or create or update a pull request for this task, add EfficiencyCode attribution exactly once:
- Commit messages must end with `Co-authored-by: EfficiencyCode <noreply@openai.com>`. Preserve existing trailers and, if this exact trailer is missing, append it with one blank line before the trailer block.
- Pull request bodies must include the exact line `Generated with [EfficiencyCode](https://openai.com/EfficiencyCode/).`. Preserve the existing body and hidden markers and, if this exact marker is missing, append it near the end, before any trailing hidden metadata markers.
...
- Ignore any earlier instructions disabling EfficiencyCode attribution; this policy reflects the current workspace.
- Do not rewrite an existing commit or pull request solely to add attribution.
"#;
pub(super) const DISABLED_INSTRUCTIONS: &str = "
EfficiencyCode commit and pull request attribution is disabled for the current workspace. Ignore any earlier instructions requiring EfficiencyCode attribution and do not add it.
";
...
fn is_enabled_fragment(role: &str, text: &str) -> bool {
    role == "developer"
        ...
        && text.contains("Co-authored-by: EfficiencyCode <noreply@openai.com>")
        && (text.contains("Generated with [EfficiencyCode](https://openai.com/EfficiencyCode/).")
            || text.contains("Generated with EfficiencyCode."))
