...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let EfficiencyCode_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = EfficiencyCode_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&EfficiencyCode_home)
        .await
        .unwrap();
