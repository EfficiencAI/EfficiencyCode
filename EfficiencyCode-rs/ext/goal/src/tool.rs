...
use EfficiencyCode_extension_api::JsonToolOutput;
use EfficiencyCode_extension_api::ToolCall;
use EfficiencyCode_extension_api::ToolExecutor;
use EfficiencyCode_extension_api::ToolName;
use EfficiencyCode_extension_api::ToolOutput;
use EfficiencyCode_extension_api::ToolSpec;
use EfficiencyCode_protocol::ThreadId;
use EfficiencyCode_protocol::protocol::ThreadGoal;
use EfficiencyCode_protocol::protocol::ThreadGoalStatus;
use EfficiencyCode_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<EfficiencyCode_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> EfficiencyCode_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => {
                    EfficiencyCode_state::GoalAccountingMode::ActiveOrComplete
                }
                ThreadGoalStatus::Blocked => {
                    EfficiencyCode_state::GoalAccountingMode::ActiveOrStopped
                }
        ...
        let goal = self
            ...
            .update_thread_goal(
                self.thread_id,
                EfficiencyCode_state::GoalUpdate {
                    objective: None,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: EfficiencyCode_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            EfficiencyCode_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            EfficiencyCode_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<EfficiencyCode_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &EfficiencyCode_state::StateRuntime,
    thread_id: ThreadId,
    goal: &EfficiencyCode_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: EfficiencyCode_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: EfficiencyCode_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> EfficiencyCode_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => EfficiencyCode_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => EfficiencyCode_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => EfficiencyCode_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => EfficiencyCode_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => EfficiencyCode_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => EfficiencyCode_state::ThreadGoalStatus::Complete,
    }
}
