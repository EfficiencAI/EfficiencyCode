...
use EfficiencyCode_extension_api::ConfigContributor;
...
use EfficiencyCode_extension_api::ThreadIdleInput;
...
use EfficiencyCode_extension_api::TokenUsageContributor;
...
use EfficiencyCode_extension_api::TurnAbortInput;
use EfficiencyCode_extension_api::TurnErrorInput;
use EfficiencyCode_extension_api::TurnLifecycleContributor;
...
pub struct GoalExtension<C> {
    state_dbs: Arc<EfficiencyCode_state::StateRuntime>,
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                EfficiencyCode_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                EfficiencyCodeErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        Arc<dyn EfficiencyCode_extension_api::ToolExecutor<EfficiencyCode_extension_api::ToolCall>>,
    > {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
