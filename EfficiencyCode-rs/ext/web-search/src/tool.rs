...
use EfficiencyCode_api::SearchClient;
use EfficiencyCode_api::SearchCommands;
use EfficiencyCode_api::SearchQuery;
use EfficiencyCode_api::SearchRequest;
use EfficiencyCode_api::SearchSettings;
use EfficiencyCode_core::X_EfficiencyCode_TURN_METADATA_HEADER;
use EfficiencyCode_core::web_search_action_detail;
use EfficiencyCode_extension_api::ExtensionTurnItem;
use EfficiencyCode_extension_api::FunctionCallError;
use EfficiencyCode_extension_api::ResponsesApiTool;
...
use EfficiencyCode_extension_api::parse_tool_input_schema_without_compaction;
use EfficiencyCode_extension_items::ExtensionItem;
use EfficiencyCode_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "EfficiencyCode.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.EfficiencyCode_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = EfficiencyCode_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_EfficiencyCode_TURN_METADATA_HEADER, header_value);
    }
