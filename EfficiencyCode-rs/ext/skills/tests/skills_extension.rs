...
use EfficiencyCode_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use EfficiencyCode_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use EfficiencyCode_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use EfficiencyCode_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use EfficiencyCode_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use EfficiencyCode_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
use EfficiencyCode_protocol::protocol::SessionSource;
use EfficiencyCode_protocol::protocol::SkillScope;
use EfficiencyCode_protocol::protocol::TruncationPolicy;
use EfficiencyCode_protocol::protocol::TurnEnvironmentSelection;
use EfficiencyCode_protocol::user_input::UserInput;
use EfficiencyCode_skills::SkillMetadata;
use EfficiencyCode_skills_extension::HostSkillProvider;
use EfficiencyCode_skills_extension::HostSkillsLoadInput;
use EfficiencyCode_skills_extension::HostSkillsService;
use EfficiencyCode_skills_extension::HostSkillsSnapshot;
use EfficiencyCode_skills_extension::InjectedHostSkillPrompts;
use EfficiencyCode_skills_extension::SkillLoadOutcome;
...
use EfficiencyCode_skills_extension::install_with_providers_and_metrics;
use EfficiencyCode_skills_extension::provider::SkillListQuery;
use EfficiencyCode_skills_extension::provider::SkillProvider;
...
static NEXT_EfficiencyCode_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn skill_world_state_fragments(
    registry: &EfficiencyCode_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: EfficiencyCode_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let EfficiencyCode_home = test_EfficiencyCode_home();
    ...
    std::fs::remove_dir_all(EfficiencyCode_home)?;
    Ok(())
}
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path =
        AbsolutePathBuf::try_from(test_EfficiencyCode_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_EfficiencyCode_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory(
            "test",
            "EfficiencyCode-skills-extension",
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "EfficiencyCode.skills.shadow_selection.catalog_entries")
...
async fn skills_list_only_returns_model_visible_bounded_metadata() -> TestResult {
    ...
    let output = list_tool
        .handle(ToolCall {
            ...
            EfficiencyCode_turn_metadata: None,
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    let test_root = test_EfficiencyCode_home();
    ...
    let EfficiencyCode_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(EfficiencyCode_home, Vec::new(), config_layer_stack),
...
fn test_EfficiencyCode_home() -> PathBuf {
    let id = NEXT_EfficiencyCode_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "EfficiencyCode-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
