...
use EfficiencyCode_skills::SkillInterface;
    ...
    assert_eq!(
        ...
        vec![SkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::EfficiencyCode, Product::Chatgpt, Product::Atlas],
            }),
...
async fn discovers_nested_plugin_namespace_without_plugin_identity() {
    ...
    let plugin_manifest = root
        .path()
        .join("nested/.EfficiencyCode-plugin/plugin.json");
...
async fn recursive_plugin_root_preserves_owner_namespace_and_shared_asset_policy() {
    ...
    let nested_manifest = root
        .path()
        .join("skills/group/.EfficiencyCode-plugin/plugin.json");
