...
use EfficiencyCode_exec_server::CapabilityRootDiscoverRequest;
use EfficiencyCode_exec_server::CapabilityRootsDiscoverParams;
use EfficiencyCode_exec_server::LOCAL_FS;
use EfficiencyCode_exec_server::discover_capability_roots;
use EfficiencyCode_protocol::protocol::Product;
use EfficiencyCode_skills::EnvironmentSkillMetadata;
use EfficiencyCode_skills::SkillDependencies;
use EfficiencyCode_skills::SkillPolicy;
...
async fn direct_environment_loader_preserves_plugin_dependencies_and_product_policy() {
    ...
    fs::create_dir_all(root.path().join(".EfficiencyCode-plugin")).expect("manifest dir");
    fs::create_dir_all(skill_dir.join("agents")).expect("metadata dir");
    fs::write(
        root.path().join(".EfficiencyCode-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )
    .expect("manifest");
    fs::write(
    ...
    assert_eq!(
        ...
        vec![EnvironmentSkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::EfficiencyCode, Product::Atlas],
            }),
        }]
    );
    let atlas =
...
async fn executor_bundle_parser_matches_direct_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".EfficiencyCode-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".EfficiencyCode-plugin/plugin.json", "EfficiencyCode-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.EfficiencyCode-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(
        bundled.skills[0].metadata.name,
        "EfficiencyCode-name:search"
    );
}
