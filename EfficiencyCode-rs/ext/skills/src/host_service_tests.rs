...
use EfficiencyCode_skills::ImplicitSkillLookup;
use EfficiencyCode_skills::LoadedSkillRoot;
use EfficiencyCode_skills::SkillRootSnapshotCache;
...
fn write_user_skill(EfficiencyCode_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = EfficiencyCode_home.path().join("skills").join(dir);
...
fn user_config_layer(EfficiencyCode_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(EfficiencyCode_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(EfficiencyCode_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(EfficiencyCode_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(EfficiencyCode_home, user_config_toml),
            ConfigLayerEntry::new(
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&EfficiencyCode_home, "");
    let skills_service = HostSkillsService::new(
        EfficiencyCode_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

    write_user_skill(&EfficiencyCode_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&EfficiencyCode_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_service, &cwd, &config_layer_stack, &[]).await;
...
async fn watchable_skill_root_paths_exclude_plugin_and_system_roots() {
    ...
    assert!(watchable_paths.contains(&EfficiencyCode_home.path().join("skills").abs()));
    assert!(!watchable_paths.contains(&plugin_skill_root.path));
    assert!(!watchable_paths.contains(&EfficiencyCode_home.path().join("skills/.system").abs()));
}
...
async fn snapshot_for_config_merges_extension_host_and_legacy_plugin_roots() {
    ...
    let config_layer_stack =
        config_stack(&EfficiencyCode_home, "[skills.bundled]\nenabled = false\n");
    let input = HostSkillsLoadInput::new(
        cwd.path().abs(),
...
async fn snapshot_for_config_preserves_host_precedence_for_symlinked_plugin_root() {
    ...
    symlink(
        ...
        EfficiencyCode_home.path().join("skills"),
    )
    ...
    assert_eq!(
        outcome.skills,
        vec![EfficiencyCode_skills::SkillMetadata {
...
async fn skills_list_snapshots_share_host_roots_only_within_one_request() {
    ...
    write_user_skill(&EfficiencyCode_home, "first", "first-skill", "first skill");
    let first = request
        .snapshot_for_cwd(
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let EfficiencyCode_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_EfficiencyCode = cwd.path().join(".EfficiencyCode");
    fs::create_dir_all(&repo_dot_EfficiencyCode).expect("create repo config dir");
    ...
    let repo_skill_dir = repo_dot_EfficiencyCode.join("skills/repo");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&EfficiencyCode_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_EfficiencyCode_folder: repo_dot_EfficiencyCode.abs(),
                },
    ...
    let other_file_system: Arc<dyn EfficiencyCode_exec_server::ExecutorFileSystem> =
        Arc::new(EfficiencyCode_exec_server::LocalFileSystem::unsandboxed());
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let EfficiencyCode_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = EfficiencyCode_home
        .path()
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_stack = config_stack(&EfficiencyCode_home, &disabled_skill_config);
