...
use EfficiencyCode_protocol::mcp::ResourceContent;
...
const ORCHESTRATOR_SKILL_DISCOVERY_DURATION_METRIC: &str =
    "EfficiencyCode.skills.orchestrator.discovery.duration_ms";
const ORCHESTRATOR_SKILL_DISCOVERY_RESOURCES_METRIC: &str =
    "EfficiencyCode.skills.orchestrator.discovery.resources_total";
...
impl SkillProvider for OrchestratorSkillProvider {
    fn list(&self, query: SkillListQuery) -> SkillProviderFuture<'_, SkillCatalog> {
        Box::pin(async move {
            ...
            if !client.has_server(EfficiencyCode_APPS_MCP_SERVER_NAME).await {
                return Ok(SkillCatalog::default());
            }

            let _discovery_timer = EfficiencyCode_otel::start_global_timer(
            ...
            for _ in 0..MAX_RESOURCE_PAGES {
                let page = match tokio::time::timeout_at(
                    discovery_deadline,
                    client.list_resources(EfficiencyCode_APPS_MCP_SERVER_NAME, cursor.clone()),
                )
                .await
                {
