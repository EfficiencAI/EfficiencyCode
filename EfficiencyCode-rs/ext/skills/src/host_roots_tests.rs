...
use EfficiencyCode_exec_server::WalkOptions;
use EfficiencyCode_exec_server::WalkOutcome;
...
fn user_layer(EfficiencyCode_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: EfficiencyCode_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn project_layer(dot_EfficiencyCode_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/EfficiencyCode"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("EfficiencyCode");
    let project_folder = absolute(temp_dir.path().join("repo/.EfficiencyCode"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.EfficiencyCode"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_EfficiencyCode_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_EfficiencyCode = workspace.join(".EfficiencyCode");
    let skill_root = dot_EfficiencyCode.join("skills");
    fs::create_dir_all(&workspace).expect("create workspace");
    let skill_path = write_skill(&skill_root, "local", "local-skill");
    let config_stack = stack(vec![project_layer(&dot_EfficiencyCode)]);
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_EfficiencyCode = repository.join(".EfficiencyCode");
    let skill_root = dot_EfficiencyCode.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let EfficiencyCode_home = home_folder.join("EfficiencyCode");
    let system_folder = absolute(temp_dir.path().join("etc/EfficiencyCode"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_EfficiencyCode = repository.join(".EfficiencyCode");
    let nested_project_dot_EfficiencyCode = repository.join("nested/.EfficiencyCode");
    let user_skills = EfficiencyCode_home.join("skills");
    let root_project_skill = write_skill(
        &project_dot_EfficiencyCode.join("skills"),
        "root-duplicate",
        "duplicate-skill",
    );
    let nested_project_skill = write_skill(
        &nested_project_dot_EfficiencyCode.join("skills"),
        "nested-duplicate",
        "duplicate-skill",
    );
    ...
    let system_skill = write_skill(
        &EfficiencyCode_home.join("skills/.system"),
        "system",
        "system-skill",
    );
    ...
    let config_stack = stack(vec![
        ...
        user_layer(&EfficiencyCode_home),
        project_layer(&project_dot_EfficiencyCode),
        project_layer(&nested_project_dot_EfficiencyCode),
    ]);
