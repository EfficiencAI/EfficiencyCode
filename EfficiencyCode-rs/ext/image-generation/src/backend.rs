...
use EfficiencyCode_api::ImageResponse;
use EfficiencyCode_api::ImagesClient;
...
const X_EfficiencyCode_IMAGE_TURN_ID_HEADER: &str = "x-EfficiencyCode-image-turn-id";
...
impl ImageBackendError {
    fn from_api(error: EfficiencyCode_api::ApiError) -> Self {
        let message = error.to_string();
        Self {
            message,
            EfficiencyCode_error: map_api_error(error),
        }
    }

    fn from_message(message: String) -> Self {
        Self {
            EfficiencyCode_error: EfficiencyCodeErr::Stream(message.clone()),
            message,
        }
    }
    ...
    pub(crate) fn EfficiencyCode_error(&self) -> &EfficiencyCodeErr {
        &self.EfficiencyCode_error
    }
}

#[derive(Clone)]
pub(crate) struct EfficiencyCodeImagesBackend {
    provider: SharedModelProvider,
    originator: Option<String>,
}

impl EfficiencyCodeImagesBackend {
...
fn image_request_headers(originator: Option<&str>, turn_id: &str) -> HeaderMap {
    ...
    if let Ok(turn_id) = HeaderValue::from_str(turn_id) {
        headers.insert(X_EfficiencyCode_IMAGE_TURN_ID_HEADER, turn_id);
    }
