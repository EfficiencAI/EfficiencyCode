...
use EfficiencyCode_api::ImageBackground;
use EfficiencyCode_api::ImageEditRequest;
use EfficiencyCode_api::ImageGenerationRequest;
use EfficiencyCode_api::ImageQuality;
use EfficiencyCode_api::ImageUrl;
...
use EfficiencyCode_extension_api::ExtensionTurnItem;
use EfficiencyCode_extension_api::FunctionCallError;
use EfficiencyCode_extension_api::ToolCall;
use EfficiencyCode_extension_api::ToolEnvironment;
...
use EfficiencyCode_extension_api::parse_tool_input_schema;
...
use EfficiencyCode_utils_image::PromptImageMode;
use EfficiencyCode_utils_image::load_for_prompt_bytes;
...
use crate::backend::EfficiencyCodeImagesBackend;
...
pub(crate) struct ImageGenerationTool {
    backend: EfficiencyCodeImagesBackend,
...
impl ToolExecutor<ToolCall> for ImageGenerationTool {
    ...
    fn handle(&self, call: ToolCall) -> EfficiencyCode_extension_api::ToolExecutorFuture<'_> {
    ...
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        .map_err(|error| {
            (
                ...
                usage_limit_failure(error.EfficiencyCode_error()),
            )
        })
        .and_then(|response| {
...
fn usage_limit_failure(error: &EfficiencyCodeErr) -> Option<ImageGenerationFailure> {
    let EfficiencyCodeErrorDetails::UsageLimitReached(usage_limit) = error.details() else {
        return None;
    };
