//! Custom CA handling shared by EfficiencyCode outbound HTTP and websocket clients.
//!
//! EfficiencyCode constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `EfficiencyCode_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because EfficiencyCode itself commonly runs spawned test processes under seatbelt, so this is not just
...
use EfficiencyCode_utils_rustls_provider::ensure_rustls_crypto_provider;
...
pub const EfficiencyCode_CA_CERT_ENV: &str = "EfficiencyCode_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set EfficiencyCode_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors EfficiencyCode custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `EfficiencyCode_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound EfficiencyCode HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a EfficiencyCode custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `EfficiencyCode_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
/// Builds a rustls client config using native roots and any configured EfficiencyCode custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the EfficiencyCode custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        EfficiencyCode_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `EfficiencyCode_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the EfficiencyCode-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(EfficiencyCode_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: EfficiencyCode_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for EfficiencyCode CA overrides.
    ///
    /// This accepts a few common real-world variants so EfficiencyCode behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// EfficiencyCode only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    use super::EfficiencyCode_CA_CERT_ENV;
    use super::EnvSource;
    ...
    #[test]
    fn ca_path_prefers_EfficiencyCode_env() {
        let env = map_env(&[
            (EfficiencyCode_CA_CERT_ENV, "/tmp/EfficiencyCode.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/EfficiencyCode.pem"))
        );
    }
