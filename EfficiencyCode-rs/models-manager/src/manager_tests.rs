...
use EfficiencyCode_login::ExternalAuth;
use EfficiencyCode_login::ExternalAuthRefreshContext;
use EfficiencyCode_login::TokenData;
...
struct TestModelsEndpoint {
    ...
    uses_EfficiencyCode_backend: bool,
...
impl TestModelsEndpoint {
    fn new(responses: Vec<Vec<ModelInfo>>) -> Arc<Self> {
        Arc::new(Self {
            ...
            uses_EfficiencyCode_backend: true,
    ...
    fn without_refresh(responses: Vec<Vec<ModelInfo>>) -> Arc<Self> {
        Arc::new(Self {
            has_command_auth: false,
            uses_EfficiencyCode_backend: false,
            responses: Mutex::new(responses.into()),
            fetch_count: AtomicUsize::new(0),
            observed_proxy_policy: Mutex::new(None),
        })
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> EfficiencyCode_login::ExternalAuthFuture<'_, EfficiencyCodeAuth> {
        Box::pin(async { Ok(EfficiencyCodeAuth::from_api_key("test-external-api-key")) })
    }

    fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> EfficiencyCode_login::ExternalAuthFuture<'_, EfficiencyCodeAuth> {
        Box::pin(async { Ok(EfficiencyCodeAuth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_EfficiencyCode_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_EfficiencyCode_backend })
    }
...
fn openai_manager_for_tests(
    EfficiencyCode_home: std::path::PathBuf,
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(EfficiencyCode_home, endpoint_client, auth_manager)
}

async fn mutate_file_cache_for_test<F>(EfficiencyCode_home: &Path, f: F)
where
    F: FnOnce(&mut ModelsCacheEntry),
{
    let client_version = crate::client_version_to_whole();
    let cache = FileModelsCache::new(
        EfficiencyCode_home.join(MODEL_CACHE_FILE),
...
async fn file_cache_implements_models_cache_contract() {
    let EfficiencyCode_home = tempdir().expect("temp dir");
    let cache = FileModelsCache::new(
        EfficiencyCode_home.path().join(MODEL_CACHE_FILE),
        DEFAULT_MODEL_CACHE_TTL,
    );
    let client_version = crate::client_version_to_whole();
...
async fn chatgpt_auth_tokens_for_tests(EfficiencyCode_home: &Path) -> EfficiencyCodeAuth {
    let auth_dot_json = EfficiencyCode_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: EfficiencyCode_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(EfficiencyCode_home).expect("EfficiencyCode home should be created");
    std::fs::write(
    ...
    EfficiencyCodeAuth::from_auth_storage(
        EfficiencyCode_home,
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager =
        openai_manager_for_tests(EfficiencyCode_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-EfficiencyCode/gpt-image".to_string();
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(EfficiencyCode_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_refetches_when_cache_stale() {
    ...
    mutate_file_cache_for_test(EfficiencyCode_home.path(), |cache| {
...
impl TestAuthAwareModelsEndpoint {
    ...
    async fn uses_EfficiencyCode_backend(&self) -> bool {
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_EfficiencyCode_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_EfficiencyCode_backend(
            self,
        ))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(EfficiencyCode_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        EfficiencyCode_home.path().to_path_buf(),
        endpoint.clone(),
