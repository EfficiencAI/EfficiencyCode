...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "EfficiencyCode-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "EfficiencyCode.turns",
        "Total number of EfficiencyCode turns.",
        /*inc*/ 1,
        &[("model", "gpt-5.1"), ("env", "dev")],
    )?;
    metrics.histogram(
        "EfficiencyCode.tool_latency",
        /*value*/ 25,
        &[("tool", "shell")],
    )?;
    ...
    let counter =
        find_metric(&resource_metrics, "EfficiencyCode.turns").expect("counter metric missing");
    assert_eq!(
        counter.description(),
        "Total number of EfficiencyCode turns."
    );
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "EfficiencyCode-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "EfficiencyCode.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "EfficiencyCode.tool_latency")
            ...
            .expect("EfficiencyCode.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge =
        find_metric(&resource_metrics, "EfficiencyCode.active").expect("gauge metric missing");
    assert_eq!(
        gauge.description(),
        "Number of active EfficiencyCode operations."
    );
...
fn send_merges_default_tags_per_line() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[
        ("service", "EfficiencyCode-cli"),
        ("env", "prod"),
        ("region", "us"),
    ])?;

    metrics.counter(
        "EfficiencyCode.alpha",
        /*inc*/ 1,
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric = find_metric(&resource_metrics, "EfficiencyCode.alpha")
        .expect("EfficiencyCode.alpha metric missing");
    ...
    let beta_metric = find_metric(&resource_metrics, "EfficiencyCode.beta")
        .expect("EfficiencyCode.beta metric missing");
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("EfficiencyCode.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
