...
use EfficiencyCode_otel::PLUGIN_INSTALL_ELICITATION_SENT_METRIC;
use EfficiencyCode_otel::PLUGIN_INSTALL_SUGGESTION_METRIC;
...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "EfficiencyCode-cli")])?;
    ...
    manager.counter(
        "EfficiencyCode.session_started",
        /*inc*/ 1,
        &[("source", "tui")],
    );
    ...
    let metric = find_metric(&resource_metrics, "EfficiencyCode.session_started")
...
fn manager_attaches_optional_service_name_tag() -> Result<()> {
    ...
    manager.counter("EfficiencyCode.session_started", /*inc*/ 1, &[]);
    manager.shutdown_metrics()?;
