...
use EfficiencyCode_otel::MetricsError;
...
fn counter_rejects_negative_increment() -> Result<()> {
    let metrics = build_in_memory_client()?;
    let err = metrics
        .counter("EfficiencyCode.turns", /*inc*/ -1, &[])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::NegativeCounterIncrement { name, inc } if name == "EfficiencyCode.turns" && inc == -1
    ));
    metrics.shutdown()?;
    Ok(())
}
