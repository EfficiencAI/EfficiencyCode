...
use EfficiencyCode_otel::OtelExporter;
use EfficiencyCode_otel::OtelHttpProtocol;
use EfficiencyCode_otel::OtelProvider;
use EfficiencyCode_otel::OtelSettings;
use EfficiencyCode_otel::Result;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter(
        "EfficiencyCode.turns",
        /*inc*/ 1,
        &[("source", "test")],
    )?;
    metrics.counter(
        "EfficiencyCode.api_request",
        /*inc*/ 1,
        &[("status", "200")],
    )?;
    metrics.record_duration(
        "EfficiencyCode.api_request.duration_ms",
    ...
    metrics.counter(
        "EfficiencyCode.tool.call",
        /*inc*/ 1,
        &[("tool", "test")],
    )?;
    metrics.record_duration(
        "EfficiencyCode.tool.call.duration_ms",
        Duration::from_millis(42),
        &[("tool", "test")],
    )?;
    metrics.histogram(
        "EfficiencyCode.turn.token_usage",
        /*value*/ 100,
        &[("token_type", "total")],
    )?;
    metrics.gauge_with_description(
        "EfficiencyCode.active",
        "Number of active EfficiencyCode operations.",
        /*value*/ 1,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    assert!(
        body.contains("EfficiencyCode.turns"),
        "expected metric name not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("EfficiencyCode.active"),
        "expected gauge not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.api_request\""),
        "expected API-request counter not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.api_request.duration_ms\""),
        "expected API-request duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.conversation.turn.count\""),
        "expected conversation turn count not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.responses_api_engine_iapi_ttft.duration_ms\""),
        "expected engine IAPI TTFT duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.responses_api_engine_service_tbt.duration_ms\""),
        "expected engine service TBT duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.responses_api_engine_service_ttft.duration_ms\""),
        "expected engine service TTFT duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.tool.call\""),
        "expected tool-call counter not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.turn.token_usage\""),
        "expected turn-token histogram not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"EfficiencyCode.tool.call.duration_ms\""),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let otel = OtelProvider::from(&OtelSettings {
        ...
        service_name: "EfficiencyCode-cli".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        EfficiencyCode_home: PathBuf::from("."),
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "EfficiencyCode.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("EfficiencyCode.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "EfficiencyCode.test.trace_event",
    ...
    assert!(
        body.contains("EfficiencyCode-cli"),
    ...
    assert!(
        body.contains("EfficiencyCode.test.trace_event"),
