...
fn snapshot_collects_metrics_without_shutdown() -> Result<()> {
    ...
    .with_tag("service", "EfficiencyCode-cli")?
    .with_runtime_reader();
    ...
    let metric =
        find_metric(&snapshot, "EfficiencyCode.tool.call").expect("counter metric missing");
...
fn observable_gauge_is_collected_on_every_delta_snapshot() -> Result<()> {
    ...
    for snapshot in [metrics.snapshot()?, metrics.snapshot()?] {
        let gauge = find_metric(&snapshot, "EfficiencyCode.active").expect("gauge metric missing");
