...
`EfficiencyCode-otel` is the OpenTelemetry integration crate for EfficiencyCode. It provides:

- Provider wiring for log/trace/metric exporters (`EfficiencyCode_otel::OtelProvider`
  and `EfficiencyCode_otel::provider`).
- Session-scoped business event emission via `EfficiencyCode_otel::SessionTelemetry`.
- Low-level metrics APIs via `EfficiencyCode_otel::metrics`.
- Trace-context helpers via `EfficiencyCode_otel::trace_context` and crate-root re-exports.
...
```rust
use EfficiencyCode_otel::config::OtelExporter;
use EfficiencyCode_otel::config::OtelHttpProtocol;
use EfficiencyCode_otel::config::OtelSettings;
...
let settings = OtelSettings {
    ...
    EfficiencyCode_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, EfficiencyCode
...
EfficiencyCode-specific session events. Rich session/business events should go through
...
`EfficiencyCode-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using EfficiencyCode-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use EfficiencyCode_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("EfficiencyCode.session_started", 1, &[("source", "tui")])?;
metrics.histogram("EfficiencyCode.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("EfficiencyCode.turns", 1, &[("model", "gpt-5.1")])?;
