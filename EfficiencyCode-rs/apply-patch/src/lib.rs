...
use EfficiencyCode_utils_path_uri::PathUriParseError;
use anyhow::Context;
use anyhow::Result;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the EfficiencyCode executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `EfficiencyCode-apply-patch` (to avoid forcing
/// `EfficiencyCode-arg0` to depend on `EfficiencyCode-core`), it remains part of the "EfficiencyCode core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const EfficiencyCode_CORE_APPLY_PATCH_ARG1: &str = "--EfficiencyCode-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const EfficiencyCode_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "EfficiencyCode_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(EfficiencyCode_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {
