# Bazel in EfficiencyCode-rs

This repository uses Bazel to build the Rust workspace under `EfficiencyCode-rs`.
...
- `rules_rs` imports third-party crates from `EfficiencyCode-rs/Cargo.toml` and
  `EfficiencyCode-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `EfficiencyCode_rust_crate`, which wraps `rust_library`,
...
- Each crate in `EfficiencyCode-rs/*/BUILD.bazel` typically uses `EfficiencyCode_rust_crate` and
...
EfficiencyCode uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/EfficiencyCode` | Yes | Yes |
| `workflow_dispatch` in `openai/EfficiencyCode` | Yes | Yes |
| Same-repository pull request in `openai/EfficiencyCode` | Yes | Yes |
| Fork pull request into `openai/EfficiencyCode` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/EfficiencyCode \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //EfficiencyCode-rs/cli:EfficiencyCode
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `openai/EfficiencyCode`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `EfficiencyCode_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `EfficiencyCode_rust_crate` to configure it.
