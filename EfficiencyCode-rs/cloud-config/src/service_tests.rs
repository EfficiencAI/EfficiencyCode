...
use EfficiencyCode_backend_client::ConfigBundleResponse;
use EfficiencyCode_backend_client::DeliveredTomlFragment;
use EfficiencyCode_config::AbsolutePathBuf;
use EfficiencyCode_config::CloudConfigFragment;
use EfficiencyCode_config::CloudConfigTomlBundle;
use EfficiencyCode_config::CloudRequirementsFragment;
use EfficiencyCode_config::CloudRequirementsTomlBundle;
...
fn write_auth_json(EfficiencyCode_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(
...
fn create_test_cache(EfficiencyCode_home: &Path) -> CloudConfigBundleCache {
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(EfficiencyCodeAuth::AgentIdentity(
...
struct TestExternalChatgptAuth {
    current: RwLock<EfficiencyCodeAuth>,
    refreshed: EfficiencyCodeAuth,
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let EfficiencyCode_home = tempdir().expect("tempdir");
    let cache = create_test_cache(EfficiencyCode_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = EfficiencyCodeAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = EfficiencyCodeAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
async fn dropping_loader_cancels_in_flight_startup_and_refresh() {
    for starts_from_cache in [false, true] {
        ...
        if starts_from_cache {
            create_test_cache(EfficiencyCode_home.path())
                .save(
                    Some("user-12345".to_string()),
                    Some("account-12345".to_string()),
                    test_bundle(),
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(
            EfficiencyCode_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            EfficiencyCode_backend_client::DeliveredRequirementsToml {
