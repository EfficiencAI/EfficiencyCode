...
use EfficiencyCode_config::CloudConfigBundle;
...
fn auth_identity(auth: &EfficiencyCodeAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &EfficiencyCodeAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let EfficiencyCode_home =
            AbsolutePathBuf::resolve_path_against_base(EfficiencyCode_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(EfficiencyCode_home.clone()),
            EfficiencyCode_home,
            timeout,
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer = EfficiencyCode_otel::start_global_timer(
            "EfficiencyCode.cloud_config_bundle.fetch.duration_ms",
            &[],
        );
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.EfficiencyCode_home)
                {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.EfficiencyCode_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",
