...
use EfficiencyCode_connectors::ConnectorMetadata;
use EfficiencyCode_connectors::ConnectorMetadataStore;
use EfficiencyCode_connectors::ConnectorToolSummary;
use EfficiencyCode_connectors::DirectoryListResponse;
use EfficiencyCode_connectors::merge::merge_connectors;
use EfficiencyCode_connectors::merge::merge_plugin_connectors;
use EfficiencyCode_core::config::Config;
pub use EfficiencyCode_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use EfficiencyCode_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use EfficiencyCode_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use EfficiencyCode_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use EfficiencyCode_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use EfficiencyCode_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "EfficiencyCode";
...
async fn connector_auth(config: &Config) -> anyhow::Result<EfficiencyCodeAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_EfficiencyCode_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        auth.uses_EfficiencyCode_backend(),
        "ChatGPT connectors require EfficiencyCode backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = EfficiencyCode_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = EfficiencyCode_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_EfficiencyCode_backend(),
        "ChatGPT backend requests require EfficiencyCode backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run EfficiencyCode login"
    );
...
mod tests {
    use super::*;
    use EfficiencyCode_connectors::metadata::connector_install_url;
    use EfficiencyCode_plugin::AppConnectorId;
