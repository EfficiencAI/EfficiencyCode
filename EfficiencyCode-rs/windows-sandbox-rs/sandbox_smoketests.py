...
# Run a suite of smoke tests against the Windows sandbox via the EfficiencyCode CLI
...
def _resolve_EfficiencyCode_cmd() -> List[str]:
    """Resolve the EfficiencyCode CLI to invoke `EfficiencyCode sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run EfficiencyCode.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "EfficiencyCode.exe",
        ws_root / "target" / "release" / "EfficiencyCode.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "EfficiencyCode.exe",
            cargo_base / "release" / "EfficiencyCode.exe",
        ])
    ...
    if shutil.which("EfficiencyCode"):
        return ["EfficiencyCode"]

    raise FileNotFoundError(
        "EfficiencyCode CLI not found. Build it first, e.g.\n"
        "  cargo build -p EfficiencyCode-cli --release\n"
        "or for debug:\n"
        "  cargo build -p EfficiencyCode-cli\n"
    )

EfficiencyCode_CMD = _resolve_EfficiencyCode_cmd()
print(EfficiencyCode_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to EfficiencyCode CLI overrides.
    ...
    argv = [*EfficiencyCode_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            proxy_home = WS_ROOT / ".EfficiencyCode_proxy_smoke"
            remove_if_exists(proxy_home)
            proxy_home.mkdir(parents=True, exist_ok=True)
            proxy_url = f"http://127.0.0.1:{proxy_port}"
            proxy_env = {
                "EfficiencyCode_HOME": str(proxy_home),
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"EfficiencyCode_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\EfficiencyCode_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.EfficiencyCode artifacts) denied
    EfficiencyCode_home = Path(os.environ["USERPROFILE"]) / ".EfficiencyCode"
    cap_sid_target = EfficiencyCode_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .EfficiencyCode\\policy.json"], WS_ROOT)
    add("WS: .EfficiencyCode cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .EfficiencyCode policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > EfficiencyCode_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://EfficiencyCode-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
