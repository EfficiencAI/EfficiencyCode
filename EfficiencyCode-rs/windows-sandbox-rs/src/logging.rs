...
pub fn current_log_file_path_for_EfficiencyCode_home(EfficiencyCode_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(EfficiencyCode_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_EfficiencyCode_home_uses_sandbox_dir() {
        let EfficiencyCode_home = Path::new("EfficiencyCode-home");

        assert_eq!(
            current_log_file_path_for_EfficiencyCode_home(EfficiencyCode_home),
            current_log_file_path(&EfficiencyCode_home.join(".sandbox"))
        );
    }
}
