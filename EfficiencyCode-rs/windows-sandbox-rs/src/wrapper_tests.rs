...
use super::EfficiencyCode_HOME_FLAG;
use super::EfficiencyCode_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "EfficiencyCode.exe".to_string(),
            "--EfficiencyCode-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.EfficiencyCode"),
    );

    assert_eq!(args[0], EfficiencyCode_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&EfficiencyCode_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["EfficiencyCode.exe", "--EfficiencyCode-run-as-fs-helper"]
    );
