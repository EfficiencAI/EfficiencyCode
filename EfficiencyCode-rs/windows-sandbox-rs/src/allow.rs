...
mod tests {
    ...
    #[test]
    fn denies_EfficiencyCode_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let EfficiencyCode_dir = command_cwd.join(".EfficiencyCode");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&EfficiencyCode_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&EfficiencyCode_dir).unwrap(),
