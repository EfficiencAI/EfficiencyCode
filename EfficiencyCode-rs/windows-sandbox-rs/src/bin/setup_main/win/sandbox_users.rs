...
use EfficiencyCode_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "EfficiencyCodeSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "EfficiencyCode sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(EfficiencyCode_home);
...
pub(super) fn prepare_setup_marker(EfficiencyCode_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(EfficiencyCode_home).join("setup_marker.json");
