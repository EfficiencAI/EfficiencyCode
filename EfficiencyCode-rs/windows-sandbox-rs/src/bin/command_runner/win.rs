...
use EfficiencyCode_utils_pty::JobObject;
use EfficiencyCode_windows_sandbox::ConsoleMode;
use EfficiencyCode_windows_sandbox::ErrorPayload;
use EfficiencyCode_windows_sandbox::ErrorStage;
use EfficiencyCode_windows_sandbox::ExitPayload;
use EfficiencyCode_windows_sandbox::FramedMessage;
use EfficiencyCode_windows_sandbox::IPC_PROTOCOL_VERSION;
use EfficiencyCode_windows_sandbox::LocalSid;
use EfficiencyCode_windows_sandbox::Message;
use EfficiencyCode_windows_sandbox::OutputPayload;
use EfficiencyCode_windows_sandbox::OutputStream;
use EfficiencyCode_windows_sandbox::PipeSpawnHandles;
use EfficiencyCode_windows_sandbox::ResizePayload;
use EfficiencyCode_windows_sandbox::SpawnReady;
use EfficiencyCode_windows_sandbox::SpawnRequest;
use EfficiencyCode_windows_sandbox::StderrMode;
use EfficiencyCode_windows_sandbox::StdinMode;
use EfficiencyCode_windows_sandbox::WindowsSandboxTokenMode;
use EfficiencyCode_windows_sandbox::allow_null_device;
use EfficiencyCode_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use EfficiencyCode_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use EfficiencyCode_windows_sandbox::decode_bytes;
use EfficiencyCode_windows_sandbox::encode_bytes;
use EfficiencyCode_windows_sandbox::get_current_token_for_restriction;
use EfficiencyCode_windows_sandbox::hide_current_user_profile_dir;
use EfficiencyCode_windows_sandbox::log_note;
use EfficiencyCode_windows_sandbox::read_frame;
use EfficiencyCode_windows_sandbox::read_handle_loop;
use EfficiencyCode_windows_sandbox::spawn_process_with_pipes;
use EfficiencyCode_windows_sandbox::to_wide;
use EfficiencyCode_windows_sandbox::token_mode_for_permission_profile;
use EfficiencyCode_windows_sandbox::write_frame;
use anyhow::Context;
use anyhow::Result;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with EfficiencyCode_exec_server::EfficiencyCode_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--EfficiencyCode-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\EfficiencyCodeSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<EfficiencyCode_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.EfficiencyCode_home.clone();
    hide_current_user_profile_dir(req.EfficiencyCode_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = EfficiencyCode_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
