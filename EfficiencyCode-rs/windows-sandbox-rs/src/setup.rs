...
pub const OFFLINE_USERNAME: &str = "EfficiencyCodeSandboxOffline";
pub const ONLINE_USERNAME: &str = "EfficiencyCodeSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "EfficiencyCode-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home.join(".sandbox")
}

pub fn sandbox_bin_dir(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(EfficiencyCode_home: &Path) -> PathBuf {
    EfficiencyCode_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(EfficiencyCode_home: &Path) -> PathBuf {
    sandbox_dir(EfficiencyCode_home).join("setup_marker.json")
}

pub fn sandbox_users_path(EfficiencyCode_home: &Path) -> PathBuf {
    sandbox_secrets_dir(EfficiencyCode_home).join("sandbox_users.json")
}
...
pub struct SandboxSetupRequest<'a> {
    ...
    pub EfficiencyCode_home: &'a Path,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, EfficiencyCode_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        EfficiencyCode_home: request.EfficiencyCode_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.EfficiencyCode_home)
    })
}

fn run_setup_refresh_payload(b64: &str, EfficiencyCode_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(EfficiencyCode_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(EfficiencyCode_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    cmd.arg(b64)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let cwd = std::env::current_dir().unwrap_or_else(|_| EfficiencyCode_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(EfficiencyCode_home) {
        log_note(
...
fn gather_helper_read_roots(EfficiencyCode_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(EfficiencyCode_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(EfficiencyCode_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, EfficiencyCode_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<EfficiencyCode_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "EfficiencyCode_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "EfficiencyCode_WINDOWS_SANDBOX_PROXY_PORTS";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(EfficiencyCode_home) {
...
fn verify_setup_completed(EfficiencyCode_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(EfficiencyCode_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, EfficiencyCode_home)
    })
}
...
fn run_setup_exe_payload(
    ...
    let cleared_report = match clear_setup_error_report(EfficiencyCode_home) {
        ...
        Err(err) => {
            log_note(
                ...
                Some(&sandbox_dir(EfficiencyCode_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(EfficiencyCode_home)?;
        if let Err(err) = clear_setup_error_report(EfficiencyCode_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.EfficiencyCode_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: EfficiencyCode_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.EfficiencyCode_home)
}
...
pub fn run_elevated_provisioning_setup(
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: EfficiencyCode_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.EfficiencyCode_home);
...
fn filter_sensitive_write_roots(
    mut roots: Vec<PathBuf>,
    EfficiencyCode_home: &Path,
) -> Vec<PathBuf> {
    // Never grant capability write access to EfficiencyCode_HOME or anything under EfficiencyCode_HOME/.sandbox,
    // EfficiencyCode_HOME/.sandbox-bin, or EfficiencyCode_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let EfficiencyCode_home_key = canonical_path_key(EfficiencyCode_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(EfficiencyCode_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(EfficiencyCode_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(EfficiencyCode_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != EfficiencyCode_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let EfficiencyCode_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(EfficiencyCode_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("EfficiencyCode.exe");
        let setup_exe = resources_dir.join("EfficiencyCode-windows-sandbox-setup.exe");
        fs::write(&exe, b"EfficiencyCode").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "EfficiencyCode_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_EfficiencyCode_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let EfficiencyCode_home = user_profile.join("EfficiencyCodeHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&EfficiencyCode_home).expect("create EfficiencyCode home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &EfficiencyCode_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let expected = dunce::canonicalize(helper_bin_dir(&EfficiencyCode_home))
            .expect("canonical helper dir");
    ...
    fn build_payload_roots_preserves_helper_roots_when_read_override_is_provided() {
        ...
        let expected_helper = dunce::canonicalize(helper_bin_dir(&EfficiencyCode_home))
            .expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&EfficiencyCode_home);
        fs::create_dir_all(&EfficiencyCode_home).expect("create EfficiencyCode home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_EfficiencyCode_home =
            dunce::canonicalize(&EfficiencyCode_home).expect("canonical EfficiencyCode home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_EfficiencyCode_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_EfficiencyCode = extra_write_root.join(".EfficiencyCode");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_EfficiencyCode).expect("create extra .EfficiencyCode");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_EfficiencyCode)
                    .expect("canonical extra .EfficiencyCode"),
                explicit_deny,
            ]
            .into_iter()
