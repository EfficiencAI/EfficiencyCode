//! Internal `EfficiencyCode.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const EfficiencyCode_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const EfficiencyCode_HOME_FLAG: &str = "--EfficiencyCode-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        EfficiencyCode_WINDOWS_SANDBOX_ARG1.to_string(),
        EfficiencyCode_HOME_FLAG.to_string(),
        EfficiencyCode_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            EfficiencyCode_home: request.EfficiencyCode_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            EfficiencyCode_HOME_FLAG => {
                EfficiencyCode_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?))
            }
            COMMAND_CWD_FLAG => {
    ...
    let EfficiencyCode_home = EfficiencyCode_home
        .ok_or_else(|| anyhow!("missing required {EfficiencyCode_HOME_FLAG}"))?;
    if !EfficiencyCode_home.is_absolute() {
        bail!(
            "{EfficiencyCode_HOME_FLAG} must be absolute: {}",
