    ...
    pub fn run_windows_sandbox_capture_for_permission_profile(
        ...
        let (sid_for_null, cap_sids) = if permissions.uses_write_capabilities_for_cwd(cwd, &env_map)
        {
            ...
            let cap_sids = write_roots
                .iter()
                .map(|root| workspace_write_cap_sid_for_root(EfficiencyCode_home, cwd, root))
        ...
        (|| -> Result<CaptureResult> {
            let spawn_request = SpawnRequest {
                ...
                EfficiencyCode_home: sandbox_base.clone(),
                real_EfficiencyCode_home: EfficiencyCode_home.to_path_buf(),
                cap_sids,
                network_proxy_restricting_sid,
                timeout_ms,
                tty: false,
