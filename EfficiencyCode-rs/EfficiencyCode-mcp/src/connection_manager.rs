//! Aggregates MCP server connections for EfficiencyCode.
//!
...
use startup::should_share_EfficiencyCode_apps_tools_cache;
...
use crate::pagination::MAX_EfficiencyCode_APPS_TOOL_CATALOG_ITEMS;
use crate::pagination::MAX_MCP_CATALOG_ITEMS;
use crate::rmcp_client::AsyncManagedClient;
use crate::rmcp_client::DEFAULT_TOOL_TIMEOUT;
use crate::rmcp_client::ManagedClient;
use crate::rmcp_client::StartupOutcomeError;
use crate::rmcp_client::prepare_EfficiencyCode_apps_tools_for_model;
...
use EfficiencyCode_diagnostics::Gauge;
use EfficiencyCode_diagnostics::GaugeGuard;
...
use EfficiencyCode_protocol::protocol::McpStartupCompleteEvent;
use EfficiencyCode_protocol::protocol::McpStartupFailure;
...
use EfficiencyCode_rmcp_client::determine_streamable_http_auth_status_from_credentials;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
...
impl McpServerView {
    async fn listed_tools(
        ...
        Some(
            if self.connection.client.is_EfficiencyCode_apps_mcp_server {
                prepare_EfficiencyCode_apps_tools_for_model(tools, tool_plugin_provenance)
            } else {
...
pub(crate) struct McpConnectionSet {
    ...
    EfficiencyCode_apps_tools_override: RwLock<Option<Vec<ToolInfo>>>,
    EfficiencyCode_apps_refresh_lock: Mutex<()>,
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let McpRuntimeInput {
            ...
            tool_catalog_cache,
            EfficiencyCode_apps_tools_cache_key,
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_EfficiencyCode_backend())
            .map(EfficiencyCode_model_provider::auth_provider_from_auth);
        let EfficiencyCode_apps_auth_provider =
            EfficiencyCode_apps_auth_manager.and_then(|auth_manager| {
                auth.filter(|auth| auth.uses_EfficiencyCode_backend())
                    .map(|auth| {
                        EfficiencyCode_model_provider::auth_provider_from_auth_manager(
                            auth_manager,
                            auth,
                        )
                    })
        ...
        {
            let is_host_owned_EfficiencyCode_apps = server_name
            ...
            let catalog_item_limit = if is_host_owned_EfficiencyCode_apps {
                MAX_EfficiencyCode_APPS_TOOL_CATALOG_ITEMS
            } else {
            ...
            // For built-in EfficiencyCode Apps, `EfficiencyCode_CONNECTORS_TOKEN` is a debug
            ...
            let shares_EfficiencyCode_apps_tools_cache = is_host_owned_EfficiencyCode_apps
                && should_share_EfficiencyCode_apps_tools_cache(
                    &server_name,
                    uses_env_bearer_token,
                );
            let EfficiencyCode_apps_tools_cache_context = shares_EfficiencyCode_apps_tools_cache
                .then(|| {
                    EfficiencyCode_apps_tools_cache.context(
                        EfficiencyCode_home.clone(),
                        EfficiencyCode_apps_tools_cache_key.clone(),
                    )
                });
            // The reserved EfficiencyCode Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == EfficiencyCode_APPS_MCP_SERVER_NAME {
                EfficiencyCode_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If EfficiencyCode Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient EfficiencyCodeAuth provider.
            let runtime_auth_provider =
                if server_name == EfficiencyCode_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                auth,
                shares_EfficiencyCode_apps_tools_cache.then(|| {
                    (
                        EfficiencyCode_home.clone(),
                        EfficiencyCode_apps_tools_cache_key.clone(),
                    )
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("EfficiencyCode_MCP_PROTOCOL_VERSION"))
                {
            ...
            let tool_catalog_cache_context = if server_name == EfficiencyCode_APPS_MCP_SERVER_NAME {
                None
    ...
    /// EfficiencyCode Apps metadata may come from its existing cache; regular MCP server information is
