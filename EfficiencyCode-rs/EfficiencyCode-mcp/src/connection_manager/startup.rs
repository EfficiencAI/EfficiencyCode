...
pub(super) fn should_share_EfficiencyCode_apps_tools_cache(
    server_name: &str,
    uses_env_bearer_token: bool,
) -> bool {
    server_name == EfficiencyCode_APPS_MCP_SERVER_NAME && !uses_env_bearer_token
}
...
pub(super) fn mcp_init_error_display(
        ...
        )
    {
        let recovery_hint = if config.is_some_and(|config| !config.is_local_environment()) {
            ...
            format!("Run `EfficiencyCode mcp login {server_name}`.")
        };
