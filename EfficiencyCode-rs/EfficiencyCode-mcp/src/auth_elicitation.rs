...
//! Session orchestration stays in `EfficiencyCode-core`.

use EfficiencyCode_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_EfficiencyCode_APPS_META_KEY: &str = "_EfficiencyCode_apps";
...
pub struct EfficiencyCodeAppsConnectorAuthFailure {
...
pub struct EfficiencyCodeAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct EfficiencyCodeAppsAuthElicitationPlan {
    pub auth_failure: EfficiencyCodeAppsConnectorAuthFailure,
    pub elicitation: EfficiencyCodeAppsAuthElicitation,
}

#[derive(Serialize)]
struct EfficiencyCodeAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<EfficiencyCodeAppsConnectorAuthFailure> {
    ...
    Some(EfficiencyCodeAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<EfficiencyCodeAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(EfficiencyCodeAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &EfficiencyCodeAppsConnectorAuthFailure,
) -> EfficiencyCodeAppsAuthElicitation {
    EfficiencyCodeAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_EfficiencyCode_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: EfficiencyCodeAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("EfficiencyCode_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &EfficiencyCodeAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in EfficiencyCode.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(
            ...
            "EfficiencyCode_apps_auth_call_123"
        );
    }
}
