...
//! and exposing cached EfficiencyCode Apps tools while a client is still connecting.
...
use crate::EfficiencyCode_apps::normalize_EfficiencyCode_apps_callable_name;
use crate::EfficiencyCode_apps::normalize_EfficiencyCode_apps_callable_namespace;
use crate::EfficiencyCode_apps::normalize_EfficiencyCode_apps_tool_title;
use crate::EfficiencyCode_apps::prepare_openai_file_params_for_model;
use crate::elicitation::ElicitationRequestManager;
use crate::mcp::EfficiencyCode_APPS_MCP_SERVER_NAME;
...
use EfficiencyCode_async_utils::CancelErr;
...
use EfficiencyCode_protocol::protocol::McpStartupStatus;
use EfficiencyCode_protocol::protocol::McpStartupUpdateEvent;
use EfficiencyCode_rmcp_client::ExecutorStdioServerLauncher;
use EfficiencyCode_rmcp_client::LocalStdioServerLauncher;
use EfficiencyCode_rmcp_client::McpProtocolMode;
use EfficiencyCode_rmcp_client::RmcpClient;
use EfficiencyCode_rmcp_client::StdioServerLauncher;
use EfficiencyCode_rmcp_client::StreamableHttpRedirectMode;
use EfficiencyCode_rmcp_client::ToolWithConnectorId;
use EfficiencyCode_rmcp_client::is_authentication_required_error;
use anyhow::Result;
use anyhow::anyhow;
...
/// MCP server capability indicating that EfficiencyCode should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "EfficiencyCode/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "EfficiencyCode/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "EfficiencyCode.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "EfficiencyCode.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const EfficiencyCode_APPS_REFRESH_DURATION_METRIC: &str =
    "EfficiencyCode.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const EfficiencyCode_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const EfficiencyCode_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) EfficiencyCode_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .EfficiencyCode_apps_tools_cache_context
            .as_ref()
        ...
        if self.EfficiencyCode_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct EfficiencyCodeAppsStartupReconnectState {
...
#[derive(Clone)]
struct EfficiencyCodeAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct EfficiencyCodeAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<EfficiencyCodeAppsStartupReconnectState>,
    startup_status_context: Option<EfficiencyCodeAppsStartupStatusContext>,
}

impl EfficiencyCodeAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(EfficiencyCodeAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context =
            tx_event.map(|tx_event| EfficiencyCodeAppsStartupStatusContext {
                submit_id,
                server_name,
                tx_event,
            });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after =
                            EfficiencyCode_apps_reconnect_backoff(state.consecutive_failures);
...
fn EfficiencyCode_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    EfficiencyCode_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(EfficiencyCode_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    EfficiencyCode_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_EfficiencyCode_apps_mcp_server = server_name == EfficiencyCode_APPS_MCP_SERVER_NAME;
        ...
        async move {
            ...
            let refresh_start = is_EfficiencyCode_apps_mcp_server.then(Instant::now);
            let outcome = match async {
                ...
                start_server_task(
                    ...
                    StartServerTaskParams {
                        is_EfficiencyCode_apps_mcp_server,
            ...
            {
                emit_duration(
                    EfficiencyCode_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_EfficiencyCode_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<EfficiencyCodeAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let cached_server_info = if is_EfficiencyCode_apps_mcp_server {
            EfficiencyCode_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            EfficiencyCode_apps_tools_cache_context: EfficiencyCode_apps_tools_cache_context
                .clone(),
        ...
        let startup_reconnect = is_EfficiencyCode_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                EfficiencyCodeAppsStartupReconnect::new(Arc::new(move || startup.start()))
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.EfficiencyCode_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_EfficiencyCode_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_EfficiencyCode_apps_mcp_server: bool,
    EfficiencyCode_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_EfficiencyCode_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", EfficiencyCode_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared EfficiencyCode Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
pub(crate) fn prepare_EfficiencyCode_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_EfficiencyCode_apps_mcp_server {
        EfficiencyCode_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a EfficiencyCode Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn EfficiencyCode_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_EfficiencyCode_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_EfficiencyCode_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title =
            normalize_EfficiencyCode_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = EfficiencyCode_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_EfficiencyCode_apps_mcp_server,
        /*EfficiencyCode_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (
        EfficiencyCode_apps_tools_cache_context.as_ref(),
        fetch_ticket,
    ) {
        ...
        _ => unreachable!("EfficiencyCode Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog =
        is_EfficiencyCode_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("EfficiencyCode-mcp-client", env!("CARGO_PKG_VERSION"))
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(EfficiencyCode_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    use super::*;
    use EfficiencyCode_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
    ...
    #[test]
    fn EfficiencyCode_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: EfficiencyCode_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "EfficiencyCode_apps__gmail".to_string(),
