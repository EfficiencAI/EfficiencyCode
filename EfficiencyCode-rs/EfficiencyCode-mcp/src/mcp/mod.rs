...
use EfficiencyCode_model_provider::CHATGPT_EfficiencyCode_BASE_URL;
use EfficiencyCode_protocol::mcp::ClientMcpExtensions;
use EfficiencyCode_protocol::mcp::McpServerInfo;
use EfficiencyCode_protocol::mcp::Resource;
use EfficiencyCode_protocol::mcp::ResourceTemplate;
use EfficiencyCode_protocol::mcp::Tool;
use EfficiencyCode_protocol::models::PermissionProfile;
use EfficiencyCode_protocol::protocol::AskForApproval;
use EfficiencyCode_protocol::protocol::McpAuthStatus;
...
pub const EfficiencyCode_APPS_MCP_SERVER_NAME: &str = "EfficiencyCode_apps";
const DEFAULT_EfficiencyCode_APPS_MCP_PRODUCT_SKU: &str = "EfficiencyCode";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const EfficiencyCode_CONNECTORS_TOKEN_ENV_VAR: &str = "EfficiencyCode_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `EfficiencyCode_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// EfficiencyCode home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `EfficiencyCode-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_EfficiencyCode_apps_enabled(
    config: &McpConfig,
    auth: Option<&EfficiencyCodeAuth>,
) -> bool {
    config.apps_enabled && auth.is_some_and(EfficiencyCodeAuth::uses_EfficiencyCode_backend)
}
...
fn is_trusted_chatgpt_mcp_server(
    ...
    if url::Url::parse(CHATGPT_EfficiencyCode_BASE_URL)
        .ok()
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_EfficiencyCode_apps_enabled(config, auth) {
        servers.remove(EfficiencyCode_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            tool_catalog_cache,
            EfficiencyCode_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn EfficiencyCode_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(EfficiencyCode_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => {
            Some(EfficiencyCode_CONNECTORS_TOKEN_ENV_VAR.to_string())
        }
        Ok(_) => None,
...
fn normalize_EfficiencyCode_apps_base_url(base_url: &str) -> String {
...
fn EfficiencyCode_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_EfficiencyCode_apps_base_url(base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/EfficiencyCode")
    {
        base_url
    } else {
        format!("{base_url}/api/EfficiencyCode")
    };
    format!("{base_url}/ps/mcp")
}

pub fn EfficiencyCode_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        EfficiencyCode_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    EfficiencyCode_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_EfficiencyCode_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: EfficiencyCode_apps_mcp_bearer_token_env_var(),
