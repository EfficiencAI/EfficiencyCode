...
pub(crate) fn test_mcp_config(EfficiencyCode_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: EfficiencyCode_config::ConfigLayerStack::default(),
        approvals_reviewer: EfficiencyCode_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: EfficiencyCode_connectors::ConnectorSnapshot::default(),
    }
}
...
#[test]
fn EfficiencyCode_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        EfficiencyCode_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        EfficiencyCode_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        EfficiencyCode_apps_mcp_url_for_base_url("http://localhost:8080/api/EfficiencyCode"),
        "http://localhost:8080/api/EfficiencyCode/ps/mcp"
    );
    assert_eq!(
        EfficiencyCode_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/EfficiencyCode/ps/mcp"
    );
}

#[test]
fn EfficiencyCode_apps_server_config_uses_plugin_service_path() {
    let config = EfficiencyCode_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for EfficiencyCode apps"),
    };
...
#[test]
fn EfficiencyCode_apps_server_config_forwards_thread_originator_header() {
    let config = EfficiencyCode_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
...
#[test]
fn EfficiencyCode_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in
        [(None, "EfficiencyCode"), (Some("tpp"), "tpp")]
    {
        let config = EfficiencyCode_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn EfficiencyCode_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = EfficiencyCode_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
fn effective_mcp_servers_preserve_chatgpt_auth_for_staging() {
    for url in [
        ...
        let server = EfficiencyCode_apps_mcp_server_config(
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(EfficiencyCode_home.path().to_path_buf());
    ...
    let EfficiencyCode_apps = effective
        .get(EfficiencyCode_APPS_MCP_SERVER_NAME)
        .expect("EfficiencyCode apps server should exist");

    let sample = sample.config();
    let docs = docs.config();
    let EfficiencyCode_apps = EfficiencyCode_apps.config();
    ...
    match &EfficiencyCode_apps.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => {
            assert_eq!(url, "https://chatgpt.com/backend-api/ps/mcp");
        }
        other => panic!("expected streamable http transport, got {other:?}"),
