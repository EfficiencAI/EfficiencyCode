...
#[cfg(unix)]
use EfficiencyCode_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR;
use anyhow::Context;
use anyhow::Result;
use anyhow::bail;
...
impl PidBackend {
    ...
    pub(crate) fn new_update_loop(EfficiencyCode_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            EfficiencyCode_bin,
            pid_file,
    ...
    pub(crate) async fn start(&self) -> Result<Option<u32>> {
        ...
        let mut command = Command::new(&self.EfficiencyCode_bin);
            ...
            Err(err) => {
                ...
                return Err(err).with_context(|| {
                    format!(
                        ...
                        self.EfficiencyCode_bin.display()
                    )
                });
            }
        };
