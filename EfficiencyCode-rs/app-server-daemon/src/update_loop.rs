...
#[cfg(unix)]
use crate::managed_install::resolved_managed_EfficiencyCode_bin;
...
#[cfg(unix)]
const INSTALL_URL: &str = "https://chatgpt.com/EfficiencyCode/install.sh";
...
async fn update_once(
    ...
    let managed_EfficiencyCode_bin =
        resolved_managed_EfficiencyCode_bin(&daemon.managed_EfficiencyCode_bin).await?;
    let managed_identity = executable_identity(&managed_EfficiencyCode_bin).await?;
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_EfficiencyCode_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_EfficiencyCode_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed EfficiencyCode binary {}",
            managed_EfficiencyCode_bin.display()
        )
    })
}
...
async fn install_latest_standalone(http: &RouteAwareClientPool) -> Result<()> {
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone EfficiencyCode updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone EfficiencyCode updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone EfficiencyCode updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone EfficiencyCode updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone EfficiencyCode updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone EfficiencyCode updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone EfficiencyCode updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone EfficiencyCode updater")?
            .to_vec();
