# EfficiencyCode-app-server-daemon

> `EfficiencyCode-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`EfficiencyCode-app-server-daemon` backs the machine-readable `EfficiencyCode app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for EfficiencyCode instances launched over SSH, including fresh developer
...
```sh
EfficiencyCode app-server daemon start
EfficiencyCode app-server daemon restart
EfficiencyCode app-server daemon enable-remote-control
EfficiencyCode app-server daemon disable-remote-control
EfficiencyCode app-server daemon stop
EfficiencyCode app-server daemon version
EfficiencyCode app-server daemon bootstrap --remote-control
```
...
$HOME/.EfficiencyCode/packages/standalone/current/EfficiencyCode app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `EfficiencyCode_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes EfficiencyCode is installed through `install.sh` and always launches
the standalone managed binary under `EfficiencyCode_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `EfficiencyCode_HOME/packages/standalone/current/EfficiencyCode` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `EfficiencyCode_HOME/packages/standalone/current/EfficiencyCode` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `EfficiencyCode remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `EfficiencyCode_HOME`, so a concurrent
...
The daemon stores its local state under `EfficiencyCode_HOME/app-server-daemon/`:
