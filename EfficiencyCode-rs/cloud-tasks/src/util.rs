...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = EfficiencyCode_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("EfficiencyCode_cloud_tasks_tui");
    let ua = EfficiencyCode_login::default_client::get_EfficiencyCode_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("EfficiencyCode-cli")),
    );
    ...
    {
        headers.extend(
            EfficiencyCode_model_provider::auth_provider_from_auth(&auth).to_auth_headers(),
        );
    }
    headers
}
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/EfficiencyCode/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/EfficiencyCode") {
        return format!("{root}/EfficiencyCode/tasks/{task_id}");
    }
    if normalized.ends_with("/EfficiencyCode") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/EfficiencyCode/tasks/{task_id}")
}
