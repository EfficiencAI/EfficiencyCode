...
pub enum Command {
    /// Submit a new EfficiencyCode Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a EfficiencyCode Cloud task.
    Status(StatusCommand),
    /// List EfficiencyCode Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a EfficiencyCode Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a EfficiencyCode Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in EfficiencyCode Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `EfficiencyCode cloud` to browse).
    ...
    /// Git branch to run in EfficiencyCode Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// EfficiencyCode Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// EfficiencyCode Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// EfficiencyCode Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
