...
- From the EfficiencyCode CLI, run `EfficiencyCode execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
EfficiencyCode execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
EfficiencyCode execpolicy check \
...
```bash
cargo run -p EfficiencyCode-execpolicy -- check --rules path/to/policy.rules git status
```
