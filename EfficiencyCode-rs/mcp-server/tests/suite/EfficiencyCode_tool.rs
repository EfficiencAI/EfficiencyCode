...
use EfficiencyCode_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use EfficiencyCode_mcp_server::EfficiencyCodeToolCallParam;
use EfficiencyCode_mcp_server::ExecApprovalElicitRequestParams;
use EfficiencyCode_mcp_server::ExecApprovalResponse;
use EfficiencyCode_mcp_server::PatchApprovalElicitRequestParams;
use EfficiencyCode_mcp_server::PatchApprovalResponse;
use EfficiencyCode_protocol::protocol::FileChange;
use EfficiencyCode_protocol::protocol::ReviewDecision;
use EfficiencyCode_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a EfficiencyCode sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "EfficiencyCode" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let EfficiencyCode_request_id = mcp_process
        .send_EfficiencyCode_tool_call(EfficiencyCodeToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            EfficiencyCode_request_id.to_string(),
            params.EfficiencyCode_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `EfficiencyCode` tool call completes and that the file was created.
    let EfficiencyCode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process
            .read_stream_until_response_message(RequestId::Number(EfficiencyCode_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(EfficiencyCode_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    EfficiencyCode_mcp_tool_call_id: String,
    EfficiencyCode_event_id: String,
    thread_id: EfficiencyCode_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow EfficiencyCode to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let EfficiencyCode_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        EfficiencyCode_elicitation: "exec-approval".to_string(),
        EfficiencyCode_mcp_tool_call_id,
        EfficiencyCode_event_id,
        EfficiencyCode_command: command,
        EfficiencyCode_cwd: workdir.to_path_buf(),
        EfficiencyCode_call_id: "call1234".to_string(),
        EfficiencyCode_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "EfficiencyCode" tool request that will trigger the apply_patch command
    let EfficiencyCode_request_id = mcp_process
        .send_EfficiencyCode_tool_call(EfficiencyCodeToolCallParam {
    ...
    // Verify the original `EfficiencyCode` tool call completes
    let EfficiencyCode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process
            .read_stream_until_response_message(RequestId::Number(EfficiencyCode_request_id)),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_EfficiencyCode_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    EfficiencyCode_tool_passes_base_instructions()
        .await
        .expect("EfficiencyCode tool should pass base instructions");
}

async fn EfficiencyCode_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `EfficiencyCode mcp` with a specific config.toml.
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml(EfficiencyCode_home.path(), &server.uri())?;
    let skill_dir = EfficiencyCode_home.path().join("skills").join("demo");
    ...
    let mut mcp_process = McpProcess::new_with_env(
        ...
        &[
            ("OPENAI_API_KEY", None),
            ("EfficiencyCode_ACCESS_TOKEN", None),
        ],
    )
    .await?;
    ...
    .await??;
    assert_eq!(EfficiencyCode_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(
        EfficiencyCode_response.id,
        RequestId::Number(EfficiencyCode_request_id)
    );
    assert_eq!(
        EfficiencyCode_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": EfficiencyCode_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("EfficiencyCode tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
    ...
    assert_eq!(
        developer_text
            .matches("Co-authored-by: EfficiencyCode <noreply@openai.com>")
            .count(),
        1
    );
    assert_eq!(
        developer_text
            .matches("Generated with [EfficiencyCode](https://openai.com/EfficiencyCode/).")
            .count(),
        1
    );
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_EfficiencyCode_tool_forwards_skills_extension_warnings() {
    skip_if_no_network!();

    EfficiencyCode_tool_forwards_skills_extension_warnings()
        .await
        .expect("EfficiencyCode tool should forward skills extension warnings");
}

async fn EfficiencyCode_tool_forwards_skills_extension_warnings() -> anyhow::Result<()> {
    let server =
        create_mock_responses_server(vec![create_final_assistant_message_sse_response("Enjoy!")?])
            .await;
    let EfficiencyCode_home = TempDir::new()?;
    create_config_toml(EfficiencyCode_home.path(), &server.uri())?;
    let skills_dir = EfficiencyCode_home.path().join("skills");
    for index in 0..200 {
    ...
    let mut mcp_process = McpProcess::new(EfficiencyCode_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp_process.initialize()).await??;
    ...
    let warning = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_EfficiencyCode_event_matching("warning", |params| {
            params["msg"]["message"]
                .as_str()
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow EfficiencyCode to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        EfficiencyCode_elicitation: "patch-approval".to_string(),
        EfficiencyCode_mcp_tool_call_id,
        EfficiencyCode_event_id,
        EfficiencyCode_reason: reason,
        EfficiencyCode_grant_root: grant_root,
        EfficiencyCode_changes: changes,
        EfficiencyCode_call_id: "call1234".to_string(),
    })?;
...
async fn create_mcp_process(responses: Vec<String>) -> anyhow::Result<McpHandle> {
    ...
    Ok(McpHandle {
        process: mcp_process,
        server,
        dir: EfficiencyCode_home,
    })
}

/// Create a EfficiencyCode config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(EfficiencyCode_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = EfficiencyCode_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
