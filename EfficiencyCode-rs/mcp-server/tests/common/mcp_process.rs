...
impl McpProcess {
    pub async fn new(EfficiencyCode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(EfficiencyCode_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-mcp-server")
            .context("should find binary for EfficiencyCode-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("EfficiencyCode-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = EfficiencyCode_login::default_client::originator().value;
        ...
        assert_eq!(
            ...
            json!({
                ...
                "serverInfo": {
                    "name": "EfficiencyCode-mcp-server",
    ...
    pub async fn send_EfficiencyCode_tool_call(
        &mut self,
        params: EfficiencyCodeToolCallParam,
    ) -> anyhow::Result<i64> {
        let EfficiencyCode_tool_call_params = CallToolRequestParams::new("EfficiencyCode")
            .with_arguments(match serde_json::to_value(params)? {
                serde_json::Value::Object(map) => map,
                _ => unreachable!("params serialize to object"),
            });
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(EfficiencyCode_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "EfficiencyCode/event" with params.msg.type == "task_complete".
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        self.read_stream_until_EfficiencyCode_event("task_complete")
            .await
    }

    pub async fn read_stream_until_EfficiencyCode_event(
        &mut self,
        event_type: &str,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        self.read_stream_until_EfficiencyCode_event_matching(event_type, |_| true)
            .await
    }

    pub async fn read_stream_until_EfficiencyCode_event_matching(
        &mut self,
        event_type: &str,
        predicate: impl Fn(&serde_json::Value) -> bool,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        eprintln!("in read_stream_until_EfficiencyCode_event({event_type})");

        loop {
            let message = self.read_jsonrpc_message().await?;
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = notification.notification.method == "EfficiencyCode/event"
                        && notification
                            .notification
                            .params
                            .as_ref()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `EfficiencyCode-mcp-server` child process.
        //
