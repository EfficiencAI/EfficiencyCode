//! Asynchronous worker that executes a **EfficiencyCode** tool-call inside a spawned
...
use EfficiencyCode_core::config::Config as EfficiencyCodeConfig;
...
/// To adhere to MCP `tools/call` response format, include the EfficiencyCode
...
/// Run a complete EfficiencyCode session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_EfficiencyCode_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: EfficiencyCodeConfig,
        ...
        .await
    {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![ContentBlock::text(format!(
                "Failed to start EfficiencyCode session: {e}"
            ))]);
    ...
    // Preserve the legacy event ID for initial `EfficiencyCode` calls. Each call starts
    ...
    run_EfficiencyCode_tool_session_inner(thread_id, thread, outgoing, id, active_turns).await;
}

pub async fn run_EfficiencyCode_tool_session_reply(
    thread_id: ThreadId,
    ...
    run_EfficiencyCode_tool_session_inner(thread_id, thread, outgoing, request_id, active_turns)
        .await;
}

async fn run_EfficiencyCode_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<EfficiencyCodeThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(EfficiencyCode_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("EfficiencyCode runtime error: {e}"),
                    Some(true),
                );
                active_turns.finish(&request_id, || {
                    outgoing.send_response(request_id.clone(), result);
