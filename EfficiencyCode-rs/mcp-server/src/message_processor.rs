...
use EfficiencyCode_login::default_client::USER_AGENT_SUFFIX;
...
use crate::EfficiencyCode_tool_config::EfficiencyCodeToolCallParam;
use crate::EfficiencyCode_tool_config::EfficiencyCodeToolCallReplyParam;
use crate::EfficiencyCode_tool_config::create_tool_for_EfficiencyCode_tool_call_param;
use crate::EfficiencyCode_tool_config::create_tool_for_EfficiencyCode_tool_call_reply_param;
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        EfficiencyCode_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.EfficiencyCode_home.clone()),
        );
        let skill_providers = EfficiencyCode_skills_extension::SkillProviders::new()
            .with_host_provider(Arc::new(
                EfficiencyCode_skills_extension::HostSkillProvider::new(),
            ));
        EfficiencyCode_skills_extension::install_with_providers_and_metrics(
            &mut extensions,
            skill_providers,
            EfficiencyCode_otel::global(),
            |config: &Config| EfficiencyCode_skills_extension::SkillsExtensionConfig {
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            EfficiencyCode_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            EfficiencyCode_core::thread_store_from_config(config.as_ref(), state_db.clone()),
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("EfficiencyCode-mcp-server", env!("CARGO_PKG_VERSION"))
                .with_title("EfficiencyCode");

        // Preserve EfficiencyCode's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert(
                ...
                json!(get_EfficiencyCode_user_agent()),
            );
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult::with_all_items(vec![
            create_tool_for_EfficiencyCode_tool_call_param(),
            create_tool_for_EfficiencyCode_tool_call_reply_param(),
        ]);
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "EfficiencyCode" => self.handle_tool_call_EfficiencyCode(id, arguments).await,
            "EfficiencyCode-reply" => {
                self.handle_tool_call_EfficiencyCode_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_EfficiencyCode(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<EfficiencyCodeToolCallParam>(json_val)
            {
                    ...
                    Err(e) => {
                        let result =
                            CallToolResult::error(vec![rmcp::model::ContentBlock::text(format!(
                                "Failed to load EfficiencyCode configuration from overrides: {e}"
                            ))]);
                        self.outgoing.send_response(id, result);
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for EfficiencyCode tool: {e}"),
                    )]);
                    self.outgoing.send_response(id, result);
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for EfficiencyCode tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result);
                return;
            }
        ...
        // Spawn an async task to handle the EfficiencyCode session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the EfficiencyCode session and stream events back to the client.
            crate::EfficiencyCode_tool_runner::run_EfficiencyCode_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_EfficiencyCode_session_reply(
        &self,
        ...
        // parse arguments
        let EfficiencyCode_tool_call_reply_param: EfficiencyCodeToolCallReplyParam = match arguments
        {
            Some(json_val) => {
                match serde_json::from_value::<EfficiencyCodeToolCallReplyParam>(json_val) {
                    Ok(params) => params,
                    Err(e) => {
                        tracing::error!(
                            "Failed to parse EfficiencyCode tool call reply parameters: {e}"
                        );
                        let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                            format!("Failed to parse configuration for EfficiencyCode tool: {e}"),
                        )]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for EfficiencyCode-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for EfficiencyCode-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match EfficiencyCode_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let EfficiencyCode = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result =
                    crate::EfficiencyCode_tool_runner::create_call_tool_result_with_thread_id(
                        thread_id,
        ...
        let prompt = EfficiencyCode_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let active_turns = Arc::clone(&active_turns);

            async move {
                crate::EfficiencyCode_tool_runner::run_EfficiencyCode_tool_session_reply(
                    thread_id,
                    EfficiencyCode,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the EfficiencyCode thread from the server.
        let EfficiencyCode_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to EfficiencyCode.
        if let Err(e) = EfficiencyCode_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: EfficiencyCode_protocol::protocol::Op::Interrupt,
            ...
            .await
        {
            tracing::error!("Failed to submit interrupt to EfficiencyCode: {e}");
            return;
        }
