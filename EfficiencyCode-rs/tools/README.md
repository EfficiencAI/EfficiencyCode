# EfficiencyCode-tools

`EfficiencyCode-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `EfficiencyCode-core`.
...
compatibility-sensitive orchestration in `EfficiencyCode-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `EfficiencyCode-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `EfficiencyCode-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `EfficiencyCode-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `EfficiencyCode-core`.
3. Leave compatibility-sensitive adapters in `EfficiencyCode-core` while downstream
...
`EfficiencyCode-core`, that is a sign to revisit the extraction boundary before adding
more here.
