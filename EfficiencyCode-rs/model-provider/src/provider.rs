...
use EfficiencyCode_api::Provider;
...
use EfficiencyCode_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that EfficiencyCode may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "EfficiencyCode-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether EfficiencyCode should have a unified provider-specific auth
    ...
    fn map_api_error(&self, error: ApiError) -> EfficiencyCodeErr {
        EfficiencyCode_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(
        &self,
    ) -> ModelProviderFuture<'_, EfficiencyCode_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            self.info()
                .to_api_provider(auth.as_ref().map(EfficiencyCodeAuth::auth_mode))
        })
    }
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, EfficiencyCode_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, EfficiencyCode_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a EfficiencyCode session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, EfficiencyCode_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| EfficiencyCode_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, EfficiencyCodeAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    EfficiencyCodeAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
...
mod tests {
    ...
    use EfficiencyCode_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn test_EfficiencyCode_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "EfficiencyCode-model-provider-test-{}",
            std::process::id()
        ))
    }
    ...
    fn bedrock_api_key_auth() -> EfficiencyCodeAuth {
        EfficiencyCodeAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn configured_provider_uses_luna_for_approval_review_with_api_key_auth() {
        let provider = create_model_provider(
            ...
            Some(AuthManager::from_auth_for_testing(
                EfficiencyCodeAuth::from_api_key("openai-api-key"),
            )),
        );
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("EfficiencyCode-bedrock".to_string()),
                region: None,
            })),
            Some(AuthManager::from_auth_for_testing(
                EfficiencyCodeAuth::from_api_key("openai-api-key"),
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = EfficiencyCode_models_manager::bundled_models_response()
