...
use EfficiencyCode_api::ModelsClient;
...
use EfficiencyCode_login::collect_auth_env_telemetry;
use EfficiencyCode_login::default_client::create_client_for_route_async;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<EfficiencyCodeAuth> {
    ...
    async fn list_models(
        ...
        let _timer = EfficiencyCode_otel::start_global_timer(
            "EfficiencyCode.remote_models.fetch_update.duration_ms",
            &[],
        );
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(EfficiencyCodeAuth::auth_mode);
        ...
        let agent_identity_telemetry =
            if let Some(EfficiencyCodeAuth::AgentIdentity(auth)) = auth.as_ref() {
        ...
        .await
        .map_err(|_| EfficiencyCodeErr::Timeout)?
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let EfficiencyCode_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.EfficiencyCode_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, EfficiencyCode_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_EfficiencyCode_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_EfficiencyCode_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "EfficiencyCode_otel.log_only",
            tracing::Level::INFO,
            event.name = "EfficiencyCode.api_request",
            ...
            auth.env_EfficiencyCode_api_key_present = self.auth_env.EfficiencyCode_api_key_env_present,
            auth.env_EfficiencyCode_api_key_enabled = self.auth_env.EfficiencyCode_api_key_env_enabled,
        ...
        tracing::event!(
            target: "EfficiencyCode_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "EfficiencyCode.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
...
mod tests {
    ...
    use EfficiencyCode_login::default_client::create_client;
