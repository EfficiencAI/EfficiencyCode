...
use EfficiencyCode_agent_identity::AgentIdentityKey;
use EfficiencyCode_agent_identity::authorization_header_for_agent_task;
...
use EfficiencyCode_login::auth::AgentIdentityAuthError;
...
struct AuthManagerAuthProvider {
    ...
    expected_auth: EfficiencyCodeAuth,
}
...
impl AuthProvider for AuthManagerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        let Some(auth) = self
            .auth_manager
            .auth_cached()
            .filter(EfficiencyCodeAuth::uses_EfficiencyCode_backend)
        else {
            return;
        };
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> EfficiencyCode_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(EfficiencyCodeAuth::BedrockApiKey(_))) {
        return Err(EfficiencyCodeErr::UnsupportedOperation(
...
) -> EfficiencyCode_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(EfficiencyCodeAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(EfficiencyCodeAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> EfficiencyCode_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party EfficiencyCode auth snapshot.
pub fn auth_provider_from_auth(auth: &EfficiencyCodeAuth) -> SharedAuthProvider {
    match auth {
        EfficiencyCodeAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        EfficiencyCodeAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        EfficiencyCodeAuth::BedrockApiKey(_) => {
            unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}")
        }
        EfficiencyCodeAuth::ApiKey(_)
        | EfficiencyCodeAuth::Chatgpt(_)
        | EfficiencyCodeAuth::ChatgptAuthTokens(_)
        | EfficiencyCodeAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use EfficiencyCode_agent_identity::generate_agent_key_material;
    use EfficiencyCode_login::AuthCredentialsStoreMode;
    ...
    use EfficiencyCode_login::auth::login_with_chatgpt_auth_tokens;
    use EfficiencyCode_model_provider_info::WireApi;
    use EfficiencyCode_model_provider_info::create_oss_provider_with_base_url;
    ...
    fn test_EfficiencyCode_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "EfficiencyCode-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp EfficiencyCode home");
        path
    }

    fn write_chatgpt_auth_json(EfficiencyCode_home: &Path) {
        let auth_json = json!({
            "tokens": {
        ...
        std::fs::write(
            EfficiencyCode_home.join("auth.json"),
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, EfficiencyCodeAuth) {
        let EfficiencyCode_home = test_EfficiencyCode_home();
        write_chatgpt_auth_json(&EfficiencyCode_home);
        ...
        (EfficiencyCode_home, auth_manager, auth)
    }
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = EfficiencyCodeAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = EfficiencyCodeAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&EfficiencyCodeAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_EfficiencyCode_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
