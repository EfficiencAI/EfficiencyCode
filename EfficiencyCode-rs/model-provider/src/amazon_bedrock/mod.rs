...
impl AmazonBedrockModelProvider {
    ...
    fn managed_auth(&self) -> Option<BedrockApiKeyAuth> {
        self.auth_manager
            ...
            .and_then(|auth| match auth {
                EfficiencyCodeAuth::BedrockApiKey(auth) => Some(auth),
                ...
                | EfficiencyCodeAuth::PersonalAccessToken(_) => None,
            })
    }
    ...
    async fn auth(&self) -> Option<EfficiencyCodeAuth> {
        if self.info.has_command_auth() {
            ...
            self.managed_auth().map(EfficiencyCodeAuth::BedrockApiKey)
        }
    }
    ...
    fn account_state(&self) -> ProviderAccountResult {
        Ok(ProviderAccountState {
            account: Some(ProviderAccount::AmazonBedrock {
                uses_EfficiencyCode_managed_credentials: self.managed_auth().is_some(),
            }),
...
mod tests {
    ...
    async fn managed_auth_takes_precedence_over_aws_auth() {
        ...
        let auth_manager = AuthManager::from_auth_for_testing(EfficiencyCodeAuth::BedrockApiKey(
            managed_auth.clone(),
        ));
        ...
        assert_eq!(
            provider.auth().await,
            Some(EfficiencyCodeAuth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
            provider.account_state(),
            Ok(ProviderAccountState {
