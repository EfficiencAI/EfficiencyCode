...
use EfficiencyCode_api::AuthError;
use EfficiencyCode_api::AuthProvider;
use EfficiencyCode_api::SharedAuthProvider;
use EfficiencyCode_aws_auth::AwsAuthContext;
use EfficiencyCode_aws_auth::AwsAuthError;
use EfficiencyCode_aws_auth::AwsRequestToSign;
use EfficiencyCode_http_client::Request;
use EfficiencyCode_http_client::RequestBody;
use EfficiencyCode_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_EfficiencyCode_error)?;
...
fn aws_auth_error_to_EfficiencyCode_error(error: AwsAuthError) -> EfficiencyCodeErr {
    EfficiencyCodeErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer EfficiencyCode agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockMantleSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> EfficiencyCode_api::AuthProviderFuture<'_> {
