...
pub(super) const BEDROCK_EXPIRED_SIGNATURE_MESSAGE: &str = concat!(
    ...
    "update or unset it, then restart EfficiencyCode",
);

pub(super) fn map_api_error(error: ApiError) -> EfficiencyCodeErr {
    let error = EfficiencyCode_api::map_api_error(error);
    if let EfficiencyCodeErrorDetails::UnexpectedStatus(response) = error.details()
        ...
        && response.body.contains("Signature expired:")
    {
        ...
        let mapped_error =
            EfficiencyCodeErr::new(EfficiencyCodeErrorDetails::UnexpectedStatus(response));
