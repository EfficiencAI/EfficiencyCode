...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "EfficiencyCode-code-mode-host.exe"
} else {
    "EfficiencyCode-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "EfficiencyCode-package.json";
const PATH_DIRNAME: &str = "EfficiencyCode-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "EfficiencyCode-resources";
...
/// Version metadata recorded in a bundled EfficiencyCode runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct EfficiencyCodePackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<EfficiencyCodePackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.EfficiencyCode/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `EfficiencyCode-resources/`, and `EfficiencyCode-path/`.
    ...
    /// A EfficiencyCode binary launched through the npm-managed `EfficiencyCode.js` shim.
    Npm,
    /// A EfficiencyCode binary launched through the bun-managed `EfficiencyCode.js` shim.
    Bun,
    /// A EfficiencyCode binary launched through the pnpm-managed `EfficiencyCode.js` shim.
    Pnpm,
    /// A EfficiencyCode binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled EfficiencyCode binaries, custom
    /// internal launchers, and tests that execute EfficiencyCode from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let EfficiencyCode_home = EfficiencyCode_utils_home_dir::find_EfficiencyCode_home().ok();
        Self::from_exe_with_EfficiencyCode_home(
            is_macos,
            current_exe,
            method_override,
            EfficiencyCode_home.as_deref(),
        )
    }

    fn from_exe_with_EfficiencyCode_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        EfficiencyCode_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(EfficiencyCodePackageLayout::from_exe);
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("EfficiencyCode_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("EfficiencyCode_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("EfficiencyCode_MANAGED_BY_BUN").is_some() {
    ...
    pub fn package_manifest(&self) -> Option<EfficiencyCodePackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under EfficiencyCode-resources
...
impl EfficiencyCodePackageLayout {
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) =
        standalone_install_method(exe_path, EfficiencyCode_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_EfficiencyCode_home = canonical_absolute_path(EfficiencyCode_home?)?;
    ...
    let releases_root = canonical_EfficiencyCode_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "EfficiencyCode-test-helper";
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = EfficiencyCode_home
            .path()
        ...
        let context = InstallContext::from_exe_with_EfficiencyCode_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*EfficiencyCode_home*/ Some(EfficiencyCode_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
  ...
  "variant": "EfficiencyCode",
  "entrypoint": "bin/EfficiencyCode",
  "resourcesDir": "EfficiencyCode-resources",
  "pathDir": "EfficiencyCode-path"
}
"#,
        )?;
        let exe_path = bin_dir.join(if cfg!(windows) {
        ...
        let package_layout = EfficiencyCodePackageLayout {
        ...
        assert_eq!(
            ...
            Some(EfficiencyCodePackageManifest {
    ...
    fn code_mode_host_program_accepts_symlinks_to_files() -> std::io::Result<()> {
        ...
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
        let exe_path = bin_dir.join("EfficiencyCode");
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        let EfficiencyCode_home = tempfile::tempdir()?;
        let package_dir = EfficiencyCode_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_EfficiencyCode_path_is_missing() -> std::io::Result<()>
    {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
    ...
    fn bundled_file_lookups_ignore_directories() -> std::io::Result<()> {
        ...
        fs::write(&exe_path, "")?;
        let fallback_exe_path = package_dir.path().join("fallback-EfficiencyCode");
    ...
    fn code_mode_host_program_is_next_to_the_executable_even_when_missing() {
        ...
        assert_eq!(
            context.code_mode_host_program_from_exe(Some(Path::new(
                "/opt/EfficiencyCode/bin/EfficiencyCode"
            ))),
            PathBuf::from("/opt/EfficiencyCode/bin").join(CODE_MODE_HOST_EXECUTABLE_NAME)
        );
    }
    ...
    fn package_manager_method_overrides_take_precedence() {
        let pnpm_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/EfficiencyCode")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_EfficiencyCode_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/EfficiencyCode")),
            /*method_override*/ None,
            /*EfficiencyCode_home*/ None,
        );
        assert_eq!(
