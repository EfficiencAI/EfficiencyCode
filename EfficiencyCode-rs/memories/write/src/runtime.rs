...
use EfficiencyCode_core::content_items_to_text;
use EfficiencyCode_core::detached_memory_responses_metadata;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<EfficiencyCodeThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<EfficiencyCode_otel::Timer> {
...
pub(crate) struct MemoryStartupContext {
    thread_id: ThreadId,
    thread: Arc<EfficiencyCodeThread>,
...
fn build_session_telemetry(
    ...
    let account_email = auth.and_then(EfficiencyCodeAuth::get_account_email);
