...
use EfficiencyCode_state::Stage1Output;
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            EfficiencyCode_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            EfficiencyCode_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            EfficiencyCode_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            EfficiencyCode_state::Phase2JobClaimOutcome::SkippedRunning => {
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[EfficiencyCode_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
        db.memories()
...
mod agent {
    ...
    pub(super) fn get_config(
        ...
        // Preserve the parent's explicit choice to skip EfficiencyCode-managed sandboxing.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<EfficiencyCode_state::Stage1Output>,
        memory_root: EfficiencyCode_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<EfficiencyCode_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &EfficiencyCode_core::EfficiencyCodeThread,
    ) -> AgentStatus {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[EfficiencyCode_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
