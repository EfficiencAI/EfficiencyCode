...
use EfficiencyCode_git_utils::diff_since_latest_init;
use EfficiencyCode_git_utils::reset_git_repository;
use EfficiencyCode_login::AuthManager;
use EfficiencyCode_login::EfficiencyCodeAuth;
use EfficiencyCode_model_provider::ModelProvider;
use EfficiencyCode_model_provider::ModelProviderFuture;
use EfficiencyCode_model_provider::ProviderAccountResult;
...
use EfficiencyCode_state::Phase2JobClaimOutcome;
...
async fn memories_startup_creates_memory_root() -> anyhow::Result<()> {
    ...
    let test = build_test_EfficiencyCode(&server, home).await?;

    assert!(!memory_root.exists());
    trigger_memories_startup(&test).await;
    wait_for_dir(&memory_root).await?;

    shutdown_test_EfficiencyCode(&test).await?;
    Ok(())
}
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_EfficiencyCode(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by EfficiencyCode before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    context
        .stream_stage_one_prompt(
            &test.config,
            &EfficiencyCode_core::Prompt::default(),
            &request_context,
        )
        .await?;
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test =
        build_test_EfficiencyCode_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test =
        build_test_EfficiencyCode_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.EfficiencyCode_home);
...
async fn build_test_EfficiencyCode(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestEfficiencyCode> {
    build_test_EfficiencyCode_with_memories_config(server, home, startup_test_memories_config())
        .await
}

async fn build_test_EfficiencyCode_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestEfficiencyCode> {
...
async fn init_state_db(
    home: &Arc<TempDir>,
) -> anyhow::Result<Arc<EfficiencyCode_state::StateRuntime>> {
    let db = EfficiencyCode_state::StateRuntime::init(
        EfficiencyCode_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestEfficiencyCode) {
    let config_snapshot = test.EfficiencyCode.config_snapshot().await;
...
async fn memory_startup_context_with_provider(
    ...
    Arc<EfficiencyCode_core::config::Config>,
) {
    let config_snapshot = test.EfficiencyCode.config_snapshot().await;
    let mut config = test.config.clone();
    config
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    fn auth(&self) -> ModelProviderFuture<'_, Option<EfficiencyCodeAuth>> {
    ...
    ) -> EfficiencyCode_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(EfficiencyCode_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &EfficiencyCode_state::StateRuntime,
    ...
    let mut metadata_builder = EfficiencyCode_state::ThreadMetadataBuilder::new(
        thread_id,
        EfficiencyCode_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = EfficiencyCode_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = EfficiencyCode_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<EfficiencyCode_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.EfficiencyCode.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        EfficiencyCode_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_EfficiencyCode(test: &TestEfficiencyCode) -> anyhow::Result<()> {
