...
Runtime orchestration for Phase 1 and Phase 2 still lives in `EfficiencyCode-core` under
`EfficiencyCode-rs/core/src/memories/`.

## Crates

- `EfficiencyCode-rs/memories/read` (`EfficiencyCode-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `EfficiencyCode-rs/memories/write` (`EfficiencyCode-memories-write`) owns the write path:
...
- In `EfficiencyCode`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.EfficiencyCode/memories/.git` by `EfficiencyCode-git-utils`
