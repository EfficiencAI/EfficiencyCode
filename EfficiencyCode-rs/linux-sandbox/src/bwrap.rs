...
//! - sensitive subpaths such as `.git`, `.agents`, and `.EfficiencyCode` remain
...
use EfficiencyCode_protocol::permissions::is_protected_metadata_name;
...
use EfficiencyCode_protocol::protocol::FileSystemSpecialPath;
use EfficiencyCode_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.EfficiencyCode`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".EfficiencyCode")
                {
                    return None;
                }
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_EfficiencyCode = temp_dir.path().join("real-EfficiencyCode");
        let logical_EfficiencyCode = logical_home.join(".EfficiencyCode");
        let real_memories = real_EfficiencyCode.join("memories");
        let logical_memories = logical_EfficiencyCode.join("memories");
        ...
        std::os::unix::fs::symlink(&real_EfficiencyCode, &logical_EfficiencyCode)
            .expect("create symlinked EfficiencyCode home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".EfficiencyCode"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_EfficiencyCode = path_to_string(&temp_dir.path().join(".EfficiencyCode"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_EfficiencyCode));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_EfficiencyCode)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.EfficiencyCode"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.EfficiencyCode"),
            ]
        );
        assert_eq!(
            args.args,
