...
use EfficiencyCode_network_proxy::write_attribution_frame;
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(EfficiencyCode_home) = std::env::var_os("EfficiencyCode_HOME") {
        let candidate = PathBuf::from(EfficiencyCode_home).join("tmp");
