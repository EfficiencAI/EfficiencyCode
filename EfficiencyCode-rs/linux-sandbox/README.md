# EfficiencyCode-linux-sandbox

This crate is responsible for producing:

- a `EfficiencyCode-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the EfficiencyCode CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `EfficiencyCode-exec` CLI can check if its arg0 is `EfficiencyCode-linux-sandbox` and, if so, execute as if it were `EfficiencyCode-linux-sandbox`
  - this should also be true of the `EfficiencyCode` multitool CLI

On Linux, EfficiencyCode prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `EfficiencyCode-resources/bwrap` binary shipped
with EfficiencyCode.
EfficiencyCode also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. EfficiencyCode surfaces the same startup warning
...
it cannot create the required user namespaces, so EfficiencyCode rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `EfficiencyCode-resources/bwrap` path.
- If `bwrap` is missing, EfficiencyCode also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, EfficiencyCode surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; EfficiencyCode rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.EfficiencyCode`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `EfficiencyCode sandbox`; the host OS selects the sandbox backend.
