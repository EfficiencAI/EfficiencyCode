...
fn linux_sandbox_command(
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_EfficiencyCode-linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn managed_proxy_full_filesystem_uses_minimal_dev_nodes() {
    ...
    if let Some(file) = &shared_memory_file {
        env.insert(
            "EfficiencyCode_TEST_SHM_PATH".to_string(),
    ...
    let output = run_linux_sandbox_direct(
        &[
            ...
            concat!(
                ...
                "fi; ",
                "if [ -n \"${EfficiencyCode_TEST_SHM_PATH:-}\" ]; then ",
                "test \"$(cat \"$EfficiencyCode_TEST_SHM_PATH\")\" = host-before; ",
                "printf sandbox-after >\"$EfficiencyCode_TEST_SHM_PATH\"; ",
                "fi; ",
                "stat -c '%d:%i' /dev",
            ),
        ],
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    let sandbox_helper_dir =
        std::path::Path::new(env!("CARGO_BIN_EXE_EfficiencyCode-linux-sandbox"))
            .parent()
