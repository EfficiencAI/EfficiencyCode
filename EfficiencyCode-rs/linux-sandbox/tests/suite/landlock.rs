...
use EfficiencyCode_core::exec::process_exec_tool_call;
...
fn EfficiencyCode_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_EfficiencyCode-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> EfficiencyCode_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<EfficiencyCode_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let EfficiencyCode_linux_sandbox_exe = Some(EfficiencyCode_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(
    output: &EfficiencyCode_protocol::exec_output::ExecToolCallOutput,
) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            EfficiencyCodeErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<EfficiencyCode_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> EfficiencyCode_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let EfficiencyCode_linux_sandbox_exe: Option<PathBuf> =
        Some(EfficiencyCode_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_EfficiencyCode_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_EfficiencyCode = tmpdir.path().join(".EfficiencyCode");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_EfficiencyCode).expect("create .EfficiencyCode");

    let git_target = dot_git.join("config");
    let EfficiencyCode_target = dot_EfficiencyCode.join("config.toml");
    ...
    let EfficiencyCode_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", EfficiencyCode_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".EfficiencyCode write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(EfficiencyCode_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_EfficiencyCode_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-EfficiencyCode");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_EfficiencyCode = tmpdir.path().join(".EfficiencyCode");
    symlink(&decoy, &dot_EfficiencyCode).expect("create .EfficiencyCode symlink");
    ...
    let EfficiencyCode_output = expect_denied(
        ...
        .await,
        ".EfficiencyCode symlink replacement should be denied",
    );
    assert_ne!(EfficiencyCode_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_EfficiencyCode_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".EfficiencyCode symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".EfficiencyCode symlink build failure should deny: {output:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|EfficiencyCode|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_EfficiencyCode_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".EfficiencyCode"],
            &subdir,
            std::slice::from_ref(&subdir),
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        "child .EfficiencyCode directory creation should be denied",
    );
    assert_ne!(mkdir_EfficiencyCode_output.exit_code, 0);
    assert!(!subdir.join(".EfficiencyCode").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = EfficiencyCode_linux_sandbox_exe()
        .parent()
