...
use EfficiencyCode_config::HookStateToml;
...
use EfficiencyCode_config::TomlValue;
use EfficiencyCode_config::version_for_toml;
...
use EfficiencyCode_protocol::protocol::HookTrustStatus;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project {
            ...
            .hooks_config_folder()
            .unwrap_or_else(|| dot_EfficiencyCode_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: EfficiencyCode_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let runs_async = r#async
                        && event_name
                            != EfficiencyCode_protocol::protocol::HookEventName::SessionEnd;
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        EfficiencyCode_protocol::protocol::HookEventName::PreToolUse
                            | EfficiencyCode_protocol::protocol::HookEventName::PostToolUse
                            | EfficiencyCode_protocol::protocol::HookEventName::SessionStart
                            | EfficiencyCode_protocol::protocol::HookEventName::UserPromptSubmit
                            | EfficiencyCode_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
fn normalize_command_hook(
    ...
    if event_name != EfficiencyCode_protocol::protocol::HookEventName::SessionEnd {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/EfficiencyCode/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.EfficiencyCode/config.toml").abs();
        let dot_EfficiencyCode_folder = test_path_buf("/tmp/worktree/.EfficiencyCode").abs();
