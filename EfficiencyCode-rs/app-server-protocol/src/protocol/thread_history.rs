...
use EfficiencyCode_extension_items::image_generation::ImageGenerationItem;
use EfficiencyCode_protocol::items::parse_hook_prompt_message;
use EfficiencyCode_protocol::models::MessagePhase;
use EfficiencyCode_protocol::protocol::AgentReasoningEvent;
use EfficiencyCode_protocol::protocol::AgentReasoningRawContentEvent;
use EfficiencyCode_protocol::protocol::AgentStatus;
use EfficiencyCode_protocol::protocol::ApplyPatchApprovalRequestEvent;
use EfficiencyCode_protocol::protocol::ContextCompactedEvent;
use EfficiencyCode_protocol::protocol::DynamicToolCallResponseEvent;
use EfficiencyCode_protocol::protocol::ErrorEvent;
use EfficiencyCode_protocol::protocol::EventMsg;
use EfficiencyCode_protocol::protocol::ExecCommandBeginEvent;
use EfficiencyCode_protocol::protocol::ExecCommandEndEvent;
...
use EfficiencyCode_protocol::protocol::ImageGenerationBeginEvent;
use EfficiencyCode_protocol::protocol::ImageGenerationEndEvent;
use EfficiencyCode_protocol::protocol::ItemCompletedEvent;
use EfficiencyCode_protocol::protocol::ItemStartedEvent;
use EfficiencyCode_protocol::protocol::McpToolCallBeginEvent;
use EfficiencyCode_protocol::protocol::McpToolCallEndEvent;
use EfficiencyCode_protocol::protocol::PatchApplyBeginEvent;
use EfficiencyCode_protocol::protocol::PatchApplyEndEvent;
...
use EfficiencyCode_protocol::protocol::ViewImageToolCallEvent;
use EfficiencyCode_protocol::protocol::WebSearchBeginEvent;
use EfficiencyCode_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use EfficiencyCode_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
use EfficiencyCode_rollout::CompactedItem;
...
#[cfg(test)]
use EfficiencyCode_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use EfficiencyCode_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `EfficiencyCode-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &EfficiencyCode_protocol::models::ResponseItem) {
        let EfficiencyCode_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &EfficiencyCode_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            EfficiencyCode_protocol::items::TurnItem::EnteredReviewMode(_)
                | EfficiencyCode_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            EfficiencyCode_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            EfficiencyCode_protocol::items::TurnItem::HookPrompt(_)
            | EfficiencyCode_protocol::items::TurnItem::CommandExecution(_)
            | EfficiencyCode_protocol::items::TurnItem::DynamicToolCall(_)
            | EfficiencyCode_protocol::items::TurnItem::CollabAgentToolCall(_)
            | EfficiencyCode_protocol::items::TurnItem::SubAgentActivity(_)
            | EfficiencyCode_protocol::items::TurnItem::Extension(_)
            | EfficiencyCode_protocol::items::TurnItem::EnteredReviewMode(_)
            | EfficiencyCode_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            EfficiencyCode_protocol::items::TurnItem::UserMessage(_)
            | EfficiencyCode_protocol::items::TurnItem::AgentMessage(_)
            | EfficiencyCode_protocol::items::TurnItem::Reasoning(_)
            | EfficiencyCode_protocol::items::TurnItem::WebSearch(_)
            | EfficiencyCode_protocol::items::TurnItem::ImageView(_)
            | EfficiencyCode_protocol::items::TurnItem::ImageGeneration(_)
            | EfficiencyCode_protocol::items::TurnItem::FileChange(_)
            | EfficiencyCode_protocol::items::TurnItem::McpToolCall(_)
            | EfficiencyCode_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &EfficiencyCode_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabCloseEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &EfficiencyCode_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                EfficiencyCode_error_info: payload
    ...
    fn handle_turn_complete(&mut self, payload: &TurnCompleteEvent) {
        let terminal_error = payload.error.as_ref().map(|error| V2TurnError {
            ...
            EfficiencyCode_error_info: error.EfficiencyCode_error_info.clone().map(Into::into),
            additional_details: None,
        });
...
fn convert_dynamic_tool_content_items(
    items: &[EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use EfficiencyCode_extension_items::ExtensionItem as CoreExtensionItem;
    use EfficiencyCode_extension_items::sleep::SleepItem as CoreSleepItem;
    use EfficiencyCode_protocol::ThreadId;
    use EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use EfficiencyCode_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use EfficiencyCode_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use EfficiencyCode_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use EfficiencyCode_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use EfficiencyCode_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use EfficiencyCode_protocol::items::TurnItem as CoreTurnItem;
    use EfficiencyCode_protocol::items::UserMessageItem as CoreUserMessageItem;
    use EfficiencyCode_protocol::items::build_hook_prompt_message;
    use EfficiencyCode_protocol::mcp::CallToolResult;
    use EfficiencyCode_protocol::models::ImageDetail;
    use EfficiencyCode_protocol::models::MessagePhase as CoreMessagePhase;
    use EfficiencyCode_protocol::models::WebSearchAction as CoreWebSearchAction;
    use EfficiencyCode_protocol::parse_command::ParsedCommand;
    ...
    use EfficiencyCode_protocol::protocol::EnteredReviewModeEvent;
    use EfficiencyCode_protocol::protocol::ExecCommandBeginEvent;
    use EfficiencyCode_protocol::protocol::ExecCommandEndEvent;
    use EfficiencyCode_protocol::protocol::ExecCommandSource;
    use EfficiencyCode_protocol::protocol::ExitedReviewModeEvent;
    use EfficiencyCode_protocol::protocol::ItemStartedEvent;
    use EfficiencyCode_protocol::protocol::McpInvocation;
    use EfficiencyCode_protocol::protocol::McpToolCallEndEvent;
    use EfficiencyCode_protocol::protocol::PatchApplyBeginEvent;
    use EfficiencyCode_protocol::protocol::ReviewTarget;
    use EfficiencyCode_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![EfficiencyCode_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn replays_image_generation_end_events_into_turn_history() {
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ImageGenerationEnd(ImageGenerationEndEvent {
                ...
                transparent_background: Some(true),
                failure: Some(
                    EfficiencyCode_extension_items::image_generation::ImageGenerationFailure::UsageLimitExceeded {
    ...
    fn reconstructs_tool_items_from_persisted_completion_events() {
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                query: "EfficiencyCode".into(),
                action: CoreWebSearchAction::Search {
                    query: Some("EfficiencyCode".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/EfficiencyCode",
                })]),
            }),
            EventMsg::ExecCommandEnd(ExecCommandEndEvent {
                call_id: "exec-1".into(),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                EfficiencyCode_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("EfficiencyCode_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    EfficiencyCode_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
        ...
        let events = vec![
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                error: Some(ErrorEvent {
                    ...
                    EfficiencyCode_error_info: Some(EfficiencyCodeErrorInfo::ServerOverloaded),
                }),
                ...
                Turn {
                    ...
                    error: Some(TurnError {
                        ...
                        EfficiencyCode_error_info: Some(
                            crate::protocol::v2::EfficiencyCodeErrorInfo::ServerOverloaded,
                        ),
                        additional_details: None,
                    }),
                    started_at: Some(10),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(EfficiencyCode_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(
                EfficiencyCode_protocol::protocol::CollabAgentSpawnEndEvent {
                    ...
                    reasoning_effort:
                        EfficiencyCode_protocol::openai_models::ReasoningEffort::Medium,
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                EfficiencyCode_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                EfficiencyCode_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                EfficiencyCode_error_info: Some(EfficiencyCodeErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                EfficiencyCode_error_info: Some(
                    EfficiencyCodeErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                EfficiencyCode_error_info: Some(
                    crate::protocol::v2::EfficiencyCodeErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt =
            CoreTurnItem::HookPrompt(EfficiencyCode_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(
                EfficiencyCode_protocol::models::ResponseItem::Message {
                    id: Some(EfficiencyCode_protocol::ResponseItemId::with_suffix(
                        "msg", "1",
                    )),
                    role: "user".into(),
                    content: vec![EfficiencyCode_protocol::models::ContentItem::InputText {
