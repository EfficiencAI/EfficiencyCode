...
use EfficiencyCode_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use EfficiencyCode_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use EfficiencyCode_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use EfficiencyCode_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use EfficiencyCode_protocol::protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub enum Account {
    ...
    AmazonBedrock {
        #[serde(default)]
        uses_EfficiencyCode_managed_credentials: bool,
    },
}
    ...
    fn from(account: ProviderAccount) -> Self {
        match account {
            ...
            ProviderAccount::AmazonBedrock {
                uses_EfficiencyCode_managed_credentials,
            } => Self::AmazonBedrock {
                uses_EfficiencyCode_managed_credentials,
            },
        }
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that EfficiencyCode-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, EfficiencyCode attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// EfficiencyCode attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that EfficiencyCode was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `EfficiencyCode`).
