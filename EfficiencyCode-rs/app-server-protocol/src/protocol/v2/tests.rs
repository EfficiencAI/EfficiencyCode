...
use EfficiencyCode_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use EfficiencyCode_protocol::config_types::MultiAgentMode;
use EfficiencyCode_protocol::items::AgentMessageContent;
use EfficiencyCode_protocol::items::AgentMessageItem;
use EfficiencyCode_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use EfficiencyCode_protocol::items::CollabAgentToolCallItem;
use EfficiencyCode_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use EfficiencyCode_protocol::items::CommandExecutionItem;
...
use EfficiencyCode_protocol::items::FileChangeItem;
use EfficiencyCode_protocol::items::ImageViewItem;
use EfficiencyCode_protocol::items::McpToolCallItem;
use EfficiencyCode_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use EfficiencyCode_protocol::items::ReasoningItem;
...
use EfficiencyCode_protocol::items::WebSearchItem as CoreWebSearchItem;
use EfficiencyCode_protocol::mcp::CallToolResult;
use EfficiencyCode_protocol::mcp::McpServerInfo;
use EfficiencyCode_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use EfficiencyCode_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use EfficiencyCode_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use EfficiencyCode_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use EfficiencyCode_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use EfficiencyCode_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use EfficiencyCode_protocol::protocol::AgentStatus as CoreAgentStatus;
use EfficiencyCode_protocol::protocol::AskForApproval as CoreAskForApproval;
use EfficiencyCode_protocol::protocol::ConversationTextRole;
use EfficiencyCode_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use EfficiencyCode_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use EfficiencyCode_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use EfficiencyCode_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use EfficiencyCode_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use EfficiencyCode_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: EfficiencyCode_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        EfficiencyCode_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        EfficiencyCode_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.EfficiencyCode/state",
            "logDir": "file:///C:/Users/alice/.EfficiencyCode/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.EfficiencyCode/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.EfficiencyCode/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.EfficiencyCode/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(
            PathUri::parse("file:///C:/Users/alice/.EfficiencyCode/models.json")
                .expect("valid URI")
        )
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        EfficiencyCode_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.EfficiencyCode/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![EfficiencyCode_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            EfficiencyCode_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = EfficiencyCode_protocol::ThreadId::default();
    let receiver_thread_id = EfficiencyCode_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: EfficiencyCode_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            EfficiencyCode_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(EfficiencyCode_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn mcp_tool_call_app_context_serializes_connector_id() {
    let item = ThreadItem::McpToolCall {
        id: "mcp-1".to_string(),
        server: "EfficiencyCode_apps".to_string(),
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@EfficiencyCode-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "EfficiencyCode-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@EfficiencyCode-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "EfficiencyCode-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn EfficiencyCode_error_info_serializes_http_status_code_in_camel_case() {
    let value = EfficiencyCodeErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn EfficiencyCode_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(EfficiencyCodeErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn EfficiencyCode_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = EfficiencyCodeErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(EfficiencyCode_protocol::config_types::CollaborationMode {
            mode: EfficiencyCode_protocol::config_types::ModeKind::Plan,
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    ...
    assert_eq!(
        params.EfficiencyCode_response_handoff_channel_prefixes,
        Some(BTreeMap::from([
