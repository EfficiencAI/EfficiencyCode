...
#[path = "../src/proto/EfficiencyCode.exec_server.relay.v1.rs"]
mod relay_proto;
...
use EfficiencyCode_exec_server::EnvironmentConnectionState;
use EfficiencyCode_exec_server::EnvironmentManager;
use EfficiencyCode_exec_server::EnvironmentReadyInfo;
use EfficiencyCode_exec_server::ExecParams;
use EfficiencyCode_exec_server::ExecResponse;
use EfficiencyCode_exec_server::ExecServerClient;
use EfficiencyCode_exec_server::ExecServerError;
use EfficiencyCode_exec_server::ExecServerRuntimePaths;
use EfficiencyCode_exec_server::FsReadFileParams;
use EfficiencyCode_exec_server::NoiseChannelIdentity;
use EfficiencyCode_exec_server::NoiseChannelPublicKey;
use EfficiencyCode_exec_server::NoiseRendezvousConnectArgs;
...
use EfficiencyCode_exec_server_protocol::ProcessSandboxType;
use EfficiencyCode_http_client::HttpClientFactory;
use EfficiencyCode_http_client::OutboundProxyPolicy;
use EfficiencyCode_http_client::cache_system_proxy_route_for_test;
...
fn static_registry_auth_provider() -> EfficiencyCode_api::SharedAuthProvider {
...
async fn pending_noise_environment_connects_and_reconnects_after_ready_report() -> Result<()> {
    ...
    let (EfficiencyCode_exe, EfficiencyCode_linux_sandbox_exe) =
        common::current_test_binary_helper_paths()?;
    let runtime_paths =
        ExecServerRuntimePaths::new(EfficiencyCode_exe, EfficiencyCode_linux_sandbox_exe)?;
...
async fn remote_environment_routes_encrypted_exec_server_rpc() -> Result<()> {
    ...
    let client_args = NoiseRendezvousConnectArgs {
        ...
        http_client_factory: EfficiencyCode_http_client::HttpClientFactory::new(
