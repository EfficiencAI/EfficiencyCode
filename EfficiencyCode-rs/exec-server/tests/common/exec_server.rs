...
use EfficiencyCode_exec_server_protocol::JSONRPCNotification;
use EfficiencyCode_exec_server_protocol::JSONRPCRequest;
...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestEfficiencyCodeHelperPaths,
    child: Child,
...
pub(crate) struct TestEfficiencyCodeHelperPaths {
    pub(crate) EfficiencyCode_exe: PathBuf,
    pub(crate) EfficiencyCode_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_EfficiencyCode_helper_paths() -> anyhow::Result<TestEfficiencyCodeHelperPaths> {
    let (helper_binary, EfficiencyCode_linux_sandbox_exe) =
        super::current_test_binary_helper_paths()?;
    Ok(TestEfficiencyCodeHelperPaths {
        EfficiencyCode_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.EfficiencyCode_exe);
    ...
    child.env("EfficiencyCode_HOME", EfficiencyCode_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn EfficiencyCode_home(&self) -> &std::path::Path {
