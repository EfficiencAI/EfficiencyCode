...
use EfficiencyCode_exec_server::CAPABILITY_ROOTS_DISCOVER_METHOD;
use EfficiencyCode_exec_server::CapabilityRootDiscovery;
use EfficiencyCode_exec_server::CapabilityRootsDiscoverParams;
use EfficiencyCode_exec_server::CapabilityRootsDiscoverResponse;
...
async fn discovers_a_complete_capability_bundle_in_one_request() -> anyhow::Result<()> {
  ...
  "apps": "./config/apps.json"
    ...
    assert_eq!(
        ...
        root_uri.join(".EfficiencyCode-plugin/plugin.json")?
    );
