...
use EfficiencyCode_exec_server::FsCloseParams;
use EfficiencyCode_exec_server::FsOpenParams;
use EfficiencyCode_exec_server::FsReadBlockParams;
use EfficiencyCode_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\EfficiencyCode-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\EfficiencyCode-fs-stream-{}", Uuid::new_v4());
