...
const RELEASED_EfficiencyCode_ENV_VAR: &str = "EfficiencyCode_TEST_RELEASED_EfficiencyCode";
const CURRENT_EfficiencyCode_ENV_VAR: &str = "EfficiencyCode_TEST_CURRENT_EfficiencyCode";
const EXECUTOR_MARKER_ENV_VAR: &str = "EfficiencyCode_EXECUTOR_VERSION_SKEW_MARKER";
...
fn version_skew_binaries() -> Result<Option<(PathBuf, PathBuf)>> {
    let Some(released) = std::env::var_os(RELEASED_EfficiencyCode_ENV_VAR) else {
        return Ok(None);
    };
    let current = std::env::var_os(CURRENT_EfficiencyCode_ENV_VAR).with_context(|| {
        format!("{CURRENT_EfficiencyCode_ENV_VAR} must name the current EfficiencyCode binary")
    })?;
    let current = PathBuf::from(current);
    let released = PathBuf::from(released);
    anyhow::ensure!(
        current.is_file(),
        "current EfficiencyCode does not exist: {}",
        current.display()
    );
    anyhow::ensure!(
        released.is_file(),
        "released EfficiencyCode does not exist: {}",
        released.display()
    );
...
async fn assert_noise_version_skew(app_binary: &Path, executor_binary: &Path) -> Result<()> {
    ...
    let model = mock_model(EfficiencyCode_home.path()).await?;
...
"#
    ...
    let mut executor = Command::new(executor_binary)
        ...
        .current_dir(EfficiencyCode_home.path())
        .env("EfficiencyCode_HOME", EfficiencyCode_home.path())
        .env("EfficiencyCode_API_KEY", REGISTRY_TOKEN)
    ...
    let mut app_server = Command::new(app_binary)
        ...
        .env(
            EfficiencyCode_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR,
            &registry_url,
        )
        .env(
            EfficiencyCode_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR,
            ENVIRONMENT_ID,
        )
        .env(
        ...
        .env_remove(EfficiencyCode_EXEC_SERVER_URL_ENV_VAR)
    ...
    let thread = app_server_request(
        ...
        /*id*/ 2,
        "thread/start",
        json!({"cwd": EfficiencyCode_home.path()}),
    )
    .await?;
    ...
    assert_eq!(
        std::fs::read_to_string(EfficiencyCode_home.path().join("version-skew-output.txt"))?,
        EXPECTED_OUTPUT
    );
...
async fn mock_model(EfficiencyCode_home: &Path) -> Result<MockServer> {
    let server = MockServer::start().await;
    let arguments = serde_json::to_string(&json!({
        "cmd": format!("printf '%s' \"${EXECUTOR_MARKER_ENV_VAR}\" > version-skew-output.txt"),
        "workdir": EfficiencyCode_home,
        "yield_time_ms": 5_000,
    }))?;
    let completed = |id| {
        json!({
