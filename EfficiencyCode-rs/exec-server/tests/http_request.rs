...
use EfficiencyCode_exec_server_protocol::JSONRPCError;
        ...
        .send_request(
            ...
            serde_json::to_value(HttpRequestParams {
                ...
                headers: vec![HttpHeader {
                    name: "x-EfficiencyCode-test".to_string(),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<EfficiencyCode_exec_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
