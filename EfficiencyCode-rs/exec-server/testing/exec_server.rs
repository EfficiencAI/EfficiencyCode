...
//!
//! Linking only exec-server avoids depending on the full EfficiencyCode CLI binary
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let runtime_paths =
        ExecServerRuntimePaths::new(current_exe, /*EfficiencyCode_linux_sandbox_exe*/ None)?;
    EfficiencyCode_exec_server::run_main(
        "ws://127.0.0.1:0",
        runtime_paths,
