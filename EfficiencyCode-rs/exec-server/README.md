...
`EfficiencyCode-exec-server` is the library backing `EfficiencyCode exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`EfficiencyCode-utils-pty`.

It provides:

- a CLI entrypoint: `EfficiencyCode exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `EfficiencyCode` binary owns hidden helper dispatch for sandboxed
filesystem operations and `EfficiencyCode-linux-sandbox`.

## Transport

The server speaks the exec-specific `EfficiencyCode-exec-server-protocol` message
...
must support it.
It uses the standard EfficiencyCode ChatGPT sign-in state; run `EfficiencyCode login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `EfficiencyCode_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; EfficiencyCode then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `EfficiencyCode_API_KEY`;
EfficiencyCode sends it as a bearer token on the registration request. For example:

```sh
EfficiencyCode_API_KEY="$OPENAI_API_KEY" \
EfficiencyCode exec-server \
  --remote ... \
...
`EfficiencyCode.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/EfficiencyCode.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `EfficiencyCode-utils-pty`.
...
hidden helper process launched from the top-level `EfficiencyCode` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `EfficiencyCode exec-server` command
builds these paths from the `EfficiencyCode` arg0 dispatch state and resolves its HTTP
client factory from the effective EfficiencyCode configuration.
...
provider from EfficiencyCode auth state before starting remote mode.
