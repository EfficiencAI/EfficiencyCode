...
use EfficiencyCode_config::host_name;
use EfficiencyCode_config::loader::LocalTomlLayerStack;
use EfficiencyCode_config::loader::load_local_config_layers;
use EfficiencyCode_exec_server_protocol::EnvironmentConfigLayer;
use EfficiencyCode_exec_server_protocol::EnvironmentConfigLayerStack;
use EfficiencyCode_exec_server_protocol::EnvironmentConfigReadParams;
use EfficiencyCode_exec_server_protocol::EnvironmentConfigReadResponse;
...
pub(crate) async fn read_environment_config(
    ...
    let EfficiencyCode_home = find_EfficiencyCode_home().map_err(|error| {
        ReadEnvironmentConfigError::Internal(format!("failed to find EfficiencyCode home: {error}"))
    })?;
    let layers = load_local_config_layers(file_system, EfficiencyCode_home.as_path(), &cwd)
        .await
        .map_err(|error| {
