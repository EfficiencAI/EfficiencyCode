...
use EfficiencyCode_network_proxy::PROXY_ATTRIBUTION_TOKEN_ENV_KEY;
...
async fn sandbox_request_routes_custom_arg0_to_inner_helper() {
    ...
    let helper_mode = prepared
        .command
        .iter()
        .position(|arg| arg == EfficiencyCode_ARG0_EXEC_HELPER_ARG1)
        .expect("sandboxed command should invoke arg0 helper");

    assert_eq!(
        prepared.command[helper_mode..],
        [
            EfficiencyCode_ARG0_EXEC_HELPER_ARG1,
            "custom-arg0",
            "/bin/sh",
            "-c",
            "true",
    ...
    assert_eq!(
        prepared.arg0,
        Some(EfficiencyCode_LINUX_SANDBOX_ARG0.to_string())
    );
    #[cfg(target_os = "macos")]
