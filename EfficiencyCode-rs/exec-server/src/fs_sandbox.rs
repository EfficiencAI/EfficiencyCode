...
use EfficiencyCode_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::EfficiencyCode_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.EfficiencyCode_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![EfficiencyCode_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.EfficiencyCode_self_exe.as_path())
        .chain(runtime_paths.EfficiencyCode_linux_sandbox_exe.as_deref())
    {
...
fn spawn_command(
    ...
    env.retain(|name, _| {
        !EfficiencyCode_protocol::shell_environment::is_non_inheritable_env_var(name)
    });
    command.env_clear();
    command.envs(env);
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let EfficiencyCode_self_exe = std::env::current_exe().expect("current exe");
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/EfficiencyCode".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let EfficiencyCode_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths = ExecServerRuntimePaths::new(
            EfficiencyCode_self_exe.clone(),
            Some(EfficiencyCode_self_exe),
        )
        .expect("runtime paths");
        ...
        let sandbox_context = {
            ...
            crate::FileSystemSandboxContext {
                windows_sandbox_level:
                    EfficiencyCode_protocol::config_types::WindowsSandboxLevel::RestrictedToken,
                ..sandbox_context
            }
        };
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context =
            EfficiencyCode_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let EfficiencyCode_self_exe = root.path().join("bin").join("EfficiencyCode");
        let EfficiencyCode_linux_sandbox_exe = root
            .path()
            .join("aliases")
            .join("EfficiencyCode-linux-sandbox");
        let runtime_paths = ExecServerRuntimePaths::new(
            EfficiencyCode_self_exe,
        ...
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let EfficiencyCode_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute EfficiencyCode parent");
        ...
        assert!(policy.can_read_path_with_cwd(EfficiencyCode_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        EfficiencyCode_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
