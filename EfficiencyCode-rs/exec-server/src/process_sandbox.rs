...
use EfficiencyCode_exec_server_protocol::JSONRPCErrorError;
...
use EfficiencyCode_network_proxy::NetworkProxyHandle;
use EfficiencyCode_network_proxy::NetworkProxyState;
use EfficiencyCode_network_proxy::RemoteNetworkProxyLaunchConfig;
use EfficiencyCode_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use EfficiencyCode_network_proxy::strip_managed_proxy_env;
use EfficiencyCode_protocol::models::PermissionProfile;
use EfficiencyCode_sandboxing::SandboxCommand;
use EfficiencyCode_sandboxing::SandboxDirectSpawnTransformRequest;
...
use EfficiencyCode_sandboxing::WindowsSandboxProxySettingsMode;
use EfficiencyCode_sandboxing::WindowsSandboxSpawnRequest;
...
#[cfg(unix)]
use crate::EfficiencyCode_ARG0_EXEC_HELPER_ARG1;
...
pub(crate) async fn prepare_exec_request(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.EfficiencyCode_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.EfficiencyCode_self_exe) {
            sandbox_helper_paths.push(runtime_paths.EfficiencyCode_self_exe.clone());
        }
        sandbox_helper_paths.extend(
            runtime_paths
    ...
    let (program, args) = params.arg0.as_ref().map_or_else(
        ...
        |arg0| {
            ...
            helper_args.push(EfficiencyCode_ARG0_EXEC_HELPER_ARG1.to_string());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            EfficiencyCode_linux_sandbox_exe: runtime_paths
                .EfficiencyCode_linux_sandbox_exe
                .as_deref(),
