...
const MISSING_BWRAP_WARNING: &str = concat!(
    "EfficiencyCode could not find bubblewrap on PATH. ",
    "Install bubblewrap with your OS package manager. ",
    "See the sandbox prerequisites: ",
    "https://developers.openai.com/EfficiencyCode/concepts/sandboxing#prerequisites. ",
    "EfficiencyCode will use the bundled bubblewrap in the meantime.",
);
const USER_NAMESPACE_WARNING: &str =
    "EfficiencyCode's Linux sandbox uses bubblewrap and needs access to create user namespaces.";
pub(crate) const WSL1_BWRAP_WARNING: &str = concat!(
    "EfficiencyCode's Linux sandbox uses bubblewrap, which is not supported on WSL1 ",
