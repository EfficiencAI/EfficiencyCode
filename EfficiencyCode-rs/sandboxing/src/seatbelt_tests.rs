...
use EfficiencyCode_network_proxy::ManagedNetworkSandboxContext;
...
use EfficiencyCode_network_proxy::build_config_state;
...
use EfficiencyCode_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/EfficiencyCode-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.EfficiencyCode".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/EfficiencyCode-readable");
    let unreadable = absolute_path("/tmp/EfficiencyCode-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/EfficiencyCode-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root =
        normalize_path_for_sandbox(Path::new("/tmp/EfficiencyCode-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/EfficiencyCode-proxy-use";
    let explicit_socket = "/tmp/EfficiencyCode-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_EfficiencyCode_subpaths() {
    ...
    // write to .EfficiencyCode/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.EfficiencyCode carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_EfficiencyCode_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .EfficiencyCode/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_EfficiencyCode_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .EfficiencyCode is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_EfficiencyCode_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_EfficiencyCode = repo_root.join(".EfficiencyCode");
    let config_toml = dot_EfficiencyCode.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_EfficiencyCode.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_EfficiencyCode = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_EfficiencyCode_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_EfficiencyCode),
        "missing {expected_dot_EfficiencyCode}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .EfficiencyCode/config.toml to
    ...
    dot_EfficiencyCode_canonical: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".EfficiencyCode")).expect("create .EfficiencyCode");
    fs::write(
        vulnerable_root.join(".EfficiencyCode").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .EfficiencyCode/config.toml");
    ...
    let dot_EfficiencyCode_canonical = vulnerable_root_canonical.join(".EfficiencyCode");
