...
pub use EfficiencyCode_windows_sandbox::WindowsSandboxProxySettingsMode;
#[cfg(target_os = "linux")]
...
pub fn system_bwrap_warning(
    _permission_profile: &EfficiencyCode_protocol::models::PermissionProfile,
) -> Option<String> {
    None
}

impl From<SandboxTransformError> for EfficiencyCodeErr {
    fn from(err: SandboxTransformError) -> Self {
        match err {
            error @ SandboxTransformError::InvalidCommandCwd { .. }
            | error @ SandboxTransformError::InvalidSandboxPolicyCwd { .. } => {
                EfficiencyCodeErr::InvalidRequest(error.to_string())
            }
            SandboxTransformError::MissingLinuxSandboxExecutable => {
                EfficiencyCodeErr::LandlockSandboxExecutableNotProvided
            }
            SandboxTransformError::EnvironmentNetworkProxy(message) => {
                EfficiencyCodeErr::UnsupportedOperation(message)
            }
            #[cfg(target_os = "linux")]
            SandboxTransformError::Wsl1UnsupportedForBubblewrap => {
                EfficiencyCodeErr::UnsupportedOperation(
                    crate::bwrap::WSL1_BWRAP_WARNING.to_string(),
                )
            }
            #[cfg(not(target_os = "macos"))]
            SandboxTransformError::SeatbeltUnavailable => EfficiencyCodeErr::UnsupportedOperation(
