...
use EfficiencyCode_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "EfficiencyCode-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/EfficiencyCode-backend/agent-identity";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"EfficiencyCode-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://chatgpt.com/EfficiencyCode"
            | "https://chatgpt.com/backend-api/EfficiencyCode"
            | "https://chat.openai.com"
            | "https://chat.openai.com/backend-api"
            | "https://chat.openai.com/EfficiencyCode"
            | "https://chat.openai.com/backend-api/EfficiencyCode" => Ok(Self::Production),
            "https://chatgpt-staging.com"
            | "https://chatgpt-staging.com/backend-api"
            | "https://chatgpt-staging.com/EfficiencyCode"
            | "https://chatgpt-staging.com/backend-api/EfficiencyCode" => Ok(Self::Staging),
            _ => anyhow::bail!(
...
/// single EfficiencyCode run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "EfficiencyCode-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "EfficiencyCode-cli".to_string(),
        },
...
mod tests {
    ...
    use EfficiencyCode_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/EfficiencyCode"),
            "http://localhost:8080/api/EfficiencyCode/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/EfficiencyCode/"),
            "http://localhost:8080/api/EfficiencyCode/agent-identities/jwks"
