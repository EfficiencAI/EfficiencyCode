...
use EfficiencyCode_app_server_client::EnvironmentManager;
use EfficiencyCode_app_server_client::ExecServerRuntimePaths;
...
use EfficiencyCode_app_server_client::InProcessServerEvent;
...
use EfficiencyCode_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use EfficiencyCode_core::StateDbHandle;
use EfficiencyCode_core::check_execpolicy_for_warnings;
...
use EfficiencyCode_core::config::ConfigTomlLoadResult;
...
use EfficiencyCode_core::config::resolve_oss_provider;
use EfficiencyCode_core::config::resolve_profile_v2_config_path;
use EfficiencyCode_core::find_thread_meta_by_name_str;
use EfficiencyCode_core::format_exec_policy_error_with_source;
use EfficiencyCode_core::path_utils;
use EfficiencyCode_core::read_session_meta_line;
...
use EfficiencyCode_login::default_client::set_default_originator;
...
use EfficiencyCode_otel::set_parent_from_context;
use EfficiencyCode_otel::traceparent_context_from_env;
...
pub use event_processor_with_jsonl_output::EfficiencyCodeStatus;
...
enum StdinPromptBehavior {
    ...
    /// `EfficiencyCode exec` behavior for `EfficiencyCode exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `EfficiencyCode exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("EfficiencyCode_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set EfficiencyCode exec originator override {err:?}"
        );
    }
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&EfficiencyCode_home, profile_v2));
    ...
    let cloud_config_bundle = cloud_config_bundle_loader_for_storage(
        bootstrap_auth_config(&EfficiencyCode_home, &bootstrap_config)?,
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        EfficiencyCode_core::otel_init::build_provider(
            &config,
    ...
    EfficiencyCode_core::otel_init::record_process_start(otel.as_ref(), "EfficiencyCode_exec");
    EfficiencyCode_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "EfficiencyCode_exec");
    ...
    let state_db = EfficiencyCode_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_EfficiencyCode_home(
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: EfficiencyCodeFeedback::new(),
        log_db: None,
        ...
        client_name: "EfficiencyCode_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary =
                EfficiencyCode_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what EfficiencyCode
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) = EfficiencyCode_core::config::system_bwrap_warning(
    ...
    info!("EfficiencyCode initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        EfficiencyCode_app_server_protocol::TurnStatus::Failed
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        EfficiencyCodeStatus::Running => {}
                        EfficiencyCodeStatus::InitiateShutdown => {
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<EfficiencyCode_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<EfficiencyCode_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<EfficiencyCode_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral
        && payload.turn.items_view != EfficiencyCode_app_server_protocol::TurnItemsView::Full
}
