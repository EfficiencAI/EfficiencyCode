...
#[command(
    version,
    override_usage = "EfficiencyCode exec [OPTIONS] [PROMPT]\n       EfficiencyCode exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running EfficiencyCode outside a Git repository.
    ...
    /// Do not load `$EfficiencyCode_HOME/config.toml`; auth still uses `EfficiencyCode_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "EfficiencyCode resume --last <prompt>" treats the positional as a prompt, not a session id.
