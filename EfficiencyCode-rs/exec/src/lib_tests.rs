...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("EfficiencyCode-exec-tests");
...
fn exec_default_stderr_filter_suppresses_otel_self_diagnostics() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::error!(target: "EfficiencyCode_exec_test", "real exec error");
    });
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = EfficiencyCode_protocol::protocol::W3cTraceContext {
...
fn runtime_warnings_are_filtered_to_the_primary_thread() {
    ...
    let turn_id = "turn-1";
    let outcomes = [
        EfficiencyCode_app_server_protocol::WarningNotification {
            thread_id: None,
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let EfficiencyCode_home = tempdir().expect("create temp EfficiencyCode home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: EfficiencyCode_app_server_protocol::SessionSource::Exec,
        ...
        name: None,
        turns: vec![
            EfficiencyCode_app_server_protocol::Turn {
                id: "turn-1".to_string(),
    ...
    let notification = ServerNotification::TurnCompleted(
        EfficiencyCode_app_server_protocol::TurnCompletedNotification {
            ...
            turn: EfficiencyCode_app_server_protocol::Turn {
                id: "turn-1".to_string(),
                items_view: EfficiencyCode_app_server_protocol::TurnItemsView::Summary,
                items: Vec::new(),
                status: EfficiencyCode_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(EfficiencyCode_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
"#,
    ...
    .expect("write config");
    let requirements_path = EfficiencyCode_home.path().join("requirements.toml");
    std::fs::write(
        &requirements_path,
        r#"
...
async fn thread_start_params_include_user_thread_source() {
    ...
    assert_eq!(
        ...
        Some(EfficiencyCode_app_server_protocol::ThreadSource::User)
    );
}
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(EfficiencyCode_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        thread: EfficiencyCode_app_server_protocol::Thread {
            ...
            can_accept_direct_input: None,
            thread_source: Some(EfficiencyCode_app_server_protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            git_info: None,
        ...
        approval_policy: EfficiencyCode_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: EfficiencyCode_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: EfficiencyCode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
