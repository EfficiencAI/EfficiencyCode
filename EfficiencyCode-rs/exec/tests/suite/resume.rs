...
fn exec_repo_root() -> anyhow::Result<std::path::PathBuf> {
    Ok(EfficiencyCode_utils_cargo_bin::repo_root()?)
}
...
async fn exec_resume_last_appends_to_existing_file() -> anyhow::Result<()> {
    ...
    let output = test
        ...
        .env(
            "RUST_LOG",
            "EfficiencyCode_app_server::outgoing_message=trace",
        )
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(&repo_root)
...
async fn exec_resume_last_repairs_rollout_missing_from_state_db() -> anyhow::Result<()> {
    ...
    let config = ConfigBuilder::default()
        .EfficiencyCode_home(test.home_path().to_path_buf())
        .build()
        .await?;
