#![cfg(unix)]
use EfficiencyCode_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use EfficiencyCode_core::exec::build_exec_request;
    ...
    let EfficiencyCode_linux_sandbox_exe = Some(
        core_test_support::find_EfficiencyCode_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_EfficiencyCode_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .EfficiencyCode && echo 'sandbox_mode = \"danger-full-access\"' > .EfficiencyCode/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .EfficiencyCode");

    let status = child
        .wait()
        .await
        .expect("should wait for .EfficiencyCode command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .EfficiencyCode creation: {status:?}"
    );
    let dot_EfficiencyCode_metadata = tokio::fs::symlink_metadata(&dot_EfficiencyCode).await;
    if let Ok(metadata) = dot_EfficiencyCode_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_EfficiencyCode.display()
        );
    } else if let Err(err) = &dot_EfficiencyCode_metadata {
        assert_eq!(
            err.kind(),
