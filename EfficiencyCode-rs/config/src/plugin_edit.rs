...
use EfficiencyCode_utils_path::resolve_symlink_write_paths;
use EfficiencyCode_utils_path::write_atomically;
use tokio::task;
...
pub async fn apply_user_plugin_config_edits(
    ...
    task::spawn_blocking(move || {
        apply_user_plugin_config_edits_blocking(&EfficiencyCode_home, edits)
    })
    .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(EfficiencyCode_home.path());
...
"#,
        ...
        clear_user_plugin(EfficiencyCode_home.path(), "demo@market".to_string())
            .await
            .unwrap();

        assert_eq!(
            fs::read_to_string(EfficiencyCode_home.path().join(CONFIG_TOML_FILE)).unwrap(),
            ""
        );
    }
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let EfficiencyCode_home = TempDir::new().unwrap();
        let target_path = EfficiencyCode_home.path().join("target_config.toml");
        symlink(
            &target_path,
            EfficiencyCode_home.path().join(CONFIG_TOML_FILE),
        )
...
enabled = true
    ...
    fn read_config(EfficiencyCode_home: &Path) -> toml::Value {
        toml::from_str(&fs::read_to_string(EfficiencyCode_home.join(CONFIG_TOML_FILE)).unwrap())
            .unwrap()
    }
}
