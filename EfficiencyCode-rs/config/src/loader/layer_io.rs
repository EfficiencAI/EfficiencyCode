...
#[cfg(unix)]
const EfficiencyCode_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/EfficiencyCode/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/EfficiencyCode/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(EfficiencyCode_home)),
    )?;
...
pub(super) fn managed_config_default_path(EfficiencyCode_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = EfficiencyCode_home;
        PathBuf::from(EfficiencyCode_MANAGED_CONFIG_SYSTEM_PATH)
    }
