...
use EfficiencyCode_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/EfficiencyCode/config.toml";
...
///
/// - system    `/etc/EfficiencyCode/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\EfficiencyCode\requirements.toml` (Windows)
...
///
/// - package:  optional default configuration supplied with the EfficiencyCode package
/// - admin:    managed preferences (*)
/// - system    `/etc/EfficiencyCode/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\EfficiencyCode\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${EfficiencyCode_HOME}/config.toml`
/// - profile   `${EfficiencyCode_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.EfficiencyCode/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.EfficiencyCode/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/EfficiencyCode/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(EfficiencyCode_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir =
                AbsolutePathBuf::from_absolute_path(EfficiencyCode_home)?;
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(EfficiencyCode_home)?;
    let base_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, EfficiencyCode_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/EfficiencyCode/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(EfficiencyCode_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/EfficiencyCode/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_EfficiencyCode_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("EfficiencyCode")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_EfficiencyCode_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_EfficiencyCode_system_dir().join("config.toml");
...
fn requirements_layers_from_legacy_scheme(
    ...
    let EfficiencyCode_home = AbsolutePathBuf::from_absolute_path(EfficiencyCode_home)?;
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for EfficiencyCode to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".EfficiencyCode"))
    }
}

fn project_layer_entry(
    dot_EfficiencyCode_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_EfficiencyCode_folder: dot_EfficiencyCode_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_EfficiencyCode_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for layer in discovered.layers {
        let config = resolve_relative_paths_in_config_toml(
            layer.config,
            layer.dot_EfficiencyCode_folder.as_path(),
        )?;
        ...
        layers.push(project_layer_entry(
            &layer.dot_EfficiencyCode_folder,
            config,
...
async fn discover_project_layers(
    ...
    let EfficiencyCode_home_abs = AbsolutePathBuf::from_absolute_path(EfficiencyCode_home)?;
    let EfficiencyCode_home_normalized = normalize_path(EfficiencyCode_home_abs.as_path())
        .unwrap_or_else(|_| EfficiencyCode_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_EfficiencyCode_abs = dir.join(".EfficiencyCode");
        let dot_EfficiencyCode_uri = PathUri::from_abs_path(&dot_EfficiencyCode_abs);
        if !fs
            .get_metadata(&dot_EfficiencyCode_uri, /*sandbox*/ None)
            .await
        ...
        let dot_EfficiencyCode_normalized = normalize_path(dot_EfficiencyCode_abs.as_path())
            .unwrap_or_else(|_| dot_EfficiencyCode_abs.to_path_buf());
        if dot_EfficiencyCode_abs == EfficiencyCode_home_abs
            || dot_EfficiencyCode_normalized == EfficiencyCode_home_normalized
        {
            continue;
        }
        let config_file = dot_EfficiencyCode_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                let config: TomlValue = match toml::from_str(&contents) {
                    ...
                    Err(e) => {
                        ...
                        layers.push(DiscoveredProjectLayer {
                            dot_EfficiencyCode_folder: dot_EfficiencyCode_abs,
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_EfficiencyCode_abs.as_path(),
                    )?;
                }
...
/// from a file like `/etc/EfficiencyCode/managed_config.toml` that has the same
