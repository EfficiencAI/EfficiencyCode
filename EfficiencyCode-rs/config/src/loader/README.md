# `EfficiencyCode-config` loader

This module is the canonical place to **load and describe EfficiencyCode configuration layers** (user config, CLI/session overrides, cloud-managed config, managed config, and MDM-managed preferences) and to produce:
...
Exported from `EfficiencyCode_config::loader`:

- `load_config_layers_state(fs, EfficiencyCode_home, cwd_opt, cli_overrides, options, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
...
4. `Project` config (`.EfficiencyCode/config.toml`)
5. `User` profile config, when present
6. `User` config (`config.toml`)
7. `EnterpriseManaged` cloud-managed config bundle layers
8. `System` config (`/etc/EfficiencyCode/config.toml` or the Windows system config path)
