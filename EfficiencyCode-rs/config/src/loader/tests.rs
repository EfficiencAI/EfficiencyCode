use super::*;
use EfficiencyCode_file_system::CopyOptions;
use EfficiencyCode_file_system::CreateDirectoryOptions;
use EfficiencyCode_file_system::ExecutorFileSystemFuture;
use EfficiencyCode_file_system::FileMetadata;
use EfficiencyCode_file_system::FileSystemReadStream;
use EfficiencyCode_file_system::FileSystemSandboxContext;
use EfficiencyCode_file_system::ReadDirectoryEntry;
use EfficiencyCode_file_system::RemoveOptions;
...
"#,
    ...
    assert!(
        message.contains("https://developers.openai.com/EfficiencyCode/config-advanced#profiles"),
        "unexpected error message: {message}"
    );
}
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    let user_file = EfficiencyCode_home.join(CONFIG_TOML_FILE);
