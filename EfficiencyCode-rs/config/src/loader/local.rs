...
pub async fn load_local_config_layers(
    ...
    load_local_config_layers_with_overrides(
        fs,
        EfficiencyCode_home,
        cwd,
...
pub(super) async fn load_local_config_layers_with_overrides(
    ...
    .await?;
    append_legacy_config_layers(&mut config_layers, loaded_managed, &EfficiencyCode_home)?;
    ...
    for layer in layers {
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_EfficiencyCode_folder: layer.dot_EfficiencyCode_folder.clone(),
            },
            base_dir: layer.dot_EfficiencyCode_folder,
            toml: config,
        });
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_EfficiencyCode_folder: hooks_config_folder.clone(),
            },
...
fn append_legacy_config_layers(
    ...
    if let Some(config) = loaded.managed_config_from_mdm {
        output.push(LocalTomlLayer {
            ...
            base_dir: EfficiencyCode_home.clone(),
            toml: config.managed_config,
        });
    }
    Ok(())
...
async fn local_requirements_layers(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        entries.extend(
            ...
            .await?
            .map(|layer| layer.with_base_dir(EfficiencyCode_home)),
        );
    }
