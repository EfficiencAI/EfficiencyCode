...
fn user_layer(EfficiencyCode_home: &TempDir, config: &str) -> ConfigLayerEntry {
...
fn stack(EfficiencyCode_home: &TempDir, user: &str, session: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![
            user_layer(EfficiencyCode_home, user),
            ConfigLayerEntry::new(
...
fn session_flags_can_reenable_user_disabled_path() {
    let EfficiencyCode_home = TempDir::new().expect("temp dir");
    let skill_path = EfficiencyCode_home.path().join("skills/demo/SKILL.md");
...
fn later_path_rule_reenables_one_skill_disabled_by_name() {
    let EfficiencyCode_home = TempDir::new().expect("temp dir");
    let root = EfficiencyCode_home.path().join("reenable-by-path");
