...
fn base_dir() -> AbsolutePathBuf {
    test_path_buf("/var/lib/EfficiencyCode").abs()
}
...
fn enterprise_layers_precede_user_and_override_system() {
    ...
    let mut layers = vec![ConfigLayerEntry::new(
        ConfigLayerSource::System {
            file: test_path_buf("/etc/EfficiencyCode/config.toml").abs(),
        },
        toml(
            r#"
model = "system"
...
"#,
    ...
    layers.push(ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: test_path_buf("/home/alice/.EfficiencyCode/config.toml").abs(),
            profile: None,
        },
        toml("model = \"user\""),
    ));
