//! Schema-heavy configuration TOML types used by EfficiencyCode.
...
use EfficiencyCode_protocol::config_types::WebSearchToolConfig;
...
/// Base config deserialized from ~/.EfficiencyCode/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by EfficiencyCode will likely degrade model performance.
    ...
    /// file: Use a file in the EfficiencyCode home directory.
    ...
    /// Directory where EfficiencyCode stores the SQLite state DB.
    /// Defaults to `$EfficiencyCode_SQLITE_HOME` when set. Otherwise uses `$EfficiencyCode_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where EfficiencyCode writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$EfficiencyCode_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned EfficiencyCode Apps MCP requests.
    ...
    /// directories for `.EfficiencyCode` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use EfficiencyCode_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use EfficiencyCode_protocol::protocol::RealtimeVoice;
