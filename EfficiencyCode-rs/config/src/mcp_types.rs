...
use EfficiencyCode_protocol::config_types::ToolExposureSurface;
...
/// OAuth client settings used when EfficiencyCode launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
/// Authentication flow EfficiencyCode attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where EfficiencyCode should start this MCP server.
    pub environment_id: String,

    /// When `false`, EfficiencyCode skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `EfficiencyCode exec` exits with an error if this MCP server fails to initialize.
