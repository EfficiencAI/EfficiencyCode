...
//! `EscalateRequest` over the inherited `EfficiencyCode_ESCALATE_SOCKET`, and the server decides whether to
...
//! specified by the `EfficiencyCode_ESCALATE_SOCKET` environment variable to _send_ escalation requests,
...
pub use EfficiencyCode_protocol::approvals::EscalationPermissions;
pub use EfficiencyCode_protocol::approvals::ResolvedPermissionProfile;
