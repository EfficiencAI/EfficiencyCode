...
use EfficiencyCode_code_mode::CodeModeSessionDelegate;
...
use EfficiencyCode_code_mode::NotificationFuture;
...
use EfficiencyCode_code_mode::ToolInvocationFuture;
use EfficiencyCode_code_mode::WaitOutcome;
use EfficiencyCode_code_mode::WaitRequest;
use EfficiencyCode_code_mode::host::MAX_FRAME_BYTES;
...
async fn child_process_loss_cleans_up_and_rebuilds_the_shared_host() {
    let host_program = EfficiencyCode_utils_cargo_bin::cargo_bin("EfficiencyCode-code-mode-host")
        .expect("host binary");
