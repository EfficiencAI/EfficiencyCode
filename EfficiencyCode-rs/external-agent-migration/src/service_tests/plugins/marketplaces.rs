...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, EfficiencyCode_home)
        .import_plugins(
            /*cwd*/ None,
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".EfficiencyCode-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".EfficiencyCode-plugin"))
...
"#
    ...
    .expect("write EfficiencyCode config");
    fs::write(
    ...
    fs::write(
        source_plugin_root.join(".EfficiencyCode-plugin/plugin.json"),
...
async fn detect_home_supports_relative_external_agent_plugin_marketplace_path() {
    ...
    let items = service_for_paths(external_agent_home.clone(), EfficiencyCode_home)
...
fn import_skills_returns_only_new_skill_directory_names() {
    ...
    let copied_names = service_for_paths(external_agent_home, EfficiencyCode_home)
...
fn import_cursor_skills_reads_user_and_managed_directories() {
    ...
    let managed_skill = external_agent_home
        .join("skills-cursor")
        .join("managed-skill");
    let target_skills = EfficiencyCode_home
        .parent()
        .map(|parent| parent.join(".agents").join("skills"))
        .unwrap_or_else(|| PathBuf::from(".agents").join("skills"));
    ...
    let mut service = service_for_paths(external_agent_home, EfficiencyCode_home);
