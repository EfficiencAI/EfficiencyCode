...
fn copies_only_selected_projects_and_recopies_changed_content() {
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &all_files).expect("detect new memories"),
        BTreeSet::from(["project-a".to_string(), "project-b".to_string()])
    );
    let selected_memory = BTreeSet::from(["project-a"]);

    let outcome =
        copy_resources(&EfficiencyCode_home, &all_files, &selected_memory).expect("copy project A");
    assert_eq!(outcome.synchronized_projects, vec!["project-a"]);
    assert_eq!(outcome.failures, Vec::new());
    assert_eq!(
        fs::read(resources_root(&EfficiencyCode_home).join("project-a/MEMORY.md"))
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &all_files)
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &updated_files)
    ...
    let updated_files = discover_external_memory_files(&source_home).expect("rediscover memories");
    copy_resources(&EfficiencyCode_home, &updated_files, &selected_memory)
    ...
    assert_eq!(
        fs::read(resources_root(&EfficiencyCode_home).join("project-a/updated.md"))
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &changed_files)
            .expect("detect changed memory"),
        BTreeSet::from(["project-a".to_string(), "project-b".to_string()])
    );
    let outcome = copy_resources(&EfficiencyCode_home, &changed_files, &selected_memory)
...
fn preserves_project_successes_and_reports_each_failed_selection() {
    ...
    let outcome = copy_resources(&EfficiencyCode_home, &memory_files, &selected_memory)
...
fn removes_project_resources_when_the_source_project_disappears() {
    ...
    let selected_memory = BTreeSet::from(["project-a"]);
    let memory_files = discover_external_memory_files(&source_home).expect("discover memories");
    copy_resources(&EfficiencyCode_home, &memory_files, &selected_memory).expect("copy project");
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &memory_files)
            .expect("detect removed project"),
        BTreeSet::from(["project-a".to_string()])
    );
    assert_eq!(
        copy_resources(&EfficiencyCode_home, &memory_files, &selected_memory)
...
fn uses_scope_file_to_identify_owned_projects() {
    let root = TempDir::new().expect("create tempdir");
    let EfficiencyCode_home = root.path().join(".EfficiencyCode");
    let resources_root = resources_root(&EfficiencyCode_home);
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &[]).expect("detect removed projects"),
...
fn project_rename_removes_the_old_target_and_imports_the_new_target() {
    ...
    let memory_files = discover_external_memory_files(&source_home).expect("discover memories");
    copy_resources(&EfficiencyCode_home, &memory_files, &project_a_selection)
        .expect("copy project");
...
fn does_not_import_a_new_project_without_a_reliable_cwd() {
    ...
    assert_eq!(
        projects_needing_import(&EfficiencyCode_home, &memory_files).expect("detect memories"),
        BTreeSet::new()
    );
    let outcome = copy_resources(&EfficiencyCode_home, &memory_files, &selected_memory)
        .expect("attempt copy");
...
fn removes_an_existing_unscoped_target_when_cwd_is_unavailable() {
    ...
    let target_root = resources_root(&EfficiencyCode_home).join("project-a");
