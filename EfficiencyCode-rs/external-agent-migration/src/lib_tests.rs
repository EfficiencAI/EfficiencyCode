...
fn migrated_quoted_hook_command(script_name: &str) -> String {
    let hook_path = Path::new("/repo/.EfficiencyCode")
...
#[test]
fn subagent_requires_minimum_EfficiencyCode_agent_fields() {
...
fn subagent_target_preserves_dotted_file_stem() {
    let target_agents = Path::new("/repo/.EfficiencyCode/agents");
    let source_file = source_path("agents/security.audit.md");

    assert_eq!(
        subagent_target_file(&source_file, target_agents),
        Some(PathBuf::from(
            "/repo/.EfficiencyCode/agents/security.audit.toml"
        ))
    );
}
...
fn hook_migration_ignores_unsupported_handlers() {
    ...
    append_convertible_hook_groups_cla(
        &settings,
        &mut migration,
        Some(Path::new("/repo/.EfficiencyCode")),
...
fn hook_script_copy_keeps_existing_target_scripts() {
    ...
    let target_config_dir = root.path().join(".EfficiencyCode");
