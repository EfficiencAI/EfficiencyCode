...
usage() {
  cat <<'EOF'
Usage: build-EfficiencyCode-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-bin <path>] \
  [--zsh-manifest <path>] \
  [--EfficiencyCode-command-runner-bin <path>] \
  [--EfficiencyCode-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --EfficiencyCode-command-runner-bin)
      resource_args+=(
        --EfficiencyCode-command-runner-bin
        "${2:?--EfficiencyCode-command-runner-bin requires a value}"
      )
    ...
    --EfficiencyCode-windows-sandbox-setup-bin)
      resource_args+=(
        --EfficiencyCode-windows-sandbox-setup-bin
        "${2:?--EfficiencyCode-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    ...
    entrypoint="EfficiencyCode"
    archive_stem="EfficiencyCode-package"
    ;;
  app-server)
    variant="EfficiencyCode-app-server"
    entrypoint="EfficiencyCode-app-server"
    archive_stem="EfficiencyCode-app-server-package"
    ;;
  *)
    echo "No EfficiencyCode package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/EfficiencyCode-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/EfficiencyCode-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/EfficiencyCode-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--EfficiencyCode-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--EfficiencyCode-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_EfficiencyCode_package.py"
  --target "$target"
  --variant "$variant"
