...
work_root="$(mktemp -d "${temp_root%/}/EfficiencyCode-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/EfficiencyCode-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${EfficiencyCode_WRAPPER_LOG:?missing EfficiencyCode_WRAPPER_LOG}"
printf '%s\n' "$@" > "$EfficiencyCode_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
  exec "$file"
...
EfficiencyCode_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" EfficiencyCode-zsh)
