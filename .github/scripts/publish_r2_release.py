#!/usr/bin/env python3
"""Mirror a EfficiencyCode GitHub Release to Cloudflare R2.
...
Objects are created under ``EfficiencyCode/releases/<version>/`` with a validated upload
...
objects are verified, the same metadata advances ``EfficiencyCode/channels/latest`` when
the release is marked latest and ``EfficiencyCode/channels/prerelease`` for prereleases.
Stable releases also update the mutable ``EfficiencyCode/install.sh`` and
``EfficiencyCode/install.ps1`` bootstrap aliases from their verified versioned assets.
"""
