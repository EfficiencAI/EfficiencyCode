...
OPENAI_REPOSITORY = "openai/EfficiencyCode"
...
def is_trusted_upstream_run(env: Mapping[str, str]) -> bool:
    ...
    if (
        ...
        return False
    # Non-PR workflow runs in `openai/EfficiencyCode` execute upstream refs, so they are
...
def bazel_command(*args: str, env: Mapping[str, str] | None = None) -> list[str]:
    ...
    bazel = env.get("EfficiencyCode_BAZEL_BIN", "bazel")
