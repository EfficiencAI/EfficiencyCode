...
if ((Test-Path "D:\") -and (Test-DevDrive "D:")) {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "EfficiencyCode-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "EfficiencyCodeDevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop
