...
`RUSTY_V8_ARCHIVE`/`RUSTY_V8_SRC_BINDING_PATH` with EfficiencyCode release assets. Bazel
...
1. Bump the `v8` crate version and refresh `EfficiencyCode-rs/Cargo.lock`.
...
by EfficiencyCode's Bazel Windows GNU toolchain.
...
downloaded `RUSTY_V8_SRC_BINDING_PATH` to point at those `openai/EfficiencyCode` release
...
the exact resolved `v8` crate version in `EfficiencyCode-rs/Cargo.lock`.
