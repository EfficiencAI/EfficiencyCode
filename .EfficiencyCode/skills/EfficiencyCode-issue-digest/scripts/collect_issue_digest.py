#!/usr/bin/env python3
"""Collect recent openai/EfficiencyCode issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a EfficiencyCode owner digest."
    )
    parser.add_argument(
        "--repo", default="openai/EfficiencyCode", help="OWNER/REPO, default openai/EfficiencyCode"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(EfficiencyCode(?: desktop| app|\.app| cli)?|desktop|windows EfficiencyCode app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def collect_digest(args):
    ...
    return {
        ...
        "source": {
            "repo": args.repo,
            "skill": "EfficiencyCode-issue-digest",
