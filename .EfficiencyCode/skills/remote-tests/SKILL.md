...
Use `TestEfficiencyCodeBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$EfficiencyCode_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `EfficiencyCode_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap EfficiencyCode_remote_env_cleanup EXIT

  cd EfficiencyCode-rs
  just test -p EfficiencyCode-core --test all
'
```
...
bash -c '
  ...
  cd EfficiencyCode-rs
  just test -p EfficiencyCode-app-server --test all
'
```
...
```sh
bazel test //EfficiencyCode-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `EfficiencyCode` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of EfficiencyCode in `~/code/EfficiencyCode`. Reset files if needed. Multiple checkouts take longer to build and take up more space.
