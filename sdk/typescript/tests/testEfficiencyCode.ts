...
import { EfficiencyCode } from "../src/EfficiencyCode";
import type { EfficiencyCodeConfigObject } from "../src/EfficiencyCodeOptions";

export const EfficiencyCodeExecPath =
  process.env.EfficiencyCode_EXEC_PATH ??
...
export function createTestClient(options: CreateTestClientOptions = {}): TestClient {
  ...
  return {
    cleanup: () => {},
    client: new EfficiencyCode({
      EfficiencyCodePathOverride: EfficiencyCodeExecPath,
...
function mergeTestConfig(
  ...
  config: EfficiencyCodeConfigObject | undefined,
): EfficiencyCodeConfigObject | undefined {
  const mergedConfig: EfficiencyCodeConfigObject | undefined =
  ...
  return {
    ...
    // sync does not race temp EfficiencyCode_HOME cleanup.
    features:
...
function hasExplicitProviderConfig(config: EfficiencyCodeConfigObject | undefined): boolean {
...
function getCurrentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE") {
      continue;
    }
    if (value !== undefined) {
      env[key] = value;
