...
import { EfficiencyCodeExecPath } from "./testEfficiencyCode";
...
function conformanceDirectory(): string {
  ...
  for (const candidate of candidates) {
    if (existsSync(path.join(candidate, "run_EfficiencyCode_compliance.py"))) {
      return candidate;
    }
  }
...
describe("MCP client conformance", () => {
  it(
    ...
    () => {
      ...
      const EfficiencyCodeBinary = requireFile(EfficiencyCodeExecPath, "built EfficiencyCode binary");
      ...
      const reportPath = path.join(reportDirectory, `EfficiencyCode-mcp-conformance-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          path.join(directory, "run_EfficiencyCode_compliance.py"),
          EfficiencyCodeBinary,
          "--mode",
          "all",
          "--transport",
          "all",
  ...
  it(
    ...
    () => {
      ...
      const reportDirectory = process.env.RUNNER_TEMP ?? os.tmpdir();
      const reportPath = path.join(reportDirectory, `EfficiencyCode-mcp-review-${process.pid}.json`);
      const result = spawnSync(
        "python3",
        [
          reviewer,
