...
import { EfficiencyCodeExecSpy } from "./EfficiencyCodeExecSpy";
...
import { createMockClient, createTestClient } from "./testEfficiencyCode";

describe("EfficiencyCode", () => {
  ...
  it("passes turn options to exec", async () => {
    ...
    const { args: spawnArgs, restore } = EfficiencyCodeExecSpy();
    const { client, cleanup } = createMockClient(url);
  ...
  it("passes EfficiencyCodeOptions config overrides as TOML --config flags", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("lets thread options override EfficiencyCodeOptions config overrides", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("writes output schema to a temporary file and forwards it", async () => {
    ...
    try {
      ...
      expect(text?.format).toEqual({
        name: "EfficiencyCode_output_schema",
        type: "json_schema",
        strict: true,
        schema,
      });
  ...
  it("forwards images to exec", async () => {
    ...
    const { args: spawnArgs, restore } = EfficiencyCodeExecSpy();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "EfficiencyCode-images-"));
  ...
  it("runs in provided working directory", async () => {
    ...
    const { args: spawnArgs, restore } = EfficiencyCodeExecSpy();
    const workingDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "EfficiencyCode-working-dir-"));
    const { client, cleanup } = createTestClient({
      baseUrl: url,
      apiKey: "test",
    });
  ...
  it("sets the EfficiencyCode sdk originator header", async () => {
    const { url, close, requests } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [sse(responseStarted(), assistantMessage("Hi!"), responseCompleted())],
    });
    ...
    try {
      ...
      if (Array.isArray(originatorHeader)) {
        expect(originatorHeader).toContain("EfficiencyCode_sdk_ts");
      } else {
        expect(originatorHeader).toBe("EfficiencyCode_sdk_ts");
      }
    } finally {
      cleanup();
      await close();
...
/**
 * Given a list of args to `EfficiencyCode` and a `key`, collects all `--config`
