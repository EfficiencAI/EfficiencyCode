...
describe("EfficiencyCodeExec", () => {
  it("rejects when exit happens before stdout closes", async () => {
    ...
    if (result.status === "rejected") {
      ...
      expect(result.error.message).toMatch(/EfficiencyCode Exec exited/);
    }
  });
  ...
  it("allows overriding the env passed to the EfficiencyCode CLI", async () => {
    const { EfficiencyCodeExec } = await import("../src/exec");
    spawnMock.mockClear();
    const child = new FakeChildProcess();
    spawnMock.mockReturnValue(child as unknown as child_process.ChildProcess);
    ...
    process.env.EfficiencyCode_ENV_SHOULD_NOT_LEAK = "leak";

    try {
      const exec = new EfficiencyCodeExec("EfficiencyCode", {
        EfficiencyCode_HOME: "/tmp/EfficiencyCode-home",
        CUSTOM_ENV: "custom",
      });
      ...
      expect(spawnEnv.EfficiencyCode_HOME).toBe("/tmp/EfficiencyCode-home");
      expect(spawnEnv.CUSTOM_ENV).toBe("custom");
      expect(spawnEnv.EfficiencyCode_ENV_SHOULD_NOT_LEAK).toBeUndefined();
      expect(spawnEnv.EfficiencyCode_API_KEY).toBe("test");
      expect(spawnEnv.EfficiencyCode_INTERNAL_ORIGINATOR_OVERRIDE).toBeDefined();
      expect(commandArgs).toContain("--config");
      expect(commandArgs).toContain(`openai_base_url=${JSON.stringify("https://example.test")}`);
    } finally {
      delete process.env.EfficiencyCode_ENV_SHOULD_NOT_LEAK;
    }
  });

  it("resolves the package-layout binary and PATH directory", async () => {
    const { resolveNativePackage } = await import("../src/exec");
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "EfficiencyCode-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "bin");
    const pathDir = path.join(packageRoot, "EfficiencyCode-path");
    mkdirSync(binDir, { recursive: true });
    mkdirSync(pathDir, { recursive: true });
    writeFileSync(path.join(packageRoot, "EfficiencyCode-package.json"), "{}");
    writeFileSync(path.join(binDir, "EfficiencyCode"), "");

    expect(resolveNativePackage(vendorRoot, "x86_64-unknown-linux-musl", "EfficiencyCode")).toEqual({
      executablePath: path.join(binDir, "EfficiencyCode"),
      pathDirs: [pathDir],
    });
  });
  ...
  it("falls back to the legacy binary layout", async () => {
    ...
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "EfficiencyCode-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "EfficiencyCode");
  ...
  it("prepends package PATH entries without duplicating them", async () => {
    ...
    const pathDir = path.join(tmpdir(), "EfficiencyCode-path");
