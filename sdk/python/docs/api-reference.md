# OpenAI EfficiencyCode Python SDK - API Reference

Public surface of `openai_EfficiencyCode` for EfficiencyCode workflows.
...
from openai_EfficiencyCode.types import (
    Account,
...
- Version: `openai_EfficiencyCode.__version__`
- Requires Python >= 3.10
- Public EfficiencyCode protocol value and event types live in `openai_EfficiencyCode.types`

## EfficiencyCode (sync)

```python
EfficiencyCode(config: EfficiencyCodeConfig | None = None)
```
...
## AsyncEfficiencyCode (async parity)

```python
AsyncEfficiencyCode(config: EfficiencyCodeConfig | None = None)
```
...
`AsyncEfficiencyCode` initializes lazily. Context entry is the standard path because it
...
When `sandbox=` is omitted, EfficiencyCode uses its configured default. A sandbox
...
- one `EfficiencyCode` instance can stream multiple active turns concurrently
...
- `stream()` and `run()` consume only notifications for their own turn ID
- one `AsyncEfficiencyCode` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public EfficiencyCode protocol models wherever possible:
...
with EfficiencyCode() as EfficiencyCode:
    thread = EfficiencyCode.thread_start(model="gpt-5.4", config={"model_reasoning_effort": "high"})
