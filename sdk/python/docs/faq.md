...
`openai-EfficiencyCode` publishes stable releases. Install the latest one with
`pip install openai-EfficiencyCode`.

## Why does the SDK install a runtime package?

The SDK version tracks the corresponding EfficiencyCode CLI release. Each SDK release
...
- `EfficiencyCode` is the sync public API.
- `AsyncEfficiencyCode` is an async replica of the same public API shape.
- Prefer `async with AsyncEfficiencyCode()` for async code. It is the standard path for
  explicit startup/shutdown, and `AsyncEfficiencyCode` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `EfficiencyCode`.
...
```python
from openai_EfficiencyCode import Sandbox
...
When `sandbox=` is omitted, EfficiencyCode uses its configured default. A turn
...
`EfficiencyCode()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-EfficiencyCode-cli-bin` dependency is missing
- local `EfficiencyCode_bin` override points to a missing file
- a custom local EfficiencyCode executable does not support the SDK operation being used
