...
class EfficiencyCodeError(Exception):
    """Base exception for SDK errors."""


class JsonRpcError(EfficiencyCodeError):
...
class TransportClosedError(EfficiencyCodeError):
    """Raised when the EfficiencyCode transport closes unexpectedly."""


class EfficiencyCodeRpcError(JsonRpcError):
    """Base typed error for JSON-RPC failures."""


class ParseError(EfficiencyCodeRpcError):
    """Raised when a request or response cannot be parsed."""


class InvalidRequestError(EfficiencyCodeRpcError):
    """Raised when the runtime rejects the request shape."""


class MethodNotFoundError(EfficiencyCodeRpcError):
    """Raised when the requested operation is unavailable."""


class InvalidParamsError(EfficiencyCodeRpcError):
    """Raised when an operation receives invalid parameters."""


class InternalRpcError(EfficiencyCodeRpcError):
    """Raised when the runtime reports an internal RPC failure."""


class ServerBusyError(EfficiencyCodeRpcError):
...
def _is_server_overloaded(data: Any) -> bool:
    ...
    if isinstance(data, dict):
        direct = data.get("EfficiencyCode_error_info") or data.get("EfficiencyCodeErrorInfo") or data.get("errorInfo")
...
def map_jsonrpc_error(code: int, message: str, data: Any = None) -> JsonRpcError:
    ...
    if -32099 <= code <= -32000:
        ...
        if _contains_retry_limit_text(message):
            return RetryLimitExceededError(code, message, data)
        return EfficiencyCodeRpcError(code, message, data)
