...
from .errors import EfficiencyCodeError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "openai-EfficiencyCode-cli-bin"
...
def _installed_EfficiencyCode_path() -> Path:
    try:
        from EfficiencyCode_cli_bin import bundled_EfficiencyCode_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned EfficiencyCode runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set EfficiencyCodeConfig.EfficiencyCode_bin "
            "explicitly."
        ) from exc

    return bundled_EfficiencyCode_path()


def _installed_EfficiencyCode_path_dirs() -> tuple[Path, ...]:
    try:
        from EfficiencyCode_cli_bin import bundled_path_dir
...
class EfficiencyCodeBinResolverOps:
    installed_EfficiencyCode_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_EfficiencyCode_bin_resolver_ops() -> EfficiencyCodeBinResolverOps:
    return EfficiencyCodeBinResolverOps(
        installed_EfficiencyCode_path=_installed_EfficiencyCode_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_EfficiencyCode_bin(config: "EfficiencyCodeConfig", ops: EfficiencyCodeBinResolverOps) -> Path:
    if config.EfficiencyCode_bin is not None:
        EfficiencyCode_bin = Path(config.EfficiencyCode_bin)
        if not ops.path_exists(EfficiencyCode_bin):
            raise FileNotFoundError(
                f"EfficiencyCode binary not found at {EfficiencyCode_bin}. Set EfficiencyCodeConfig.EfficiencyCode_bin "
                "to a valid binary path."
            )
        return EfficiencyCode_bin

    return ops.installed_EfficiencyCode_path()


def _resolve_EfficiencyCode_bin(config: "EfficiencyCodeConfig") -> Path:
    return resolve_EfficiencyCode_bin(config, _default_EfficiencyCode_bin_resolver_ops())


@dataclass(slots=True)
class EfficiencyCodeConfig:
    """Configuration for launching and identifying the local EfficiencyCode runtime.

    Most callers can use ``EfficiencyCode()`` without configuration. Set ``EfficiencyCode_bin``
    only when intentionally using a specific local EfficiencyCode executable.
    """

    EfficiencyCode_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "EfficiencyCode_python_sdk"
    client_title: str = "EfficiencyCode Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class EfficiencyCodeClient:
    """Synchronous typed JSON-RPC client for `EfficiencyCode app-server` over stdio."""

    def __init__(
        self,
        config: EfficiencyCodeConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or EfficiencyCodeConfig()
    ...
    def __enter__(self) -> "EfficiencyCodeClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            EfficiencyCode_bin = _resolve_EfficiencyCode_bin(self.config)
            if self.config.EfficiencyCode_bin is None:
                path_dirs = _installed_EfficiencyCode_path_dirs()
            args = [str(EfficiencyCode_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise EfficiencyCodeError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("EfficiencyCode process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"EfficiencyCode process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise EfficiencyCodeError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise EfficiencyCodeError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_EfficiencyCode_home() -> str:
    return str(Path.home() / ".EfficiencyCode")
