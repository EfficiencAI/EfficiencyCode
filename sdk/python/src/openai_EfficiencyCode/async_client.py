...
class AsyncEfficiencyCodeClient:
    """Async wrapper around EfficiencyCodeClient using thread offloading."""
    ...
    def __init__(self, config: EfficiencyCodeConfig | None = None) -> None:
        ...
        self._sync = EfficiencyCodeClient(config=config)

    async def __aenter__(self) -> "AsyncEfficiencyCodeClient":
        """Start the EfficiencyCode process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the EfficiencyCode process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the EfficiencyCode session."""
    ...
    async def start_goal_operation(
        ...
        worker = threading.Thread(
            ...
            name="EfficiencyCode-goal-start",
            daemon=True,
        )
        worker.start()
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="EfficiencyCode-goal-start-cleanup",
                    daemon=True,
                ).start()
