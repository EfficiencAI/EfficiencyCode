...
from .client import EfficiencyCodeClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncEfficiencyCode needed by async login handles."""

    _client: AsyncEfficiencyCodeClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live EfficiencyCode connection."""
        ...


def start_chatgpt_login(client: EfficiencyCodeClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: EfficiencyCodeClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `EfficiencyCode.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `EfficiencyCode.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncEfficiencyCode.login_chatgpt()`."""

    _EfficiencyCode: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._EfficiencyCode._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._EfficiencyCode._ensure_initialized()
        return await self._EfficiencyCode._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncEfficiencyCode.login_chatgpt_device_code()`."""
