...
from .async_client import AsyncEfficiencyCodeClient
from .client import EfficiencyCodeClient, EfficiencyCodeConfig
...
class EfficiencyCode:
    """Synchronous client for creating threads and running EfficiencyCode turns.

    The client starts its runtime connection during construction. Use it as a
    context manager so resources are closed promptly.
    """

    def __init__(self, config: EfficiencyCodeConfig | None = None) -> None:
        self._client = EfficiencyCodeClient(config=config)
        try:
            self._client.start()
    ...
    def __enter__(self) -> "EfficiencyCode":
        return self
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate EfficiencyCode with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current EfficiencyCode account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current EfficiencyCode account session."""
        self._client.account_logout()

    # BEGIN GENERATED: EfficiencyCode.flat_methods
    def thread_start(
        self,
        *,
    ...
    # END GENERATED: EfficiencyCode.flat_methods

    def models(self, *, include_hidden: bool = False) -> ModelListResponse:
        """List available models reported by EfficiencyCode."""
        return self._client.model_list(include_hidden=include_hidden)


class AsyncEfficiencyCode:
    """Async mirror of :class:`EfficiencyCode`.

    Prefer ``async with AsyncEfficiencyCode()`` so initialization and shutdown are
    explicit and paired. The async client initializes lazily on context entry
    or first awaited API use.
    """

    def __init__(self, config: EfficiencyCodeConfig | None = None) -> None:
        self._client = AsyncEfficiencyCodeClient(config=config)
        self._init: InitializeResponse | None = None
        self._initialized = False
        self._init_lock = asyncio.Lock()

    async def __aenter__(self) -> "AsyncEfficiencyCode":
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncEfficiencyCode is not initialized yet. Prefer `async with AsyncEfficiencyCode()`; "
    ...
    # BEGIN GENERATED: AsyncEfficiencyCode.flat_methods
    ...
    # END GENERATED: AsyncEfficiencyCode.flat_methods
...
class AsyncThread:
    ...
    _EfficiencyCode: AsyncEfficiencyCode
    id: str
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        """Read this thread, optionally including its turn history."""
        await self._EfficiencyCode._ensure_initialized()
        return await self._EfficiencyCode._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._EfficiencyCode._ensure_initialized()
        return await self._EfficiencyCode._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._EfficiencyCode._ensure_initialized()
        return await self._EfficiencyCode._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        """Send additional input to this active turn."""
        await self._EfficiencyCode._ensure_initialized()
        return await self._EfficiencyCode._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        """Request interruption of this active turn."""
        await self._EfficiencyCode._ensure_initialized()
        return await self._EfficiencyCode._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._EfficiencyCode._ensure_initialized()
        self._EfficiencyCode._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._EfficiencyCode._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._EfficiencyCode._client.unregister_turn_notifications(self.id)
