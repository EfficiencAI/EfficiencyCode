...
with EfficiencyCode(config=runtime_config()) as EfficiencyCode:
    ...
    reopened = EfficiencyCode.thread_resume(thread.id)
    listing_active = EfficiencyCode.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = EfficiencyCode.thread_archive(reopened.id)
    listing_archived = EfficiencyCode.thread_list(limit=20, archived=True)
    unarchived = EfficiencyCode.thread_unarchive(reopened.id)

    resumed = EfficiencyCode.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = EfficiencyCode.thread_fork(unarchived.id, model="gpt-5.4")
