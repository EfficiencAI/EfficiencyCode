...
async def main() -> None:
    async with AsyncEfficiencyCode(config=runtime_config()) as EfficiencyCode:
        ...
        reopened = await EfficiencyCode.thread_resume(thread.id)
        listing_active = await EfficiencyCode.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await EfficiencyCode.thread_archive(reopened.id)
        listing_archived = await EfficiencyCode.thread_list(limit=20, archived=True)
        unarchived = await EfficiencyCode.thread_unarchive(reopened.id)

        resumed = await EfficiencyCode.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await EfficiencyCode.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (
