...
- `sync.py` (public sync surface: `EfficiencyCode`)
- `async.py` (public async surface: `AsyncEfficiencyCode`)

All examples intentionally use only public SDK exports from `openai_EfficiencyCode`
and `openai_EfficiencyCode.types`.
...
```bash
python -m pip install openai-EfficiencyCode
```

The SDK installs its pinned `openai-EfficiencyCode-cli-bin` runtime dependency.
