...
from openai_EfficiencyCode import ApprovalMode, AsyncEfficiencyCode, EfficiencyCode
from openai_EfficiencyCode.generated.v2_all import AskForApprovalValue, ThreadResumeParams
...
def test_thread_resume_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with EfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
            source = EfficiencyCode.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            resumed = EfficiencyCode.thread_resume(source.id)
            resumed_state = EfficiencyCode._client.thread_resume(  # noqa: SLF001
                resumed.id,
...
def test_thread_fork_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with EfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
            source = EfficiencyCode.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            forked = EfficiencyCode.thread_fork(source.id)
            forked_state = EfficiencyCode._client.thread_resume(  # noqa: SLF001
                forked.id,
...
def test_thread_fork_can_override_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with EfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
            source = EfficiencyCode.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            forked = EfficiencyCode.thread_fork(
                source.id,
...
def test_turn_approval_mode_persists_until_next_turn(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with EfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
            ...
            after_turn_override = EfficiencyCode._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            second_result = thread.run("inherit previous approval mode")
            after_omitted_turn = EfficiencyCode._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_thread_run_approval_mode_persists_until_explicit_override(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with EfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
            thread = EfficiencyCode.thread_start(approval_mode=ApprovalMode.deny_all)

            first_result = thread.run("keep approvals denied")
            after_default_run = EfficiencyCode._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            ...
            after_override_run = EfficiencyCode._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_async_thread_run_approval_mode_persists_until_explicit_override(
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncEfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
                thread = await EfficiencyCode.thread_start(approval_mode=ApprovalMode.deny_all)
                first_result = await thread.run("keep async approvals denied")
                after_default_run = await EfficiencyCode._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
                ...
                after_override_run = await EfficiencyCode._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
