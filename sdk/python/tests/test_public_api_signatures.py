...
import openai_EfficiencyCode.types as public_types
from openai_EfficiencyCode import (
    ApprovalMode,
    AsyncEfficiencyCode,
    AsyncThread,
    AsyncTurnHandle,
...
from openai_EfficiencyCode._initialize_metadata import validate_initialize_metadata
from openai_EfficiencyCode.types import InitializeResponse
...
def test_root_exports_EfficiencyCode_config() -> None:
    """The root package should expose the process configuration object."""
    assert EfficiencyCodeConfig.__name__ == "EfficiencyCodeConfig"
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert openai_EfficiencyCode.__version__ == pyproject["project"]["version"]
    assert EfficiencyCodeConfig().client_version == openai_EfficiencyCode.__version__
...
def test_curated_public_api_has_builtin_help_documentation() -> None:
    ...
    documented = {
        "module": openai_EfficiencyCode,
        "EfficiencyCode": EfficiencyCode,
        "AsyncEfficiencyCode": AsyncEfficiencyCode,
        "EfficiencyCodeConfig": EfficiencyCodeConfig,
        "Thread": Thread,
        ...
        "Sandbox": Sandbox,
        "thread_start": EfficiencyCode.thread_start,
        "thread_resume": EfficiencyCode.thread_resume,
...
def test_package_includes_py_typed_marker() -> None:
    ...
    marker = resources.files("openai_EfficiencyCode").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert openai_EfficiencyCode.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(openai_EfficiencyCode, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "EfficiencyCodeClient": hasattr(openai_EfficiencyCode, "EfficiencyCodeClient"),
        "AsyncEfficiencyCodeClient": hasattr(openai_EfficiencyCode, "AsyncEfficiencyCodeClient"),
        "InitializeResponse": hasattr(openai_EfficiencyCode, "InitializeResponse"),
        "ThreadStartParams": hasattr(openai_EfficiencyCode, "ThreadStartParams"),
        "TurnStartParams": hasattr(openai_EfficiencyCode, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(openai_EfficiencyCode, "TurnCompletedNotification"),
        "TurnStatus": hasattr(openai_EfficiencyCode, "TurnStatus"),
    } == {
        "EfficiencyCodeClient": False,
        "AsyncEfficiencyCodeClient": False,
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from openai_EfficiencyCode import *", namespace)
...
def test_types_module_exports_curated_public_types() -> None:
    """The public type module should expose EfficiencyCode protocol models."""
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from openai_EfficiencyCode.types import *", namespace)
...
def test_examples_use_public_import_surfaces() -> None:
    ...
    private_import_markers = [
        "openai_EfficiencyCode.api",
        "openai_EfficiencyCode.client",
        "openai_EfficiencyCode.generated",
        "openai_EfficiencyCode.models",
        "openai_EfficiencyCode.retry",
    ]
...
def test_generated_public_signatures_are_snake_case_and_typed() -> None:
    ...
    expected = {
        ...
        AsyncEfficiencyCode.thread_start: [
            "approval_mode",
            "base_instructions",
            "config",
            "cwd",
        ...
        AsyncEfficiencyCode.thread_list: [
            "archived",
            "cursor",
            "cwd",
            "limit",
        ...
        AsyncEfficiencyCode.thread_resume: [
            "approval_mode",
            "base_instructions",
            "config",
            "cwd",
        ...
        AsyncEfficiencyCode.thread_fork: [
            "approval_mode",
            "base_instructions",
            "config",
            "cwd",
...
def test_lifecycle_methods_are_EfficiencyCode_scoped() -> None:
    """Lifecycle operations should hang off the client rather than thread objects."""
    assert hasattr(EfficiencyCode, "thread_resume")
    assert hasattr(EfficiencyCode, "thread_fork")
    assert hasattr(EfficiencyCode, "thread_archive")
    assert hasattr(EfficiencyCode, "thread_unarchive")
    assert hasattr(AsyncEfficiencyCode, "thread_resume")
    assert hasattr(AsyncEfficiencyCode, "thread_fork")
    assert hasattr(AsyncEfficiencyCode, "thread_archive")
    assert hasattr(AsyncEfficiencyCode, "thread_unarchive")
    assert not hasattr(EfficiencyCode, "thread")
    assert not hasattr(AsyncEfficiencyCode, "thread")
    ...
    for fn in (
        EfficiencyCode.thread_archive,
        EfficiencyCode.thread_unarchive,
        AsyncEfficiencyCode.thread_archive,
        AsyncEfficiencyCode.thread_unarchive,
    ):
        _assert_no_any_annotations(fn)
...
def test_initialize_metadata_parses_user_agent_shape() -> None:
    ...
    payload = InitializeResponse.model_validate({"userAgent": "EfficiencyCode-cli/1.2.3"})
    parsed = validate_initialize_metadata(payload)
    assert parsed is payload
    assert parsed.userAgent == "EfficiencyCode-cli/1.2.3"
    assert parsed.serverInfo is not None
    assert parsed.serverInfo.name == "EfficiencyCode-cli"
