...
from openai_EfficiencyCode._goal import _GoalNotificationStream
from openai_EfficiencyCode._run import _collect_turn_result
from openai_EfficiencyCode.generated.notification_registry import notification_turn_id
from openai_EfficiencyCode.generated.v2_all import TurnStatus
...
def test_private_goal_operation_coalesces_runtime_continuations(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with EfficiencyCode(config=harness.app_server_config()) as EfficiencyCode:
            ...
            state, turn_id = EfficiencyCode._client.start_goal_operation(  # noqa: SLF001
                thread.id,
            ...
            stream = _GoalNotificationStream(
                ...
                lambda: EfficiencyCode._client.unregister_goal_operation(state),  # noqa: SLF001
                lambda: EfficiencyCode._client.cancel_goal_operation(state),  # noqa: SLF001
            )
            events = list(stream)
            result = _collect_turn_result(iter(events), turn_id=turn_id)
            routes = EfficiencyCode._client._router._goal_operations.copy()  # noqa: SLF001
