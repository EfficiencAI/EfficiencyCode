...
remote_path='~/code/EfficiencyCode-sync'
local_exec_server_port="${EfficiencyCode_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${EfficiencyCode_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
rsync \
  ...
  --itemize-changes \
  --exclude '.git/' \
  --exclude 'EfficiencyCode-rs/target/' \
  --filter=':- .gitignore' \
  "$@" \
  "${repo_root}/" \
...
remote_exec_server_log_path="/tmp/EfficiencyCode-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/EfficiencyCode-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/EfficiencyCode-sync"
remote_EfficiencyCode_rs="$remote_repo_root/EfficiencyCode-rs"

cd "${remote_EfficiencyCode_rs}"
cargo build -p EfficiencyCode-cli --bin EfficiencyCode

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/EfficiencyCode exec-server --listen ws://127.0.0.1:0 \
...
echo "Start EfficiencyCode via: "
printf '  EfficiencyCode_EXEC_SERVER_URL=ws://127.0.0.1:%s EfficiencyCode -C %q\n' \
