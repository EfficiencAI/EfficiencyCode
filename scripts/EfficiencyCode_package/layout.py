"""Canonical EfficiencyCode package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "EfficiencyCode-resources"
    path_dir = package_dir / "EfficiencyCode-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"EfficiencyCode-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.EfficiencyCode_command_runner_bin is not None:
        copy_executable(
            inputs.EfficiencyCode_command_runner_bin,
            resources_dir / "EfficiencyCode-command-runner.exe",
            is_windows=True,
        )

    if inputs.EfficiencyCode_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.EfficiencyCode_windows_sandbox_setup_bin,
            resources_dir / "EfficiencyCode-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    write_json(package_dir / "EfficiencyCode-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    required_dirs = [
        Path("bin"),
        Path("EfficiencyCode-resources"),
        Path("EfficiencyCode-path"),
    ]
    ...
    metadata_path = package_dir / "EfficiencyCode-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: EfficiencyCode-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"EfficiencyCode-code-mode-host{spec.exe_suffix}",
        Path("EfficiencyCode-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("EfficiencyCode-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("EfficiencyCode-resources") / "bwrap")
        executable_files.append(Path("EfficiencyCode-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("EfficiencyCode-resources") / "EfficiencyCode-command-runner.exe",
                Path("EfficiencyCode-resources") / "EfficiencyCode-windows-sandbox-setup.exe",
            ]
        )
