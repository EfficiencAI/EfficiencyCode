"""Command-line interface for building EfficiencyCode package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical EfficiencyCode package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt EfficiencyCode-code-mode-host executable. If omitted, "
    ...
    zsh_source.add_argument(
        ...
        help=(
            ...
            "scripts/EfficiencyCode_package/EfficiencyCode-zsh."
        ),
    )
    zsh_source.add_argument(
        "--zsh-bin",
    ...
    parser.add_argument(
        "--EfficiencyCode-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows EfficiencyCode-command-runner.exe executable. "
            "If omitted for Windows targets, EfficiencyCode-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--EfficiencyCode-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows EfficiencyCode-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "EfficiencyCode-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
        ...
        help=(
            ...
            "scripts/EfficiencyCode_package/rg."
        ),
    )
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="EfficiencyCode-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        EfficiencyCode_command_runner_bin=resolve_optional_input_path(
            args.EfficiencyCode_command_runner_bin,
            "prebuilt Windows EfficiencyCode-command-runner.exe executable",
            "--EfficiencyCode-command-runner-bin",
        ),
        EfficiencyCode_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.EfficiencyCode_windows_sandbox_setup_bin,
            "prebuilt Windows EfficiencyCode-windows-sandbox-setup.exe executable",
            "--EfficiencyCode-windows-sandbox-setup-bin",
        ),
    )
    ...
    inputs = PackageInputs(
        ...
        EfficiencyCode_command_runner_bin=source_outputs.EfficiencyCode_command_runner_bin,
        EfficiencyCode_windows_sandbox_setup_bin=source_outputs.EfficiencyCode_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built EfficiencyCode package archive at {archive_path}")

    print(f"Built EfficiencyCode package directory at {package_dir}")
    return 0
