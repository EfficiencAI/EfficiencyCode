# EfficiencyCode package builder

This package contains the implementation behind `scripts/build_EfficiencyCode_package.py`.
...
The builder creates a canonical EfficiencyCode package directory:
...
├── EfficiencyCode-resources
│   ├── bwrap                             # Linux only
│   ├── zsh/bin/zsh                       # supported Unix targets only
│   ├── EfficiencyCode-command-runner.exe          # Windows only
│   └── EfficiencyCode-windows-sandbox-setup.exe   # Windows only
└── EfficiencyCode-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match EfficiencyCode release
...
`EfficiencyCode` and `EfficiencyCode-app-server`. The `version` field in `EfficiencyCode-package.json` is
read from `[workspace.package].version` in `EfficiencyCode-rs/Cargo.toml`.
...
- all targets: `EfficiencyCode-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `EfficiencyCode-command-runner` and `EfficiencyCode-windows-sandbox-setup`,
...
`--EfficiencyCode-command-runner-bin` plus `--EfficiencyCode-windows-sandbox-setup-bin` for
...
downloads and verifies the matching EfficiencyCode-built V8 release pair before invoking
...
DotSlash manifest at `scripts/EfficiencyCode_package/rg`. Downloaded archives are cached
under `$TMPDIR/EfficiencyCode-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/EfficiencyCode_package/EfficiencyCode-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/EfficiencyCode-package/<target>-zsh` and installed at
`EfficiencyCode-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed
