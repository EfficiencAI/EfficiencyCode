...
import EfficiencyCode_conformance_adapter
import official_conformance
import pytest
import run_EfficiencyCode_compliance
from EfficiencyCode_conformance_adapter import (
...
def test_official_adapter_launcher_preserves_strict_production_auth_mode(
    ...
    adapter.write_text(
        'import os\nprint(os.environ.get("EfficiencyCode_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH", ""))\n',
        encoding="utf-8",
    )
...
def test_windows_official_adapter_launcher_quotes_checkout_paths(
    ...
    assert (
        f'set "EfficiencyCode_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH={expected_auth}"\n' in contents
    )
...
def test_strict_auth_does_not_inject_a_second_login(
    ...
    monkeypatch.setattr(
        EfficiencyCode_conformance_adapter,
        "_oauth_login",
        lambda _client, **_kwargs: (True, None),
    )
    monkeypatch.setattr(EfficiencyCode_conformance_adapter, "_reload_mcp", lambda _client: None)
    monkeypatch.setattr(
        EfficiencyCode_conformance_adapter, "_auth_inventory", lambda _client: {}
    )

    def fail_tool_call(_client: object, _workspace: Path) -> None:
        raise AdapterFailure("reauthorization is required")

    monkeypatch.setattr(EfficiencyCode_conformance_adapter, "_auth_tool_call", fail_tool_call)
    monkeypatch.setattr(
        EfficiencyCode_conformance_adapter,
...
def test_retained_artifacts_remove_oauth_credential_store(tmp_path: Path) -> None:
    EfficiencyCode_home = tmp_path / "EfficiencyCode-home"
    EfficiencyCode_home.mkdir()
    credentials = EfficiencyCode_home / ".credentials.json"
...
def test_official_cli_rejects_an_empty_versioned_scenario_run(
    ...
    monkeypatch.setattr(run_EfficiencyCode_compliance, "run_compliance", unexpected_run)

    exit_code = run_EfficiencyCode_compliance.main(
        [
            sys.executable,
            "--mode",
            SHIPPING_LEGACY_VERSION,
...
pathlib.Path(os.environ["EfficiencyCode_CONFORMANCE_ADAPTER_REPORT"]).write_text(
...
""".lstrip(),
    ...
    results = run_official_mode(
        ...
        EfficiencyCode_binary=Path("/opt/EfficiencyCode"),
        mode=MODERN_VERSION,
