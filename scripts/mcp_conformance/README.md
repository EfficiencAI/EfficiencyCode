...
This directory tests the actual EfficiencyCode executable against the official Model
...
First install the frozen workspace dependencies and build EfficiencyCode:

```bash
pnpm install --frozen-lockfile
cargo build --locked --manifest-path EfficiencyCode-rs/Cargo.toml -p EfficiencyCode-cli --bin EfficiencyCode
```

From a published EfficiencyCode checkout, run:

```bash
python3 scripts/mcp_conformance/run_EfficiencyCode_compliance.py \
  EfficiencyCode-rs/target/debug/EfficiencyCode \
  ...
  --report /tmp/EfficiencyCode-mcp-conformance.json
```

The positional executable can also point to an already built EfficiencyCode binary.
...
```bash
python3 scripts/mcp_conformance/run_EfficiencyCode_compliance.py \
  /absolute/path/to/EfficiencyCode \
...
The separate reviewer gate tests the real EfficiencyCode app-server across all three
...
keepalives, and catalog boundaries. In a published EfficiencyCode checkout, run:
...
python3 scripts/mcp_conformance/review_regressions.py \
  ...
  --report /tmp/EfficiencyCode-mcp-review-regressions.json
```
...
Its TypeScript job builds the actual EfficiencyCode executable, sets `EfficiencyCode_EXEC_PATH`,
...
```bash
EfficiencyCode_EXEC_PATH=/absolute/path/to/EfficiencyCode \
  pnpm --filter @openai/EfficiencyCode-sdk test -- \
