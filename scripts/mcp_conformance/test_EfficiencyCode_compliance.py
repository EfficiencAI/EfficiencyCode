...
import pytest
from run_EfficiencyCode_compliance import (
...
def test_registration_commands_select_stdio_then_http_shapes() -> None:
    EfficiencyCode = Path("/opt/EfficiencyCode")
    ...
    assert stdio == [
        ...
        TEST_SERVER_NAME,
        "--env",
        f"EfficiencyCode_MCP_PROTOCOL_VERSION={MODERN_VERSION}",
        "--",
        sys.executable,
        "/src/server.py",
        "--mode",
    ...
    valid, _ = _validate_registration(
        {
            ...
            "transport": {
                ...
                "env": {"EfficiencyCode_MCP_PROTOCOL_VERSION": MODERN_VERSION},
            },
        },
        transport="stdio",
        mode=MODERN_VERSION,
...
def test_isolated_environment_drops_model_credentials(
    ...
    monkeypatch.setenv("EfficiencyCode_API_KEY", "secret")
    monkeypatch.setenv("EfficiencyCode_ACCESS_TOKEN", "secret")
    monkeypatch.setenv("OPENAI_API_KEY", "secret")

    env = _isolated_environment(tmp_path)

    assert env["EfficiencyCode_HOME"] == str(tmp_path)
    assert "EfficiencyCode_API_KEY" not in env
    assert "EfficiencyCode_ACCESS_TOKEN" not in env
...
def test_scenario_ratio_includes_adapter_failures() -> None:
    checks = [
        ...
        CheckResult(
            "harness/auth/example/EfficiencyCode-adapter",
            False,
            "adapter failed",
            source="harness",
            scenario="auth/example",
...
def test_official_case_preserves_adapter_check_identity_on_failure_and_success(
    ...
    monkeypatch.setattr(
        "run_EfficiencyCode_compliance.run_official_mode",
        lambda **_kwargs: [result],
    )

    case = _run_official_case(
        Path("/opt/EfficiencyCode"),
        Path("/src/EfficiencyCode_conformance_adapter.py"),
    ...
    assert case.checks == [
        CheckResult(
            name=f"harness/{scenario}/EfficiencyCode-adapter",
            success=adapter_success,
...
def test_official_case_keeps_cimd_registration_checks_in_regression_gated_http_case(
    ...
    assert auto_env.get("EfficiencyCode_CONFORMANCE_CLIENT_REGISTRATION") is None
    assert forced_env["EfficiencyCode_CONFORMANCE_CLIENT_REGISTRATION"] == "cimd"
    ...
    assert gate["fixedChecks"] == [
        {
            ...
            "source": "harness",
            "scenario": scenario,
            "check_id": f"harness/{scenario}/EfficiencyCode-adapter",
        },
        {
            "mode": MODERN_VERSION,
            "transport": "official-http",
...
def test_regression_cli_accepts_a_baseline_report() -> None:
    args = _parse_args(
        ["/opt/EfficiencyCode", "--baseline-report", "/tmp/EfficiencyCode-mcp-baseline.json"]
    )

    assert args.baseline_report == Path("/tmp/EfficiencyCode-mcp-baseline.json")
...
def test_regression_cli_extracts_compact_baseline_without_starting_conformance(
    ...
    EfficiencyCode = tmp_path / "EfficiencyCode"
    EfficiencyCode.write_text("#!/bin/sh\nexit 99\n", encoding="utf-8")
    EfficiencyCode.chmod(0o755)
...
def test_regression_cli_rejects_extraction_without_a_source_report(
    ...
    assert main([str(EfficiencyCode), "--extract-baseline", str(tmp_path / "compact.json")]) == 2
...
def test_regression_cli_rejects_malformed_baselines(
    ...
    assert main([str(EfficiencyCode), "--baseline-report", str(baseline)]) == 2
...
def test_regression_cli_rejects_missing_baseline_before_starting_conformance(
    ...
    assert main([str(EfficiencyCode), "--baseline-report", str(tmp_path / "missing.json")]) == 2
