#!/usr/bin/env python3
"""Black-box MCP compliance runner for a supplied EfficiencyCode binary."""
...
class AppServerClient:
    """Minimal JSONL client for the EfficiencyCode app-server protocol."""

    def __init__(
        self,
        EfficiencyCode_binary: Path,
        *,
        env: Mapping[str, str],
        cwd: Path,
        ...
        self.process = subprocess.Popen(
            [str(EfficiencyCode_binary), "app-server"],
            cwd=cwd,
            env=dict(env),
        ...
        self._stdout_thread = threading.Thread(
            ...
            name="EfficiencyCode-app-server-stdout",
            daemon=True,
        )
        self._stderr_thread = threading.Thread(
            target=self._read_stderr,
            name="EfficiencyCode-app-server-stderr",
            daemon=True,
        )
    ...
    def _send(self, message: Mapping[str, object]) -> None:
        if self.process.poll() is not None:
            raise AppServerError(
                f"EfficiencyCode app-server exited with code {self.process.returncode}"
            )
    ...
    def request(
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"EfficiencyCode app-server closed stdout while handling {method}"
                )
    ...
    def wait_for_notification(
        ...
        *,
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"EfficiencyCode app-server closed stdout while waiting for {method}"
                )
            if "id" in message and isinstance(message.get("method"), str):
                self._handle_server_request(message)
                continue
...
def _isolated_environment(EfficiencyCode_home: Path) -> dict[str, str]:
    env = dict(os.environ)
    env["EfficiencyCode_HOME"] = str(EfficiencyCode_home)
    # Direct app-server MCP calls do not need a model or credentials.
    for name in ("EfficiencyCode_API_KEY", "EfficiencyCode_ACCESS_TOKEN", "OPENAI_API_KEY"):
        env.pop(name, None)
    return env
...
def _registration_command(
    ...
    *,
    ...
    command = [str(EfficiencyCode_binary), "mcp", "add", TEST_SERVER_NAME]
    if transport == "stdio":
        if mode == MODERN_VERSION:
            command.extend(["--env", f"EfficiencyCode_MCP_PROTOCOL_VERSION={MODERN_VERSION}"])
        return [
            *command,
            "--",
            sys.executable,
...
def _validate_registration(
    ...
    *,
    ...
    if transport == "stdio":
        ...
        modern_opt_in = (
            isinstance(env, dict)
            and env.get("EfficiencyCode_MCP_PROTOCOL_VERSION") == MODERN_VERSION
        )
...
def _exercise_app_server(
    ...
    *,
    ...
    sentinel = f"EfficiencyCode-mcp-{transport}-{mode}"
...
def _run_case(
    ...
    *,
    ...
    try:
        ...
        if registered:
            remove = _run_command(
                [str(EfficiencyCode_binary), "mcp", "remove", TEST_SERVER_NAME],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
            ...
            listed = _run_command(
                [str(EfficiencyCode_binary), "mcp", "list", "--json"],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
...
def _run_official_case(
    ...
    *,
    ...
    case_home.mkdir(parents=True)
    env = _isolated_environment(case_home)
    env["EfficiencyCode_CONFORMANCE_TIMEOUT"] = str(timeout_seconds)
    env["EfficiencyCode_CONFORMANCE_ENABLE_MODERN_FEATURE"] = (
        "1" if enable_modern_feature else "0"
    )
    env["EfficiencyCode_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = (
        "1" if require_automatic_auth else "0"
    )
    results = run_official_mode(
        conformance_command=conformance_command,
        adapter_script=adapter_script,
        EfficiencyCode_binary=EfficiencyCode_binary,
        mode=mode,
        scenarios=scenarios,
    ...
    for result in results:
        ...
        case.checks.append(
            CheckResult(
                name=f"harness/{result.scenario}/EfficiencyCode-adapter",
...
def _cimd_registration_checks(
    ...
    *,
    ...
    for check_name, client_registration in (
        ...
        env["EfficiencyCode_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = "1"
        if client_registration is not None:
            env["EfficiencyCode_CONFORMANCE_CLIENT_REGISTRATION"] = client_registration
        results = run_official_mode(
            conformance_command=conformance_command,
            adapter_script=adapter_script,
            EfficiencyCode_binary=EfficiencyCode_binary,
...
def run_compliance(
    ...
    *,
    ...
    run_root = Path(
        tempfile.mkdtemp(
            prefix="EfficiencyCode-mcp-compliance-",
            dir=artifact_parent,
        )
    )
    version_env = _isolated_environment(run_root)
    version = _run_command(
        [str(EfficiencyCode_binary), "--version"],
        env=version_env,
        cwd=run_root,
        timeout_seconds=timeout_seconds,
    )
    ...
    report: dict[str, object] = {
        ...
        "EfficiencyCodeBinary": str(EfficiencyCode_binary),
        "EfficiencyCodeVersion": version.stdout or None,
...
def _required_regression_checks(
    ...
    *,
    ...
    if not isinstance(version_check, dict) or version_check.get("success") is not True:
        errors.append(f"{label} does not contain a successful EfficiencyCode version check")
...
def _print_human_report(report: Mapping[str, object]) -> None:
    ...
    print(f"EfficiencyCode MCP compliance: {'PASS' if success else 'FAIL'}")
    print(f"Binary: {report.get('EfficiencyCodeBinary')}")
    print(f"Version: {report.get('EfficiencyCodeVersion') or 'unknown'}")
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "supplied EfficiencyCode binary, plus the supplemental local stdio suite."
        )
    )
    parser.add_argument("EfficiencyCode_binary", type=Path)
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--adapter-script",
        type=Path,
        default=Path(__file__).resolve().with_name("EfficiencyCode_conformance_adapter.py"),
        help="Path to the EfficiencyCode adapter used by the official client suite.",
    )
    parser.add_argument(
        "--conformance-cli",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require EfficiencyCode to recover from OAuth scope escalation and "
    ...
    parser.add_argument(
        "--artifact-parent",
        type=Path,
        help="Parent directory for isolated temporary EfficiencyCode homes.",
    )
    parser.add_argument(
        "--keep-artifacts",
        action="store_true",
        help="Keep isolated EfficiencyCode homes and include their path in the report.",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    EfficiencyCode_binary = args.EfficiencyCode_binary.expanduser().resolve()
    ...
    if not EfficiencyCode_binary.is_file() or not os.access(EfficiencyCode_binary, os.X_OK):
        print(f"error: EfficiencyCode binary is not executable: {EfficiencyCode_binary}", file=sys.stderr)
        return 2
