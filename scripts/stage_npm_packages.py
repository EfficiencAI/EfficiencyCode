#!/usr/bin/env python3
"""Stage one or more EfficiencyCode npm packages for release."""
...
BUILD_SCRIPT = REPO_ROOT / "EfficiencyCode-cli" / "scripts" / "build_npm_package.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
GITHUB_REPO = "openai/EfficiencyCode"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("EfficiencyCode_build_npm_package", BUILD_SCRIPT)
...
EfficiencyCode_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "EfficiencyCode_PLATFORM_PACKAGES", {})
EfficiencyCode_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "EfficiencyCode_PACKAGE_COMPONENT", "EfficiencyCode-package"
)
...
BINARY_COMPONENTS = {
    "EfficiencyCode-responses-api-proxy": BinaryComponent(
        artifact_prefix="EfficiencyCode-responses-api-proxy",
        dest_dir="EfficiencyCode-responses-api-proxy",
        binary_basename="EfficiencyCode-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if EfficiencyCode_PACKAGE_COMPONENT in components:
        install_EfficiencyCode_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = EfficiencyCode_PACKAGE_COMPONENT in components or any(
...
def install_EfficiencyCode_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing EfficiencyCode package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_EfficiencyCode_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_EfficiencyCode_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"EfficiencyCode-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in EfficiencyCode_PLATFORM_PACKAGES:
        platform = package.removeprefix("EfficiencyCode-")
        return f"EfficiencyCode-npm-{platform}-{version}.tgz"
