[CmdletBinding()]
param(
    [string]$Release = $env:EfficiencyCode_RELEASE
)
...
$NonInteractive = $env:EfficiencyCode_NON_INTERACTIVE -match "^(?i:1|true|yes)$"
$DefaultPreferReleasesOpenAICom = $true
$PreferReleasesOpenAICom = if ([string]::IsNullOrWhiteSpace($env:EfficiencyCode_INSTALLER_USE_RELEASES_OPENAI_COM)) {
    $DefaultPreferReleasesOpenAICom
} else {
    $env:EfficiencyCode_INSTALLER_USE_RELEASES_OPENAI_COM -match "^(?i:1|true|yes)$"
}
$ReleasesBaseUri = "https://releases.openai.com/EfficiencyCode"
...
function Assert-ValidReleaseVersion {
    ...
    if ($Version -cne "latest" -and $Version -cnotmatch "^[0-9]+\.[0-9]+\.[0-9]+(?:-alpha(?:\.[0-9]+){0,2}|-beta(?:\.[0-9]+)?)?$") {
        throw "Invalid EfficiencyCode release version: $Version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]."
    }
}
...
function Resolve-ReleaseAssetSelection {
    ...
    $packageAsset = "EfficiencyCode-package-$Target.tar.gz"
    $checksumAsset = "EfficiencyCode-package_SHA256SUMS"
    $packageUrl = $null
    ...
    if ($ResolvedRelease.Source -eq "ReleasesOpenAICom") {
        ...
        $packageFallbackUrl = "https://github.com/openai/EfficiencyCode/releases/download/rust-v$version/$packageAsset"
        $checksumUrl = "$ReleasesBaseUri/releases/$version/$checksumAsset"
        $checksumFallbackUrl = "https://github.com/openai/EfficiencyCode/releases/download/rust-v$version/$checksumAsset"
    }
    ...
    $packageAsset = "EfficiencyCode-npm-$NpmTag-$version.tgz"
    $packageUrl = $null
    $packageFallbackUrl = $null
    if ($ResolvedRelease.Source -eq "ReleasesOpenAICom") {
        $packageUrl = "$ReleasesBaseUri/releases/$version/$packageAsset"
    ...
    if ($null -eq $packageMetadata) {
        throw "Could not find EfficiencyCode package or platform npm release assets for EfficiencyCode $version."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded EfficiencyCode archive checksum did not match expected digest. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Get-PackageArchiveDigest {
    ...
    throw "Could not find SHA-256 digest for $AssetName in EfficiencyCode-package_SHA256SUMS."
}
...
function Resolve-VersionFromReleaseMetadata {
    ...
    if (-not $ReleaseMetadata.tag_name) {
        throw "Failed to resolve the latest EfficiencyCode release version."
    }
...
function Resolve-ReleaseFromGitHub {
    ...
    if ($NormalizedVersion -eq "latest") {
        ...
        $metadataUri = "https://api.github.com/repos/openai/EfficiencyCode/releases/latest"
    } else {
        $resolvedVersion = $NormalizedVersion
        $requestedRelease = $resolvedVersion
        $metadataUri = "https://api.github.com/repos/openai/EfficiencyCode/releases/tags/rust-v$resolvedVersion"
    }

    try {
        $releaseMetadata = Invoke-RestMethod -Uri $metadataUri
    } catch {
        throw "Could not fetch GitHub release metadata for EfficiencyCode $requestedRelease. GitHub API may be unavailable or rate limited. $($_.Exception.Message)"
    }
...
function Resolve-ReleaseFromReleases {
    ...
    try {
        ...
        if ($NormalizedVersion -ne "latest" -and $resolvedVersion -cne $NormalizedVersion) {
            throw "Release metadata version did not match requested EfficiencyCode version $NormalizedVersion."
        }
...
function Get-VersionFromBinary {
    param(
        [string]$EfficiencyCodePath
    )

    if (-not (Test-Path -LiteralPath $EfficiencyCodePath -PathType Leaf)) {
        return $null
    }

    try {
        $versionOutput = & $EfficiencyCodePath --version 2>$null
    } catch {
        return $null
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -EfficiencyCodePath (Join-Path $StandaloneCurrentDir "bin\EfficiencyCode.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }

    $standaloneVersion = Get-VersionFromBinary -EfficiencyCodePath (Join-Path $StandaloneCurrentDir "EfficiencyCode.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }
...
function Test-OldStandaloneBinLayout {
    ...
    $requiredFiles = @("EfficiencyCode.exe", "rg.exe")
    ...
    $knownFiles = @(
        "EfficiencyCode.exe",
        "rg.exe",
        "EfficiencyCode-command-runner.exe",
        "EfficiencyCode-windows-sandbox.exe",
        "EfficiencyCode-windows-sandbox-setup.exe"
    )
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older EfficiencyCode install at $VisibleBinDir"
    Write-WarningStep "To continue, EfficiencyCode needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current EfficiencyCode setup now?")) {
...
function Add-JunctionSupportType {
    if (([System.Management.Automation.PSTypeName]'EfficiencyCodeInstaller.Junction').Type) {
        return
    }
...
namespace EfficiencyCodeInstaller
{
...
function Set-JunctionTarget {
    ...
    [EfficiencyCodeInstaller.Junction]::SetTarget($LinkPath, $TargetPath)
}
...
function Test-PackageContentsAreComplete {
    ...
    $expectedFiles = @(
        "EfficiencyCode-package.json",
        "bin\EfficiencyCode.exe",
        "bin\EfficiencyCode-code-mode-host.exe",
        "EfficiencyCode-path\rg.exe",
        "EfficiencyCode-resources\EfficiencyCode-command-runner.exe",
        "EfficiencyCode-resources\EfficiencyCode-windows-sandbox-setup.exe"
    )
...
function Test-LegacyPlatformNpmContentsAreComplete {
    ...
    $expectedFiles = @(
        ...
        "EfficiencyCode-resources\EfficiencyCode-command-runner.exe",
        "EfficiencyCode-resources\EfficiencyCode-windows-sandbox-setup.exe",
        "EfficiencyCode-resources\rg.exe"
    )
    foreach ($name in $expectedFiles) {
        if (-not (Test-Path -LiteralPath (Join-Path $PackageDir $name) -PathType Leaf)) {
            return $false
...
function Test-ReleaseIsComplete {
    ...
    switch ($Layout) {
        "Package" {
            ...
            $EfficiencyCodePath = Join-Path $ReleaseDir "bin\EfficiencyCode.exe"
        }
        "LegacyPlatformNpm" {
            if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $ReleaseDir)) {
                return $false
            }
            $EfficiencyCodePath = Join-Path $ReleaseDir "EfficiencyCode.exe"
        }
        default {
            throw "Unknown EfficiencyCode installer layout: $Layout"
        }
    }

    return (Split-Path -Leaf $ReleaseDir) -eq "$ExpectedVersion-$ExpectedTarget" -and
        (Get-VersionFromBinary -EfficiencyCodePath $EfficiencyCodePath) -ceq $ExpectedVersion
}

function Get-ExistingEfficiencyCodeCommand {
    $existing = Get-Command EfficiencyCode -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ExistingEfficiencyCodeManager {
    param(
        [string]$ExistingPath,
        [string]$VisibleBinDir
    )
...
function Get-ConflictingInstall {
    ...
    $existingPath = Get-ExistingEfficiencyCodeCommand
    $manager = Get-ExistingEfficiencyCodeManager -ExistingPath $existingPath -VisibleBinDir $VisibleBinDir
    if ($null -eq $manager) {
        return $null
    }

    Write-Step "Detected existing $manager-managed EfficiencyCode at $existingPath"
    Write-WarningStep "Multiple managed EfficiencyCode installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        @("remove", "-g", "@openai/EfficiencyCode")
    } else {
        @("uninstall", "-g", "@openai/EfficiencyCode")
    }
    $uninstallCommand = if ($manager -eq "bun") { "bun" } else { "npm" }

    if (Prompt-YesNo "Uninstall the existing $manager-managed EfficiencyCode now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed EfficiencyCode. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed EfficiencyCode installed. PATH order will determine which EfficiencyCode runs."
    }
}

function Test-VisibleEfficiencyCodeCommand {
    param(
        [string]$VisibleBinDir
    )

    $EfficiencyCodeCommand = Join-Path $VisibleBinDir "EfficiencyCode.exe"
    & $EfficiencyCodeCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed EfficiencyCode command failed verification: $EfficiencyCodeCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "EfficiencyCode requires a 64-bit version of Windows."
    exit 1
}
...
$EfficiencyCodeHome = if ([string]::IsNullOrWhiteSpace($env:EfficiencyCode_HOME)) {
    Join-Path $env:USERPROFILE ".EfficiencyCode"
} else {
    $env:EfficiencyCode_HOME
}
$standaloneRoot = Join-Path $EfficiencyCodeHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\OpenAI\EfficiencyCode\bin"
if ([string]::IsNullOrWhiteSpace($env:EfficiencyCode_INSTALL_DIR)) {
    $visibleBinDir = $defaultVisibleBinDir
} else {
    $visibleBinDir = $env:EfficiencyCode_INSTALL_DIR
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating EfficiencyCode CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating EfficiencyCode CLI"
} else {
    Write-Step "Installing EfficiencyCode CLI"
}
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("EfficiencyCode-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading EfficiencyCode CLI"
            ...
            if ($installLayout -eq "Package") {
                ...
                if (-not (Test-PackageContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded EfficiencyCode package archive did not contain the expected package layout."
                }
            } else {
                ...
                $resourcesDir = Join-Path $stagingDir "EfficiencyCode-resources"
                New-Item -ItemType Directory -Force -Path $resourcesDir | Out-Null
                $copyMap = @{
                    "EfficiencyCode/EfficiencyCode.exe" = "EfficiencyCode.exe"
                    "EfficiencyCode/EfficiencyCode-command-runner.exe" = "EfficiencyCode-resources\EfficiencyCode-command-runner.exe"
                    "EfficiencyCode/EfficiencyCode-windows-sandbox-setup.exe" = "EfficiencyCode-resources\EfficiencyCode-windows-sandbox-setup.exe"
                    "path/rg.exe" = "EfficiencyCode-resources\rg.exe"
                }
                ...
                if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded EfficiencyCode npm archive did not contain the expected legacy platform package layout."
                }
            }
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            throw "Installed EfficiencyCode command did not report expected version $resolvedVersion."
        }
        ...
        try {
            ...
            Test-VisibleEfficiencyCodeCommand -VisibleBinDir $visibleBinDir
        } catch {
...
Write-Step "Current PowerShell session: EfficiencyCode"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: EfficiencyCode"
Write-Host "EfficiencyCode CLI $resolvedVersion installed successfully."

$EfficiencyCodeCommand = Join-Path $visibleBinDir "EfficiencyCode.exe"
if (Prompt-YesNo "Start EfficiencyCode now?") {
    Write-Step "Launching EfficiencyCode"
    & $EfficiencyCodeCommand
}
