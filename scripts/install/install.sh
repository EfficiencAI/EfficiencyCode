...
RELEASE="${EfficiencyCode_RELEASE:-latest}"
NON_INTERACTIVE="${EfficiencyCode_NON_INTERACTIVE:-false}"
DEFAULT_PREFER_RELEASES_OPENAI_COM="true"
PREFER_RELEASES_OPENAI_COM="${EfficiencyCode_INSTALLER_USE_RELEASES_OPENAI_COM:-$DEFAULT_PREFER_RELEASES_OPENAI_COM}"
RELEASES_BASE_URL="https://releases.openai.com/EfficiencyCode"
RELEASES_CONNECT_TIMEOUT=10
RELEASES_METADATA_TIMEOUT=30
RELEASES_ASSET_TIMEOUT=300
release_source="github"

BIN_DIR="${EfficiencyCode_INSTALL_DIR:-$HOME/.local/bin}"
BIN_PATH="$BIN_DIR/EfficiencyCode"
CODE_MODE_HOST_BIN_PATH="$BIN_DIR/EfficiencyCode-code-mode-host"
EfficiencyCode_HOME_DIR="${EfficiencyCode_HOME:-$HOME/.EfficiencyCode}"
STANDALONE_ROOT="$EfficiencyCode_HOME_DIR/packages/standalone"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-alpha(\.[0-9]+){0,2}|-beta(\.[0-9]+)?)?$'; then
    echo "Invalid EfficiencyCode release version: $version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]." >&2
    return 1
  fi
}
...
Environment:
  EfficiencyCode_RELEASE          Version to install; overridden by --release.
  EfficiencyCode_NON_INTERACTIVE  Set to 1, true, or yes to skip prompts.
  EfficiencyCode_INSTALLER_USE_RELEASES_OPENAI_COM
...
download_file() {
  ...
  echo "curl or wget is required to install EfficiencyCode." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/openai/EfficiencyCode/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}
...
release_metadata_url() {
  ...
  printf 'https://api.github.com/repos/openai/EfficiencyCode/releases/tags/rust-v%s\n' "$resolved_version"
}
...
parse_downloaded_release_metadata() {
  ...
  if ! release_metadata="$(printf '%s\n' "$release_json" | parse_release_metadata)"; then
    echo "Could not parse $source_name release metadata for EfficiencyCode $requested_release." >&2
    return 1
  fi
}
...
resolve_metadata_version() {
  ...
  if [ -z "$metadata_version" ]; then
    echo "Failed to resolve the latest EfficiencyCode release version." >&2
    return 1
  fi
...
resolve_release_from_github() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/openai/EfficiencyCode/releases/latest"
  else
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for EfficiencyCode $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi
...
resolve_release_from_releases() {
  ...
  if [ "$normalized_version" != "latest" ] && [ "$metadata_version" != "$normalized_version" ]; then
    echo "Release metadata version did not match requested EfficiencyCode version $normalized_version." >&2
    return 1
  fi
  resolved_version="$metadata_version"
...
select_release_assets() {
  package_asset="EfficiencyCode-package-$vendor_target.tar.gz"
  checksum_asset="EfficiencyCode-package_SHA256SUMS"
  ...
  if release_asset_exists "$package_asset" &&
    ...
    asset="$package_asset"
  elif release_asset_exists "EfficiencyCode-npm-$npm_tag-$resolved_version.tgz"; then
    install_layout="legacy-platform-npm"
    asset="EfficiencyCode-npm-$npm_tag-$resolved_version.tgz"
  else
    echo "Could not find EfficiencyCode package or platform npm release assets for EfficiencyCode $resolved_version." >&2
    return 1
  fi
...
package_archive_digest() {
  ...
  if [ -z "$digest" ]; then
    echo "Could not find SHA-256 digest for $asset in EfficiencyCode-package_SHA256SUMS." >&2
    return 1
  fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the EfficiencyCode download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded EfficiencyCode archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install EfficiencyCode." >&2
    exit 1
  fi
}
...
add_to_path() {
  ...
  begin_marker="# >>> EfficiencyCode installer >>>"
  end_marker="# <<< EfficiencyCode installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.EfficiencyCode.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  EfficiencyCode_path="$1"

  if [ ! -x "$EfficiencyCode_path" ]; then
    return 1
  fi

  "$EfficiencyCode_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/EfficiencyCode" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/EfficiencyCode" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_EfficiencyCode() {
  command -v EfficiencyCode 2>/dev/null || true
}

classify_existing_EfficiencyCode() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && EfficiencyCode"
      step "Future terminals: open a new terminal and run: EfficiencyCode"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: EfficiencyCode"
      step "Future terminals: open a new terminal and run: EfficiencyCode"
      ;;
  esac
}

maybe_launch_EfficiencyCode_now() {
  if prompt_yes_no "Start EfficiencyCode now?"; then
    step "Launching EfficiencyCode"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_EfficiencyCode)"
  manager="$(classify_existing_EfficiencyCode "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed EfficiencyCode at $existing_path"
  warn "Multiple managed EfficiencyCode installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask EfficiencyCode"
      ;;
    bun)
      uninstall_cmd="bun remove -g @openai/EfficiencyCode"
      ;;
    *)
      uninstall_cmd="npm uninstall -g @openai/EfficiencyCode"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed EfficiencyCode now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed EfficiencyCode. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed EfficiencyCode installed. PATH order will determine which EfficiencyCode runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 \
    "$stage_release/bin/EfficiencyCode" \
    "$stage_release/bin/EfficiencyCode-code-mode-host" \
    "$stage_release/EfficiencyCode-path/rg"
  if [ -f "$stage_release/EfficiencyCode-resources/bwrap" ]; then
    chmod 0755 "$stage_release/EfficiencyCode-resources/bwrap"
  fi
  ln -sf "bin/EfficiencyCode" "$stage_release/EfficiencyCode"
...
install_legacy_platform_npm_release() {
  ...
  mkdir -p "$stage_release/EfficiencyCode-resources" "$extract_dir"
  tar -xzf "$archive_path" -C "$extract_dir"

  cp "$vendor_root/EfficiencyCode/EfficiencyCode" "$stage_release/EfficiencyCode"
  cp "$vendor_root/path/rg" "$stage_release/EfficiencyCode-resources/rg"
  chmod 0755 "$stage_release/EfficiencyCode" "$stage_release/EfficiencyCode-resources/rg"
  if [ -f "$vendor_root/EfficiencyCode-resources/bwrap" ]; then
    cp "$vendor_root/EfficiencyCode-resources/bwrap" "$stage_release/EfficiencyCode-resources/bwrap"
    chmod 0755 "$stage_release/EfficiencyCode-resources/bwrap"
  fi
...
release_dir_is_complete() {
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/EfficiencyCode-package.json" ] &&
        [ -x "$release_dir/bin/EfficiencyCode" ] &&
        [ -x "$release_dir/bin/EfficiencyCode-code-mode-host" ] &&
        [ -x "$release_dir/EfficiencyCode" ] &&
        [ -x "$release_dir/EfficiencyCode-path/rg" ] ||
        return 1
      ;;
    legacy-platform-npm)
      [ -x "$release_dir/EfficiencyCode" ] &&
        [ -x "$release_dir/EfficiencyCode-resources/rg" ] ||
        return 1
      ;;
    *)
      return 1
  ...
  case "$layout:$expected_target" in
    package:*linux* | legacy-platform-npm:*linux*)
      [ -x "$release_dir/EfficiencyCode-resources/bwrap" ] || return 1
      ;;
  esac

  installed_version="$(version_from_binary "$release_dir/bin/EfficiencyCode" || version_from_binary "$release_dir/EfficiencyCode" || true)"
...
release_EfficiencyCode_relative_path() {
  release_dir="$1"

  if [ -x "$release_dir/bin/EfficiencyCode" ]; then
    printf 'bin/EfficiencyCode\n'
  else
    printf 'EfficiencyCode\n'
  fi
}
...
update_visible_command() {
  release_dir="$1"
  mkdir -p "$BIN_DIR"
  tmp_link="$BIN_DIR/.EfficiencyCode.$$"
  EfficiencyCode_relative_path="$(release_EfficiencyCode_relative_path "$release_dir")"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/$EfficiencyCode_relative_path" "$tmp_link"

  if [ "$os" = "darwin" ] && [ -x "$release_dir/bin/EfficiencyCode-code-mode-host" ]; then
    replace_path_with_symlink \
      "$CODE_MODE_HOST_BIN_PATH" \
      "$CURRENT_LINK/bin/EfficiencyCode-code-mode-host" \
      "$tmp_link"
  elif [ "$(readlink "$CODE_MODE_HOST_BIN_PATH" 2>/dev/null || true)" = \
    "$CURRENT_LINK/bin/EfficiencyCode-code-mode-host" ]; then
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating EfficiencyCode CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating EfficiencyCode CLI"
else
  step "Installing EfficiencyCode CLI"
fi
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  ...
  step "Downloading EfficiencyCode CLI"
  if [ "$install_layout" = "package" ]; then
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  echo "Installed EfficiencyCode command did not report expected version $resolved_version." >&2
  exit 1
fi
...
printf 'EfficiencyCode CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_EfficiencyCode_now
