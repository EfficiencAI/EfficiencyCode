...
./scripts/stage_npm_packages.py \
  ...
  --package EfficiencyCode \
  --package EfficiencyCode-responses-api-proxy \
  --package EfficiencyCode-sdk
```
...
When `--package EfficiencyCode` is provided, the staging helper builds the lightweight
`@openai/EfficiencyCode` meta package plus all platform-native `@openai/EfficiencyCode` variants
